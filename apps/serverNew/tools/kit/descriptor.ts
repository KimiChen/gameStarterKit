...
export type DomainDeclaration = {
  readonly serverRuntime?: "serverNew";
  ...
  readonly exposesOperationGroupTo: {
...
function fail(label: string, message: string): never {
  throw new Error(`[plugin-codegen] ${label}: ${message}`);
}
...
type ModuleIndex = {
  ...
  readonly interfaces: ReadonlyMap<
    string,
    { readonly exported: boolean; readonly node: ts.InterfaceDeclaration }
  >;
...
function indexModule(sourceFile: ts.SourceFile, label: string): ModuleIndex {
  ...
  const interfaces = new Map<
    string,
    { exported: boolean; node: ts.InterfaceDeclaration }
  >();
  ...
  for (const statement of sourceFile.statements) {
    if (ts.isImportDeclaration(statement)) {
      ...
      if (!ts.isStringLiteral(specifier))
        fail(label, "import specifier 必须是字符串字面量");
      ...
      if (ts.isNamespaceImport(bindings))
        fail(label, "domain 文件不允许 namespace import");
    ...
    for (const declaration of statement.declarationList.declarations) {
      ...
      if (
        ...
        annotation.typeName.text === "RuntimeValidator" &&
      ...
      const initializer = declaration.initializer
        ? unwrapAsConst(declaration.initializer)
        : null;
...
function resolveType(
  ...
  if (declaredInterface) {
    if (!declaredInterface.exported)
      fail(
        label,
        `${context} 的类型 "${typeName}" 必须导出（registry 需要 import 它）`,
      );
  ...
  fail(
    label,
    `${context} 的类型 "${typeName}" 既不在本文件声明也不在 import 内`,
  );
}
...
function resolveValidator(
  ...
  if (!info) {
    fail(
      label,
      `${context} 引用的 "${name}" 不是本文件的 annotated validator` +
  ...
  if (!info.exported)
    fail(
      label,
      `${context} 引用的 validator "${name}" 必须导出（registry 需要 import 它）`,
    );
  if (!info.typeName) {
    fail(
      label,
      `${context} 引用的 validator "${name}" 缺少单一类型标识符的载荷类型注解`,
    );
  }
  return {
    identifier: name,
    payloadType: resolveType(
      index,
      info.typeName,
      label,
...
function parseStringArray(
  ...
  if (!ts.isArrayLiteralExpression(literal))
    fail(label, `${context} 必须是数组字面量`);
  const values: string[] = [];
  for (const element of literal.elements) {
    if (!ts.isStringLiteral(element))
      fail(label, `${context} 的元素必须是字符串字面量`);
    if (values.includes(element.text))
      fail(label, `${context} 重复声明："${element.text}"`);
    values.push(element.text);
  }
  return values;
}

function parseRouteType(
  index: ModuleIndex,
  node: ts.Expression,
  label: string,
  context: string,
  ...
  if (
    ...
    if (value === undefined) {
      fail(
        label,
        `${context} 引用的 ${node.expression.text}.${node.name.text} 不是本文件 as const 路由名表的成员`,
      );
    }
    return value;
  }
...
function objectEntries(
  ...
  if (!ts.isObjectLiteralExpression(node))
    fail(label, `${context} 必须是对象字面量`);
  const entries = new Map<string, ts.Expression>();
  for (const property of node.properties) {
    if (ts.isSpreadAssignment(property))
      fail(label, `${context} 不允许 spread`);
    if (!ts.isPropertyAssignment(property))
      fail(label, `${context} 只允许普通「键: 值」赋值`);
    if (ts.isComputedPropertyName(property.name))
      fail(label, `${context} 不允许 computed property`);
    if (!ts.isIdentifier(property.name))
      fail(label, `${context} 的键必须是标识符`);
    if (entries.has(property.name.text))
      fail(label, `${context} 重复键：${property.name.text}`);
    entries.set(property.name.text, property.initializer);
  }
  return entries;
}

function assertRequiredClientReqId(
  index: ModuleIndex,
  requestType: TypeRef,
  label: string,
  route: string,
  ...
  if (requestType.specifier !== null) {
    fail(
      label,
      `idempotent-write 路由 ${route} 的 request 接口必须在本域文件内声明（读到 import 的 "${requestType.name}"）`,
    );
  }
  ...
  if (!declared) {
    fail(
      label,
      `idempotent-write 路由 ${route} 的 request 类型 "${requestType.name}" 必须是接口声明`,
    );
  }
  for (const member of declared.node.members) {
    ...
    if (member.questionToken) {
      fail(
        label,
        `idempotent-write 路由 ${route} 的 "${requestType.name}.clientReqId" 不得是可选字段` +
    ...
    if (!member.type || member.type.kind !== ts.SyntaxKind.StringKeyword) {
      fail(
        label,
        `idempotent-write 路由 ${route} 的 "${requestType.name}.clientReqId" 必须是 string`,
      );
    }
    return;
  }
  fail(
    label,
    `idempotent-write 路由 ${route} 的 request 接口 "${requestType.name}" 字面缺少必选 clientReqId: string`,
  );
}
...
function parsePushCall(
  ...
  if (
    ...
    !ts.isIdentifier(node.expression) ||
    node.expression.text !== "defineLobbyPush"
  ) {
    fail(
      label,
      `${context} 的元素必须是 defineLobbyPush(key, type, validator) 调用`,
    );
  }
  if (node.arguments.length !== 3)
    fail(label, `${context} 的 defineLobbyPush 必须是三参形态`);
  const [keyArg, typeArg, validatorArg] = node.arguments;
  if (!ts.isStringLiteral(keyArg))
    fail(label, `${context} 的推送 key 必须是字符串字面量`);
  if (!ts.isStringLiteral(typeArg))
    fail(label, `${context} 的推送消息名必须是字符串字面量`);
  if (!/^[A-Za-z][A-Za-z0-9]*$/u.test(keyArg.text)) {
    fail(
      label,
      `${context} 的推送 key "${keyArg.text}" 必须是标识符形态（聚合 LobbyPush 常量的成员名）`,
    );
  }
  const resolved = resolveValidator(
    ...
    validatorArg,
    label,
    `${context} 的推送 "${typeArg.text}"`,
  );
  return {
    key: keyArg.text,
    type: typeArg.text,
    validator: resolved.identifier,
    dataType: resolved.payloadType,
  };
}
...
function parseRouteCall(
  ...
  if (!ts.isCallExpression(node) || !ts.isIdentifier(node.expression)) {
    fail(
      label,
      "routes 的元素必须是 defineRpcQuery/defineRpcNaturalWrite/defineRpcIdempotentWrite 调用",
    );
  }
  ...
  if (!mode) {
    fail(
      label,
      `routes 内不允许 "${node.expression.text}" 调用（define* 之外的函数调用一律拒绝）`,
    );
  }
  if (node.arguments.length !== 2)
    fail(
      label,
      `${node.expression.text} 必须是 (type, {request, response, ...}) 两参形态`,
    );
  const type = parseRouteType(
    index,
    node.arguments[0],
    label,
    `${node.expression.text} 的路由名`,
  );
  const options = objectEntries(
    node.arguments[1],
    label,
    `路由 ${type} 的选项`,
  );
  ...
  if (mode === "idempotent-write") {
    allowed.add("operationGroup");
    allowed.add("inspectable");
  }
  for (const key of options.keys()) {
    if (!allowed.has(key))
      fail(label, `路由 ${type}（${mode}）的选项含未知键：${key}`);
  }
  const requestNode = options.get("request");
  const responseNode = options.get("response");
  if (!requestNode || !responseNode)
    fail(label, `路由 ${type} 必须同时声明 request 与 response validator`);
  const request = resolveValidator(
    index,
    requestNode,
    label,
    `路由 ${type} 的 request`,
  );
  const response = resolveValidator(
    index,
    responseNode,
    label,
    `路由 ${type} 的 response`,
  ...
  if (contractVersionNode) {
    ...
    if (!Number.isSafeInteger(parsed) || parsed < 1) {
      fail(
        label,
        `路由 ${type} 的 contractVersion 必须是 ≥1 的安全整数（读到 "${contractVersionNode.text}"）`,
      );
    }
  ...
  if (operationGroupNode) {
    if (!ts.isStringLiteral(operationGroupNode))
      fail(label, `路由 ${type} 的 operationGroup 必须是字符串字面量`);
  ...
  if (inspectableNode) {
    if (
      inspectableNode.kind !== ts.SyntaxKind.TrueKeyword &&
      inspectableNode.kind !== ts.SyntaxKind.FalseKeyword
    ) {
  ...
  if (inspectsNode) {
    if (!ts.isStringLiteral(inspectsNode))
      fail(label, `路由 ${type} 的 inspectsOperationGroup 必须是字符串字面量`);
...
function parseExposesOperationGroupTo(
  ...
  for (const [group, value] of entries) {
    out[group] = parseStringArray(
      value,
      label,
      `exposesOperationGroupTo.${group}`,
    );
  }
  return out;
}
...
export function parseDomainModule(
  source: string,
  label: string,
): DomainDeclaration {
  ...
  if (!index.defaultExport)
    fail(label, "缺少 export default defineLobbyRpcDomain({...})");
  ...
  if (call.arguments.length !== 1)
    fail(label, "defineLobbyRpcDomain 必须是单对象参数形态");
  const descriptor = objectEntries(
    call.arguments[0],
    label,
    "defineLobbyRpcDomain 的参数",
  );
  ...
  if (!domainNode || !ts.isStringLiteral(domainNode))
    fail(label, "domain 必须是字符串字面量");
  const domain = domainNode.text;
  const runtime = descriptor.get("serverRuntime");
  if (
    runtime &&
    (!ts.isStringLiteral(runtime) || runtime.text !== "serverNew")
  ) {
    fail(label, "serverRuntime 必须是 serverNew 字符串字面量");
  }
  ...
  if (contractVersionNode) {
    if (!ts.isNumericLiteral(contractVersionNode))
      fail(label, "域 contractVersion 必须是数字字面量（正整数）");
    const parsed = Number(contractVersionNode.text);
    if (!Number.isSafeInteger(parsed) || parsed < 1) {
      fail(
        label,
        `域 contractVersion 必须是 ≥1 的安全整数（读到 "${contractVersionNode.text}"）`,
      );
    }
    contractVersion = parsed;
  }
  ...
  const ownsOperationGroups = ownsNode
    ? parseStringArray(ownsNode, label, "ownsOperationGroups")
    : [];
  const exposesNode = descriptor.get("exposesOperationGroupTo");
  const exposesOperationGroupTo = exposesNode
    ? parseExposesOperationGroupTo(exposesNode, label)
    : {};
  ...
  if (pushesNode) {
    ...
    if (!ts.isArrayLiteralExpression(literal))
      fail(label, "pushes 必须是数组字面量");
    for (const element of literal.elements) {
      const push = parsePushCall(index, element, label, "pushes");
      if (pushes.some((existing) => existing.type === push.type))
        fail(label, `重复声明推送消息名：${push.type}`);
      if (pushes.some((existing) => existing.key === push.key))
        fail(label, `重复声明推送 key：${push.key}`);
      pushes.push(push);
    }
  }
  ...
  if (!ts.isArrayLiteralExpression(routesLiteral))
    fail(label, "routes 必须是数组字面量");
  const routes: RouteDeclaration[] = [];
  for (const element of routesLiteral.elements) {
    const route = parseRouteCall(index, element, label);
    if (routes.some((existing) => existing.type === route.type))
      fail(label, `重复声明路由：${route.type}`);
    routes.push(route);
  }
  ...
  return {
    domain,
    ...(runtime ? { serverRuntime: "serverNew" as const } : {}),
    contractVersion,
    errorCodes,
    ownsOperationGroups,
    exposesOperationGroupTo,
...
export function parseCoreErrorsModule(
  source: string,
  label: string,
): CoreErrorsDeclaration {
  const sourceFile = ts.createSourceFile(
    label,
    source,
    ts.ScriptTarget.Latest,
  ...
  const coreErrorCodes = parseStringArray(
    findExportedConst(sourceFile, "CORE_RPC_ERROR_CODES", label),
    label,
    "CORE_RPC_ERROR_CODES",
  );
  const errorCodeOrder = parseStringArray(
    findExportedConst(sourceFile, "RPC_ERR_CODE_ORDER", label),
    label,
    "RPC_ERR_CODE_ORDER",
  );

  const pushesLiteral = unwrapAsConst(
    findExportedConst(sourceFile, "CORE_LOBBY_PUSHES", label),
  );
  if (!ts.isArrayLiteralExpression(pushesLiteral))
    fail(label, "CORE_LOBBY_PUSHES 必须是数组字面量");
  const pushes: PushDeclaration[] = [];
