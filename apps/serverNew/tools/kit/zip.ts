...
function assertEntryPath(raw: string, seen: Set<string>): void {
  ...
  if (raw.includes("\\"))
    throw new Error(`[plugin-zip] 条目路径含反斜杠：${raw}`);
  if (raw.startsWith("/") || /^[A-Za-z]:/u.test(raw))
    throw new Error(`[plugin-zip] 条目路径不是相对路径：${raw}`);
  for (const segment of raw.split("/")) {
    if (segment === "" || segment === "." || segment === "..")
      throw new Error(`[plugin-zip] 条目路径含非法段：${raw}`);
  }
  const folded = raw.toLowerCase();
  if (seen.has(folded))
    throw new Error(`[plugin-zip] 条目重名（含大小写归一化）：${raw}`);
  seen.add(folded);
}
...
export function writeZip(entries: readonly ZipEntry[]): Buffer {
  ...
  if (sorted.length > MAX_ENTRIES)
    throw new Error(`[plugin-zip] 条目数超过上限 ${MAX_ENTRIES}`);
  ...
  for (const entry of sorted) {
    if (entry.data.length > MAX_ENTRY_BYTES)
      throw new Error(`[plugin-zip] 条目超过单文件上限：${entry.path}`);
...
export function readZip(buffer: Buffer): readonly ZipEntry[] {
  ...
  if (diskNumber !== 0 || centralDisk !== 0)
    throw new Error("[plugin-zip] 不支持多卷");
  ...
  if (centralOffset + centralSize > eocd)
    throw new Error("[plugin-zip] central directory 越界");
  ...
  for (let index = 0; index < entryCount; index += 1) {
    if (cursor + 46 > eocd || buffer.readUInt32LE(cursor) !== CENTRAL_SIG)
      throw new Error("[plugin-zip] central directory 损坏");
    ...
    if ((flags & 0x0001) !== 0)
      throw new Error(`[plugin-zip] 不支持加密条目：${name}`);
    if (compressedSize === 0xffffffff || uncompressedSize === 0xffffffff)
      throw new Error("[plugin-zip] 不支持 zip64");
    const unixMode = (externalAttributes >>> 16) & 0xf000;
    if (unixMode === 0xa000)
      throw new Error(`[plugin-zip] 拒绝符号链接条目：${name}`);
    if (name.endsWith("/")) {
      if (uncompressedSize !== 0)
        throw new Error(`[plugin-zip] 目录条目携带数据：${name}`);
      continue;
    }
    assertEntryPath(name, seen);
    if (method !== METHOD_STORE && method !== METHOD_DEFLATE)
      throw new Error(`[plugin-zip] 不支持的压缩方法 ${method}：${name}`);
    if (uncompressedSize > MAX_ENTRY_BYTES)
      throw new Error(`[plugin-zip] 条目超过单文件上限：${name}`);
    total += uncompressedSize;
    if (total > MAX_TOTAL_BYTES)
      throw new Error("[plugin-zip] 包解压总量超过上限");

    if (
      localOffset + 30 > buffer.length ||
      buffer.readUInt32LE(localOffset) !== LOCAL_SIG
    ) {
    ...
    if (dataEnd > buffer.length)
      throw new Error(`[plugin-zip] 条目数据越界：${name}`);
    ...
    if (data.length !== uncompressedSize)
      throw new Error(`[plugin-zip] 尺寸不符：${name}`);
