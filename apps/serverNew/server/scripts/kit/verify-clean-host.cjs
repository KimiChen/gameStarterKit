...
try {
    ...
    const artifact = path.join(host, 'gameDemo-package')
    pack(path.join(repositoryRoot, 'apps/serverNew/kits/gameDemo'), artifact)
    ...
    const schema = path.join(artifact, 'files/apps/shared/schema/protocols/C2S/gameDemo.json')
    ...
    assert.equal(fs.existsSync(path.join(host, 'apps/shared/schema/protocols/C2S/gameDemo.json')), false)
    ...
    const brokenSchema = path.join(brokenArtifact, 'files/apps/shared/schema/protocols/C2S/gameDemo.json')
    const brokenValue = JSON.parse(fs.readFileSync(brokenSchema, 'utf8'))
    brokenValue.types.IGameDemoPingReq.kind = 'invalid'
    ...
    brokenLock.hashes['apps/shared/schema/protocols/C2S/gameDemo.json'] = crypto
        .createHash('sha256')
        .update(brokenBytes)
        .digest('hex')
    ...
    assert.equal(fs.existsSync(path.join(host, 'apps/serverNew/server/src/modules/gameDemo')), false)
    run('node', [kit, 'install', artifact, '--root', host])
    rejects('node', [kit, 'install', artifact, '--root', host], /kit is installed/)
    const installedSchema = path.join(host, 'apps/shared/schema/protocols/C2S/gameDemo.json')
    fs.appendFileSync(installedSchema, '\n')
    rejects('node', [kit, 'uninstall', 'gameDemo', '--root', host], /installed kit file changed/)
    fs.writeFileSync(installedSchema, originalSchema)
    run('node', [kit, 'check', 'gameDemo', '--root', host])
    run('pnpm', ['typecheck'], path.join(host, 'apps/serverNew/server'))
    run('node', ['scripts/kit/verify-game-demo.cjs'], path.join(host, 'apps/serverNew/server'))
    run('node', [kit, 'uninstall', 'gameDemo', '--root', host])
    run('pnpm', ['check:kit-protocol'], path.join(host, 'apps/serverNew/server'))
    assert.deepEqual(fs.readFileSync(registry), originalRegistry)
    run('node', ['scripts/kit/verify-game-demo.cjs', '--absent'], path.join(host, 'apps/serverNew/server'))
    assert.equal(fs.existsSync(path.join(host, 'apps/serverNew/server/kits-installed/gameDemo.lock.json')), false)
    assert.equal(fs.existsSync(path.join(host, 'apps/serverNew/server/src/modules/gameDemo')), false)
