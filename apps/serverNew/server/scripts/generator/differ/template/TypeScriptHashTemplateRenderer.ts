...
export class TypeScriptHashTemplateRenderer {
    public gen(args: RecBean) {
        ...
        constructor(id: ${args.idFiledType}, loadOpts?: HashLoadOpts, byLoad = false) {
            super(id, loadOpts, byLoad)
            ...
            if (f.isBaseType()) {
                ...
                if (f.isCollectionType()) {
                    ...
                    `
                        ...
                        get ${f.name}(): ${f.beanType()} {
                            ...
                            if (this._${f.name} === undefined) {
                                ...
                                ${f.modType !== 'ModType.None' ? `this.__versions!.${f.name} = 1` : ''}
                                if (this.writable) {
                    ...
                    get ${f.name}(): ${f.type} ${f.hasQuestionToken ? '| undefined' : ''}{
                        this.tryLoad('${f.name}')
                         if (!this._${f.name}) {
                            ${f.hasQuestionToken ? 'return undefined' : `this._${f.name} = new ${f.type}()`}
                            ${f.modType !== 'ModType.None' ? `this.__versions!.${f.name} = 1` : ''}
                         }
        ...
        ${args.forEach(args.propsExist, (f) => {
            if (f.isBeanType()) {
                return `
                static readonly f_${f.name}: FieldInfo & { [K in keyof typeof ${
                    f.type
                }]: FieldInfo } = new FieldInfo(this._class_info, '${f.name}', ${f.id}, ${f.getFieldType()},
                ${f.defaultValue}, ${f.collectionTypes}, ${f.saveType}, ${f.modType}, ${f.listen}) as any
                `
            } else {
                return `
