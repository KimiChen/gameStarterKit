...
export class TypeScriptBeanTemplateRenderer {
    public gen(args: RecBean) {
        ...
        ${args.forEach(
         ...
         constructor(data?:Partial<${args.className}>) {
                    super()
        ...
        ${args.forEach(
            ...
            (f) =>
                `
                    if (data?.${f.name}) {
                        this._${f.name} = ${f.type === 'int' ? `Math.trunc(data.${f.name})` : `data.${f.name}`}
                    }
                    ...
                    if (f.type === 'Array') {
                        str += `
                         ...
                         get ${f.name}(): ${f.beanType()} {
                             ...
                             } else if (!(this._${f.name} instanceof DiffArray)) {
                                const val = this._${f.name} as ${f.collectionTypes?.[0]}[]
                        ...
                        get ${f.name}(): ${f.beanType()} {
                            ...
                            } else if (!(this._${f.name} instanceof DiffMap)) {
                                const data = this._${f.name}
                                this._${f.name} = new ${f.beanType()}(${className}.f_${f.name})
                                this._${f.name}.parseFromData(data)
                                this._${f.name}.initDiff(this, ${className}.f_${f.name})
                            } else if (this.diff) {
                                this._${f.name}.initDiff(this, ${className}.f_${f.name})
                            }
        ...
        ${args.forEach(args.propsExist, (f) => {
            ...
            ${f.defaultValue}, ${f.beanCollectionTypes()}, ${f.saveType})
            `
        })}
