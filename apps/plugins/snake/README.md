...
插件包 `1.2.5` 接入 SC1-B9 的正式 `GameplayPresentationHost.rawInput` 端口：订阅绑定 gameplay
owner 的 signal / isActive，在玩法 router 前固定 pointer 归属；模态、关闭、重挂与 hide 先取消拖拽 / boost，
必要的 release 意图在宿主关闭业务输入前完成。HUD 与全局后备输入由同一框架适配器调度，表现件不再自行订阅全局触摸。
验收状态见 [3D-PLAN §8](../../../docs/3D-PLAN.md#8-批次状态只在本文回写阶段级完成回写-3dmd-10)。
