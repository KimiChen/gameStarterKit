    ...
    const timeoutResult = spawnSync(
      ...
      ["--import", "tsx", "--input-type=module", "-e", timeoutScript],
      {
        ...
        env: {
          ...
          //
          // ⚠ 预算取值不追求「刚好够」。这段历史值得写清楚，因为它被误诊过两轮：
          //
          // 冷启动实测在不同观测者手里差出一个数量级（174ms / 1142ms / 1913ms，最大约 11×），
          // 一度被当成「机器差异」。真正的原因是**注入 worker 继承了它不需要的 tsx loader**——见上面
          // worker 工厂处的 execArgv 注释。去掉之后同一段窗口降到 18–31ms（8 核满载），
          // 而观测差异则来自负载形态（复核期间曾有 74 个 agent 并发）。
          //
          // 也就是说前两轮把预算从 250 抬到 1200 再抬到 2500，都是在治症状。根因修掉后，
          // **旧的贴边值 1200/4000 在 12 核满载下也连跑 3 次全绿**。这里保留 2500/8000 只作
          // 纵深，不是因为需要这么大。⛔ 不要因为「现在余量很大」就把 execArgv 删掉去换预算——
          // 那会把根因放回来。respawn 8000 维持 任务超时 < 退避 这个本场景必需的次序。
