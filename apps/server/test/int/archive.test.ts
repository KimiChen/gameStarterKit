...
test("thawRestore overwrite 契约：计数器反超 hwm 也不回退（评审修正：overwrite 分支保留计数器）", async () => {
  // janitor 门控下 ARCHIVE_NEWER 恒有 hwm > max(hash, counter)（resolve 已含计数器），
  // 该场景走不到——本用例**直测脚本自身契约**：不依赖上游门控，防未来新调用方破不变量
  //（旧实现 overwrite 先 UNLINK 计数器再写 hwm：计数回退、已发号可被复用）。
  ...
  assert.equal(await thawRestore(u, lease.fence, hwm, snap, true), "ok");
  await lease.release();
