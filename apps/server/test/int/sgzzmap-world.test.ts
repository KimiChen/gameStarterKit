...
import { sgzzCellOf, sgzzIsPassable, sgzzNeighbours } from "@game/shared/kits/sgzzmap/api/hexmap/index";
...
test("真库：出生落地 → 连地扩张 → 稀疏行与持地计数一致", async () => {
    ...
    const [rows] = await pool.execute<{ cell: number; owner_uid: string; durability: number }[] & object[]>(
    ...
    const [hold] = await pool.execute<object[]>(
...
test("真库：并发占同一邻环不死锁，revision 严格递增且无重号", async () => {
    const pool = getPool();
    const [before] = await pool.execute<object[]>(
...
test("真库：回执跨事务重放原样返回，⛔ 不再改一次世界", async () => {
    ...
    const [beforeRows] = await pool.execute<object[]>(
    ...
    const [afterRows] = await pool.execute<object[]>(
        "SELECT durability FROM k_sgzzmap_tile WHERE server_id = ? AND cell = ?", [SID, cell]);
...
test("真库：弃地删行、日志带 tombstone、持地计数同步减", async () => {
    ...
    const [rows] = await pool.execute<object[]>(
