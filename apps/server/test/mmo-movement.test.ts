...
test("content v2：灰盒包两职业 + 墙过闸并可索引；classes 缺 / 空 / 重复 / 未知技能 / 速度域、位图长度 / 字符、出生点 / 复活点 / 刷新点落阻挡格一律 ContentPackError 并点名路径", () => {
    ...
    assert.deepEqual([pack.version, pack.maps[0]!.collision?.bitmap.length], [6, 400]);
    ...
    expectError(mutate((_copy, map) => { map.spawnPoints = [{ spawnPointId: "start", pos: { x: 1550, y: 950 } }, map.spawnPoints[1]!]; }), /maps\[0\]\.spawnPoints\[0\]\.pos/u, "出生点落阻挡格（保留东郊门引用的 gate 点）");
