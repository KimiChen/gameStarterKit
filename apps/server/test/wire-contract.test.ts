...
test("C2S/S2C：每个消息都有 exact runtime validator，坏包不进入回调", () => {
  ...
  //  Snake S1 契约增 c2s.snake.input 与 s2c.snake.snapshot → 8/8）。
  ...
  assert.equal(Object.keys(C2S_RUNTIME_VALIDATORS).length, 8);
  assert.equal(Object.keys(S2C_RUNTIME_VALIDATORS).length, 8);
});
