...
import {
  ...
  parseDomainModule,
} from "../tools/plugin-codegen/astReader";
import {
...
const OPTIONAL_INPUT_DIRS = new Set([
  "apps/client/src/plugins",
  "apps/client/src/kits",
  "apps/kits",
]);
const FIXTURE_INPUT_DIRS = [
  ...
  "apps/client/src/logic",
  "apps/client/src/plugins",
  "apps/client/src/kits",
  "apps/client/src/generated",
  VECTORS_DIR,
  "apps/shared/schema/gameplays",
  "apps/shared/src/kits",
  "apps/server/src/kits",
] as const;
...
function createFixture(): {
  ...
  readonly options: PluginCodegenOptions;
} {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "plugin-codegen-"));
  FIXTURE_ROOTS.push(root);
  for (const dir of FIXTURE_INPUT_DIRS) {
    if (
      OPTIONAL_INPUT_DIRS.has(dir) &&
      !fs.existsSync(path.join(REPOSITORY_ROOT, dir))
    )
      continue;
    fs.cpSync(path.join(REPOSITORY_ROOT, dir), path.join(root, dir), {
      recursive: true,
    });
  }
  fs.mkdirSync(path.join(root, "docs"), { recursive: true });
  fs.cpSync(
    path.join(REPOSITORY_ROOT, PLUGIN_INDEX_RELATIVE),
    path.join(root, PLUGIN_INDEX_RELATIVE),
  );
  fs.cpSync(
    ...
    path.join(root, "apps/art/fairygui/assets"),
    {
      recursive: true,
      filter: (source) =>
        fs.statSync(source).isDirectory() || source.endsWith(".xml"),
    },
  );
  return {
    root,
    options: { repositoryRoot: root, skipSchemaDomainCodegen: true },
  };
}

/** schema writer 的真实输入面；用于证明 schema 域在其它门禁失败前绝不写盘。 */
function createSchemaFixture(): {
  ...
  fs.cpSync(
    path.join(REPOSITORY_ROOT, "apps/shared/schema"),
    path.join(fixture.root, "apps/shared/schema"),
    { recursive: true },
  );
  return { root: fixture.root, options: { repositoryRoot: fixture.root } };
}

/** registry 里记录的域 contractVersion —— 域契约闸的「上一次」。 */
function recordedContractVersion(root: string, domain: string): number {
  const registry = fs.readFileSync(path.join(root, REGISTRY_RELATIVE), "utf8");
  const match = new RegExp(
    `^ {4}${domain}: \\{ contractVersion: (\\d+), digest: "[0-9a-f]{64}" \\},$`,
    "mu",
  ).exec(registry);
  assert.ok(match, `registry 未记录域 ${domain}`);
  return Number(match[1]);
}

/** 把 descriptor 里 `domain: "<域>",` 紧跟的 contractVersion 设为指定值（缺行则插入）。 */
function withContractVersion(
  ...
  assert.ok(anchor.test(source), `fixture 域源码缺 domain: "${domain}", 行`);
  return source.replace(anchor, `$1    contractVersion: ${version},\n`);
}

/**
 * 用 fixture 域源码覆盖**已登记**的域文件。域契约闸要求「descriptor 字节变化必须同批递增 contractVersion」，
 * 而 fixture 源码里的版本号是字面量、会随真仓版本漂移（真仓 bump 一次，这些用例就集体变红）。所以覆盖一律走
 * 本助手，按 registry 现有记录自动取「记录值 + 1」；⛔ 不要在 fixture 源码里写死 contractVersion。
 * 覆盖**新**域（registry 无记录，闸放行）不走本助手，直接 fs.writeFileSync。
 */
function writeFixtureDomain(
  root: string,
  domain: string,
  source: string,
): void {
  const bumped = withContractVersion(
    domain,
    source,
    recordedContractVersion(root, domain) + 1,
  );
  fs.writeFileSync(
    path.join(root, LOBBY_RPC_DIR, `domains/${domain}.ts`),
    bumped,
  );
}
...
test("--check 只读：stale/missing/extra 三态失败并点名", () => {
  ...
  const shopSource = fs.readFileSync(
    path.join(root, LOBBY_RPC_DIR, "domains/shop.ts"),
    "utf8",
  );
  ...
  writeFixtureDomain(
    ...
    shopSource.replace(
      ...
      'errorCodes: ["INSUFFICIENT_BALANCE","GRANTING","ORDER_MISMATCH","SHOP_TEST_ONLY"]',
    ),
  );
  ...
  assert.throws(
    () => assertPluginArtifactsFresh(options),
    /missing: .*registry\.generated\.ts/u,
  );
...
test("descriptor 运行时值 ⇔ generated 表双向相等（route/mode/errorCodes/pushes 全覆盖）", async () => {
  ...
  const domains = await Promise.all(
    [...LOBBY_RPC_DOMAINS].sort().map(async (domain) => {
      const module = (await import(
        `../../shared/src/protocol/lobbyRpc/domains/${domain}`
      )) as {
  ...
  assert.deepEqual(
    ...
    [...LOBBY_RPC_DOMAINS].sort(),
  );
  for (const domain of domains) {
    assert.equal(
      LOBBY_RPC_DOMAIN_CONTRACTS[domain.domain]?.contractVersion,
  ...
  for (const domain of domains) {
    for (const route of domain.routes)
      declaredModes.set(route.type, route.mode);
  }
  ...
  for (const type of ALL_LOBBY_RPC_TYPES as readonly LobbyRpcType[]) {
    assert.equal(
      LOBBY_RPC_ROUTE_MODES[type],
      declaredModes.get(type),
      `${type} 的 mode 与 descriptor 不一致`,
    );
  }
  ...
  const declaredCodes = [
    ...CORE_RPC_ERROR_CODES,
    ...domains.flatMap((d) => [...d.errorCodes]),
  ];
  ...
  assert.deepEqual(
    [...RPC_ERR_CODES].slice(roomStart, roomStart + roomCodes.length),
    roomCodes,
  );

  // pushes：key/type/validator 双向
  const declaredPushes = [
    ...CORE_LOBBY_PUSHES,
    ...domains.flatMap((d) => [...d.pushes]),
  ];
  assert.deepEqual(
  ...
  for (const push of declaredPushes) {
    ...
    assert.equal(
      typeof validator,
      "function",
      `${push.type} 缺少生成的 push validator`,
    );
    assert.equal(
      validator.name,
      push.data.name,
      `${push.type} 的生成 validator 必须与 descriptor 引用同名函数`,
    );
  }
  ...
  for (const domain of domains) {
    assert.deepEqual(
      [...domain.ownsOperationGroups],
      [],
      `${domain.domain} 生产域不得声明 ownsOperationGroups`,
    );
    for (const route of domain.routes) {
      assert.equal(
        route.operationGroup ?? undefined,
        undefined,
        `${route.type} 生产路由不得声明 operationGroup`,
      );
      assert.notEqual(
        route.inspectable,
        true,
        `${route.type} 生产路由不得声明 inspectable`,
      );
      assert.equal(
        route.contractVersion ?? undefined,
        undefined,
        `${route.type} 当前不显式声明 contractVersion（缺省 1）`,
      );
    }
  }
  ...
  for (const type of ALL_LOBBY_RPC_TYPES as readonly LobbyRpcType[]) {
    assert.equal(
      LOBBY_RPC_CONTRACT_VERSIONS[type],
      1,
      `${type} 的生成契约版本应缺省 1`,
    );
  }
...
test("domain 形态违规点名拒绝：副作用/let/computed/spread/未知调用/未导出/缺注解/重复路由", () => {
  ...
  assert.throws(
    () =>
      parseDomainModule(
        BASE_DOMAIN.replace('domain: "ping",', '["domain"]: "ping",'),
        "fixture",
      ),
    /computed property/u,
  );
  ...
  assert.throws(
    () =>
      parseDomainModule(
        BASE_DOMAIN.replace("defineRpcQuery(", "defineRpcMystery(").replace(
          ", defineRpcQuery }",
          ", defineRpcMystery }",
        ),
        "fixture",
  ...
  assert.throws(
    () =>
      parseDomainModule(
        BASE_DOMAIN.replace(
          "export const validatePingReq",
          "const validatePingReq",
        ),
        "fixture",
      ),
    /必须导出/u,
...
test("idempotent-write 的 request 接口必须字面含必选 clientReqId: string（AST 层校验反例）", () => {
  ...
  assert.doesNotThrow(() =>
    parseDomainModule(
      build("export interface ICommitReq { clientReqId: string; }"),
      "fixture",
    ),
  );
  assert.throws(
    () =>
      parseDomainModule(
        build("export interface ICommitReq { other: string; }"),
        "fixture",
      ),
  ...
  assert.throws(
    () =>
      parseDomainModule(
        build("export interface ICommitReq { clientReqId: number; }"),
        "fixture",
      ),
    /必须是 string/u,
  );
...
test("coreErrors 解析 + 顺序钉：钉表引用不存在的码必须拒绝", () => {
  ...
  assert.doesNotThrow(() =>
    parseCoreErrorsModule(fs.readFileSync(coreFile, "utf8"), "coreErrors"),
  );
  ...
  fs.writeFileSync(
    shopFile,
    fs
      .readFileSync(shopFile, "utf8")
      .replace(
        'errorCodes: ["INSUFFICIENT_BALANCE","GRANTING","ORDER_MISMATCH"]',
        'errorCodes: ["INSUFFICIENT_BALANCE","GRANTING"]',
      ),
  );
  assert.throws(
    () => readPluginDescriptors(options),
    /RPC_ERR_CODE_ORDER 引用了不属于任何 descriptor 的码：ORDER_MISMATCH/u,
  );
});
...
test("向量 sidecar ⇔ domain 双向对齐：孤儿 sidecar 与缺失 sidecar 都必须 fail-fast", () => {
  ...
  try {
    ...
    fs.writeFileSync(
      path.join(root, VECTORS_DIR, "ghost.ts"),
      FIXTURE_CHAMBER_VECTORS,
    );
    assert.throws(
      () => assertPluginArtifactsFresh(options),
      /lobbyRpcVectors\/ghost\.ts.*没有对应的 domain descriptor/u,
    );
    ...
    assert.throws(
      () => assertPluginArtifactsFresh(options),
      /lobbyRpcVectors\/guild\.ts: missing required file/u,
    );
    // 只读发现函数直接暴露同一判定（供其他工具复用）。
    assert.throws(
      () => readVectorSidecars(root, readPluginDescriptors(options)),
      /guild\.ts/u,
    );
  } finally {
    fs.rmSync(root, { recursive: true, force: true });
...
test("退出条件：新增 fixture domain 只加 domains/chamber.ts + 向量 sidecar，生成 registry/向量表即收录，人工中央源码零改", () => {
  ...
  fs.writeFileSync(
    path.join(root, LOBBY_RPC_DIR, "domains/chamber.ts"),
    FIXTURE_CHAMBER_DOMAIN,
  );
  // 只有 descriptor 没有向量 sidecar：生成器点名缺失（新增域必须同批带最小合法向量），⛔ 不静默。
  assert.throws(
    () => writePluginArtifacts(options),
    /lobbyRpcVectors\/chamber\.ts: missing required file/u,
  );
  fs.writeFileSync(
    path.join(root, VECTORS_DIR, "chamber.ts"),
    FIXTURE_CHAMBER_VECTORS,
  );
  const result = writePluginArtifacts(options);
  assert.deepEqual(
    result.changed,
    [REGISTRY_RELATIVE, VECTORS_INDEX_RELATIVE],
  ...
  const vectorsIndex = fs.readFileSync(
    path.join(root, VECTORS_INDEX_RELATIVE),
    "utf8",
  );
  assert.match(
    vectorsIndex,
    /^import chamberVectors from "\.\/chamber";$/mu,
    "向量登记表静态 import 新 sidecar",
  );
  ...
  assert.match(
    registry,
    /^ {4}\| "chamber\.commit"[;,]?$/mu,
    "LobbyRpcIdemType 显式收录 chamber.commit",
  );
  assert.ok(
    registry.includes('ChamberEvent: "chamber.event",'),
    "LobbyPush 聚合 chamber.event",
  );
  assert.ok(
    registry.indexOf('"INTERNAL",') <
      registry.indexOf('"CHAMBER_TEST_FAILED",'),
    "未上钉的新码追加在历史钉之后",
  );
...
test("阶段 4 fixture：operationGroup/inspectable/inspects/contractVersion 经生成进四张表", () => {
  const { root, options } = createFixture();
  writeFixtureDomain(root, "room", STAGE4_ROOM_DOMAIN);
  writePluginArtifacts(options);
  const registry = fs.readFileSync(path.join(root, REGISTRY_RELATIVE), "utf8");
  assert.match(
    registry,
    /^ {4}"room\.commit": "roomOps",$/mu,
    "LOBBY_RPC_OPERATION_GROUPS 收录写路由",
  );
  const inspectableBlock =
    registry.match(/LOBBY_RPC_INSPECTABLE[^;]+;/u)?.[0] ?? "";
  assert.match(
    inspectableBlock,
    /"room\.commit",/u,
    "LOBBY_RPC_INSPECTABLE 收录写路由",
  );
  const inspectsBlock = registry.match(/LOBBY_RPC_INSPECTS[^;]+;/u)?.[0] ?? "";
  assert.match(
    inspectsBlock,
    /"room\.peek": "roomOps",/u,
    "LOBBY_RPC_INSPECTS 收录查询路由",
  );
  const versionsBlock =
    registry.match(/LOBBY_RPC_CONTRACT_VERSIONS[^;]+;/u)?.[0] ?? "";
  assert.match(
    versionsBlock,
    /"room\.commit": 3,/u,
    "显式 contractVersion 逐字进表",
  );
...
test("operation group 所有权 fail closed：无主/越权/跨域重复/无组可查全部点名拒绝", () => {
  const withRoomDomain = (source: string): PluginCodegenOptions => {
    const { root, options } = createFixture();
    writeFixtureDomain(root, "room", source);
    return options;
  };
  ...
  // 跨域重复所有权 → 重复 id 拒绝清单
  {
    ...
    writeFixtureDomain(root, "room", STAGE4_ROOM_DOMAIN);
    fs.writeFileSync(
      path.join(root, LOBBY_RPC_DIR, "domains/annex.ts"),
      stage4Domain(
        [
          "export default defineLobbyRpcDomain({",
          '    domain: "annex",',
...
test("跨域 inspects：缺 exposesOperationGroupTo 即 fail closed，显式暴露后放行", () => {
  ...
  // owner 显式暴露：放行且 generated 表收录
  {
    ...
    fs.writeFileSync(
      path.join(root, VECTORS_DIR, "annex.ts"),
      FIXTURE_ANNEX_VECTORS,
    );
    writePluginArtifacts(options);
    ...
    assert.match(
      registry,
      /^ {4}"annex\.peek": "roomOps",$/mu,
      "获准跨域查询的路由进 LOBBY_RPC_INSPECTS",
    );
  }
});

test("contractVersion AST 校验：非数字字面量 / 0 / 小数一律点名拒绝", () => {
  const build = (literal: string): string =>
    stage4Domain(
      [
        "export default defineLobbyRpcDomain({",
        '    domain: "room",',
  ...
  assert.throws(
    () => parseDomainModule(build('"2"'), "fixture"),
    /必须是数字字面量/u,
  );
  assert.throws(
    () => parseDomainModule(build("0"), "fixture"),
    /≥1 的安全整数/u,
  );
  assert.throws(
    () => parseDomainModule(build("1.5"), "fixture"),
    /≥1 的安全整数/u,
  );
});
...
test("删除保护：域源文件消失必须显式 --allow-delete，放行后 registry 同步收缩", () => {
  ...
  assert.ok(
    !fs
      .readFileSync(path.join(root, VECTORS_INDEX_RELATIVE), "utf8")
      .includes("guildVectors"),
    "向量登记表必须随域删除收缩",
  );
  const registry = fs.readFileSync(path.join(root, REGISTRY_RELATIVE), "utf8");
...
test("渲染确定性：相同输入重复渲染字节相同（registry + 客户端三件）", () => {
  ...
  const second = renderPluginArtifacts(
    readPluginDescriptors(),
    readViewCatalog(REPOSITORY_ROOT),
  );
...
function addFixtureView(root: string): void {
  ...
  fs.writeFileSync(
    path.join(root, "apps/client/src/view/FxView.ts"),
    "export class FxView {}\n",
  );
  fs.writeFileSync(
  ...
  fs.writeFileSync(
    path.join(root, "apps/client/src/logic/page/FxLogic.ts"),
    "export class FxLogic {}\n",
  );
...
function addFixtureCocosView(
  ...
  fs.writeFileSync(
    path.join(root, "apps/client/src/view/CxView.ts"),
    "export class CxView {}\n",
  );
  fs.writeFileSync(
    ...
    `${JSON.stringify(
      {
        ...
        ...(options.restore === false
          ? {}
          : { group: "authenticated", restore: "reopen" }),
      },
      null,
      2,
    )}\n`,
  ...
  fs.writeFileSync(
    path.join(root, "apps/client/src/logic/page/CxLogic.ts"),
    "export class CxLogic {}\n",
  );
  const manifestFile = path.join(root, "apps/plugins/builtin/plugin.json");
  const manifest = JSON.parse(fs.readFileSync(manifestFile, "utf8"));
...
test("cocos View 进 ViewMgr catalog 的判别信号 = 被 plugin routes 引用（⛔ 无标记字段）", () => {
  {
    ...
    assert.match(
      views,
      /\{ name: "Cx", owner: "builtin", kind: "cocos", /u,
  ...
  }
  {
    ...
    assert.throws(
      ...
      /必须在 sidecar 声明 group 与 restore/u,
    );
  }
});
...
test("checked-in client view artifacts are fresh（客户端 freshness 由 server 侧断言，§5.4 口径）", () => {
  ...
  for (const relative of CLIENT_ARTIFACTS) {
    assert.ok(
      fs.existsSync(path.join(REPOSITORY_ROOT, relative)),
      `${relative} 缺失`,
    );
  }
  assertPluginArtifactsFresh();
...
test("阶段 6 退出条件：fixture view 只新增文件 + plugin.json 登记，三产物收录且既有条目字节不动", () => {
  ...
  const contractsBefore = fs.readFileSync(
    path.join(root, FGUI_CONTRACTS_RELATIVE),
    "utf8",
  );
  addFixtureView(root);
  ...
  assert.deepEqual(
    result.changed,
    [FGUI_CONTRACTS_RELATIVE, VIEWS_RELATIVE],
  ...
  assert.ok(
    views.includes(
      'load: () => import("../view/FxView").then((m) => m.FxView),',
    ),
    "load 必须是生成的字面量动态 import（铁律 10）",
  );
  assert.match(
    views,
    /^ {4}\{ name: "Fx", owner: "builtin", kind: "fgui", pkg: "View_Fixture_Fx", comp: "Fx", /mu,
    "VIEW_SOURCE_RECORDS 收录 Fx",
  );
  for (const line of viewsBefore
    .split("\n")
  ...
  assert.ok(
    contracts.includes('"name": "txt_fx"'),
    "required 必须从 XML 按 binding 规则算出",
  );
  assert.ok(
    contracts.includes('"name": "btn_go"'),
    "required 必须包含带前缀 component 元素",
  );
  for (const line of contractsBefore
    .split("\n")
...
test("阶段 6 --check 三态：sidecar 变更 → stale 点名客户端产物；删/多余 generated 也红", () => {
  const { root, options } = createFixture();
  assertPluginArtifactsFresh(options);
  const homeSidecar = path.join(
    root,
    "apps/client/src/view/HomeView.view.json",
  );
  ...
  assert.throws(
    () => assertPluginArtifactsFresh(options),
    /stale: .*views\.generated\.ts/u,
  );
  assert.equal(
    fs.readFileSync(viewsFile, "utf8"),
    viewsBytes,
    "--check 不得改写生成物",
  );
  ...
  assert.throws(
    () => assertPluginArtifactsFresh(options),
    /missing: .*fguiContracts\.generated\.ts/u,
  );
  writePluginArtifacts(options);
...
test("sidecar⇔View 双向：未登记的 *View.ts 红；登记的 sidecar 缺 View/logic 也红", () => {
  {
    const { root } = createFixture();
    fs.writeFileSync(
      path.join(root, "apps/client/src/view/OrphanView.ts"),
  ...
  }
  {
    ...
    assert.throws(
      () => readViewCatalog(root),
      /FxView\.view\.json → apps\/client\/src\/view\/FxView\.ts: missing required file/u,
    );
  }
  {
    const { root } = createFixture();
    ...
    assert.throws(
      () => readViewCatalog(root),
      /FxLogic\.ts: missing required file/u,
    );
  }
  {
    const { root } = createFixture();
    ...
    fs.writeFileSync(
      path.join(root, "apps/client/src/view/FxView.view.json"),
      fixtureSidecar({
        logic: "apps/client/src/logic/rooms/ballMove/BallMoveGameplay.ts",
      }),
    );
...
test("sharedPkgs ⊇ 闭包 fail-fast：漏声明依赖包点名拒绝（art 闭包与 assetUrls 所属包同口径）", () => {
  ...
  assert.throws(
    () => readViewCatalog(root),
    /HomeView\.view\.json: sharedPkgs 缺依赖包 \["Common_Btn"\]/u,
  );
});
...
test("aliasOf 迁移期兼容：无 alias 的重复 package\\/component 必败；显式 aliasOf 放行且不产重复所有权", () => {
  const makeAliasFixture = (aliasOf?: string): string => {
    const { root } = createFixture();
    fs.writeFileSync(
      path.join(root, "apps/client/src/view/HomeNextView.ts"),
      "export class HomeNextView {}\n",
    );
    fs.writeFileSync(
  ...
  assert.throws(
    () => readViewCatalog(makeAliasFixture("HomeNext")),
    /aliasOf 不得指向自身|aliasOf 必须指向同组件的 canonical View/u,
  );
  ...
  assert.ok(
    names.includes("Home") && names.includes("HomeNext"),
    "alias 条目与 canonical 同时在册",
  );
  const result = writePluginArtifacts({
    repositoryRoot: root,
    skipSchemaDomainCodegen: true,
  });
  assert.deepEqual(result.changed, [FGUI_CONTRACTS_RELATIVE, VIEWS_RELATIVE]);
  const views = fs.readFileSync(path.join(root, VIEWS_RELATIVE), "utf8");
  assert.match(
    views,
    /^ {4}HomeNext: defineView\(\{$/mu,
    "alias 条目仍是独立 catalog 键（不抢占 canonical）",
  );
  assertPluginArtifactsFresh({
    repositoryRoot: root,
    skipSchemaDomainCodegen: true,
...
test("阶段 6 删除保护：View 真源消失必须显式 --allow-delete，放行后三产物同步收缩", () => {
  ...
  const manifestFile = path.join(root, "apps/plugins/builtin/plugin.json");
  const manifest = JSON.parse(fs.readFileSync(manifestFile, "utf8"));
  manifest.views = manifest.views.filter(
    (entry: string) => !entry.endsWith("FxView.view.json"),
  );
  fs.writeFileSync(manifestFile, `${JSON.stringify(manifest, null, 2)}\n`);
  ...
  assert.ok(
    !fs
      .readFileSync(path.join(root, FGUI_CONTRACTS_RELATIVE), "utf8")
      .includes("FX_CONTRACT"),
  );
  assertPluginArtifactsFresh(options);
});
...
test("入口治理闸：entryId 全仓唯一、一 gameplayId 一贡献者、route 形态 launch 必须引用已登记 route", () => {
  const withSnakeMenu = (
    mutate: (menu: Record<string, unknown>[]) => void,
  ): string => {
    const { root } = createFixture();
      ...
      readViewCatalog(
        withSnakeMenu((menu) =>
          menu.push({
            ...
            label: "x",
            labelKey: "menu.x",
            launch: { kind: "gameplay", gameplayId: "idle" },
          }),
        ),
      ),
      ...
      readViewCatalog(
        withSnakeMenu((menu) =>
          menu.push({
            ...
            label: "x",
            labelKey: "menu.x",
            launch: { kind: "route", routeId: "nowhere" },
          }),
        ),
      ),
    /launch 引用未登记的 route "nowhere"/u,
  );
  const root = withSnakeMenu((menu) =>
    menu.push({
      entryId: "snakePanel",
      label: "面板",
      labelKey: "menu.snakePanel",
      launch: { kind: "route", routeId: "settings" },
    }),
  );
  const catalog = readViewCatalog(root);
  const rendered =
    renderViewCatalogArtifacts(catalog).get(PLUGINS_RELATIVE) ?? "";
  assert.match(
    rendered,
    /entryId: "snakePanel".*launch: \{ kind: "route", routeId: "settings" \}/u,
  );
  ...
  assert.throws(
    () =>
      readViewCatalog(
        withSnakeMenu((menu) => {
          menu[0].launch = { kind: "route", gameplayId: "snake" };
        }),
      ),
  ...
  assert.throws(
    () =>
      readViewCatalog(
        withSnakeMenu((menu) => {
          menu[0].launch = { kind: "gameplay", gameplayId: "privateFixture" };
        }),
      ),
  ...
  assert.throws(
    () =>
      readViewCatalog(
        withSnakeMenu((menu) => {
          menu[0].launch = { kind: "gameplay", gameplayId: "snak" };
        }),
      ),
    /引用未登记的玩法 "snak"/u,
  );
...
test("plugin entry：entry 必须是本 plugin 的 apps/client/src/plugins/<id>/index.ts 且导出 createPluginModule，渲染为静态字面量 load", () => {
  ...
  ): { root: string; options: PluginCodegenOptions } => {
    const { root, options } = createFixture();
    const manifestFile = path.join(root, "apps/plugins/snake/plugin.json");
    const manifest = JSON.parse(fs.readFileSync(manifestFile, "utf8"));
  ...
  const writeModule = (root: string, id: string, source: string): void => {
    fs.mkdirSync(path.join(root, `apps/client/src/plugins/${id}`), {
      recursive: true,
    });
    fs.writeFileSync(
      path.join(root, `apps/client/src/plugins/${id}/index.ts`),
      source,
      "utf8",
    );
  };
  ...
  assert.throws(
    () =>
      readViewCatalog(
        withModule((root, manifest) => {
          writeModule(
            root,
            "other",
            "export function createPluginModule() { return {}; }\n",
          );
    ...
    () =>
      readViewCatalog(
        withModule((root, manifest) => {
          ...
          fs.rmSync(path.join(root, "apps/client/src/plugins/snake/index.ts"), {
            force: true,
          });
        }).root,
      ),
  ...
  assert.throws(
    () =>
      readViewCatalog(
        withModule((_root, manifest) => {
          manifest.entry = "apps/client/src/logic/page/HomeLogic.ts";
        }).root,
      ),
  ...
  const { root, options } = withModule((root, manifest) => {
    writeModule(
      root,
      "snake",
      "export function createPluginModule(): { install(): void } {\n  return { install() {} };\n}\n",
    );
    manifest.entry = "apps/client/src/plugins/snake/index.ts";
  });
  writePluginArtifacts(options);
  ...
  assert.match(
    rendered,
    /^import type \{ PluginModule \} from "\.\.\/app\/PluginHost";$/mu,
  );
  ...
  assert.doesNotMatch(
    rendered,
    /id: "builtin",\n {8}resident: true,\n {8}load:/u,
    "无 entry 的 plugin ⛔ 不带 load",
  );
  // 真仓：有 entry 就必须带上类型 import，一个 entry 都没有时 ⛔ 不引入未使用的类型（noUnusedLocals）。
  const real = fs.readFileSync(
    path.join(REPOSITORY_ROOT, PLUGINS_RELATIVE),
    "utf8",
  );
  if (real.includes("load: () => import("))
    assert.match(real, /import type \{ PluginModule \}/u);
...
test("宿主 placement（apps/plugins/host.json）：缺失即 fail-fast；defaultLaunch 须有贡献者；home 引用必须存在且渲染进 GENERATED_HOST", () => {
  {
    ...
    assert.throws(
      () => readViewCatalog(root),
      /plugins\/host\.json.*宿主必须显式声明 defaultLaunch/u,
    );
  }
  const withHost = (host: unknown): string => {
    const { root } = createFixture();
    fs.writeFileSync(
      path.join(root, "apps/plugins/host.json"),
  ...
  assert.throws(
    () =>
      readViewCatalog(
        withHost({
          schemaVersion: 1,
          defaultLaunch: { kind: "gameplay", gameplayId: "idle" },
          home: [],
        }),
      ),
  ...
  assert.throws(
    () =>
      readViewCatalog(
        withHost({
          schemaVersion: 1,
          defaultLaunch: { kind: "gameplay", gameplayId: "dropInFixture" },
          home: [],
        }),
      ),
  ...
  assert.throws(
    () =>
      readViewCatalog(
        withHost({
          schemaVersion: 1,
          defaultLaunch: { kind: "gameplay", gameplayId: "nope" },
          home: [],
        }),
      ),
  ...
  const root = withHost({
    schemaVersion: 1,
    defaultLaunch: { kind: "gameplay", gameplayId: "ballMove" },
    home: ["builtin/ballMove", "snake/snake"],
  });
  const rendered =
    renderViewCatalogArtifacts(readViewCatalog(root)).get(PLUGINS_RELATIVE) ??
    "";
  assert.match(
    rendered,
    /defaultLaunch: \{ kind: "gameplay", gameplayId: "ballMove" \}/u,
  );
  assert.match(
    rendered,
    /\{ pluginId: "builtin", entryId: "ballMove" \},\n {8}\{ pluginId: "snake", entryId: "snake" \},/u,
  );
});
...
test("宿主分组（host.json groups）：成员必须存在、⛔ 不得跨组重复、id ⛔ 不得与入口/单元撞名，且渲染进 GENERATED_HOST", () => {
  ...
  const group = (over: Record<string, unknown> = {}): unknown => ({
    id: "arenaHub",
    label: "竞技场",
    labelKey: "menu.group.arena",
    members: ["arena/board", "arenaShop/arenaShop"],
    ...over,
  });
  ...
  assert.throws(
    () =>
      readViewCatalog(
        withHost({
          ...base,
          groups: [group({ members: ["arena/board", "arena/nope"] })],
        }),
      ),
  ...
  assert.throws(
    () =>
      readViewCatalog(
        withHost({ ...base, groups: [group({ members: ["arena/board"] })] }),
      ),
    /至少要收 2 条入口/u,
  );
  assert.throws(
    () =>
      readViewCatalog(
        withHost({ ...base, groups: [group(), group({ id: "arenaHub2" })] }),
      ),
    ...
    () =>
      readViewCatalog(
        withHost({
          ...
          groups: [
            group(),
            group({ id: "arenaHub", members: ["arena/capture", "arena/duel"] }),
          ],
        }),
      ),
  ...
  assert.throws(
    () =>
      readViewCatalog(
        withHost({ ...base, groups: [group({ id: "builtin" })] }),
      ),
  ...
  const ok = withHost({
    ...base,
    groups: [
      group({ members: ["arenaShop/arenaShop", "arena/duel", "arena/board"] }),
    ],
  });
  const rendered =
    renderViewCatalogArtifacts(readViewCatalog(ok)).get(PLUGINS_RELATIVE) ?? "";
  assert.match(
    rendered,
    /id: "arenaHub", label: "竞技场", labelKey: "menu\.group\.arena", members: \[/u,
  );
  assert.match(
    rendered,
  ...
  const none =
    renderViewCatalogArtifacts(readViewCatalog(withHost(base))).get(
      PLUGINS_RELATIVE,
    ) ?? "";
...
test("域契约闸：descriptor 字节变化而 contractVersion 未增，writer 与只读闸都拒绝；bump 后放行并渲染 LOBBY_RPC_DOMAIN_CONTRACTS", () => {
  ...
  try {
    ...
    // 版本号从 registry 推，⛔ 不写死：真仓 bump 一次，写死版本号的用例就集体变红。
    const current = recordedContractVersion(root, "guild");
    // 只改一个注释也算：digest 与 gameplay 的 wire.ts 同口径按字节算（⛔ 不做语义豁免）。
    fs.writeFileSync(guildFile, `${original}// touched\n`, "utf8");
    const gate = new RegExp(
      `domains/guild\\.ts: domain contract digest changed but contractVersion did not increase ` +
        `\\(kept ${current}, previous ${current}\\)`,
      "u",
    );
    ...
    fs.writeFileSync(
      guildFile,
      `${withContractVersion("guild", original, current + 1)}// touched\n`,
      "utf8",
    );
    const result = writePluginArtifacts(options);
    ...
    assert.match(
      registry,
      new RegExp(
        `^ {4}guild: \\{ contractVersion: ${current + 1}, digest: "[0-9a-f]{64}" \\},$`,
        "mu",
      ),
    );
    ...
    fs.writeFileSync(
      guildFile,
      `${withContractVersion("guild", original, 1)}// touched twice\n`,
      "utf8",
    );
    assert.throws(
      () => assertPluginArtifactsFresh(options),
      new RegExp(`kept 1, previous ${current + 1}`, "u"),
    );
    // 非法 contractVersion 形态在 AST 层点名。
    const withLiteral = (literal: string): string =>
      original.replace(
        /( {4}domain: "guild",\n)(?: {4}contractVersion: \d+,\n)?/u,
        `$1    contractVersion: ${literal},\n`,
      );
    fs.writeFileSync(guildFile, withLiteral("0"), "utf8");
    assert.throws(
      () => readPluginDescriptors(options),
      /域 contractVersion 必须是 ≥1 的安全整数/u,
    );
    fs.writeFileSync(guildFile, withLiteral('"2"'), "utf8");
    assert.throws(
      () => readPluginDescriptors(options),
      /域 contractVersion 必须是数字字面量/u,
    );
  } finally {
    fs.rmSync(root, { recursive: true, force: true });
  }
...
test("schema 域在 contractVersion 闸失败时不留下半写生成物", () => {
  const { root, options } = createSchemaFixture();
  const incomeDomain = path.join(root, LOBBY_RPC_DIR, "domains/income.ts");
  const registry = path.join(root, REGISTRY_RELATIVE);
  const beforeDomain = fs.readFileSync(incomeDomain, "utf8");
  const beforeRegistry = fs.readFileSync(registry, "utf8");
  const schemaFile = path.join(
    root,
    "apps/shared/schema/protocols/C2S/income.json",
  );
  const schema = JSON.parse(fs.readFileSync(schemaFile, "utf8")) as {
    types: {
      IIncomeClaimOfflineRes: { fields: Array<{ name: string; min?: number }> };
    };
  };
  schema.types.IIncomeClaimOfflineRes.fields.find(
    (field) => field.name === "copper",
  )!.min = 1;
  fs.writeFileSync(schemaFile, `${JSON.stringify(schema, null, 2)}\n`, "utf8");

  assert.throws(
    () => writePluginArtifacts(options),
    /domain contract digest changed but contractVersion did not increase/u,
  );
  assert.equal(
    fs.readFileSync(incomeDomain, "utf8"),
    beforeDomain,
    "失败不得先写 schema 域生成物",
  );
  assert.equal(
...
test("CLI 沿用惯例：--check、--root <dir>/--root=<dir>、--allow-delete；重复/未知参数 throw", () => {
  ...
  assert.deepEqual(parseCli(["--root", "/tmp/x"]), {
    check: false,
  ...
  assert.deepEqual(parseCli(["--allow-delete", "guild"]), {
    check: false,
    allowDelete: ["guild"],
  });
  assert.deepEqual(parseCli(["--allow-delete=guild"]), {
  ...
  assert.throws(
    () => parseCli(["--allow-delete", "guild", "--allow-delete", "guild"]),
    /duplicate/u,
  );
  assert.throws(
    () => parseCli(["--check", "--allow-delete", "guild"]),
    /read-only/u,
  );
...
test("阶段 7：fixture extra plugin 只加新文件即进索引；fragment 状态按 defaultEntry 存在性判定", () => {
  ...
  assert.match(
    index,
    /^\| `fixtureExtra` \| plugin \| extra \| registered \| /mu,
    "plugin 行收录",
  );
  assert.match(
    index,
...
test("⛔ 生成器不写当前计划文件：把 docs/plan-v5.md 加进 writer 允许输出集合 → 自检红（§5.7 反例）", () => {
  ...
  assert.throws(
    () => assertWriterOutputSetSafe(["docs/other.generated.md"]),
    /不在生成器允许输出集合内/u,
  );
  assert.throws(
    () => assertWriterOutputSetSafe(["apps/shared/src/protocol/rooms.ts"]),
    /不在生成器允许输出集合内/u,
  );
});
...
const KFIX_KIT_JSON = {
  ...
  sql: {
    files: ["sql/001-init.sql"],
    tables: [{ name: "k_kfix_board", zone: "per-zone" }],
  },
  userKeys: ["board"],
  ...
  menu: [
    {
      entryId: "kfixBoard",
      label: "棋盘",
      labelKey: "menu.kfixBoard",
      launch: { kind: "route", routeId: "kfixBoard" },
    },
  ],
};
...
function fixtureCollector(): {
  readonly create: () => ReturnType<typeof createFixture>;
...
function addFixtureKit(
  ...
  writeText(
    root,
    "apps/client/src/kits/kfix/view/KfixBoardView.ts",
    "export class KfixBoardView {}\n",
  );
  writeJson(root, "apps/client/src/kits/kfix/view/KfixBoardView.view.json", {
    ...
    permanent: false,
    interactive: false,
    logic: "apps/client/src/kits/kfix/logic/KfixBoardLogic.ts",
    group: "authenticated",
    restore: "discard",
  });
  writeText(
    root,
    "apps/client/src/kits/kfix/logic/KfixBoardLogic.ts",
    "export class KfixBoardLogic {}\n",
  );
  writeText(root, `${LOBBY_RPC_DIR}/domains/kfix.ts`, FIXTURE_KFIX_DOMAIN);
  writeText(root, `${VECTORS_DIR}/kfix.ts`, FIXTURE_KFIX_VECTORS);
}

function addFixtureKitShop(
  root: string,
  pluginJson: Record<string, unknown> = KFIX_SHOP_PLUGIN_JSON,
): void {
  writeJson(root, "apps/plugins/kfixShop/plugin.json", pluginJson);
  writeText(
    root,
    "apps/client/src/plugins/kfixShop/index.ts",
...
test("K0 kit：真仓零 kit 时双端 kit catalog 与占位生成物字节相同且在盘新鲜", () => {
  ...
  for (const relative of KIT_ARTIFACTS) {
    assert.equal(
      fs.readFileSync(path.join(REPOSITORY_ROOT, relative), "utf8"),
  ...
  if (!fs.existsSync(path.join(REPOSITORY_ROOT, "apps/kits"))) {
    assert.match(
      rendered.get(KIT_CATALOG_SHARED_RELATIVE) ?? "",
      /^export const KIT_CATALOG: readonly KitCatalogEntry\[\] = \[\];$/mu,
    );
    assert.match(
      rendered.get(KIT_CATALOG_SHARED_RELATIVE) ?? "",
      /^export const KIT_EFFECT_KINDS: Readonly<Record<string, KitEffectSpec>> = \{\};$/mu,
    );
    assert.match(
      rendered.get(KIT_CATALOG_SERVER_RELATIVE) ?? "",
      /^export const SERVER_KIT_CATALOG: readonly ServerKitCatalogEntry\[\] = \[\];$/mu,
    );
  }
});
...
test("K0 kit：fixture kit + 建在其上的插件只加新文件 → 双端 kit catalog 收录、requires.kits 并入 dependencies（kit 先）、索引 class 列；手写源零 diff", () => {
  ...
  try {
    ...
    assert.deepEqual(
      shop?.dependencies,
      ["kfix", "redeem"],
      "有效依赖 = required kit（有 entry）先 ++ 声明依赖，去重",
    );
    // 真仓新增 kit 不得要求改写本 fixture 的期望基线。
    assert.deepEqual(
      catalog.plugins.map((unit) => unit.id),
      [...existingIds, "kfix", "kfixShop"].sort(),
    );
    ...
    for (const relative of [
      ...
      assert.ok(
        result.changed.includes(relative),
        `${relative} 应随 fixture kit 变化`,
      );
    }
    const after = snapshotHandwritten(root);
    for (const [key, bytes] of before)
    ...
    const shared = fs.readFileSync(
      path.join(root, KIT_CATALOG_SHARED_RELATIVE),
      "utf8",
    );
    assert.match(
      shared,
      /^\/\*\* AUTO-GENERATED by apps\/server\/tools\/plugin-codegen\/cli\.ts from apps\/kits\/<id>\/kit\.json\. Do not edit\. \*\/$/mu,
    );
    assert.match(
      shared,
    ...
    // 真仓 arena 在 kfix 前、slg 在其后；夹具条目不再假定自己是数组末项。
    assert.match(shared, /^ {8}id: "slg",$/mu);
    assert.match(
      shared,
      /^export const KIT_EFFECT_KINDS: Readonly<Record<string, KitEffectSpec>> = \{\n(?: {4}"kit:arena:[a-z]+": \{[^\n]*\},\n)* {4}"kit:kfix:bump": \{ kitId: "kfix", name: "bump", userKey: "board", field: "score", max: 10 \},\n {4}"kit:slg:trophy": \{ kitId: "slg", name: "trophy", userKey: "stats", field: "trophies", max: 1000000 \},\n\};$/mu,
    );
    const server = fs.readFileSync(
      path.join(root, KIT_CATALOG_SERVER_RELATIVE),
      "utf8",
    );
    assert.match(
      server,
    ...
    assert.match(
      plugins,
      /^ {4}"kfix",\n {4}"kfixShop",/mu,
      "kit 与插件同进 PLUGIN_IDS（class 对 PluginHost 不可见）",
    );
    assert.match(
      plugins,
      /id: "kfix",\n {8}resident: false,\n {8}load: \(\) => import\("\.\.\/kits\/kfix\/index"\)\.then\(\(m\) => m\.createPluginModule\(\)\),/u,
      "kit entry 渲染为 kits/ 命名空间的静态字面量 load",
    );
    assert.match(
      plugins,
    ...
    assert.match(
      plugins,
      /entryId: "kfixBoard", pluginId: "kfix"/u,
      "kit 的菜单贡献照常进 contribution",
    );
    ...
    assert.match(
      index,
      /^\| `kfixShop` \| plugin \| extra \| registered \| /mu,
    );
    assertPluginArtifactsFresh(options);
    // 渲染确定性：相同输入两次渲染字节相同。
...
test("K0 kit：requires.kits 闸——kit 未安装 / api 面不存在 / 版本低于 minSupported / 高于 version 全部点名拒绝；dependencies 直接点名 kit 也拒绝（唯一通道）", () => {
  ...
  try {
    const withRequires = (
      kits: Record<string, Record<string, number>>,
      kitApi?: Record<string, { version: number; minSupported: number }>,
    ): string => {
      ...
      addFixtureKit(
        ...
        kitApi === undefined
          ? KFIX_KIT_JSON
          : { ...KFIX_KIT_JSON, api: kitApi },
      );
      addFixtureKitShop(root, { ...KFIX_SHOP_PLUGIN_JSON, requires: { kits } });
      return root;
    };
    assert.throws(
      () => readViewCatalog(withRequires({ ghost: { board: 1 } })),
      /插件 kfixShop 需要 kit ghost，但该 kit 未安装/u,
    );
    assert.throws(
      () => readViewCatalog(withRequires({ kfix: { ranking: 1 } })),
      /插件 kfixShop 需要 kit kfix 的 api 面 ranking 版本 1，宿主 kit 没有该 api 面（已有：board）/u,
    );
    assert.throws(
      () =>
        readViewCatalog(
          withRequires(
            { kfix: { board: 1 } },
            { board: { version: 2, minSupported: 2 } },
          ),
        ),
      /插件 kfixShop 需要 kit kfix 的 api 面 board 版本 1，宿主 kit 提供 \[2, 2\]/u,
    );
    assert.throws(
      () => readViewCatalog(withRequires({ kfix: { board: 3 } })),
      /插件 kfixShop 需要 kit kfix 的 api 面 board 版本 3，宿主 kit 提供 \[1, 2\]/u,
    );
    // 边界：声明 == version 与 == minSupported 都放行。
    assert.doesNotThrow(() =>
      readViewCatalog(withRequires({ kfix: { board: 2 } })),
    );
    ...
    const shop = readViewCatalog(root).plugins.find(
      (unit) => unit.id === "kfixShop",
    );
    assert.deepEqual(
      shop?.dependencies,
      ["redeem"],
      "无 client entry 的 required kit 不并入 dependencies",
    );
    addFixtureKitShop(root, {
      ...KFIX_SHOP_PLUGIN_JSON,
      requires: { kits: { kfix: { ranking: 1 } } },
      dependencies: [],
    });
    assert.throws(
      () => readViewCatalog(root),
      /没有该 api 面/u,
      "无 entry 的 kit 的 api 面闸照判",
    );
    ...
    // 不管有没有同时写 requires.kits 都拒绝（⛔ 不写两遍）。
    {
      ...
      assert.throws(
        () => readViewCatalog(bypass),
        /插件 kfixShop 的 dependencies 直接引用 kit "kfix"——对 kit 的依赖只能经 requires\.kits 声明/u,
      );
      addFixtureKitShop(bypass, {
        ...KFIX_SHOP_PLUGIN_JSON,
        dependencies: ["redeem", "kfix"],
      });
      assert.throws(
        () => readViewCatalog(bypass),
        /插件 kfixShop 的 dependencies 直接引用 kit "kfix"/u,
        "写两遍同样拒绝",
      );
      addFixtureKitShop(bypass, KFIX_SHOP_PLUGIN_JSON);
      assert.deepEqual(
        readViewCatalog(bypass).plugins.find((unit) => unit.id === "kfixShop")
          ?.dependencies,
        ["kfix", "redeem"],
        "只经 requires.kits 声明 → 自动并入",
      );
    }
  } finally {
    fixtures.dispose();
...
test("K0 kit：无 entry/route/menu 的纯库 kit 只进 KIT_CATALOG，⛔ 不进 PLUGIN_IDS", () => {
  ...
  try {
    ...
    assert.doesNotMatch(
      plugins,
      /^ {4}"kfix",$/mu,
      "纯库 kit 不是 PluginHost 单元",
    );
    assert.doesNotMatch(plugins, /id: "kfix",/u);
    assert.match(
      fs.readFileSync(path.join(root, KIT_CATALOG_SHARED_RELATIVE), "utf8"),
      /^ {8}id: "kfix",$/mu,
    );
    assert.match(
      fs.readFileSync(path.join(root, PLUGIN_INDEX_RELATIVE), "utf8"),
      /^\| `kfix` \| kit \| /mu,
    );
    assertPluginArtifactsFresh(options);
  } finally {
  ...
  try {
    {
      const { root } = fixtures.create();
      addFixtureKit(root);
      writeJson(root, "apps/kits/kfix/gameplays/kfixExtra/manifest.json", {
        schemaVersion: 1,
        id: "kfixExtra",
        constantName: "KfixExtra",
        modeVersion: 1,
        maxPlayers: 2,
        profiles: [],
      });
      assert.throws(
        () => readViewCatalog(root),
        /gameplays\/kfixExtra\/ 存在但 modes\[\] 未登记该 mode/u,
      );
    }
    {
      const { root } = fixtures.create();
      addFixtureKit(root);
      fs.rmSync(path.join(root, "apps/kits/kfix/gameplays/kfixArena"), {
        recursive: true,
      });
      assert.throws(
        () => readViewCatalog(root),
        /modes\[\] 登记了 "kfixArena" 但 apps\/kits\/kfix\/gameplays\/kfixArena\/ 不存在/u,
      );
    }
    {
      const { root } = fixtures.create();
      ...
      mutateJson(
        root,
        "apps/kits/kfix/gameplays/kfixArena/manifest.json",
        (manifest) => {
          manifest.constantName = "Arena";
        },
      );
      assert.throws(
        () => readViewCatalog(root),
        /mode "kfixArena" 的 constantName "KfixArena" 与 apps\/kits\/kfix\/gameplays\/kfixArena\/manifest\.json 的 constantName "Arena" 不一致/u,
      );
    }
    {
      const { root } = fixtures.create();
      ...
      assert.throws(
        () => readViewCatalog(root),
        /manifest\.id（"arena"）必须等于 mode 目录名 "kfixArena"/u,
      );
    }
  } finally {
    fixtures.dispose();
...
test("K0 kit：域名前缀规则 (i) 一域一主 (ii) 边界前缀 + 最长前缀单元 + descriptor 存在 (iii) 未声明域不得占带版本单元前缀；真仓与宿主自有单元豁免", () => {
  const descriptorDomains = (root: string): readonly string[] =>
    readPluginDescriptors({ repositoryRoot: root }).domains.map(
      (domain) => domain.domain,
    );
  // 真仓：guild/mail/room/shop/user 未声明（都不是任何带版本单元的边界前缀），snakeCosmetic 由带版本的 snake 声明（衣柜已并入），redeem 由带版本的 redeem 声明。
  assert.doesNotThrow(() =>
    assertDomainOwnership(
      descriptorDomains(REPOSITORY_ROOT),
      readViewCatalog(REPOSITORY_ROOT).plugins,
    ),
  );
  const fixtures = fixtureCollector();
  try {
    ...
    assert.throws(
      check((root) =>
        mutateJson(root, "apps/plugins/tally/plugin.json", (m) => {
          m.domains = ["kfix"];
        }),
      ),
    ...
    assert.throws(
      check((root) =>
        mutateJson(root, "apps/plugins/redeem/plugin.json", (m) => {
          m.domains = ["redeem", "redeemx"];
        }),
      ),
    ...
    assert.throws(
      check((root) => {
        mutateJson(root, "apps/plugins/arenaShop/plugin.json", (m) => {
          m.domains = [];
        });
        mutateJson(root, "apps/kits/arena/kit.json", (m) => {
          m.domains = ["arena", "arenaShop"];
        });
      }),
    ...
    assert.throws(
      check((root) =>
        mutateJson(root, "apps/kits/kfix/kit.json", (m) => {
          m.domains = ["kfix", "kfixAdmin"];
        }),
      ),
  ...
  ): ReturnType<typeof readViewCatalog>["plugins"][number] => ({
    class: "plugin",
    schemaVersion: 2,
    id,
    version,
    domains,
    requires: { pluginApiVersion: null, kits: {} },
    contributes: {},
    category: "extra",
    docs: [],
    capabilities: [],
  ...
  assert.throws(
    ...
    /域 "slgAdmin" 未被任何单元声明，却等于或以带版本的 plugin "slg" 为前缀/u,
  );
  assert.doesNotThrow(
    () =>
      assertDomainOwnership(["slg", "slgx"], [unit("slg", "1.0.0", ["slg"])]),
    "slgx 不是 slg 的边界前缀形态，不算占用",
  );
  ...
  assert.throws(
    () =>
      assertDomainOwnership(
        ["slgAdminOps"],
        [unit("slg", null, ["slgAdminOps"]), unit("slgadmin", "1.0.0", [])],
      ),
    /域 "slgAdminOps" 的最长前缀单元是 "slgadmin"/u,
  );
  assert.throws(
    () =>
      assertDomainOwnership(
        ["slgAdmin"],
        [unit("slg", null, ["slgAdmin"]), unit("slgAdmin", "1.0.0", [])],
      ),
    /域 "slgAdmin" 的最长前缀单元是 "slgAdmin"/u,
  );
});
...
test("K0 kit：kit 与 plugin 共享 id 空间（大小写归一撞名拒绝）、保留字拒绝、kit 目录缺 kit.json 拒绝、apps/kits 非目录拒绝、kit 不得依赖 kit 或插件", () => {
  ...
  try {
    {
      const { root } = fixtures.create();
      writeJson(root, "apps/kits/snakE/kit.json", {
        schemaVersion: 1,
        id: "snakE",
      });
      assert.throws(
        () => readViewCatalog(root),
        /kit id 与 "snake" 大小写归一化后冲突（apps\/plugins\/snake ⟷ apps\/kits\/snakE）/u,
      );
    }
    {
      const { root } = fixtures.create();
      addFixtureKit(root);
      writeJson(root, "apps/plugins/kFix/plugin.json", {
        schemaVersion: 2,
        id: "kFix",
      });
      assert.throws(
        () => readViewCatalog(root),
        /大小写归一化后冲突（apps\/plugins\/kFix ⟷ apps\/kits\/kfix）|大小写归一化后冲突（apps\/kits\/kfix ⟷ apps\/plugins\/kFix）/u,
      );
    }
    {
      const { root } = fixtures.create();
      writeJson(root, "apps/kits/registry/kit.json", {
        schemaVersion: 1,
        id: "registry",
      });
    ...
    }
    {
      ...
      assert.throws(
        () => readViewCatalog(root),
        /apps\/kits\/ghost\/kit\.json: missing required file/u,
      );
    }
    {
      ...
      assert.throws(
        () => readViewCatalog(root),
        /apps\/kits: must be a directory/u,
      );
    }
    {
      const { root } = fixtures.create();
      addFixtureKit(root);
      writeJson(root, "apps/kits/kfixTwo/kit.json", {
        schemaVersion: 1,
        id: "kfixTwo",
        dependencies: ["kfix"],
      });
      ...
      assert.throws(
        () => readViewCatalog(root),
        /kit "kfixTwo" 不得依赖插件 "redeem"（KIT\.md §1\/§4：kit 只依赖框架，依赖解析只做 plugin → kit 单向）/u,
      );
      writeJson(root, "apps/kits/kfixTwo/kit.json", {
        schemaVersion: 1,
        id: "kfixTwo",
    ...
    }
    {
      ...
      addFixtureKit(root, {
        ...KFIX_KIT_JSON,
        entry: "apps/client/src/plugins/kfix/index.ts",
      });
      assert.throws(
        () => readViewCatalog(root),
        /entry.*does not match pattern/u,
  ...
  try {
    {
      ...
      writeText(
        root,
        "apps/client/src/plugins/kfix/index.ts",
        CREATE_MODULE_SOURCE,
      );
      writePluginArtifacts(options);
      assert.match(
        fs.readFileSync(path.join(root, PLUGINS_RELATIVE), "utf8"),
        /^ {4}"kfix",$/mu,
      );
      fs.rmSync(path.join(root, "apps/plugins/kfix"), { recursive: true });
      fs.rmSync(path.join(root, "apps/client/src/plugins/kfix"), {
        recursive: true,
      });
      addFixtureKit(root);
      assert.throws(
        () => writePluginArtifacts(options),
        /已登记但真源消失的域\/plugin\/kit\/View：kfix。/u,
        "plugin → kit 不得被普通 --write 静默接受",
      );
      const result = writePluginArtifacts({
        ...options,
        allowDelete: ["kfix"],
    ...
    }
    {
      ...
      fs.rmSync(path.join(root, "apps/client/src/kits/kfix"), {
        recursive: true,
      });
      writeJson(root, "apps/plugins/kfix/plugin.json", {
        schemaVersion: 2,
      ...
      assert.throws(
        () => writePluginArtifacts(options),
        /已登记但真源消失的域\/plugin\/kit\/View：kfix, KfixBoard。/u,
        "kit → plugin 不得被普通 --write 静默接受",
      );
      const result = writePluginArtifacts({
        ...options,
        allowDelete: ["kfix", "KfixBoard"],
      });
...
const KFIX_CONTENT_SCHEMA = {
  ...
  required: ["title"],
  properties: {
    title: { type: "string", pattern: "^.{1,32}$" },
...
const KFIX_KIT_WITH_CONTRIBUTIONS = {
  ...
  contributions: {
    content: {
      kind: "data",
      ends: ["shared", "client"],
      schema: KFIX_CONTENT_SCHEMA,
    },
    hook: { kind: "module", ends: ["server"], export: "hook" },
  },
};
const CONTRIBUTIONS_SHARED =
  "apps/shared/src/kits/kfix/contributions.generated.ts";
const CONTRIBUTIONS_CLIENT =
  "apps/client/src/kits/kfix/contributions.generated.ts";
const CONTRIBUTIONS_SERVER =
  "apps/server/src/kits/kfix/contributions.generated.ts";
const KFIX_SHOP_CONTRIBUTING = {
  ...KFIX_SHOP_PLUGIN_JSON,
  contributes: {
    kfix: {
      content: "apps/plugins/kfixShop/contributions/kfix/content.json",
      hook: "apps/server/src/core/kfixShop/hook.ts",
    },
  },
};
...
test("MF9 贡献点收录：kit 声明即每端恒生成（空列表）；插件只加文件 + 声明即被收录且既有手写源零 diff；撤销声明后孤儿生成物自动收回", () => {
  ...
  try {
    ...
    for (const relative of [
      CONTRIBUTIONS_SHARED,
      CONTRIBUTIONS_CLIENT,
      CONTRIBUTIONS_SERVER,
    ])
      assert.ok(first.changed.includes(relative), `${relative} 应恒生成`);
    assert.match(
      read(CONTRIBUTIONS_SHARED),
      /^\/\*\* AUTO-GENERATED [^\n]* Do not edit\. \*\/\n/u,
    );
    assert.match(
      read(CONTRIBUTIONS_SHARED),
    ...
    assert.ok(
      !read(CONTRIBUTIONS_SERVER).includes("content:") &&
        read(CONTRIBUTIONS_SERVER).includes("hook: [],"),
      "server 端只有 hook（content 未声明 server 端）",
    );
    assertPluginArtifactsFresh(options);
    ...
    writeJson(
      root,
      "apps/plugins/kfixShop/plugin.json",
      KFIX_SHOP_CONTRIBUTING,
    );
    writeJson(root, "apps/plugins/kfixShop/contributions/kfix/content.json", {
      title: "shop",
      weight: 3,
    });
    writeText(
      root,
      "apps/server/src/core/kfixShop/hook.ts",
      'export const hook = { id: "kfixShop" };\n',
    );
    const before = snapshotHandwritten(root);
    assert.throws(
      () => assertPluginArtifactsFresh(options),
    ...
    assert.deepEqual(
      [...second.changed].sort(),
      [CONTRIBUTIONS_CLIENT, CONTRIBUTIONS_SERVER, CONTRIBUTIONS_SHARED].sort(),
      "只有贡献收录文件变化",
    );
    const dataBlock =
    ...
    assert.match(
      server,
      /^import \{ hook as kfixShop_hook \} from "\.\.\/\.\.\/core\/kfixShop\/hook";$/mu,
      "module 贡献 = 静态字面量 import（相对路径、无扩展名）",
    );
    assert.match(
      server,
    ...
    const after = snapshotHandwritten(root);
    const handwritten = [...before].filter(
      ([relative]) => !relative.endsWith(".generated.ts"),
    );
    assert.ok(handwritten.length > 0);
    for (const [relative, text] of handwritten)
      assert.equal(
        after.get(relative),
        text,
        `手写文件被生成器改动：${relative}`,
    ...
    assert.deepEqual(
      [...after.keys()].filter((relative) => !before.has(relative)),
      [],
      "生成器没有新增任何快照面之外的文件",
    );

    // 撤销 hook 声明（kit 与插件同批）⇒ server 端文件成孤儿：--check 报 extra，写盘自动收回
    writeJson(root, "apps/kits/kfix/kit.json", {
      ...KFIX_KIT_WITH_CONTRIBUTIONS,
      contributions: {
        content: KFIX_KIT_WITH_CONTRIBUTIONS.contributions.content,
      },
    });
    writeJson(root, "apps/plugins/kfixShop/plugin.json", {
      ...KFIX_SHOP_CONTRIBUTING,
      contributes: {
        kfix: { content: KFIX_SHOP_CONTRIBUTING.contributes.kfix.content },
      },
    });
    assert.throws(
      () => assertPluginArtifactsFresh(options),
      /extra: apps\/server\/src\/kits\/kfix\/contributions\.generated\.ts/u,
    );
...
test("MF9 贡献点闸：越界路径（别的插件目录 / 生成物形态）、未登记贡献点、未登记 kit、schema 不合、缺 export、module 端不符、data 非 .json 一律在 codegen 拒", () => {
  ...
  try {
    ...
    const contributes =
      (value: unknown, prepare?: () => void): (() => void) =>
      () => {
    ...
    writeText(
      root,
      "apps/server/src/core/redeem/hook.ts",
      "export const hook = 1;\n",
    );
    assert.throws(
      contributes({ kfix: { hook: "apps/server/src/core/redeem/hook.ts" } }),
      /不在插件 "kfixShop" 的所有权推导集内[^\n]*越界贡献/u,
    );
    writeText(
      root,
      "apps/server/src/core/kfixShop/x.generated.ts",
      "export const hook = 1;\n",
    );
    assert.throws(
      contributes({
        kfix: { hook: "apps/server/src/core/kfixShop/x.generated.ts" },
      }),
      /不在插件 "kfixShop" 的所有权推导集内/u,
    );
    assert.throws(
      contributes({
        kfix: { nope: "apps/plugins/kfixShop/contributions/kfix/content.json" },
      }),
      /kit "kfix" 没有贡献点 "nope"（已声明：content, hook）/u,
    );
    assert.throws(() => {
      mutateJson(root, "apps/plugins/kfixShop/plugin.json", (m) => {
        m.requires = { kits: { kfix: { board: 1 }, ghost: { x: 1 } } };
        m.contributes = {
          ghost: { a: "apps/plugins/kfixShop/contributions/kfix/content.json" },
        };
      });
      readViewCatalog(root);
    }, /ghost/u);
    mutateJson(root, "apps/plugins/kfixShop/plugin.json", (m) => {
      m.requires = { kits: { kfix: { board: 1 } } };
    });
    const content = {
      kfix: {
        content: "apps/plugins/kfixShop/contributions/kfix/content.json",
    ...
    assert.throws(
      contributes(content, () =>
        writeJson(
          root,
          "apps/plugins/kfixShop/contributions/kfix/content.json",
          { title: "" },
        ),
      ),
      /content\.json/u,
    ...
    assert.throws(
      contributes(
        { kfix: { hook: "apps/server/src/core/kfixShop/hook.ts" } },
        () =>
          writeText(
            root,
            "apps/server/src/core/kfixShop/hook.ts",
    ...
    writeText(
      root,
      "apps/plugins/kfixShop/hook.ts",
      "export const hook = 1;\n",
    );
    assert.throws(
      contributes({ kfix: { hook: "apps/plugins/kfixShop/hook.ts" } }),
      /必须落在 apps\/server\/src\//u,
    );
    assert.throws(
      contributes({ kfix: { content: "apps/plugins/kfixShop/hook.ts" } }),
      /data 贡献必须是 \.json/u,
    );
    assert.throws(
      contributes({
        kfix: { hook: "apps/server/src/core/kfixShop/missing.ts" },
      }),
      /文件不存在/u,
    );
  } finally {
...
test("MF9 kit fragments：kit.json.fragments 声明的每个 fragment 必须有 apps/kits/<id>/fragments/<name>.state.json（缺文件拒）", () => {
  ...
  try {
    ...
    assert.throws(
      () => readViewCatalog(root),
      /apps\/kits\/kfix\/fragments\/spot\.state\.json/u,
    );
    writeJson(root, "apps/kits/kfix/fragments/spot.state.json", {
      schemaVersion: 1,
      root: [],
    });
    assert.doesNotThrow(() => readViewCatalog(root));
    assert.match(
      fs.readFileSync(path.join(root, "apps/kits/kfix/kit.json"), "utf8"),
      /"fragments"/u,
    );
  } finally {
    fixtures.dispose();
...
test("MF9 带参 launch：payload / profile 渲染进 GeneratedLaunchTarget；profile 必须是该玩法 manifest.profiles 成员", () => {
  ...
  try {
    ...
    const setLaunch = (extra: Record<string, unknown>): void => {
      mutateJson(root, "apps/plugins/builtin/plugin.json", (m) => {
        ...
        const item = items.find(
          (entry) => entry.launch.gameplayId === "ballMove",
        );
    ...
    assert.match(
      rendered,
      /launch: \{ kind: "gameplay", gameplayId: "ballMove", payload: \{"arena":"north","size":3\}, profile: "default" \}/u,
    );
    assert.match(
      rendered,
      /readonly payload\?: Readonly<Record<string, unknown>>;/u,
    );
    assert.match(rendered, /readonly profile\?: string;/u);
    setLaunch({ profile: "ghost" });
    assert.throws(
      () => readViewCatalog(root),
      /launch\.profile "ghost" 不在玩法 "ballMove" 的 manifest\.profiles 内（default）/u,
    );
    setLaunch({});
    assert.match(
      renderViewCatalogArtifacts(readViewCatalog(root)).get(PLUGINS_RELATIVE) ??
        "",
      /launch: \{ kind: "gameplay", gameplayId: "ballMove" \}/u,
      "不带参时渲染形态不变",
    );
  } finally {
    fixtures.dispose();
