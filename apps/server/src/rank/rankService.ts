...
import { kLbDedup, kRank, kRankSub } from "../infra/keys";
...
import { decodeScore } from "./score";

/**
 * 榜类型注册表（key 的 `{type}` 段）。现阶段仅段位星数总榜（客户端契约 shared/protocol/rank.ts
 * 的 star 分；省榜等后续追加）。seasonRotation 按此表给旧季 key 设 TTL。
 */
...
export async function updateScore(
  ...
  await evalshaWithReload(
    ...
    [uid, String(delta), String(SEASON_BASE), String(SEASON_LEN_S), JSON.stringify(subInfo)],
  );
  return "ok";
}
...
export async function selfEntry(type: string, season: string, uid: string): Promise<RankEntry> {
  const rankKey = kRank(type, season);
  const client = clientForKey(rankKey);
