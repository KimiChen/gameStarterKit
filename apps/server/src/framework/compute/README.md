...
- worker error/exit 会使在途任务失败，并在 `COMPUTE_RESPAWN_DELAY_MS`（默认 1s）退避后补位；
  ...
  故障注入用例用后一种排序来覆盖「排队超时」分支。⚠ 但该分支**并非只在这个排序下可达**：
  `onJobTimeout` 只看定时器触发时任务是否还在 queue 里——所有 worker 被长任务占满同样会走到它，
  与退避无关。排序只决定崩溃-重生场景里排队任务的归宿。⛔ 两种排序都合法，无配对校验。
