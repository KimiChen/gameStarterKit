 ...
 * currentZoneId，与「⛔ kit 不直接 import framework/infra 其他模块」自相矛盾）：
 ...
 *
 * kit 从 `apps/server/src/kits/<id>/**` 以相对路径 `../../framework/infra/kitApi` 导入本文件；kit 需要的错误类型、
 * CUR_GOLD、`kKitUser` 与 `currentZoneId` 一并从这里再导出，⛔ kit 不直接 import framework/infra 其他模块
...
import { creditInTx, debitInTx, invalidateBalanceCache } from "../../modules/economy/currency";
import {
  assertOutboxIntentMatches, canonicalizeEffect, deriveOpId, insertOutboxIntent, markOutboxDone, redisApply,
} from "../../modules/economy/outbox";
