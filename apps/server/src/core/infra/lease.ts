/**
 * `singleton_lease` 抢占 / 续租（[05 · singleton_lease](docs/SERVER.md)）。
 *
 ...
 *
 * ⚠ 判定用 `info` 的 `Rows matched` 而非 affectedRows：池已关 CLIENT_FOUND_ROWS，
 * affectedRows = **Changed**——同毫秒内连续续租算出的 expires_at 与库中值相同（连调/快 tick
 * 可达）时 UPDATE 报 0 行，会把合法持约误判成被顶替而自杀。Rows matched 只看 WHERE 命中。
 */
