 ...
 *         requested / prepared ⇒ WORLD_TRANSFER_INVALID）→ 目录解析目标分线（幂等建行）→ 签发凭据（绑定 uid / persona / worldAddress / 当前 controlEpoch）；
...
import { WORLD_PUBLIC_WS_URL } from "../infra/config";
import { readPersonaOwner, type PersonaOwner } from "../../rooms/core/control";
import { DEFAULT_WORLD_LINE, worldAddressOf, worldDirectory } from "../../rooms/core/WorldDirectory";
...
export interface WorldEnterDeps {
    ...
    /** 承载分线的 world 进程公开地址；空串 = 同当前区 gameWsUrl。 */
    endpoint(sId: number, instanceId: string): string;
    now(): number;
}
...
async function issueEntry(uid: string, sId: number, personaId: string, owner: PersonaOwner, mapId: string, line: number, deps: WorldEnterDeps): Promise<IWorldEnterRes> {
    const instance = await guard("directory", () => deps.resolveInstance(sId, mapId, line));
    ...
    return { worldAddress, mapId: instance.mapId, line: instance.line, endpoint: deps.endpoint(sId, instance.instanceId), ticket: issued.ticket, expiresAt: issued.expiresAt, transferId: null };
}
...
async function resolveCommitted(uid: string, sId: number, row: WorldTransferRow, owner: PersonaOwner, deps: WorldEnterDeps): Promise<IWorldEnterRes> {
    ...
    return {
        worldAddress, mapId: row.toMap, line: row.toLine, endpoint: deps.endpoint(sId, row.toInstance), ticket: issued.ticket, expiresAt: issued.expiresAt, transferId: row.transferId,
    };
}
...
export async function handleWorldEnter(uid: string, sId: number, req: IWorldEnterReq, deps: WorldEnterDeps = productionWorldEnterDeps): Promise<IWorldEnterRes> {
    ...
    return issueEntry(uid, sId, req.personaId, owner, req.mapId, req.line ?? DEFAULT_WORLD_LINE, deps);
}
