        ...
        for (const event of claimed) {
            ...
            if (event.kind === MMO_EVENT_GRANT_ITEM) {
                ...
                try {
                    ...
                    // CAS 冲突可能发生在前一堆已写入之后：必须回滚整轮，下一轮重新认领与计算。
                    if (!(error instanceof MmoInventoryError) || error.code === "conflict") throw error;
                    await tx.deadLetterWorldEvent(MMO_WORLD_EVENT_TABLE, event.eventId);
                }
                continue;
            }
