...
当前六个游戏服 endpoint 均由 `GameHttpContractMap` 登记 method/path/request/response；`createGameEndpoint`
从 contract key 派生 path、校验 method，并在序列化前验证 response。带 body 的 endpoint 仍由 better-call
执行 strict Zod request schema，且没有统一的应用层 body 大小上限。新增核心 HTTP 契约时应先补齐 shared
contract，再登记 router，并补 request 接受集合对照测试。
