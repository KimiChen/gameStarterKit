/**
 * POST /admin/world/events（MMO MF10-B3 运维只读面）：各 kit role world-event 表的事件积压。只读，⛔ 改任何行；鉴权走共享密钥头（同 /admin/kick / notice），
...
import { readAdminWorldEvents } from "../../core/world/adminRead";
import { createGameEndpoint } from "../contract";

export default createGameEndpoint("AdminWorldEvents", {
  method: "POST",
}, async (ctx) => {
  ...
  return readAdminWorldEvents(ctx.body.sId, ctx.body.kitId, ctx.body.instanceId);
});
