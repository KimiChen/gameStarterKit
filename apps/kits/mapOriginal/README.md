...
```
plate 90 < terrain 100 < blocks 110 < region 300 < road 900 < grid 1400
...
O1 桌面探针实测 17 张已加载 PNG 的 RGBA8 驻留为 **129.1253 MiB**（已含概览），
O0 基线为 151.1253 MiB。当前裁边产物在 Creator 横竖版实测为 **82.6253 MiB**，
17 张源纹理的 `GFXTexture.size` 与 RGBA8 尺寸换算一致，比 O1 再少 **46.5 MiB**；验收证据见优化方案 §9。
O3-B1 分组后，L3 停留 5 秒仅保留 **8.50 MiB** 概览/小地图源纹理，RT 为 0；流畅 L0
在宽限后为 **34.3753 MiB**。普通 L0 源纹理上限保持不变。十次真实关页均确认源纹理、RT、
BufferAsset、reader ArrayBuffer 及展开对象归零。选中地块时，L3 独立保留 **4,500,008 B**
显示/通行数据；不因此保留资源图集。
...
烘焙队列空闲时即销毁源材质；缓存 key 绑定地图、内容版本及布局版本。实际分组、边界和验收数字见优化方案 §9。
...
流畅档使用原始 2×2 填充图。这是宿主画质映射，不能宣称与原版设备条件相同。
本仓额外 `MAPO_RIVER_TINT` 已删除。水面共享法线虽位于 `scene_3d/water/`，有原 2D 消费证据，白名单仅放行该精确文件。
...
| 族 | 组 | 件 | 贴图 | 图集 |
|---|---:|---:|---:|---|
| river | 102 | **597** 静态记录 + 引用/动画节点 | 79 | 1024² |
| desert | 51 | **481** | 5 | 512² |
| snow | 52 | **821** | 8 | 1024×256 |
...
每族维持一张图集及独立材质，仍按原图层顺序提交。
⚠ 图集按 **0.4× 缩存**，图片条目的 `nativeSize` 记**原图像素**；世界显示尺寸使用 prefab.size × scale：
...
资源件、山体、道路、城市和 top 的逻辑条目以 `textureId` 引用独立图片表；原编号、地貌变体、
clientResId、prefab 变换、摆位与层内次序均保留。基础/雪地道路仍先选逻辑条目，再解析共享图片。

| 图集 | 逻辑条目 / 独立图片 | 尺寸 | RGBA8 |
|---|---:|---|---:|
| 山体 | 26 / 19 | 2048×2048 | 16 MiB |
| 资源件 | 320 / 320 | 2048×4096 | 32 MiB |
| 道路 | 36 / 18 | 512×512 | 1 MiB |
| 城市 | 158 / 158 | 512×1024 | 2 MiB |
| 河岸 top | 79 / 79 | 1024×1024 | 4 MiB |
| 雪 top | 8 / 8 | 1024×256 | 1 MiB |

`atlas-layout.types.ts` 的统一图片契约包括 `textureId / atlasId / rect / nativeSize / storageSize /
trimRect / layoutVersion / contentHash`。资源件、道路、河岸及雪 top 仅裁透明区，保留源内 2 px 护边，
另加每侧 2 px 图集间隔；`storageSize` 仍是裁前画布，原采样比例不变。山体、城市、沙 top 维持完整画布。
原版 `size / pivot / scale / offset` 属于逻辑件，修改图集排布不改变世界显示位置。
顶点按原画布中的 `trimRect` 展开后再经过父矩阵；UV 镜像同时镜像裁剪窗口，帧切换不移动节点锚点。
资源件维持单页；“其他切片 / 动画相关切片”双页候选虽为 18 MiB，但五个场景实际产生 253～1263 次提交，
单页只有 6～9 次。不能按纹理页重排透明件来减少这些提交。
小地图存储为 512×256，原导航画布仍为正方形，内容占中间半幅；点选和视口框使用同一个内容矩形。
纹理 ID 由图集族和同图来源中排序最前的规范路径派生，独立于像素哈希和装箱坐标；完整来源别名留在 info JSON。
源切片变更后重建图片表；道路基础/雪地原画布不再相等时打包器报错，要求重新审核变体绑定。
...
| 件 | 原图宽 | 占格 |
|---|---:|---|
| 资源 res_field（基础季） | 50–329 px | **0.17–1.10** 格（仅贴图尺寸，实际保留完整 prefab） |
...
资源件按完整 prefab 层级渲染，三套 135 个入口展开为 659 节点、320 种纹理；保留父级变换、
size/pivot/mirror/skew/color/add_color、阴影、引用、时间线和帧动画。
城池和手摆细节也使用 prefab 显示尺寸，不能拿贴图 native 代替。
原生 skew 量化、镜像、动画循环的依据见 [机制 §2.2](../../../docs/MAPORIGINAL-2D.md#22-资源件与选中框共用格心图片按-prefab-锚点与层级变换定位)。
旧版统一把图底放在格心下方 8 世界单位，使 `(750,749)` 粮田上浮约 30 个原版像素；现在根节点对格心，图片保留自身局部偏移。
普通选中框使用 `city_shape.GRID.click_res=2080` 的 8 片 UI XML 布局与 Scale/Color 动画，
原包缺省 24 fps、Quad.Out；不再拼白矩形，也不混用行军选择 11009。
...
- 先前的 8 项邻级替代全部撤销：135 个 prefab 实际均存在，原因是旧解析器吞掉错误。
  现在未知组件、非法边界、缺纹理会令构建失败，`substitutions=[]`；禁止恢复按面积挑主片或最近等级兜底。
- 道路使用 18 基础 + 18 雪地皮肤，42,018 片中 6,012 片由路片中心对应的 cell 级地貌切为雪路。
- 图集直通 RGBA；节点颜色由材质处理，禁止用图片本身作 paste mask 再乘一次 alpha。
