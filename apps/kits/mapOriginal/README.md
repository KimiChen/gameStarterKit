...
桌面探针分项实测：当前 17 张已加载 PNG 的 RGBA8 驻留合计 **129.1253 MiB**（已含概览），
O0 基线为 151.1253 MiB；图集去重重排减少 **22 MiB**。
...
| 族 | 组 | 件 | 贴图 | 图集 |
|---|---:|---:|---:|---|
| river | 102 | **597** 静态记录 + 引用/动画节点 | 79 | 2048²（填充 59%） |
| desert | 51 | **481** | 5 | 512² |
| snow | 52 | **821** | 8 | 512×1024 |
...
重建与核验入口见 [素材工具 O1](../../../tools/maporiginal-assets/README.md#o1图集去重重排)。
本次增加布局元数据后内容 TS 为 17 文件、1,466,700 B；配置外置仍是后续独立批次。
