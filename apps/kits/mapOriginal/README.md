...
| `region` 山林 | L0–L3 | ✅ ★ **多格地形每区一件**：原版 `mountain_patch.bytes` 的 3,942 条锚点优先，没锚点的连通区每区补一件（锚在区内屏幕最低格）。件是原版 2D 山体 `m1..m10` / 树簇 / 草丛 |
...
值 48..61（占全图 8.8%）是跨多格的地貌，原作里是一个模型跨整片区，⛔ 不是每格一棵树。
本 kit 用 `regions.bin`（2.78 万条 / 217 KB，Cocos BufferAsset）摆它，两个来源：
...
再叠一件「整区一件」就重了。
⚠ 连通域在**族内**求（山脉 60/61、林丛 52..55+57..59、散落 48..51），
⛔ 不能逐值求：60 和 61 在同一座山上交错出现，逐值会把一座山劈成两半。
⚠ 表**按 s = row+col 升序落盘 = 画家序**，客户端只做二分 + 矩形裁剪，
⛔ 不要每帧对 2.8 万条排序。二分下界要往下多放 `S_MARGIN` 格，否则「锚点在屏幕下方、
身子探进来」的大山会漏掉。
...
| 山体 `m1..m10` | 281–674 px | 0.94–2.25 格 |
| 树簇 | 35–139 px | 0.12–0.45 格 |
| 草丛 `a1..a8` | 246–609 px | 0.82–2.03 格 |

⚠ 两版都踩过：先按**固定 1.0 格宽**拉资源件（把等级差抹平了），再按**连通区跨度**拉区域件
（真机上是糊成一团的大绿斑）。⛔ 都别改回去 —— `native`（原图像素）才是尺寸的唯一依据，
`wTiles` 只在打包期用来**挑件**（大区用大件）。
