...
⚠ **id → 精灵的绑定是 `[推断]`**：`client_res` 在未解的 `base.cw` 里查不到名字，
改用**结构签名** —— 逐 id 算路网邻接度（纯度基本 1.00），分桶恰好是
**12 个 2 度 + 2 个 3 度 + 1 个 4 度 + 3 个 1 度**，与 `road.xml` 一套皮肤的
line4 + horizonalturn4 + up/downverticalturn4 + tcross2 + xcross1 + end4 吻合；
再与精灵按**完整路径字母序**的度序列比对，**18 位逐位全等**。
