...
`slg` 是 SQL 权威的大地图机制样例，提供分别版本化的 `worldmap` / `march` 两个 API 面。阶段 1 包含地图页与即时占领，阶段 2a 包含行军 Lobby RPC 与懒结算；二者同批登记 SQL 与 API，不登记 `slgWorld` mode。地图现为《杖剑传说》「森之国」主题（真实布局复刻中心区 + 策展摆件 + 程序化地表），尺寸改用《三国志·战略版》标准图 1500×1500，默认入口仍由宿主 placement 决定。
...
| 面 | 能力 |
| --- | --- |
| `worldmap` v1 | 1500×1500 地图坐标（三战标准图格数，正方形格）、16×16 chunk 数学、四档 LOD；按 chunk 矩形读稀疏地块；免费即时占领、攻击、加固 |
...
`data/terrain.json` 是「森之国」主题地图（1500×1500，沿用三战标准图格数），客户端资源 `apps/Cocos/assets/resources/kits/slg/terrain.json` 为其逐字节镜像。现有布局为四边海环 + 中心复刻区：六类地形（草地/林地/水面/岩石/沙滩/裸土）与 43 个矩形区域（含气泡湖内陆湖与归木村裸土场）。格式仍为 `name / width / height / palette / regions`：id 为 0 的草地隐式覆盖全图，矩形按登记顺序覆盖；保持 16 色与 512 区域上限。

世界为 1500×1500 格（225 万格），每个 chunk 为 16×16 格（尾块 12 格）。SQL 只保存发生变化的地块，客户端只加载视口附近的 chunk；静态内容和总览均不生成或遍历 225 万条地形记录。

美术与布局源文件见 [森之国素材包](art/senzhiguo-v1/README.md)：地表/装饰图集各为 1536×1024（3 列×2 行，每格 512×512），绘卷为 1024×1024。运行时图片位于 `apps/Cocos/assets/resources/kits/slg/senzhiguo/`，逐字节镜像已批准的成品；装饰图集由 zjcs 学习包的部件图人工策展切片，地表与绘卷为程序化生成（真实 Tilemap 地砖所在 bundle 未入包，提取失败走登记的 fallback，见素材包 README）。**真实布局复刻**：`data/forest-layout.json`（镜像到 `resources/kits/slg/forest-layout.json`）由森之国布局包解出（77 区 1803 实体，坐标 5× 放大居中），中心复刻区的 chunk 装饰用真实点位，其余 chunk 维持确定性哈希；图集索引、地标和装饰配置的唯一真源是 `apps/client/src/kits/slg/logic/mapArt.ts`，没有另建 `atlas.json`。

- 地标为森之国地名：归木村 / 世界树半岛 / 气泡湖 / 狂花海岸 / 蛛后巢穴（布局复刻坐标经 chunk 足迹与旱地校验微调）。
...
- 装饰双源布置：中心复刻区的 chunk 用 `forest-layout.json` 的真实点位，其余 chunk 维持确定性哈希；每块最多 7 件（含最多 1 个地标），每个非空块一个透明贴图网格。内部 LOD 0–3 的普通装饰数量上限为 6/4/2/0，已加载块内的地标四档均保留；装饰完整范围留在所属块内。缓存只随块增删和 LOD 更新，权威地块版本变化不重建静态装饰；离开保留范围后销毁对应节点和网格，未实现对象池。
- LOD 分档有 ±8% 滞回带（`slgLodForScaleStable`，shared 单源），阈值附近往复缩放不再让网格线/装饰层反复闪现；`MapCamera` 持有滞回后的状态化 LOD。**LOD 3（远档）不再逐 chunk 建网格**：一张静态区域色块地表 + 一张全图地标 + 一张稀疏归属 overlay（合计 3~5 张网格），数据仍走逐 chunk `mapTiles` 管线；进出远档整批硬切。近景 chunk 与装饰的建销走 240ms 顶点 alpha 淡入淡出（淡出并发上限 12，批量场景硬切兜底）。桌面控制台调试：页面打开期间安装 `globalThis.slgMapDebug`（`setLod(0..3|null)` 强制档、`hide/show` 逐层隐藏、`reset`），关闭时注销并重置；⛔ 不进存档与网络。
...
本轮只改变静态布局和显示，不新增地形阻挡、寻路、宗门经营、采矿、传送或行军规则。改动地形或图片时保持源文件与资源镜像一致；源码镜像仍通过 `sync:client` 刷新。森之国的文件与接入状态见 [素材包](art/senzhiguo-v1/README.md) 与其 pipeline/ 提取脚本；早期「青原仙洲」主题的接入记录留存于 [art/qingyuan-v1](art/qingyuan-v1/README.md)（git 历史）。
