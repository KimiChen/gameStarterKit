...
| 地表**图集** | L0–L2 | ✅ ⚠ **已退居兜底**：现役是连续覆盖场，图集只在 effect/图集缺席或覆盖场没铺满时用 |
| `field` 连续覆盖场 | L0–L2 | ✅ **现役地表**。逐块权重图 + 自定义着色器，见下节 |
...
### ★ 连续覆盖场（v2，2026-09-22）—— 近档地表的现役实现

近档地表**不再逐格铺菱形**。改成：逐块烘一张「每类地形的覆盖率」权重图，交给自定义着色器
按世界坐标混合 8 路地形。目标是 v2 验收线：**正常视图里看不出菱形边、半边接点或逐格折角**。

⚠ 逐格那套（`atlas-lod*` + `sgzz-blend` 过渡片）**仍然保留**，作为 effect/图集缺席时的兜底，
⛔ 不要因为「用不到了」把它删掉 —— 覆盖场没铺满时也靠它垫底。

#### 三层地基

| 件 | 位置 | 要点 |
|---|---|---|
| `sgzz-terrain.effect` | `apps/Cocos/assets/resources/kits/sgzzmap/shaders/` | **本仓第一个自定义着色器**。8 路逐像素权重混合 |
| `sgzzField.ts` | logic | 覆盖率 → 平滑 → 三次幂归一 → 两张 RGBA8；海岸的有符号距离场也在这 |
| `SgzzFieldRenderer.ts` | view | 逐块烘 + 建材质 + 铺四边形 + 缓存淘汰 |

#### map 平面：一切几何都在这里算

**X = worldX、Y = −2·worldY**，一格边长 **R = 32√2 ≈ 45.25**。这一步把等距剪切还原成正方格。
⚠ ⛔ 不在这个平面上算高斯，核在屏幕上会是椭圆，平滑出来的边界一边胖一边瘦。

#### 分档（tier）

| 档 | LOD | 块尺寸 | 步长 | LOD2 时块数 |
|---|---|---|---|---|
| tier 0 | 0–1 | 20 格 | 4 | —— |
| tier 1 | 2 | 40 格 | 8 | 36 → 6~8 块 |

⚠ 块放大几倍、步长就放大几倍 ⇒ **单块采样点数不变**（226²），成本恒定。
⚠ 缓存 key **必须带档号**，⛔ 否则两档同坐标的块会互相顶掉。
⚠ LOD2 的步长最多粗一倍（8）：试过 16，一格才 2.8 个采样点，单格宽的河直接被抹掉，
而 LOD2 一格仍有 22×11 屏幕像素 —— 河看不见是**玩法可读性**问题，⛔ 不是观感问题。

#### 海岸（v2 §3.2）

陆水合成一个有符号距离场（正 = 水），平滑后的**零等值线**才是视觉海岸，⛔ 不再是菱形边。

1. chamfer 3-4 两遍求距离 —— ⛔ 不用「模糊指示函数」代替：模糊会把窄河小岛整个吃掉；
2. 截断到 ±3R（⛔ 不截的话远处巨值把平滑拉偏）→ 盒滤波 σ=0.60R；
3. **限位移 0.38R** —— ⛔ 不限的话平滑把小岛整个吃掉；
4. 叠世界坐标低频扰动（与覆盖率同一套，连续 ⇒ 块间无缝）；
5. 格心保护：格心附近岸线⛔不得翻面；
6. 打包时按 `smoothstep(±0.08R)` 把水类/陆类分开缩放再归一
   ⇒ **海岸成为权重的属性**，⛔ 不需要额外贴图、不需要改着色器。

#### 窄特征保护（v2 §6）

同类六邻 ≤ 2 的细特征（窄河 / 陆桥 / 孤岛）用**更宽**的保护带 **0.55R**。

⚠ 只给细特征加宽，⛔ 全局加宽会把开阔岸线压成一块块方的。
⚠ 半宽必须 ≥ 0.5R：细特征上相邻格心相距 R，保护圆盘直径不到 R 就搭不上，河会被打散成一串圆点。
⛔ 别照抄美术规范里的 0.42R —— 那是「沿中心线展开的管带」的半宽，这里是逐格圆盘，几何不同。

#### 性能（实测，⛔ 别凭感觉调）

| 阶段 | 单块耗时 |
|---|---|
| 初版（逐点高斯卷积 + 闭包 chamfer） | **122 ms** |
| 高斯 → 三遍盒滤波（O(1)/像素） | 45 ms |
| chamfer 去闭包、内联边界 | **31 ms** |

⚠ 每帧只烘 **1 块**（60fps 的预算是 16.7 ms）。⛔ 调回 2 会让进图那几帧掉到 20fps。
⚠ 实测**没有**改善的三项，⛔ 别再当优化点重做：噪声查表、复用输出缓冲、修 `sgzzPos2GridRaw` 三次调用。
真正的大头是**卷积抽头数**与**闭包调用**。

#### ⛔ 五个只有真机能看见的坑（都已修，都有回归）

| # | 症状 | 原因 |
|---|---|---|
| 1 | 地表渲成一整片纯色 | `setProperty` 传 **JS 数组**给 `vec4` uniform **静默失败** ⇒ uniform 留 0 ⇒ `fract(x/0)` = NaN。必须传 `Vec4` |
| 2 | 未烘区域**全黑**，摆件浮在黑底上 | 第一块烘好就撤了逐格地表，而填满要十几帧。⇒ `isCovered` 为真才撤 |
| 3 | 只烘出一块就停住 | `render(force)` 在「相机没动、数据没变」时**整体早退**，分帧烘焙被饿死。⚠ 往 render 里加任何分帧逻辑都要先确认它会被调到 |
| 4 | 切档后 LOD2 挂着 26 块（应为 8） | 旧档的块留在缓存里**并且还在画**。⇒ 不在视野的块停用节点（只停用不销毁，来回缩放不重烘） |
| 5 | 窄河断成一串水点 | 用例用 `SGZZ_FIELD_STEP`（规范里的理想值 2）烘，而生产走 `tier.step`（4/8）—— **测试验了一个从没发布的配置** |

⚠ 第 5 条是这一轮最该记住的：**测试和被测代码共享了同一个错误前提**，所以再多条也测不出来。
现在 fixture 一律用 `tier.step` 且**两档都验**。

#### 未实现

- **岸条材质**：美术交的 `shore-strip-*.png` 还没消费（需要第 3 张贴图传岸距，+3 MB/16 块）。
- **LOD3+ 轮廓**：LOD3 起改用整幅底图，岸线会跳回旧轮廓（LOD3 一格 13.4 px、位移上限 2.8 px）。
  扩 tier 到 LOD3–4 可解，块数算过是 8–18 块。
- **烘焙挪 worker**：⛔ 没做。Cocos 的 worker 要单独打包，2.25 MB 地形要跨线程传，
  微信小游戏限制更多；降采样已把 tier1 压到一帧内，只有 tier0 的 31 ms 会在进图/快速平移时掉帧。
