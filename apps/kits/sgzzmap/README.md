...
菱形边界取六点 **S → W → NW中点 → N → E → SE中点 → S**，`resDir` 第 i 段即第 i 点到第 i+1 点。
`resDir` 1(SW) / 4(NE) 是**整条**菱形边，2/3(NW) 与 5/6(SE) 各是**半条**——
因为这套铺法是错缝砌砖：NE/SW 各只挨一个邻格，NW/SE 各挨两个。与行奇偶**无关**（实测两种一致）。

⛔ 两版错误实现都已封印在用例里：① 丢掉 `resDir` 每方向铺一整格（孤地叠 6 层 ⇒ 不透明，盖住领地色）；
② 改画「到邻格连线的中垂线」——方向对位置错，六段互不相接还戳出格外，看着像六道乱划的斜杠。
美术规范见 `~/Downloads/maps/美术规范-sgzzmap地图资产.md` §5。
...
| `plate` 世界底图 | L3–L5 | ✅ |
| `birdview` 聚合色块 | L4–L5 | ✅ |
| `decor` 摆件 | L0–L1 | ✅ 唯一**超出菱形**的一层；地形+坐标的纯函数决定种类与密度；⛔ 水里不种树。美术未到，先用梯形剪影 |
| `banner` 目标旗 | L0–L1 | ⛔ **未实现**（等图集） |
| `label` 地名 | L0–L2 | ⛔ **未实现**（还缺地名数据） |

绘制序 = 兄弟序：地表(0) → 网格线(1) → 领地(2) → 描边(3) → 摆件(4)。
⚠ 摆件在最上：它是唯一超出菱形的一层，压在半透明领地叠色之下会被糊掉。
⚠ 摆件层内部必须按**画家序**排（屏幕越低越靠前），⛔ 顺着可视模板的遍历序画会前后颠倒。
⚠ 可视模板的预取余量按 `SGZZ_DECOR_MARGIN_TILES`（3 格 = 96）取，必须盖得住**含尺寸抖动**的最高摆件（71.3），
⛔ 否则下边缘的树会突然弹出来。加抖动那次这条用例就红了一次（62 → 71.3 越过了默认的 2 格 = 64）。
⚠ 余量涨一格 ⇒ LOD0 的 span 从 32 涨到 34，仍在 `SGZZ_MAX_QUERY_CHUNKS = 64`（8×8）之内 —— 再涨就要一起调。
...
- **逐格 UV 翻转**（`sgzzTileVariant`，位置的纯函数、4 种变体）：每格采样的是同一张片，
  不打散的话整片地读作"铺地砖"而不是连续地貌（真机 run 16 实证）。
  ⚠ 必须是位置的纯函数，⛔ 随机数会让平移时纹理乱闪；翻转在**格内**镜像，⛔ 不翻整张图集。
- **网格线 alpha 0.13**：这个数是**贴上真实纹理之后**定的 —— 平涂时要 0.2 才看得见，
  有纹理后同样的值会把整片地压成一格一格的瓦片。
