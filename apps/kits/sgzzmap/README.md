...
⚠ 可点区由重放**实测** `sgzz-header` / `sgzz-footer` 两块底板算出，⛔ 不按百分比猜：
早先猜 20%/68%，而 View 用的是 `min(150, h*0.14)` / `min(270, h*0.26)`，高 1542 时中心差 32 px ——
恰好一格，于是「回领地」之后点中心选到的是家旁边那一格。这两块底板的名字因此是**重放契约**。
