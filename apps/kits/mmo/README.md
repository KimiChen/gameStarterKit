...
`hud` 是第四个贡献点（module · client · export `hud`）：`IMmoHudContribution { packId, load }` 必须匹配同一插件的内容包；`load()` 动态导入 View 并返回 `create(context)`。世界实体层与点地 / 点选保留，匹配的 HUD 替换默认控制层；未匹配保留默认 HUD。`context` 提供节点、尺寸、输入派发与房间绑定的 `subscribeScriptState(packId, listener)`，插件不接触 transport。重复包、越权绑定和非法模块拒绝装载。content 面 v4 同时公开 HUD 类型与摇杆 / 轮盘纯几何助手。

客户端 orchestration 面 v3 提供 `IMmoScriptStateSource`、`IMmoScriptStateSnapshot` 与 `subscribeScriptState`：订阅立即回放最近快照；`rev:null` 表示尚未取得本连接的全量状态。离线立即清空，重连重置代际后接受新快照，旧 / 重复 rev 不回放。服务端准入及 baselineRequest 都补发当前全量，静止比分的新进玩家也能读到。
...
### 5. 编排：事件 / 命令清单与预算（`orchestration` 面 v3；v2 加服务端检查点列表，v3 加客户端订阅和批量奖励；MMO.md §8）
...
批量奖励（orchestration v3）：`grantItem` / `grantCurrency` 仍支持 `toCharacterId`；也可互斥提供 `toCharacterIds`（1–100 个唯一 id）。一个命令先校验全部角色在场与模板，再展开每角色独立 durable 事件；任一目标非法则该命令零发奖。单人 opId 保持原值；批量 opId 附带角色 id，重排名单不改奖励身份。64 条命令、4 KB vars 与其他冻结预算不变；每次 dispatch 累计奖励目标最多 200（100 人各领道具与金币），跨事件累计，超出整批回滚并以 `grant-targets` 暂停，不能用 64 条批量命令展开 6400 次同步工作。
...
### MG2 前置反馈修复（2026-09-23）

kit 0.1.28：content 面 3 → 4（hud 贡献与纯操作几何），orchestration 面 2 → 3（客户端脚本快照订阅、100 人批量奖励）；minSupported 不变。首次进图 / baseline 补全量快照；HUD 选择不含样本 id；旧单人发奖兼容，每次 dispatch 奖励目标合计限 200。真实 Creator 查询暴露 MySQL prepared statement 的数字 LIMIT 参数兼容问题：分线查询使用固定常量 LIMIT 64，奖励回执轮询先校验 limit 为 1–256 整数再内插；真库回归覆盖区 / 图 / 包隔离与最新检查点。世界初始化只生成 managed=kit 的怪，编排怪由脚本生成；混合管理方式及旧错误检查点恢复回归防止重复出生。公开 harness 新增 `snapshot()` / `restore(snapshot, currentTick)`，封装真实 timer / region / vars 恢复并校验包、图、seed 与步长身份，插件恢复测试不导入 kit 内部。此批单独提交并重打本地 `mmo-kit-v1-frozen`，再以冻结基线验收第二内容样本。
