...
const NOT_READY: IncomeNotice = {
  kind: "error",
  text: "铜币收益未就绪（plugin 未装载）",
};
...
export function describeIncomeError(error: unknown): string {
  ...
  switch (code) {
    ...
    case "TIMEOUT":
      return "网络不可用，稍后自动重试";
    default:
      return code ? `铜币收益请求失败（${code}）` : "铜币收益请求失败";
  }
}
...
export function offlinePopupText(snapshot: IIncomeGetPendingRes): string {
  const preview = {
    seconds: snapshot.offlineSeconds,
    copper: snapshot.offlineCopper,
  };
  return (
    `你离线了 ${formatDuration(preview.seconds)}，` +
...
export class IncomeLogic {
  ...
  currentNotice(): IncomeNotice {
    return this.notice;
  }
  snapshotOf(): IIncomeGetPendingRes | null {
  ...
  pendingOffline(): IOfflinePreview {
    ...
    return {
      seconds: snapshot?.offlineSeconds ?? 0,
      copper: snapshot?.offlineCopper ?? 0,
    };
  }

  hasOffline(): boolean {
    return this.pendingOffline().copper > 0;
  }

  /**
   * 当前余额：**统一模块状态优先**，快照只在同步尚未到位时兜底。
   *
   * 领取的权威值由服务端 `reply.sync` / 主动 sync 送进 `lobbyDataSync`，所以本方法不需要
   * 任何「领取后自己加一笔」的本地记账 —— 那样只会与服务端口径分叉。
   */
  balance(): number {
    const synced = lobbyDataSync.field<number>("copper");
    return typeof synced === "number" ? synced : (this.snapshot?.copper ?? 0);
  }
   ...
   *
   * 「当前铜币」取统一模块状态（`balance()`），等级/周期/单拍收益才取本次预览快照 ——
   * 后者本来就不在同步面上，属于「本次领取展示和确认」的那部分数据。
   */
  balanceText(): string {
    const snapshot = this.snapshot;
    if (!snapshot) return "";
    return `当前铜币 ${this.balance()} · ${snapshot.level} 级每 ${snapshot.intervalSeconds} 秒 ${snapshot.perInterval}`;
  }
    ...
    try {
      ...
      this.notice =
        result.copper > 0
          ? {
              kind: "success",
              text: `已领取 ${result.copper} 铜币，当前余额 ${result.balance}`,
            }
