/**
 * arenaDuel presentation（Cocos 纯节点，挂在玩法 presentation host 节点下；坐标以 host 中心为原点）。
 * 分工（铁律 9）：本文件只管节点与触摸；视图模型来自 logic/rooms/arenaDuel/ArenaDuelGameplay.ts 的 model()。
 * 手搓粗糙版（同 TallyView 口径）：纯色底板 + Label，每帧按模型重建状态区。
 */
import { Color, Label, Node, UITransform, view } from "cc";
import type { ArenaDuelInput, ArenaDuelPresentation, ArenaDuelViewModel } from "../../../logic/rooms/arenaDuel/ArenaDuelGameplay";
import { GamePhase } from "../../../shared/index";
import { createSolidPlate } from "../../uiPlate";

const BG = new Color(26, 20, 28, 255);
const PANEL = new Color(30, 36, 52, 255);
const ACCENT = new Color(70, 130, 210, 255);
const STRIKE = new Color(190, 70, 90, 255);
...
export class ArenaDuelView implements ArenaDuelPresentation {
    ...
    constructor(
        ...
        private readonly dispatchInput: (input: ArenaDuelInput) => void,
    ) {}
    ...
    mount(): void {
        ...
        const layer = this.node("ArenaDuelLayer", this.host, this.width, this.height);
        this.layer = layer;
        this.plate(layer, this.width, this.height, BG, 0, 0, "bg");
        this.label(layer, "决斗 · arenaDuel", Math.round(this.width * 0.06), TEXT, 0, this.height * 0.4);
        const buttonSize = Math.min(this.width, this.height) * 0.42;
        this.button(layer, "出击", buttonSize, buttonSize, 0, -this.height * 0.08, () => this.dispatchInput({ type: "strike" }), STRIKE);
    ...
    render(model: ArenaDuelViewModel): void {
        ...
        const key = JSON.stringify([model.phase, model.hp, model.selfHits, model.opponentHits, model.winnerName, model.selfWon, Math.ceil(model.lingerLeft ?? -1),
            model.players.map((player) => [player.name, player.hits, player.isSelf])]);
        ...
        const headline = settled
            ...
            : model.phase === GamePhase.Playing
                ? `HP ${model.hp} · 你已命中 ${model.selfHits}${model.opponentHits === null ? "" : ` · 对手 ${model.opponentHits}`}`
                : "等待对手…";
        ...
        let y = -h * 0.12;
        for (const player of model.players.slice(0, 2)) {
            this.label(status, `${player.isSelf ? "▶ " : ""}${player.name}  ${player.hits}`, line, player.isSelf ? ACCENT : DIM, -w * 0.4, y, "left");
            y -= line * 1.5;
        }
    }
    ...
    unmount(): void {
        ...
        try {
            ...
            console.error("[arenaDuel] 卸载 presentation 失败", error);
        }
    }
