/**
 * arenaCapture presentation（Cocos 纯节点，挂在玩法 presentation host 节点下；坐标以 host 中心为原点）。
 * 分工（铁律 9）：本文件只管节点与触摸；视图模型来自 logic/rooms/arenaCapture/ArenaCaptureGameplay.ts 的 model()。
 * 手搓粗糙版（同 TallyView 口径）：纯色底板 + Label，每帧按模型重建状态区。
 */
import { Color, Label, Node, UITransform, view } from "cc";
import type { ArenaCaptureInput, ArenaCapturePresentation, ArenaCaptureViewModel } from "../../../logic/rooms/arenaCapture/ArenaCaptureGameplay";
import { GamePhase } from "../../../shared/index";
import { createSolidPlate } from "../../uiPlate";

const BG = new Color(20, 24, 30, 255);
const PANEL = new Color(30, 36, 52, 255);
const ACCENT = new Color(70, 130, 210, 255);
const CAPTURE = new Color(200, 96, 64, 255);
...
export class ArenaCaptureView implements ArenaCapturePresentation {
    ...
    constructor(
        ...
        private readonly dispatchInput: (input: ArenaCaptureInput) => void,
    ) {}
    ...
    mount(): void {
        ...
        const layer = this.node("ArenaCaptureLayer", this.host, this.width, this.height);
        this.layer = layer;
        this.plate(layer, this.width, this.height, BG, 0, 0, "bg");
        this.label(layer, "占领赛 · arenaCapture", Math.round(this.width * 0.06), TEXT, 0, this.height * 0.4);
        const buttonSize = Math.min(this.width, this.height) * 0.42;
        this.button(layer, "占领", buttonSize, buttonSize, 0, -this.height * 0.08, () => this.dispatchInput({ type: "capture" }), CAPTURE);
    ...
    render(model: ArenaCaptureViewModel): void {
        ...
        const key = JSON.stringify([model.phase, model.captureGoal, model.selfCaptures, model.winnerName, model.selfWon, Math.ceil(model.lingerLeft ?? -1),
            model.players.map((player) => [player.name, player.captures, player.isSelf])]);
        ...
        const headline = settled
            ...
            : model.phase === GamePhase.Playing ? `目标 ${model.captureGoal} 格 · 你已占 ${model.selfCaptures}` : "等待开局…";
        ...
        for (const player of model.players.slice(0, 4)) {
            this.label(status, `${player.isSelf ? "▶ " : ""}${player.name}  ${player.captures}`, line, player.isSelf ? ACCENT : DIM, -w * 0.4, y, "left");
            y -= line * 1.5;
        }
    }
    ...
    unmount(): void {
        ...
        try {
            ...
            console.error("[arenaCapture] 卸载 presentation 失败", error);
        }
    }
