 ...
 * ⚠ `coord_util.lua` 在反编译产物里**只有 disasm 形态**（`disasm/script/util/coord_util.lua.disasm`，
 *   grid2pos 在 Proto ld=121..131、pos2grid_raw 在 ld=133..151），上面标的是**源行号**。
 *   环序 `get_around_list_order` 的名字在该文件常量表里是损坏长串，实证出处是调用侧
 *   `scene/mapview/common/map/grid_range_border.lua:67`。
 * ★ 原版自身分了 `coord_util.lua`(2D) 与 `coord_util_3d.lua`(3D)，本 kit 取**前者** ——
 *   这是「只承载 2D 沙盘」的正面佐证。
 */
...
/**
 * 近景 chunk 边长（格）。硬锚是 `config_2d.lua:12` 的 `CHUNK_TILE_NUM = 10`（常量名对得上的是它）。
 * ⚠ `BLOCK_SIZE = 10` 在 `config_2d.lua:9` 与 `config_3d.lua:7` **两边都有**（维度中立），
 *   ⛔ 别拿它当 2D 侧的判据。
 * ⚠ `sgzzmap/api/hexmap/index.ts` 的同源行还带着「与最细一档鸟瞰 chunk 同尺寸」那半句 ——
 *   本 kit **有意去掉**（鸟瞰是 3D 侧），⛔ 下次对拷别抄回来。
 */
...
export function mapoNextPos(row: number, col: number, dirIndex: number): IMapoCell {
    if (!Number.isInteger(dirIndex) || dirIndex < 1 || dirIndex > 6) {
        throw new RangeError("mapOriginal dirIndex must be 1..6");
    }
...
export function mapoDirIndex(src: IMapoCell, dest: IMapoCell): number {
    ...
    if (dr === 0) {
        if (dc < 0) return 1;
        if (dc > 0) return 4;
        throw new RangeError("mapOriginal dirIndex needs two distinct cells");
    }
...
export function mapoCellOf(row: number, col: number,
    ...
    if (!Number.isInteger(row) || !Number.isInteger(col)
        ...
        throw new RangeError("mapOriginal grid outside map");
    }
...
//   `GETFIELD self.is_in_2d_scene → TEST → RETURN`，**2D 下永远触发不到**。
//   ⛔ 别再加回来；3D 侧要用就在 `mapOriginal3d` 的契约里重建。

export function mapoLodForScale(scale: number): number {
    if (!Number.isFinite(scale) || scale <= 0) throw new RangeError("mapOriginal scale must be positive");
...
export function mapoLodForScaleStable(prevLod: number, scale: number): number {
    if (!Number.isInteger(prevLod) || prevLod < 0 || prevLod > MAPO_LOD_MAX) {
        throw new RangeError("mapOriginal prev LOD invalid");
    }
    if (!Number.isFinite(scale) || scale <= 0) throw new RangeError("mapOriginal scale must be positive");
    let lod = prevLod;
    while (lod > 0
...
export function mapoTerrainAt(t: IMapoTerrain, row: number, col: number): number {
    if (!mapoInBounds(row, col, t.maxRow, t.maxCol)) throw new RangeError("mapOriginal grid outside map");
...
// ⚠ 这里曾有两整段**承自 sgzzmap 的死码**（长程邻接 `MAPO_MAX_LINKS`… 与 AOI 分块矩形
//   `MAPO_MAX_QUERY_CHUNKS`…，共 17 个导出），2026-09-22 整段删除。
//   删的理由不是「它们属于 3D」，而是：① 外部引用全 0；② 它们的 JSDoc 引了**四个本仓不存在
//   的东西**（`MAPO_MAX_VIEW_TILES` 其实是 sgzzmap 的、`mapOriginal-aoi.test.ts` 不存在、
//   `mapOriginal.tile` RPC 不存在——kit.json 无 domains、`MapomapWorldLogic` 是错的类名），
//   留着会让人以为本 kit 有服务端 AOI 链路。
//   ★ 本 kit v1 是**纯客户端本地数据**：地形随 shared 模块与 BufferAsset 走，⛔ 无 RPC 域。
//   将来真接服务端时按那时的契约重写，⛔ 不要把 sgzzmap 那份再拷一遍。
