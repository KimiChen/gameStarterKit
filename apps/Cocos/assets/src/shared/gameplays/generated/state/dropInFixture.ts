...
/** Root players map capacity. Source: apps/shared/schema/gameplays/dropInFixture/manifest.json (maxPlayers). */
const MAX_PLAYERS = 8;

/** Dependency-free mirrors of the Colyseus wire state. */
export interface IDropInFixturePlayerState {
    /** Colyseus sessionId */
    id: string;
    name: string;
}

export interface IDropInFixtureState {
    ...
    players: Map<string, IDropInFixturePlayerState>;
}
...
function entriesOfDropInFixtureStatePlayers(input: unknown, path: string): Array<[string, unknown]> {
...
export function validateDropInFixturePlayerState(input: unknown, path = "dropInFixturePlayer"): IDropInFixturePlayerState {
    return guardWire(path, () => {
        const value = stateRecord(input, path);
        assertExactKeys(value, ["id","name"], [], path);
...
export function validateDropInFixtureState(input: unknown): IDropInFixtureState {
    const path = "dropInFixtureState";
    return guardWire(path, () => {
        const value = stateRecord(input, path);
        ...
        const entries = entriesOfDropInFixtureStatePlayers(value.players, path + ".players");
        if (entries.length > MAX_PLAYERS) throw new WireValidationError("STATE_PLAYERS", path + ".players");
        const players = new Map<string, IDropInFixturePlayerState>();
        for (const [entryKey, entryValue] of entries) {
            ...
            const parsed = validateDropInFixturePlayerState(entryValue, path + ".players." + entryKey);
