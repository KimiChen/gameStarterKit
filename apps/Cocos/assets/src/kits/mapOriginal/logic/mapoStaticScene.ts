...
export function mapoStaticDecorSprites(rect: IMapoGroundRect): MapoSpriteInput[] {
    ...
    for (let row = Math.max(0, Math.floor((s0 + d0) / 2)); row <= Math.min(MAPO_MAP_ROWS - 1, Math.ceil((s1 + d1) / 2)); row++) {
        for (let col = Math.max(0, Math.floor((s0 - d1) / 2)); col <= Math.min(MAPO_MAP_COLS - 1, Math.ceil((s1 - d0) / 2)); col++) {
            ...
            if (!place) continue;
            out.push(...mapoSceneSprites(place.cell.scene, MAPO_DECOR_TEXTURES, [MAPO_DECOR_ATLAS_W, MAPO_DECOR_ATLAS_H], 0, place));
        }
    }
    return out;
}
...
export function mapoStaticScene(rect: IMapoGroundRect, resources: boolean, details = true): MapoStaticBatch[] {
    ...
    for (const kind of ["desert", "snow"]) {
        ...
        add(`${kind}-top-atlas`, buildMapoSpriteMeshes(mapoTopsFor(kind, polys, Infinity, 0)));
    }
    ...
    add("river-top-atlas", buildMapoSpriteMeshes(mapoTopsFor("river", rivers, Infinity, 0)));
    if (resources && details) add("decor-atlas", buildMapoSpriteMeshes(mapoStaticDecorSprites(rect)));
    return out;
}
