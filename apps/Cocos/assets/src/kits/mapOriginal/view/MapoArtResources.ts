...
import {
    MAPO_DECOR_ATLAS_ASSET, MAPO_MINIMAP_ASSET, MAPO_TERRAIN_ASSET, mapoAtlasAsset, mapoPlateAsset,
} from "../logic/mapoFar";
...
export async function loadMapoArt(): Promise<MapoArtResources> {
    ...
    const [plate4, plate5, minimap, decorAtlas, terrain, ...atlases] = await Promise.all([
    ...
    let released = false;
    return {
        plate4, plate5, minimap, terrain, decorAtlas,
        atlasFor: (lod) => byLod.get(lod) ?? null,
        release() {
            if (released) return;
            released = true;
            for (const a of [plate4, plate5, minimap, decorAtlas, ...atlases]) a?.decRef();
            terrain?.decRef();
        },
    };
}
