...
export interface SgzzOffsetStencil {
    /** 打包成 dr*65536+dc+偏移，解包用 sgzzUnpackOffset。 */
...
export function sgzzPackOffset(dr: number, dc: number): number {
...
function buildStencil(parity: 0 | 1, halfWorldW: number, halfWorldH: number,
    ...
    const base = sgzzGrid2Pos(baseRow, baseCol);
    const marginX = marginTiles * SGZZ_TILE_HALF_W * 2;
    const marginY = marginTiles * SGZZ_TILE_HALF_H * 2;
    ...
    // ⛔ 不能用 sgzzPos2GridRaw —— 它带奇偶行的半格分支，会把包围盒算小一格，
    //    表现就是视口角上的格漏进不了可视集合。
    const linear = (x: number, y: number) => {
        const a = x / SGZZ_TILE_HALF_W;
        const b = -y / SGZZ_TILE_HALF_H - 1;
    ...
    for (let row = minRow; row <= maxRow; row += 1) {
        for (let col = minCol; col <= maxCol; col += 1) {
            ...
            if (p.x < left - SGZZ_TILE_HALF_W || p.x > right + SGZZ_TILE_HALF_W) continue;
            if (p.y < bottom - SGZZ_TILE_HALF_H || p.y > top + SGZZ_TILE_HALF_H) continue;
            out.push(sgzzPackOffset(row - baseRow, col - baseCol));
            if (out.length > SGZZ_MAX_STENCIL_CELLS) return new Int32Array(out.slice(0, SGZZ_MAX_STENCIL_CELLS));
        }
    }
...
export class SgzzViewportStencil {
    ...
    forEach(centreRow: number, centreCol: number, rows: number, cols: number,
        ...
        for (let i = 0; i < offsets.length; i += 1) {
            const { dr, dc } = sgzzUnpackOffset(offsets[i]);
