...
import { sgzzMinimapCell, sgzzMinimapViewport } from "../logic/sgzzFar";
...
export class SgzzMinimap {
    ...
    constructor(parent: Node, private readonly size: number, x: number, y: number,
                art: SgzzArtResources | null, private readonly onLocate: (row: number, col: number) => void) {
        this.node = new Node("sgzz-minimap");
        ...
        if (art?.minimap) {
            ...
            createSolidPlate(this.node, size, size, FALLBACK, 0, 0, "sgzz-minimap-fallback");
        }
    ...
    private onTap(event: EventTouch): void {
        ...
        const cell = sgzzMinimapCell(u, v);
        this.onLocate(cell.row, cell.col);
    }

    update(logic: SgzzmapWorldLogic): void {
        if (this.disposed) return;
        const rect = sgzzMinimapViewport(logic.camera.x, logic.camera.y,
