        ...
        class TextEncoderPolyfill {
            ...
            encode(input = ""): Uint8Array {
                ...
                for (let i = 0; i < input.length; i++) {
                    ...
                    // 孤代理（lone surrogate，如被切开的 emoji 半边）：原生输出 U+FFFD，
                    // ⛔ 不能按 CESU-8 编成 3 字节（与原生行为不符，对端解码出乱码）
        ...
        class TextDecoderPolyfill {
            ...
            decode(input?: ArrayBuffer | ArrayBufferView): string {
                ...
                while (i < bytes.length) {
                    ...
                    let need = 0;
                    if (b0 < 0x80) { code = b0; }
                    else if (b0 >= 0xc0 && b0 < 0xe0) { code = b0 & 0x1f; need = 1; }
                    else if (b0 >= 0xe0 && b0 < 0xf0) { code = b0 & 0x0f; need = 2; }
                    else if (b0 >= 0xf0) { code = b0 & 0x07; need = 3; }
                    else { code = 0xfffd; } // 孤续字节（0x80-0xbf 落在首字节位）：U+FFFD
                    let truncated = false;
                    for (let k = 0; k < need; k++) {
                        // 截断/非法续字节：原生输出 U+FFFD，⛔ 不能越界读出 undefined 当 0 拼进字符（塞 \0）
                        if (i >= bytes.length || (bytes[i] & 0xc0) !== 0x80) { truncated = true; break; }
                        code = (code << 6) | (bytes[i++] & 0x3f);
                    }
                    if (truncated) { code = 0xfffd; }
                    if (code >= 0x10000) {
                        code -= 0x10000;
    ...
    try {
        if (typeof g.wx?.connectSocket === "function") {
            ...
            g.wx.connectSocket = (opts: any) => {
                // ⛔ 不用 flatMap：ES2019 API，超出本项目 ES2017 运行时下限（铁律 4）——
                // 且本函数恰在建连路径上，老 JSCore 会在兼容层自身抛 TypeError（raw 来自
                // any 时 tsc 的 lib 钉板拦不住，此处显式钉 unknown[] + ES2017 写法双保险）
                const raw: unknown[] = Array.isArray(opts?.protocols) ? opts.protocols : [];
                const protocols: string[] = [];
                for (const x of raw) {
                    if (typeof x === "string") {
                        protocols.push(x);
                    } else if (x && typeof x === "object" && (x as { protocols?: unknown }).protocols) {
                        for (const s of ([] as unknown[]).concat((x as { protocols: never }).protocols)) {
                            if (typeof s === "string") { protocols.push(s); }
                        }
                    }
                }
                const next = { ...opts };
