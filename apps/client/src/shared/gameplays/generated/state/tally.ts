/** AUTO-GENERATED by apps/server/tools/gameplay-codegen/cli.ts from apps/shared/schema/gameplays/tally/{manifest.json,state.json}. Do not edit. */
...
/** Root players map capacity. Source: apps/shared/schema/gameplays/tally/manifest.json (maxPlayers). */
const MAX_PLAYERS = 4;

/** Dependency-free mirrors of the Colyseus wire state. */
export interface ITallyPlayerState {
    /** Colyseus sessionId */
    id: string;
    name: string;
...
export interface ITallyRoomState {
    ...
    players: Map<string, ITallyPlayerState>;
}
...
function entriesOfTallyRoomStatePlayers(input: unknown, path: string): Array<[string, unknown]> {
...
export function validateTallyPlayerState(input: unknown, path = "tallyPlayer"): ITallyPlayerState {
    return guardWire(path, () => {
        const value = stateRecord(input, path);
        assertExactKeys(value, ["id","name","taps"], [], path);
        const id = boundedString(value.id, path + ".id", 1, 64);
        const name = boundedString(value.name, path + ".name", 1, 128);
        const taps = finiteInteger(value.taps, path + ".taps", 0, Number.MAX_SAFE_INTEGER);
        return {
            id,
            name,
            taps,
...
export function validateTallyRoomState(input: unknown): ITallyRoomState {
    const path = "tallyState";
    return guardWire(path, () => {
        const value = stateRecord(input, path);
        assertExactKeys(value, ["tick","phase","matchId","tapGoal","winnerId","players"], [], path);
        ...
        const tapGoal = finiteInteger(value.tapGoal, path + ".tapGoal", 1, Number.MAX_SAFE_INTEGER);
        const winnerId = boundedString(value.winnerId, path + ".winnerId", 0, 128);
        const entries = entriesOfTallyRoomStatePlayers(value.players, path + ".players");
        if (entries.length > MAX_PLAYERS) throw new WireValidationError("STATE_PLAYERS", path + ".players");
        const players = new Map<string, ITallyPlayerState>();
        for (const [entryKey, entryValue] of entries) {
            ...
            const parsed = validateTallyPlayerState(entryValue, path + ".players." + entryKey);
