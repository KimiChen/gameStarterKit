 ...
 */
import { NativeLobbyTransport } from "../net/NativeLobbyTransport";
  ...
  return {
    ...
    lobbyRpc: {
      ...
      sendIdempotent: async (type, payload) => {
        const clientReqId =
          lobbyTransportHub.currentConfig.kind === "native-websocket"
            ? NativeLobbyTransport.newClientReqId()
            : WebSocketClient.newClientReqId();
    ...
    lifecycle: {
      subscribeConnection: (listener) =>
        deps.track(lobbyTransportHub.subscribeConnection(listener)),
      getConnectionState: () => lobbyTransportHub.getConnectionState(),
    ...
    launch: {
      ...
      launch: (target) =>
        deps.launch ? deps.launch(target) : deps.enterBattle(),
    },
  };
}
