...
import type { GameDemoAssets, GameDemoMail } from "../../../shared/kits/gameDemo/api/growth/index";
import { GAME_DEMO_CONFIG } from "../../../shared/kits/gameDemo/config";
import type { GameDemoHeroState } from '../../../shared/kits/gameDemo/api/growth/index';
...
export class GameDemoLogic {
    ...
    mails: GameDemoMail[] = [];
    mailPage = 0;
    assets: GameDemoAssets | null = null;
    heroState: GameDemoHeroState | null = null;
    alchemyState: GameDemoAlchemyState | null = null;
    seasonState: GameDemoSeasonState | null = null;
    rankPage = 0;
    guildState: GameDemoGuildState | null = null;
    guildName = '修行仙盟';
    inviteTarget = '';
    invitePage = 0;
    bossList: GameDemoBossList | null = null;
    bossState: GameDemoBossState | null = null;
    bossId: GameDemoBossId | null = null;
    private bossGeneration = 0;
    private bossRefreshedAt = 0;
    private bossDirty = false;
    private alchemyReceivedAt = 0;
    ...
    watchClock(signal: AbortSignal): void {
        this.runtime?.onBossChanged?.(event => this.receiveBossChange(event), signal);
        let second = -1;
        this.runtime?.onTick(() => {
            ...
            if (this.alchemyAnimationUntil && !this.alchemyAnimationRemaining) {
                ...
                if (this.page === 'alchemy' && this.alchemyState) {
                    this.showAlchemy(this.alchemyState, false);
                    this.onChanged();
                }
            }
            if (!this.alchemyAnimationUntil && this.alchemyState?.batch?.phase !== 'running') return;
    ...
    receiveBossChange(event: GameDemoBossChanged): void {
        const current = this.bossState;
        if (this.page !== 'boss' || !current || event.bossId !== this.bossId || event.generation !== current.generation) return;
        if (event.runNumber < current.room.runNumber || event.ownerEpoch < current.room.ownerEpoch) return;
        if (event.runNumber === current.room.runNumber && event.revision <= current.room.revision && event.ownerEpoch === current.room.ownerEpoch) return;
        this.bossDirty = true;
    }
    ...
    async attackBoss(autoAttack?: boolean): Promise<void> {
        ...
        this.busy = true;
        try { this.showBoss(await this.runtime.attackBoss(current.room.bossId, current.room.runId, current.generation, autoAttack)); }
        catch (error) {
    ...
    private showBossList(state: GameDemoBossList): void {
        if (state.generation < this.bossGeneration) return;
        this.bossGeneration = state.generation;
        this.bossList = state;
        this.text = '三个独立共享房间 · 选择 Boss 进入\n切换后旧房间攻击不会生效';
    }
    private showBoss(state: GameDemoBossState): void {
        if (state.generation < this.bossGeneration || state.room.bossId !== this.bossId) return;
        const prior = this.bossState;
        if (prior && prior.room.bossId === state.room.bossId && (state.room.runNumber < prior.room.runNumber || state.room.ownerEpoch < prior.room.ownerEpoch || (state.room.runNumber === prior.room.runNumber && state.room.revision < prior.room.revision))) return;
    ...
    async guildAction(kind: 'create' | 'invite' | 'leave' | 'accept' | 'reject', inviteId = ''): Promise<void> {
        if (!this.runtime || this.busy) return;
        this.busy = true;
        try {
            const result = kind === 'create' ? await this.runtime.createGuild(this.guildName.trim())
                : kind === 'invite' ? await this.runtime.inviteGuild(this.inviteTarget.trim())
    ...
    private showGuild(state: GameDemoGuildState): void {
    ...
    async endSeason(): Promise<void> {
        const id = this.seasonState?.id;
        if (!this.runtime || this.busy || !id) return;
        this.busy = true;
        try { this.showSeason(await this.runtime.endSeason(id)); }
    ...
    private showSeason(state: GameDemoSeasonState): void {
        this.seasonState = state;
        this.turnRankPage(0);
        const phase = { running: '进行中', settling: '正在结算', settled: '奖励已投递' }[state.phase];
        const rank = state.myRank === null ? (state.myScore ? '20 名外' : '未上榜') : `第 ${state.myRank} 名`;
        this.text = `活动 ${state.id} · ${phase}\n截止剩余 ${Math.max(0, Math.ceil((state.endsAt - state.serverNow) / 1000))} 秒（刷新更新）\n我的积分 ${state.myScore} · ${rank}\n邮件奖励 ${state.deliveredRewards}/${state.totalRewards}`;
    }
    ...
    async startAlchemy(count: number): Promise<void> {
        ...
        catch { this.text = this.alchemyState?.batch?.phase === 'running'
            ? '炼丹未完成，请先领取旧批次成品' : '炼丹未完成，请检查材料数量后重试'; }
        finally { this.busy = false; this.onChanged(); }
    }
    async finishAlchemy(early: boolean): Promise<void> {
        const batchId = this.alchemyState?.batch?.id;
        if (this.busy || !this.runtime || !batchId) return;
        this.busy = true;
        try { this.showAlchemy(await this.runtime.finishAlchemy(batchId, early)); }
        catch { this.text = '结束失败，请刷新确认批次和完成时间'; }
        finally { this.busy = false; this.onChanged(); }
    }
    private showHero(state: GameDemoHeroState): void {
    ...
    private showAlchemy(state: GameDemoAlchemyState, resetClock = true): void {
        this.alchemyState = state; this.assets = state.assets;
        if (resetClock) this.alchemyReceivedAt = this.runtime?.now() ?? 0;
        const batch = state.batch;
        const elapsed = Math.max(0, (this.runtime?.now() ?? 0) - this.alchemyReceivedAt);
        const remaining = batch ? Math.max(0, Math.ceil((batch.endsAt - state.serverNow - elapsed) / 1000)) : 0;
        ...
        if (batch.phase === 'running') this.text += `\n当前 ${batch.count} 炉 · 剩余 ${remaining} 秒\n完成后领取；提前结束会退还未完成材料`;
        else this.text += `\n已领：经验丹 ${batch.pill} · 极品丹 ${batch.finePill}\n返还：灵草 ${batch.refundedHerb} · 灵露 ${batch.refundedDew}\n本批积分：${batch.score}`;
    }
    ...
    async mailAction(mailId: string, claim: boolean): Promise<void> {
        if (this.busy || !this.runtime) return;
        this.busy = true;
        try {
            if (claim) {
    ...
    private showAssets(assets: GameDemoAssets): void {
        this.assets = assets;
        this.text = `原生服务端 · ${assets.initialized ? "已初始化" : "未初始化"}\n金币：${assets.gold}\n灵草：${assets.items.herb}   灵露：${assets.items.dew}\n经验丹：${assets.items.pill}   极品丹：${assets.items.finePill}`;
    }
    ...
    alchemyNow(): number { return (this.alchemyState?.serverNow ?? 0) + Math.max(0, (this.runtime?.now() ?? 0) - this.alchemyReceivedAt); }
