...
export class GameDemoView extends CocosView {
    ...
    private alchemy(p: Node, h: number, l: GameDemoLogic): void {
        ...
        const remaining = batch ? Math.max(0, Math.ceil((batch.endsAt - l.alchemyNow()) / 1000)) : 0;
        text(
            p,
            animating ? `正在炼制 ${batch?.count ?? 0} 炉 · ${Math.ceil(l.alchemyAnimationRemaining / 1000)} 秒` : batch?.phase === 'running'
                ? `炼制 ${batch.count} 炉 · ${remaining ? remaining + ' 秒' : '可领取'}`
                : batch
        ...
        if (animating || l.busy) {
            ...
            button(p, '领取成品', 0, -h / 2 + 415, 660, () => this.act(() => l.finishAlchemy(false)), true);
            button(p, '提前结束 · 退还未完成材料', 0, -h / 2 + 305, 660, () =>
                this.act(() => l.finishAlchemy(true)),
            );
        } else {
    ...
    private season(p: Node, h: number, l: GameDemoLogic): void {
        ...
        rows.forEach((r, i) => {
            ...
            text(p, r.uid.slice(-14), -15, y, 390, 24);
