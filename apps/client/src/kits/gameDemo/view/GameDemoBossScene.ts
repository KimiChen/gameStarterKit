...
import type {
    ...
    GameDemoBossFighter,
    GameDemoBossState,
} from '../../../shared/kits/gameDemo/api/boss/index';
...
export class GameDemoBossScene {
    ...
    private state: GameDemoBossState | null = null;
    ...
    private toggle(): void {
        const p = this.state?.room.battle?.players.find((p) => p.uid === this.state?.uid);
        if (this.state?.room.phase !== 'running') return;
        this.act(() => this.logic.attackBoss(!p?.autoAttack));
    }
    private turn(delta: number): void {
        const count = this.state?.room.battle?.players.filter((p) => p.active).length ?? 0;
    ...
    update(state: GameDemoBossState): void {
        this.state = state;
        ...
        this.rank.string =
            state.room.damage
                ...
                .map(
                    (d) =>
                        `${d.rank === 1 ? '冠' : d.rank} ${d.uid === state.uid ? '我' : d.uid.slice(-3)}\n${d.damage}`,
                )
                .join('\n') || '暂无\n伤害';
        const players = [...(state.room.battle?.players.filter((p) => p.active) ?? [])].sort((a, b) =>
            a.uid === state.uid ? -1 : b.uid === state.uid ? 1 : a.uid.localeCompare(b.uid),
        );
        ...
        visible.forEach((p, i) => {
            ...
            if (!seat) {
                ...
                seat = {
                    ...
                    name: text(
                        root,
                        p.uid === state.uid ? '我' : p.uid.slice(-6),
                        0,
                        -65,
                        166,
                        22,
    ...
    tick(): void {
        ...
        const mine = this.state.room.battle?.players.find((p) => p.uid === this.state?.uid);
        ...
        for (const p of this.state.room.battle?.players ?? []) {
    ...
    private playerText(seat: Seat, p: GameDemoBossFighter): void {
    ...
    private animate(event: GameDemoBossEvent): void {
    ...
    private renderRanks(): void {
        ...
        text(
            ...
            this.state!.room.damage.slice(0, 10)
                .map(
                    (d) =>
                        `${d.rank}    ${d.uid === this.state!.uid ? '我' : d.uid.slice(-10)}       ${d.damage}`,
                )
                .join('\n') || '尚无伤害记录',
            0,
            10,
