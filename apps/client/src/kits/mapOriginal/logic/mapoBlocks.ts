...
export function mapoSetBlockGeo(kind: string, buf: ArrayBuffer | Uint8Array): void {
...
export function mapoSetBlocks(kind: string, buf: ArrayBuffer | Uint8Array): void {
    const layer = LAYERS.get(kind);
    if (!layer) throw new Error(`mapOriginal 没有 ${kind} 这一层`);
...
export function mapoHasBlocks(kind: string): boolean {
    const l = LAYERS.get(kind);
    return !!l && l.view !== null && l.geos.length > 0;
}

export function mapoBlockCount(kind: string): number { return LAYERS.get(kind)?.count ?? 0; }
 ...
 */
export function mapoBlocksInRect(kind: string, rect: IMapoBlockRect,
