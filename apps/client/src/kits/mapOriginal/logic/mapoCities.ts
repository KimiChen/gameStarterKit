...
export function createMapoCitiesData() {
    let PIECES: CitySprite[][] = [];

    let PLACED: IMapoCityPlacement[] = [];

    /** 注入 `cities.bin`。⚠ 件数/精灵数/摆位数/长度任一对不上就拒收。 */
    function mapoSetCities(buf: ArrayBuffer | Uint8Array): void {
    ...
    function mapoHasCities(): boolean {
        return PIECES.length > 0 && PLACED.length > 0;
    }

    /** 全部摆位（⚠ 只读；裁剪由调用方按视口做）。 */
    function mapoCityPlacements(): readonly IMapoCityPlacement[] {
        return PLACED;
    }

    /** 图集格 → 归一化 UV [u0, v0, uw, vh]（v 原点在上）。 */
    function mapoCityUv(cell: IMapoCityCell): readonly [number, number, number, number] {
     ...
     */
    function mapoCitiesIn(minX: number, maxX: number, minY: number, maxY: number,
    ...
    function mapoCitiesDataUsage(): Readonly<Record<string, number>> {
        return { arrayBufferBytes: 0, pieces: PIECES.length,
            sprites: PIECES.reduce((sum, p) => sum + p.length, 0), placements: PLACED.length };
    }

    return { mapoSetCities, mapoHasCities, mapoCityPlacements, mapoCityUv, mapoCitiesIn, mapoCitiesDataUsage,
        dispose(): void { PIECES = []; PLACED = []; },
    };
}

/** 兼容离线烘焙与已有测试；地图运行时必须使用 MapoDataStore 的独立读取器。 */
export const { mapoSetCities, mapoHasCities, mapoCityPlacements, mapoCityUv, mapoCitiesIn, mapoCitiesDataUsage } = createMapoCitiesData();
