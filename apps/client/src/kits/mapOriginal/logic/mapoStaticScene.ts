...
export function mapoRegionSprites(rect: IMapoGroundRect, data?: MapoDataStore): MapoSpriteInput[] {
    return (data?.regions.mapoRegionsInRect ?? mapoRegionsInRect)(rect, Infinity).map((p) => ({
...
export function mapoStaticScene(rect: IMapoGroundRect, resources: boolean, details = true, seconds = 0, data?: MapoDataStore): MapoStaticBatch[] {
    ...
    for (const kind of ["desert", "snow"]) {
        const polys = (data?.blocks.mapoBlocksInRect ?? mapoBlocksInRect)(kind, sourceRect, Infinity);
        add(`${kind}-base`, buildMapoPolygonMeshes(polys), true);
        add(`${kind}-top-atlas`, buildMapoSpriteMeshes((data?.tops.mapoTopsFor ?? mapoTopsFor)(kind, polys, Infinity, seconds)));
    }
    if (details) add("region-atlas", buildMapoSpriteMeshes(mapoRegionSprites(rect, data)));
    add("road-atlas", buildMapoSpriteMeshes((data?.roads.mapoRoadsInRect ?? mapoRoadsInRect)(rect, Infinity)));
    const rivers = (data?.rivers.mapoRiversInRect ?? mapoRiversInRect)(sourceRect, Infinity,
        (s) => [(s * 2 + 1) / (MAPO_RIVER_SYSTEMS.length * 2), 0.5], [1, 1, 1, 1]);
    add("river-fill", buildMapoPolygonMeshes(rivers));
    add("river-top-atlas", buildMapoSpriteMeshes((data?.tops.mapoTopsFor ?? mapoTopsFor)("river", rivers, Infinity, seconds)));
    if (resources && details) add("decor-atlas", buildMapoSpriteMeshes(mapoStaticDecorSprites(rect, seconds, undefined, undefined, data)));
    return out;
}
