...
export function createMapoRoadsData(bandAt: (row: number, col: number) => number = mapoBandAt) {
    let view: DataView | null = null;

    let count = 0;

    function mapoSetRoads(buf: ArrayBuffer | Uint8Array): void {
    ...
    function mapoHasRoads(): boolean { return view !== null; }

    function mapoRoadCount(): number { return count; }
     ...
     */
    function mapoRoadPos(row: number, col: number): { x: number; y: number } {
    ...
    function mapoRoadCellAt(cellId: number, x: number, y: number): IMapoRoadCell | undefined {
        const cell = CELL_BY_ID.get(cellId);
        if (!cell) return undefined;
        const grid = mapoPos2GridRaw(x, y);
        return bandAt(grid.row, grid.col) === MAPO_BAND_SNOW ? CELL_BY_ID.get(cell.snowId) : cell;
    }

    /** 可视矩形内的路片，**已是画家序**（表的落盘序）。 */
    function mapoRoadsInRect(rect: IMapoRoadRect, limit: number): MapoSpriteInput[] {
    ...
    function mapoRoadsDataUsage(): Readonly<Record<string, number>> {
        return { arrayBufferBytes: view?.buffer.byteLength ?? 0, placements: count };
    }

    return { mapoSetRoads, mapoHasRoads, mapoRoadCount, mapoRoadPos, mapoRoadCellAt, mapoRoadsInRect, mapoRoadsDataUsage,
        dispose(): void { view = null; count = 0; },
    };
}

/** 兼容离线烘焙与已有测试；地图运行时必须使用 MapoDataStore 的独立读取器。 */
export const { mapoSetRoads, mapoHasRoads, mapoRoadCount, mapoRoadPos, mapoRoadCellAt, mapoRoadsInRect, mapoRoadsDataUsage } = createMapoRoadsData();
