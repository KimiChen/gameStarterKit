...
import { MAPO_MINIMAP_ASSET, MAPO_TERRAIN_ASSET, mapoAtlasAsset, mapoPlateAsset } from "../logic/mapoFar";
...
export async function loadMapoArt(): Promise<MapoArtResources> {
    ...
    const [plate4, plate5, minimap, terrain, ...atlases] = await Promise.all([
    ...
    let released = false;
    return {
        plate4, plate5, minimap, terrain,
        atlasFor: (lod) => byLod.get(lod) ?? null,
        release() {
            if (released) return;
            released = true;
            for (const a of [plate4, plate5, minimap, ...atlases]) a?.decRef();
            terrain?.decRef();
        },
    };
}
