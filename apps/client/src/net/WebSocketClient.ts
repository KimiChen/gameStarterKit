...
import { RpcError, type LocalErrCode } from "./LobbyRpcError";

export { RpcError, type LocalErrCode } from "./LobbyRpcError";
...
export class WebSocketClient {
  ...
  private readonly ownershipSlots = new WeakMap<
  ...
  private connectionSnapshot: LobbyConnectionSnapshot = {
    state: "idle",
    connGeneration: 0,
    lastSeq: 0,
  };
  ...
  subscribeConnection(listener: LobbyConnectionListener): () => void {
    ...
    if (snapshot.state !== "idle") {
      ...
      try {
        listener({
          kind,
          connGeneration: snapshot.connGeneration,
  ...
  private publishConnectionAuthInvalid(
    slot: LobbySlot,
   ...
   */
  private publishConnectionClosed(
    slot: LobbySlot,
    reason: "voluntary" | "final-loss",
  ): void {
  ...
  init(endpoint: string): void {
    const validated = validateOrigin(
      endpoint,
      ["http", "https", "ws", "wss"],
      "endpoint",
    );
  ...
  async join(
    ...
    if (slot && !slot.cancelled && this.slot === slot)
  ...
  joinOwned(
    ...
    if (!this.client)
      throw new Error("[WebSocketClient] 未初始化，请先调用 init(endpoint)");
    ...
    if (slot && slot.connectionKey !== key) {
      throw new Error(
        "[WebSocketClient] 当前大厅连接参数与本次 join 不一致，请先 leave() 再 join()",
      );
    }
    if (!slot) {
      slot = {
    ...
    try {
      ...
      if (owner.active && alreadyAborted) {
        cancelOwner(
          new JoinError("CANCELLED", "[WebSocketClient] join 已取消"),
        );
        ...
        timer = setTimeout(
          () =>
            cancelOwner(
              new JoinError("TIMEOUT", "[WebSocketClient] join 超时"),
            ),
          waitMs,
        );
    ...
    const ownership: LobbyConnectionOwnership = {
      ...
      leave: () => {
        ...
        if (owner.active)
          cancelOwner(new Error("[WebSocketClient] ownership 已释放"));
  ...
  private bindRoom(slot: LobbySlot, room: Colyseus.Room): void {
    const current = () =>
      this.slot === slot && slot.room === room && !slot.cancelled;
    ...
    room.onMessage(LOBBY_MSG_PUSH, (raw: unknown) => {
      ...
      if (msg.type === LobbyPush.ForceLogout) {
        this.publishConnectionAuthInvalid(
          slot,
          FORCE_REASON_MAP[msg.data.reason],
        );
      }
    ...
    room.onLeave((code?: number) => {
      ...
      if (forced)
        this.publishConnectionAuthInvalid(slot, FORCE_REASON_MAP[forced]);
  ...
  private closePhysicalRoom(
    ...
    const leave = Promise.resolve()
      .then(() => {
        try {
          const reconnection = (
            room as unknown as { reconnection?: { enabled?: boolean } }
          ).reconnection;
          if (reconnection && typeof reconnection === "object")
            reconnection.enabled = false;
        } catch {
          /* malformed adapter: continue with best-effort leave */
        }
        try {
  ...
  rpc<T extends LobbyRpcType>(type: T, payload: RpcReq<T>): Promise<RpcRes<T>> {
    ...
    try {
      ...
      return Promise.reject(
        this.invalidPayloadError(`request ${String(type)}`, error),
      );
    }
    ...
    for (let attempt = 0; ; attempt++) {
      try {
        ...
        const retriable =
          ...
          (e.code === "BUSY" || e.code === "STALE_FENCE");
  ...
  onPush<K extends keyof LobbyPushMap>(
    ...
    set.add(cb);
    return () => {
      this.pushHandlers.get(type)?.delete(cb);
    };
  }
  ...
  private abandonOnReplayGuardLoss(slot: LobbySlot, room: Colyseus.Room): void {
    ...
    // 这是一次非自愿的最终断线。置下 `cancelled` 之后迟到的 onLeave 会被
    // `current()` 挡掉，因此 owner 释放与最终断线上报必须**在这里**做完，
    // 否则 owner 永远停在 active、页面组合根也收不到 connLost 而无法进入对账。
    // 只做一次由 `cancelled` 保证（两个调用点先后触发时第二次已在上面 return）。
    slot.cancelled = true;
    this.rejectAll("CONN_LOST", slot);
    for (const owner of slot.owners) {
      owner.active = false;
