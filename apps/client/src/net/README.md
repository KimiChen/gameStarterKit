...
| （无，纯客户端状态） | `serverSession.ts`（当前选中区服、列表、`isOps` 与哈希） | 页面写入，Lobby/GameRoom 读取 `gameWsUrl` + `listHash` |
...
matchmaking HTTP URL），`gameHttpUrl` 仍只用于 Portal/游戏 HTTP 请求。每次 Lobby/Game join 都带上
同一份目录 `listHash`，服务端可据此拒绝陈旧目录；列表刷新采用 `serverSession` 的整体快照，成功时保留
仍存在的当前区，失败时不抹掉旧快照。不要从旧字段或宿主地址猜测区服端点。
