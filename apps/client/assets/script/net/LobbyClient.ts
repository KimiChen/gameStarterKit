 ...
 *
 * 与 NetManager（GameRoom：fire-and-forget + 状态同步）职责分离：本类只管 lobby 房的
...
export class LobbyClient {
    private static _inst: LobbyClient | null = null;
    static get inst(): LobbyClient {
        if (!this._inst) this._inst = new LobbyClient();
        return this._inst;
    }
    ...
    async join(token: string): Promise<void> {
        if (!this.client) throw new Error("[LobbyClient] 未初始化，请先调用 init(endpoint)");
        ...
        if (this.joinedToken !== token) {
            throw new Error("[LobbyClient] 已用其他 token 在线：换号必须先 leave() 再 join()");
        }
    }
    ...
    private async doJoin(token: string): Promise<void> {
        ...
        room.onMessage(LOBBY_MSG_PUSH, (msg: { type: string; data: unknown }) => {
            ...
            if (!set) return;
            for (const cb of set) {
                try { cb(msg.data); } catch (e) { console.error("[LobbyClient] push 处理器异常", e); }
            }
        });
    ...
    /**
     * 主动离开大厅房。掉线重连窗口里调用也安全：先停自动重连（防 LEAVE 帧丢失后
     ...
     *
     *   const { uid } = await LobbyClient.inst.rpc(UserRpc.GetUserId, {});
     */
    ...
    async rpcIdem<T extends LobbyRpcIdemType>(
        ...
        clientReqId: string = LobbyClient.newClientReqId(),
    ): Promise<RpcRes<T>> {
