...
└── game/                # ECS 用法示例：组件/系统/世界管理
    └── ui/              # FairyGUI 三层模型：契约(fguiContracts) + 纯 presenter（无头测）
        └── fgui/        # 绑定层（FguiView 薄基类；依赖 fairygui-cc，Creator 侧验证）

assets/resources/ui/     # FairyGUI 发布产物（Original 等公司库的 .bin + 图集；源在 apps/art/fairygui）
