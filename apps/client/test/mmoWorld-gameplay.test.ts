 ...
 *  - MmoWorldGameplay：观察者回调 → 视图模型（本人 / 实体 / hp・mp / synced / notice）；输入 → 房间意图（move 钳到 [-1,1]、stop、leave ⇒ requestExit）；
...
function fakeRoom() {
    ...
    const room: MmoWorldRoom = {
        ...
        move: (dir) => { calls.push(["move", dir]); return true; },
        moveTo: (target) => { calls.push(["moveTo", target]); return true; },
...
test("MmoWorldGameplay：实体表 → 模型（本人 / 视野 / hp）；输入 → 意图；leave ⇒ requestExit(user-exit)；踢出 ⇒ settled", async () => {
    ...
    assert.deepEqual(calls, [["move", { x: 1, y: -0.5 }], ["moveTo", { x: 0, y: 40 }], ["stop"]]);
...
test("createMmoWorldRoom：意图 seq 递增；baseline 三件 → 实体表（synced）；update 合并；私有流推进 cursor；seq 断裂 ⇒ 重同步请求", () => {
