...
type Stub = {
    ...
    readonly calls: { pending: number; settle: number; claim: number; opened: number; closed: number };
};
...
function stub(options: {
    ...
    const runtime: IncomeRuntime = {
        pending: async () => { calls.pending += 1; return (options.pending ?? (async () => SNAPSHOT))(); },
        settleOnline: async () => {
            calls.settle += 1;
            return (options.settleOnline ?? (async () => ({ copper: 334, balance: 2002 })))();
        },
        claimOffline: async () => {
            calls.claim += 1;
            return (options.claimOffline ?? (async () => ({ copper: 8016, offlineSeconds: 120, balance: 9684 })))();
        },
...
test("income：错误保留可重试提示，收益账户不因请求失败停用", async () => {
    const lost = stub({ pending: async () => { throw new FakeRpcError("USER_DATA_LOST"); } });
    ...
    const offline = stub({ settleOnline: async () => { throw new FakeRpcError("CONN_LOST"); } });
