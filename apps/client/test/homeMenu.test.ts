 ...
 *  - 菜单唯一数据源 = generated menu contributions；排序 pluginId → entryId 由独立比较器
 *    重算核对，并与手写 apps/plugins/<id>/plugin.json 双向核对（⛔ manifest 无 slot/order）；
 *  - **位置归宿主**（docs/PLUGIN.md §6）：首屏入口顺序与默认 launch target 都来自手写
 *    apps/plugins/host.json（经 codegen 生成 GENERATED_HOST），⛔ 本文件不写死是哪个玩法；
 *  - HomeLogic：主入口点击唯一走 entry.launch；disabled/failed 叠加 = handler 拒绝
 *    （无 FGUI 视觉时以拒绝 + 状态查询断言表达；GList 视觉是编辑器待办）；
 *  - PluginHost 运行时可用性叠加：install 失败的 fixture plugin → failed → 入口不可用；
...
} from "../src/generated/plugins.generated";
import {
  APP_PLUGINS,
  BUILTIN_PLUGIN,
...
} from "../src/app/builtinPlugin";
import { PluginRegistry } from "../src/app/PluginRegistry";
import { PluginHost, type PluginModule } from "../src/app/PluginHost";
...
/** 手写真源侧的宿主 placement：直接读 apps/plugins/host.json（⛔ 不复用生成物）。 */
function readHostManifest(): { defaultLaunch: { gameplayId: string }; home: string[] } {
  return JSON.parse(readFileSync(join(REPO_ROOT, "apps/plugins/host.json"), "utf8"));
}

test("generated contributions 按 pluginId → entryId 排序且 = 手写 manifest 并集；manifest ⛔ 无 slot/order", () => {
...
test("位置归宿主：首屏入口顺序 = 手写 apps/plugins/host.json 的 home；每条都能解析到一条 contribution", () => {
  const host = readHostManifest();
  assert.deepEqual(GENERATED_HOST.home.map((entry) => `${entry.pluginId}/${entry.entryId}`), host.home,
    "GENERATED_HOST.home 必须逐字来自手写 host.json（⛔ 无第二真源）");
  const registry = new PluginRegistry(APP_PLUGINS);
  assert.deepEqual(
    registry.homeContributions().map((item) => `${item.pluginId}/${item.entryId}`),
    host.home,
...
test("默认 launch target = apps/plugins/host.json 的 defaultLaunch：AppRuntime/Main 兜底只读生成值，⛔ 不硬编码玩法名", () => {
  // ① 值：默认兜底恒等于宿主手写声明（⛔ 不再从菜单排序推导——排序首条会静默翻成回归样例）。
  const host = readHostManifest();
  assert.equal(DEFAULT_LAUNCH_GAMEPLAY_ID, host.defaultLaunch.gameplayId,
    "默认 launch target 必须跟随 apps/plugins/host.json 的 defaultLaunch（改 host.json 即换默认入口）");
  assert.equal(new PluginRegistry(APP_PLUGINS).defaultLaunchGameplayId(), host.defaultLaunch.gameplayId);
  assert.ok(
...
test("contribution 归属：拥有自己 plugin 的玩法，菜单入口必须由该 plugin 贡献（⛔ 不得留在 builtin）", () => {
  // §3.2 第 8 条的例外已关闭（apps/plugins/snake/ 落地）：玩法只要有自己的 apps/plugins/<id>/，
  // 它的 Home 入口就必须写在自己的 manifest 里，⛔ 不得再回写 built-in 的中央菜单表。
  // 反之，尚无独立 plugin 的玩法（ballMove：保留为可选入口与内部回归样例）不受此约束。
  const pluginIds = new Set(PLUGIN_IDS);
  ...
  const misplaced = gameplayItems
    .filter(({ item, gameplayId }) => pluginIds.has(gameplayId) && item.pluginId !== gameplayId)
    .map(({ item, gameplayId }) => `${item.pluginId}/${item.entryId} → ${gameplayId}`);
  assert.deepEqual(misplaced, [],
    "玩法自持 plugin 时其 contribution 必须由自己贡献（把入口搬回 apps/plugins/builtin 即红）");
  // 自洽闭合：本仓当前确实存在这样一个玩法（否则上面的过滤恒空，断言退化为永真）。
  const selfOwned = gameplayItems.filter(({ gameplayId }) => pluginIds.has(gameplayId));
  assert.ok(selfOwned.length >= 1, "至少一个玩法拥有自己的 plugin（否则本断言无判别力）");
});

test("菜单排序：多 contribution fixture 按 pluginId → entryId；首屏顺序只听宿主 placement；引用不存在的入口 fail-fast", () => {
  const fixture = (pluginId: string, entryId: string): GeneratedMenuContribution => ({
  ...
  assert.deepEqual(
    registry.menuContributions().map((item) => `${item.pluginId}/${item.entryId}`),
    ["alpha/aA", "alpha/aC", "alpha/aD", "zeta/zA", "zeta/zB"],
    "全量列表：pluginId 先行，entryId 兜底（⛔ 无 slot/order）",
  );
  assert.deepEqual(
    registry.homeContributions().map((item) => `${item.pluginId}/${item.entryId}`),
    ["zeta/zB", "alpha/aD"],
  ...
  assert.throws(
    () => new PluginRegistry(descriptors, { ...host, home: [{ pluginId: "alpha", entryId: "zB" }] }).homeContributions(),
    /宿主 placement 引用不存在的入口: alpha\/zB/u,
    "placement 的 pluginId/entryId 必须与某条 contribution 精确一致",
  );
  assert.throws(
...
test("HomeLogic：主入口点击唯一走 entry.launch；disabled 时 handler 拒绝；无入口回退旧回调", async () => {
  ...
  const entry = (enabled: boolean): HomeMenuEntryModel => ({
    entryId: "ballMove", pluginId: "builtin", label: "进入战斗", enabled,
    launch: () => { launched++; },
  });
...
test("PluginHost 可用性叠加（fixture）：install 失败 → failed 入口拒绝；显式重试成功 → 可用", async () => {
  let failNext = true;
  ...
  // AppRuntime.pluginAvailability 的同款映射（availability 是 catalog 之外的可变叠加层）。
  const availabilityOf = (pluginId: string): "available" | "failed" | "disabled" => {
    const status = host.statusOf(pluginId);
  ...
  const entryFor = (contribution: GeneratedMenuContribution): HomeMenuEntryModel => ({
    ...
    enabled: availabilityOf(contribution.pluginId) === "available",
    launch: () => { launched++; },
  });
  const contribution: GeneratedMenuContribution = {
    entryId: "fxEntry", pluginId: "fx", label: "夹具玩法", labelKey: "menu.fx",
  ...
  assert.equal(logic.primaryEntry()?.enabled, false, "failed plugin 的入口必须渲染为不可用占位");
  await logic.enterBattle();
