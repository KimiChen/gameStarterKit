/**
 * 阶段 5 退出条件 fixture feature（Non-intrusive §9 阶段 5）。
 *
 ...
 * ⛔ 不回写 value。
 */
import type { FeatureInstallContext, FeatureModule } from "../../src/app/FeatureHost";
import type { AppPorts } from "../../src/app/ports";

export interface CounterFeatureProbe {
...
export function createCounterFeature(): CounterFeatureProbe {
    let ports: AppPorts | null = null;
    let installContext: FeatureInstallContext | null = null;
    ...
    return {
        ...
        async refresh(routeSignal: AbortSignal): Promise<boolean> {
            ...
            if (!activePorts || !context) throw new Error("counterFeature 未安装");
