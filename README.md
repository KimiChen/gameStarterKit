...
`ui:import-psd` / `ui:export-psd` / `ui:roundtrip` 使用锁定的 `vendor/web-ui-to-psd-0.1.14.tgz`，
...
| `npm run dev:split` | 本地 lobby / game / world 三进程 watch，先检查栈与 schema，端口默认 2568 / 2569 / 2570 |
| `npm run start:lobby` | 启动大厅进程（`LOBBY_PORT` 缺省 `PORT`） |
| `npm run start:game` | 启动对局进程（`GAME_PORT` 缺省 `PORT`） |
| `npm run start:world` | 启动世界进程（`WORLD_PORT` 缺省 `PORT`） |
...
| `npm run ui:export-fgui -- --screen confirm --out .cache/fgui/confirm` | 从 UniFlex snapshot 写出候选独立 FairyGUI 工程 + DOM 预览包（只写 `--out`） |
| `npm run ui:export-fgui -- --screens small-popup,confirm --out .cache/fgui/popups` | 多页收进同一候选工程，共享 `UniFlex_Common` |
...
| `npm run ui:roundtrip -- --screen confirm --out .cache/psd/roundtrip-001` | UniFlex → PSD → UniFlex 项目包往返；默认不写项目源，加 `--apply` 才导入 |
...
| `npm run dev:uniflex-web` | 启动独立 WebProvider 预览；`/` 是目录壳（组件卡片 + 界面卡片），`?ui=` / `?screen=` 打开已登记页面，`?psd=1` 为导出用未缩放画布 |
