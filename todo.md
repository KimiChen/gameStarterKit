...
- **⚠ 先厘清「组」是什么**（定义已在 [DUAL_MODE §1](../docs/DUAL_MODE.md) 的物理组条目，⛔ 别另立）：
  **一个物理组 = 一整套独立部署** —— N 个 Colyseus app 节点（RedisDriver/RedisPresence 组成横向集群）
  + 该组**独占**的 `{durable, cache, coord}` 三类 Redis + 该组**自己的一套 MySQL**。
  ⇒ A 组与 B 组是**两套互不相通的服务器 + 缓存 + 数据库**，唯一共享的是**账号库**（WebPlatform 权威）。
  物理组是**爆炸半径与开关服的最小单位**；逻辑区（sId）是叠在组内的第二级分区，⛔ 两者不是一回事。
...
- **⚠ 「旧端」的准确定义**（常被误读）：指**一条仍然活着的 WebSocket 连接**，⛔ 不是"没重开的 App"。
  它靠**本组** Redis 的 `sess` 走每消息快路径（纯 hash 比对、零回源），而新登录写的是**另一组**的 Redis
  ⇒ 本组这条永远比不出变化。**连接一断就回不来**：重连要过 `onAuth`（strict 点，回 MySQL 权威），
  那时 `token_hash` 已换，直接拒。故网络抖动/切后台被杀/服务端重启任一发生即终结。
  **3d 是硬上界**：快路径只读不续期（只有 `writeGroupSess` 会 `EXPIRE`），⛔ 不会被活跃度续命。
