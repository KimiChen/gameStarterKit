...
| 跨用户推送（MF6） | **MF6a 已落地（2026-09-19，MMO.md §12）**：`websocket/push.ts` 的 `pushToUsers / pushToRealm / pushToGuild` 经 coord `stream:push` 投递总线跨节点扇出（`core/push/pushBus.ts`）+ `defineLobbyPush` + 客户端 `onPush` + party / chat 域（`domains/{party,chat}.ts`，客户端 `PartyLogic` / `ChatLogic` 样板）+ presence（`core/presence`）+ `POST /admin/notice` 全区公告 | 联盟/聊天/援助可做：联盟事件用 `pushToUsers`（名册在 kit 自有表）、联盟频道可借 chat 的 `setChatPolicy` / 或新增 kit 域；**名册与事件落 kit 自有表** |
...
其 A0 阶段是由 **LVR 项目组主责验证的可行性 spike**（取 1 个模型 + 1 套动画 + 1 张海面贴图渲出来），走不通由项目组调整或重估 LVR 内容管线。框架 SC0–SC5 已于 2026-09-24 收口，框架侧提供接入支持与通用缺陷修复，交付无需等待 A0；责任与接入状态见 [lvr-3d.md §8](lvr-3d.md#8-实施状态回写)。
...
微信/抖音渠道账号、登录、支付、广告、分享 SDK；渠道打包、审核、灰度（⚠ 与 docs/3d.md SD10 不冲突：SD10 要求 WebGL1 下 low 档表现及 lvr-3d.md R0 的 A3 low 退化 / 故障恢复证据；微信测试项目 / AppID 与可运行构建证据已从 3D 前置 / 验收 / 退出要求中移除）；**热更新**；合规与商店发布；
