...
8. **服务端写路径对照 63 条规则**（[docs/SERVER.md §12](docs/SERVER.md#12-开发约束63-条规则目录)；代码注释 `09·XX` 即编号；⛔ 禁 HGETALL/INSERT IGNORE/ZINCRBY/无 fence 写档等）。新增常量/key/错误码先进 [§13 契约表](docs/SERVER.md#13-契约与配置redis-key--字段--错误码--常量) 再进 `core/infra/config.ts`/`keys.ts`（错误码另进 shared `RPC_ERR_CODES`）；登记点清单见 `apps/server/src/core/README.md`。
...
- 验证基线（近期全绿）：typecheck 三端 + verify:sync / 服务端单测 36 / 客户端 test:fgui 79 / 集成测试 130 / 真实链冒烟 13。
