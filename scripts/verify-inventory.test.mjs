...
/**
 * 这套用例每个都要**一份自己的检出**加**一次 verifier 进程**；107 个串行跑要 97s，其中约八成
 * 花在建夹具上（逐文件复制 1999 个文件 ≈ 570ms/次，spawn 只要 112ms）。两处改动把它压下来，
 * ⛔ 都没有削弱隔离：
 *
 ...
 *
 * ⚠ 并发的前提是 `verify-inventory.mjs` 只读（它不写盘），且用例只写自己的夹具、⛔ 从不写
 *   REPO_ROOT。这两条任意一条变了，就得重新审这里的并发。
 */
/**
 * 实测（10 核 / APFS）：并发 2→46.6s、3→38.5s、4→41.7s、6→51.3s、8→46.3s、12→47.4s。
 * ⚠ 瓶颈是**每文件系统调用**（每个夹具 1998 个文件），不是 CPU——并发开大反而更慢。
 */
const CONCURRENCY = Number(process.env.INVENTORY_TEST_CONCURRENCY)
  || Math.max(2, Math.min(4, availableParallelism()));

/** 收集用例，文件末尾统一注册进并发 suite（node:test 的顶层 `test()` 之间是串行的）。 */
const pendingTests = [];
...
function runVerifierArgs(args) {
  return new Promise((settle, fail) => {
    const child = spawn(process.execPath, [VERIFY_SCRIPT, ...args], { cwd: REPO_ROOT });
    let stdout = "";
    let stderr = "";
...
function runVerifier(root) {
  return runVerifierArgs(["--root", root]);
}
...
async function assertRejected(root, expected) {
...
test("inventory verifier accepts an isolated checkout fixture", async () => {
...
test("inventory verifier rejects an unregistered default workspace entry", async () => {
  const root = await createFixture();
  try {
    const inventory = readInventory(root);
...
test("inventory verifier discovers app.config as a workspace composition root", async () => {
  const root = await createFixture();
  try {
    const inventory = readInventory(root);
    inventory.defaultModules = inventory.defaultModules.filter(
...
test("inventory verifier discovers Main from the scene's compressed Creator UUID", async () => {
  const root = await createFixture();
  try {
    const inventory = readInventory(root);
    inventory.defaultModules = inventory.defaultModules.filter(
...
test("inventory verifier rejects a standalone relayer classified as core", async () => {
  const root = await createFixture();
  try {
    const inventory = readInventory(root);
...
test("inventory verifier rejects a launch command that does not start its declared entry", async () => {
  const root = await createFixture();
  try {
    const inventory = readInventory(root);
...
test("inventory verifier rejects a vanished verification command", async () => {
  const root = await createFixture();
  try {
    const inventory = readInventory(root);
...
test("inventory verifier rejects the historical plan as route of truth", async () => {
  const root = await createFixture();
  try {
    const inventory = readInventory(root);
...
test("inventory verifier rejects removal of the current plan truth declaration", async () => {
  const root = await createFixture();
  try {
...
test("inventory verifier rejects removal of the current plan from README", async () => {
  const root = await createFixture();
  try {
...
test("inventory verifier rejects removal of the Godogen plan from README", async () => {
  const root = await createFixture();
  try {
    const readme = join(root, "README.md");
...
test("inventory verifier rejects a referenceDocs entry whose file does not exist", async () => {
...
test("inventory verifier rejects a registered doc that points at an archive as current truth", async () => {
...
test("inventory verifier scans unregistered docs too (未登记目录也要扫)", async () => {
...
test("inventory verifier 不因文件名子串误红（live-plan.md 不是 plan.md）", async () => {
...
test("inventory verifier still allows citing an archive when its identity is stated", async () => {
...
test("inventory verifier 归档闸随 referenceDocs 自动生效（新归档一进清单就被守）", async () => {
  ...
  try {
    ...
    assert.equal((await runVerifier(root)).status, 0, "未登记的归档不该被这道闸拒");
...
test("inventory verifier rejects listing the current plan as a historical reference", async () => {
...
test("inventory verifier keeps the Godogen plan registered as a checked reference", async () => {
  const root = await createFixture();
  try {
    const inventory = readInventory(root);
...
test("inventory verifier requires EXTRAS to register the Godogen plan", async () => {
  const root = await createFixture();
  try {
...
test("inventory verifier checks stable local anchors in the Godogen plan", async () => {
  const root = await createFixture();
  try {
...
test("inventory verifier rejects a broken registered Markdown link", async () => {
  const root = await createFixture();
  try {
...
test("inventory verifier rejects AGENTS/CLAUDE semantic drift even when key markers remain", async () => {
  const root = await createFixture();
  try {
...
test("inventory verifier rejects synchronized removal of a required assistant instruction", async () => {
  const root = await createFixture();
  try {
...
test("inventory verifier rejects synchronized removal of the state codegen registration", async () => {
  const root = await createFixture();
  try {
    for (const filename of ["AGENTS.md", "CLAUDE.md"]) {
      const file = join(root, filename);
...
test("inventory verifier rejects synchronized removal of the HTTP manifest registration", async () => {
  const root = await createFixture();
  try {
    for (const filename of ["AGENTS.md", "CLAUDE.md"]) {
      const file = join(root, filename);
...
test("inventory verifier rejects synchronized removal of the project metadata registration", async () => {
  const root = await createFixture();
  try {
    for (const filename of ["AGENTS.md", "CLAUDE.md"]) {
      const file = join(root, filename);
...
test("inventory verifier rejects synchronized removal of the Godogen assistant entry", async () => {
  const root = await createFixture();
  try {
    for (const filename of ["AGENTS.md", "CLAUDE.md"]) {
      const file = join(root, filename);
...
test("inventory verifier rejects a root command removed from both assistant command blocks", async () => {
  const root = await createFixture();
  try {
    for (const filename of ["AGENTS.md", "CLAUDE.md"]) {
      const file = join(root, filename);
...
test("inventory verifier rejects a newly added undocumented root command", async () => {
  const root = await createFixture();
  try {
...
test("inventory verifier rejects a stale root command in the README command table", async () => {
  const root = await createFixture();
  try {
    const readme = join(root, "README.md");
...
test("inventory verifier rejects synchronized removal of the current plan entry", async () => {
  const root = await createFixture();
  try {
    for (const filename of ["AGENTS.md", "CLAUDE.md"]) {
      const file = join(root, filename);
...
test("inventory verifier rejects duplicate or empty --root arguments", async () => {
  const duplicate = await runVerifierArgs(["--root", REPO_ROOT, "--root", REPO_ROOT]);
  assert.notEqual(duplicate.status, 0);
  assert.match(outputOf(duplicate), /参数重复：--root/);

  const empty = await runVerifierArgs(["--root", ""]);
  assert.notEqual(empty.status, 0);
  assert.match(outputOf(empty), /--root 需要非空目录参数/);
});

test("inventory verifier rejects a newly added unregistered workspace script", async () => {
  const root = await createFixture();
  try {
...
test("inventory verifier rejects a workspace scope entry whose root command stops invoking it", async () => {
  const root = await createFixture();
  try {
...
test("inventory verifier rejects a workspace scope entry whose document drops the command", async () => {
  const root = await createFixture();
  try {
...
test("inventory verifier rejects a workspace scope entry that is already in the command table", async () => {
  const root = await createFixture();
  try {
    const inventory = readInventory(root);
...
test("inventory verifier rejects a root document citing a missing workspace command", async () => {
  const root = await createFixture();
  try {
...
test("inventory verifier rejects a workspace scope entry that supersedes itself", async () => {
  const root = await createFixture();
  try {
...
test("inventory verifier rejects two workspace scope entries that supersede each other", async () => {
  const root = await createFixture();
  try {
...
test("inventory verifier accepts a workspace anchor that is itself in the command table", async () => {
  const root = await createFixture();
  try {
...
test("inventory verifier rejects an echoed workspace command as coverage", async () => {
  const root = await createFixture();
  try {
...
test("inventory verifier rejects a commented-out workspace command as coverage", async () => {
  const root = await createFixture();
  try {
...
test("inventory verifier rejects an echoed root command in a verification chain", async () => {
  const root = await createFixture();
  try {
...
test("inventory verifier rejects an echoed launch entry", async () => {
  const root = await createFixture();
  try {
    const packageFile = join(root, "apps", "server", "package.json");
    const pkg = JSON.parse(readFileSync(packageFile, "utf8"));
...
test("inventory verifier rejects a prototype-chain property posing as a root script", async () => {
  const root = await createFixture();
  try {
    const inventory = readInventory(root);
...
test("inventory verifier rejects a prototype-chain property posing as a workspace script", async () => {
  const root = await createFixture();
  try {
    const inventory = readInventory(root);
...
test("inventory verifier rejects a prototype-chain workspace scope registration", async () => {
  const root = await createFixture();
  try {
...
test("inventory verifier rejects a non-string script value", async () => {
  const root = await createFixture();
  try {
    const packageFile = join(root, "package.json");
    const pkg = JSON.parse(readFileSync(packageFile, "utf8"));
...
test("inventory verifier rejects a self-referencing verification requirement", async () => {
  const root = await createFixture();
  try {
...
test("inventory verifier rejects mutually requiring verification commands", async () => {
  const root = await createFixture();
  try {
    const inventory = readInventory(root);
    inventory.capabilities[0].verification.push({
...
test("inventory verifier does not let a self-wrapped requirement whitewash a broken chain", async () => {
  const root = await createFixture();
  try {
...
test("inventory verifier rejects a commented-out workspace command in the assistant block", async () => {
  const root = await createFixture();
  try {
...
test("inventory verifier rejects an echoed workspace command in the assistant block", async () => {
  const root = await createFixture();
  try {
...
test("inventory verifier rejects documentedIn outside README and docs", async () => {
  const root = await createFixture();
  try {
    const inventory = readInventory(root);
...
test("inventory verifier rejects documentedIn pointing at an archived plan", async () => {
  const root = await createFixture();
  try {
    const inventory = readInventory(root);
    const entry = inventory.workspaceCommandScope.find((item) => item.command.script === "loadtest");
...
test("inventory verifier rejects documentedIn pointing at a directory instead of crashing", async () => {
  const root = await createFixture();
  try {
    const inventory = readInventory(root);
    const entry = inventory.workspaceCommandScope.find((item) => item.command.script === "loadtest");
...
test("inventory verifier rejects an npx-launched entry", async () => {
  const root = await createFixture();
  try {
...
test("inventory verifier rejects a quoted pseudo-call as coverage", async () => {
  const root = await createFixture();
  try {
    const packageFile = join(root, "package.json");
    const pkg = JSON.parse(readFileSync(packageFile, "utf8"));
...
test("inventory verifier rejects an entry mentioned only in a trailing comment", async () => {
  const root = await createFixture();
  try {
...
test("inventory verifier rejects a command-substitution launch as coverage", async () => {
  const root = await createFixture();
  try {
...
test("inventory verifier rejects a heredoc pseudo-call as coverage", async () => {
  const root = await createFixture();
  try {
    const packageFile = join(root, "package.json");
    const pkg = JSON.parse(readFileSync(packageFile, "utf8"));
...
test("inventory verifier rejects an argument-position pseudo-call as coverage", async () => {
  const root = await createFixture();
  try {
...
test("inventory verifier rejects a non-executing launcher flag as launch", async () => {
  const root = await createFixture();
  try {
    const packageFile = join(root, "apps", "server", "package.json");
    const pkg = JSON.parse(readFileSync(packageFile, "utf8"));
...
test("inventory verifier still accepts launchers with value-taking flags", async () => {
  const root = await createFixture();
  try {
...
test("inventory verifier rejects a suffix-workspace npm run posing as a root command", async () => {
  const root = await createFixture();
  try {
...
test("inventory verifier rejects an inline suffix-workspace npm run", async () => {
  const root = await createFixture();
  try {
...
test("inventory verifier still accepts a prefix-workspace npm run", async () => {
  const root = await createFixture();
  try {
...
test("inventory verifier rejects a backgrounded pseudo-launch", async () => {
  const root = await createFixture();
  try {
...
test("inventory verifier still accepts a launcher with shell redirection", async () => {
  const root = await createFixture();
  try {
...
test("inventory verifier still accepts a heredoc mentioned only inside a comment", async () => {
  const root = await createFixture();
  try {
...
test("inventory verifier rejects a blacklisted flag after a value-taking flag", async () => {
  const root = await createFixture();
  try {
...
test("inventory verifier rejects documentedIn escaping through a parent segment", async () => {
  const root = await createFixture();
  try {
...
test("inventory verifier rejects a CR-glued npm run script name", async () => {
  const root = await createFixture();
  try {
...
test("inventory verifier rejects NBSP-glued npm run tokens", async () => {
  const root = await createFixture();
  try {
...
test("inventory verifier rejects a glued long non-executing flag before the entry", async () => {
  const root = await createFixture();
  try {
...
test("inventory verifier rejects a glued short non-executing flag before the entry", async () => {
  const root = await createFixture();
  try {
...
test("inventory verifier still accepts a value-taking flag before the entry", async () => {
  const root = await createFixture();
  try {
...
test("inventory verifier rejects an entry that only appears as a redirect target", async () => {
  const root = await createFixture();
  try {
...
test("inventory verifier still accepts an entry followed by a log redirect", async () => {
  const root = await createFixture();
  try {
...
test("inventory verifier rejects an entry behind a noclobber redirection", async () => {
  const root = await createFixture();
  try {
...
test("inventory verifier rejects an entry behind an fd-allocating redirection", async () => {
  const root = await createFixture();
  try {
...
test("inventory verifier still accepts a launcher piped into another command", async () => {
  const root = await createFixture();
  try {
    // 反向锁：真实管道不得被 `>|` 守卫误伤。
    const packageFile = join(root, "apps", "server", "package.json");
...
test("inventory verifier accepts a shell launcher with clustered short options", async () => {
  const root = await createFixture();
  try {
...
test("inventory verifier rejects a shell launcher whose option cluster contains -c", async () => {
  const root = await createFixture();
  try {
...
test("inventory verifier rejects a shell launch that reads stdin instead of the entry", async () => {
  const root = await createFixture();
  try {
...
test("inventory verifier rejects a shell option that eats the entry as its value", async () => {
  const root = await createFixture();
  try {
...
test("inventory verifier still accepts an idiomatic shell option cluster with a value", async () => {
  const root = await createFixture();
  try {
...
test("inventory verifier rejects an entry after the &| garbage sequence", async () => {
  const root = await createFixture();
  try {
...
test("inventory verifier rejects an entry after a spaced > | sequence", async () => {
  const root = await createFixture();
  try {
...
test("inventory verifier rejects a mid-cluster shell option that eats the entry", async () => {
  const root = await createFixture();
  try {
...
test("inventory verifier rejects a positional operand before the shell entry", async () => {
  const root = await createFixture();
  try {
...
test("inventory verifier rejects a bare dash as the node entry position (stdin script)", async () => {
  const root = await createFixture();
  try {
...
test("plugin fragment：合法 extra fragment 绿，且不修改中央 inventory.json（阶段 7 退出条件）", async () => {
  const root = await createFixture();
  try {
...
test("plugin fragment：声明 core 必须拒绝（core 身份不经 fragment 通道）", async () => {
  const root = await createFixture();
  try {
...
test("plugin fragment：extra fragment 缺 docs/EXTRAS.md 必须拒绝（沿用双向断言）", async () => {
  const root = await createFixture();
  try {
...
test("plugin fragment：plugin.json 出现中央 inventory 专属键（routeOfTruth 等）必须拒绝", async () => {
  const root = await createFixture();
  try {
...
test("plugin fragment：fragment 未知键未通过真实 JSON Schema 校验（additionalProperties:false）", async () => {
  const root = await createFixture();
  try {
...
test("plugin fragment：verification 登记存在但不发现 fragment 的命令 → 假绿拒绝（须覆盖 verify:inventory）", async () => {
  const root = await createFixture();
  try {
...
test("plugin fragment：与中央能力重复 id / defaultEntry 不存在，均并入既有 fail-closed 检查", async () => {
  const root = await createFixture();
  try {
...
test("kit fragment：合法 extra fragment 绿（apps/kits/ 缺席也绿），且不修改中央 inventory.json", async () => {
  const root = await createFixture();
  try {
...
test("kit fragment：kit.json 未知键未通过 kit-schema-v1 校验（additionalProperties:false）；requires 对 kit 也是未知键", async () => {
  const root = await createFixture();
  try {
    writeFragmentKit(root, { bogus: true });
    await assertRejected(root, /apps\/kits\/fixtureKit\/kit\.json 未通过 kit-schema-v1 校验.*unknown key\(s\): bogus/);
...
test("kit fragment：声明 core / 中央专属键 与插件同规则拒绝", async () => {
  const root = await createFixture();
  try {
    writeFragmentKit(root, {}, { category: "core", docs: ["docs/SERVER.md"] });
    await assertRejected(root, /apps\/kits\/fixtureKit\/kit\.json capabilities\[0\] 只能声明 extra/);
...
test("kit fragment：apps/kits/ 下的子目录缺 kit.json 即拒绝（有根则每个子目录都是 kit）", async () => {
  const root = await createFixture();
  try {
...
test("kit fragment：apps/kits 存在但不是目录即拒绝（可选根 ⛔ 不得 fail-open）", async () => {
  const root = await createFixture();
  try {
    rmSync(join(root, "apps", "kits"), { recursive: true, force: true });
...
test("plugin fragment：plugins/ 目录缺失即 fail closed（发现面不可静默为空）", async () => {
  const root = await createFixture();
  try {
...
describe("verify-inventory", { concurrency: CONCURRENCY }, () => {
