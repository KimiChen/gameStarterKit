...
| `packages/<id>.lock` | 已安装包（插件或 kit）`<id>` 的 plugin.json / kit.json 归一化值 + 全部写入文件的 sha256 清单 | `npm --workspace @game/server run plugin -- install`（uninstall 删除） | `apps/server/test/plugin-lock.test.ts`（随 `npm --workspace @game/server run test` 进 `verify:all`）与 `plugin -- check` | 已安装包登记面 + 内容锁（docs/PLUGIN.md §5） |
