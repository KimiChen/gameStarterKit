...
缺 instancing 或可用关节纹理时，单位上限保守降到 25 / 50 / 50；SC4 已验证桌面 WebGL1 low 的
25 单位退化路径，medium / high 的策略上限不等于该负载的实测承诺。
...
low 的 cubes 属 details，因此不激活。容量不等于性能承诺，各画质帧时证据见 [SC3 汇总](../../docs/perf/stage3d/2026-09-24-sc3-review.json)。
棋盘纹理九个格子共用 SC0 的 64² 精确尺寸例外，供真实资源路径验证；普通内容必须登记实际离线变体。
纹理降档不改变网格 lod。SC3-B2 已验证九格精确选择与缺失处理；SC5-B1 的纹理工具支持
按 `textureMaxSize` 生成离线尺寸，但不会自动填九格 catalog，内容方仍须显式登记并验证实际变体。
...
实体池已在 `Stage3dDevScene` 装配，性能、预热缓存和回收证据已由 SC3 / SC4 完成。
...
`quality-assets.mjs` 校验预设与回落，由资产闸复用。引擎按实际格式支持选择下载资源，见
...
`stage3d/P_Stage3d_Baked`，经正式 Stage3D 和异步 AssetLease 持有；不经过 Main，不读取烘焙工位场景。
...
最初判档与压缩配置证据见 [SC1-B8 摘要](../../docs/perf/stage3d/2026-09-22-sc1-b8.json)，最终验收见
[v1 冻结索引](../../docs/3d.md#101-v1-冻结面与证据索引)。WebGL1 使用单独测试页禁止 WebGL2 context 创建，
实际由 Creator WebGLDevice 渲染；证据限于桌面。构建过滤在 Creator 中调用注册钩子验证，不代表完整平台构建。
SC4 的实际约 60Hz 容量样本使用显式 `--frame-rate 120`；默认 60 上限仍有调度跳帧，未改生产配置。
