...
import { buildMapoSpriteMeshes } from "../../apps/client/src/kits/mapOriginal/logic/mapoMesh";
...
async function capture(name: string, row: number, col: number, span: number) {
    ...
    const bounds = { minX: center.x-span/2, maxX: center.x+span/2, minY: center.y-span/2, maxY: center.y+span/2 };
    const batches = mapoStaticScene({ left: bounds.minX, right: bounds.maxX, bottom: bounds.minY, top: bounds.maxY }, true);
    batches.push(...buildMapoSpriteMeshes(mapoCitiesIn(bounds.minX-1000, bounds.maxX+1000, bounds.minY-1000, bounds.maxY+1000, Infinity))
    ...
    const spec = { bounds, packets, width: 1024, height: 1024 };
    ...
    try {
        ...
        fs.writeFileSync(path.join(out, "report.json"), JSON.stringify({ name, row, col, bounds, animationSeconds: 0,
...
async function main() {
    for (const [name, row, col, span] of [
