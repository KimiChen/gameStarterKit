...
| `ctable_cw.py` | ★ `base.cw`（66.8 MB ctable）**定向读取**：头/串池/串索引已解，`client_res`（25 列 / 100 B，848 行）与 `land`（**49 列 / 392 B、键按字母序**，353 行）两张表的定长布局已定死。⛔ 不是通用解析器，换表要重定布局；⛔ 列表列是**连号句柄**、数组池未解 |
