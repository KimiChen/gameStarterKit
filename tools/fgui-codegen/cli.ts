 ...
 *   apps/client/src/view；未来 plugin/gameplay 自有 View 目录用，再由 sync:client 灌入
 *   apps/Cocos/assets/src）。
 * 目标已存在 → 幂等重写四个 AUTO 区块（区块外业务代码不动）；不存在 → 生成脚手架并
 * 打印接入清单（sidecar/plugin 登记/typecheck 自动纳入/.meta）。
 * 本工具只写 View AUTO 区（§7.5）：⛔ 不覆盖 registry/contracts 生成物，也不自动执行
 * FGUI manifest --write——那两件分别归 codegen:plugins 与显式资源审计锁。
 */
...
if (fs.existsSync(target)) {
  ...
  if (regen === source) {
    ...
    console.log(`[codegen] REQUIRED 可能已变化：运行 npm --workspace @game/server run codegen:plugins -- --check 校验 generated contracts 新鲜度`);
  }
} else {
  ...
  console.log(`  2. apps/plugins/<id>/plugin.json：把 sidecar 路径登记进 views（route 需要时同步登记）`);
  console.log(`  3. logic 配对：sidecar.logic 指向的行为层文件（无头单测）`);
  console.log(`  4. npm --workspace @game/server run codegen:plugins 刷新 registry/contracts 生成物（只读校验用 -- --check）`);
