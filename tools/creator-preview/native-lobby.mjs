  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    switch (key) {
      ...
      case "portal":
        ...
        break;
      case "lobby-url":
        options.lobbyUrl = take();
        break;
      case "internal":
        options.internal = take();
        break;
      case "secret":
        options.secret = take();
        break;
      ...
      case "format":
        ...
        break;
      case "boot-timeout":
        options.bootTimeoutMs = Number(take());
        break;
      case "step-timeout":
        options.stepTimeoutMs = Number(take());
        break;
      case "hero-buy-only":
        options.heroBuyOnly = true;
        break;
      case "help":
        options.help = true;
        break;
...
const usage = () =>
  ...
  "[--dev-key <key>] [--hero-buy-only] [--scene <uuid>] [--out <dir>] [--format jpeg|png] " +
  "[--boot-timeout <ms>] [--step-timeout <ms>]";
...
class Runner {
  ...
  async tap(node, label) {
    ...
    return {
      tapped: label,
      at: [Math.round(node.center.x), Math.round(node.center.y)],
    };
  }
  ...
  async tapText(text, query = {}) {
    const node = await this.waitFor(
      `文本「${text}」`,
      (walk) => selectNodes(walk, { text, ...query })[0] ?? null,
    );
  ...
  async tapSettingsCard(name) {
    const scroll = await this.client.evaluate(
      `(${revealSettingsCard.toString()})(${JSON.stringify(name)})`,
    );
    if (!scroll.ok)
      throw new Error(`无法定位设置卡片 ${name}：${scroll.error}`);
    // ScrollView 的零时长滚动也需等下一帧更新世界变换；不复用滚动前的 walk。
    if (scroll.scrolled) await sleep(100);
    await this.walk();
    const target = this.find({
      name,
      pathIncludes: "SettingsView/panel/viewport/content/",
    })[0];
    const viewport = this.find({
      name: "viewport",
      pathIncludes: "SettingsView/panel/",
    })[0];
    if (!target || !viewport)
      throw new Error(`滚动后找不到设置卡片 ${name} 或其视口`);
    const scaleY = this.lastWalk.canvas.height / this.lastWalk.visible.height;
    const halfHeight = (viewport.center.height * scaleY) / 2;
    if (
      target.center.y <= viewport.center.y - halfHeight ||
      target.center.y >= viewport.center.y + halfHeight
    ) {
      throw new Error(
        `设置卡片 ${name} 的点击中心仍在裁剪视口外（scroll=${JSON.stringify(scroll)}）`,
      );
    }
  ...
  async shot(name) {
    ...
    const file = path.join(
      this.outDir,
      `${String(this.shotIndex).padStart(2, "0")}-${slug(name)}.${this.options.format}`,
    );
...
function revealSettingsCard(name) {
  if (typeof cc === "undefined" || !cc.director?.getScene())
    return { ok: false, error: "场景尚未就绪" };
  const find = (node) => {
  ...
  if (
    ...
    return {
      ok: false,
      error: `卡片 ${name} 或 ScrollView 不存在（可能正停在通用设置详情）`,
    };
  }
  ...
  const top =
    -card.position.y - cardTransform.height * (1 - cardTransform.anchorY);
  ...
  const offset = outside
    ? Math.max(0, Math.min(max, (top + bottom - height) / 2))
    : before;
...
function readAccount(walk) {
  const labels = selectNodes(walk, {
    pathIncludes: "PromoHomeView",
    kind: "label",
  })
    .map((node) => node.text)
    .filter(Boolean);
...
async function main() {
  ...
  if (!options.portal || !options.lobbyUrl)
    throw new Error(`--portal 与 --lobby-url 必填\n${usage()}`);
  const sceneUuid =
    options.scene ??
    sceneUuidFromMeta(
      fs.readFileSync(path.join(REPO_ROOT, DEFAULTS.sceneMeta), "utf8"),
    );
  const outDir = path.resolve(
    options.out ??
      path.join(
        DEFAULT_OUT_ROOT,
        `creator-native-lobby-${new Date().toISOString().replace(/[:.]/gu, "-")}`,
      ),
  );
  ...
  const report = {
    ...
    startedAt: new Date().toISOString(),
    // 报告是便于分享的本地证据，内部动作密钥绝不能随截图/JSON 留存。
    options: { ...options, secret: options.secret ? "[redacted]" : null },
    scene: sceneUuid,
    expectedUserId: devUserId(options.devKey, options.sid),
    ok: false,
    steps: [],
    console: [],
    sockets: [],
  ...
  try {
    const tab = await acquireTab({
      ...
      throw new Error(
        `Chrome 调试端口不可用（${options.devtools}）：${error.message}`,
      );
    });
    ...
    client.on((message) => {
      if (message.method === "Network.webSocketCreated")
        runner.sockets.push(message.params.url);
    });
    ...
    await runner.step(
      `加载预览并显式选择原生通道（?lobby=native&lobbyUrl=${options.lobbyUrl}）`,
      async () => {
    ...
    await runner.step(
      ...
      async () => {
        ...
        return {
          all: [...runner.sockets],
          previewChannel: runner.sockets.filter(
            (url) => hostOf(url) === previewHost,
          ),
    ...
    await runner.step("登录页可见（FGUI btn_login）", async () => {
      const button = await runner.waitFor(
        "btn_login",
        (walk) => selectNodes(walk, { name: "btn_login" })[0] ?? null,
        60_000,
      );
      return {
        via: "fgui:btn_login",
        shot: await runner.shot("login"),
        at: button.center,
      };
    });

    const firstAccount = await runner.step(
      "点「开始游戏」→ Lobby ready → 首屏 PromoHomeView（GetInfo 查询）",
      async () => {
        await runner.tap(runner.find({ name: "btn_login" })[0], "btn_login");
        await runner.waitFor(
          "PromoHomeView",
          (walk) =>
            selectNodes(walk, { name: "PromoHomeView" }).length > 0
              ? true
              : null,
          90_000,
        );
        ...
        if (!account.userId)
          throw new Error(
            `首屏卡片读不到账号：${JSON.stringify(account.labels)}`,
          );
        if (account.userId !== report.expectedUserId)
          throw new Error(
            `账号与本地 WebPlatform 副本签发的不一致：期望 ${report.expectedUserId}，实际 ${account.userId}`,
          );
        const protocolLine =
          account.labels.find((text) => text.startsWith("协议 ")) ?? null;
        return {
          account: account.line,
          userId: account.userId,
          protocolLine,
          shot: await runner.shot("home-native"),
        };
      },
    );

    const sockets = await runner.step(
      "游戏连接只落在原生端点（Network.webSocketCreated）",
      async () => {
        ...
        const game = [
          ...new Set(
            runner.sockets.filter((url) => hostOf(url) !== previewHost),
          ),
        ];
        const expected = new URL(options.lobbyUrl).origin;
        if (game.length === 0)
          throw new Error(
            `客户端没有连任何游戏端点（期望 ${options.lobbyUrl}）`,
          );
        const foreign = game.filter((url) => new URL(url).origin !== expected);
        if (foreign.length > 0)
          throw new Error(
            `客户端连了非原生端点：${foreign.join(", ")}（原生端点 ${options.lobbyUrl}）`,
          );
        return {
          game,
          previewChannel: runner.sockets.filter(
    ...
    const write = await runner.step(
      "设置面板：音乐开关幂等写（user.updateProfile）被服务端接受",
      async () => {
        await runner.tapText("设置", { pathIncludes: "PromoHomeView" });
        await runner.waitFor("SettingsView", (walk) =>
          selectNodes(walk, { name: "SettingsView" }).length > 0 ? true : null,
        );
        ...
        const before = await runner.waitFor(
          ...
          (walk) => {
            ...
            const label = selectNodes(walk, {
              pathIncludes: `${control.path}/`,
              kind: "label",
            })[0];
        ...
        if (before.state !== "开" && before.state !== "关")
          throw new Error(
            `音乐开关状态不可读：${JSON.stringify(before.state)}`,
          );
        ...
        return {
          ...
          generalTap,
          shotBefore,
          shotAfter: await runner.shot(`settings-after-${after.state}`),
        };
      },
    );

    await runner.step("设置面板：英雄招募读取目录并实际购买英雄", async () => {
      const back = await runner.waitFor(
        "通用设置返回按钮",
        (walk) =>
          selectNodes(walk, {
            name: "btn-back",
            pathIncludes: "SettingsView",
          })[0] ?? null,
        10_000,
      );
      await runner.tap(back, "btn-back");
      const launch = await runner.tapSettingsCard("card-heroRecruit");
      await runner.waitFor(
        "HeroRecruitSceneView",
        (walk) =>
          selectNodes(walk, { name: "HeroRecruitSceneView" }).length > 0
            ? true
            : null,
        20_000,
      );
      await runner.walk();
      const labels = selectNodes(runner.lastWalk, {
        pathIncludes: "HeroRecruitSceneView",
        kind: "label",
      })
        .map((node) => node.text)
        .filter(Boolean);
      for (const expected of ["铜币：", "赵云", "貂蝉", "李白"]) {
        if (!labels.some((text) => text.includes(expected)))
          throw new Error(
            `英雄招募场景缺少 ${expected}：${JSON.stringify(labels)}`,
          );
      }
      const readState = (walk) => {
        const sceneNodes = selectNodes(walk, {
          pathIncludes: "HeroRecruitSceneView",
        });
        const texts = sceneNodes
          .filter((node) => node.kind === "label" && node.text)
          .map((node) => node.text);
        const copperText = texts.find((text) => /^铜币：\d+$/u.test(text));
        const copper = copperText
          ? Number(copperText.slice("铜币：".length))
          : null;
        const ownedButtons = sceneNodes.filter(
          (node) => node.name === "button-已拥有",
        ).length;
        const recruitButtons = sceneNodes
          .filter((node) => /^button-招募 \d+$/u.test(node.name ?? ""))
          .map((node) => ({
            node,
            price: Number(node.name.slice("button-招募 ".length)),
          }))
          .filter(
            (entry) => Number.isSafeInteger(entry.price) && entry.price > 0,
          )
          .sort((left, right) => left.price - right.price);
        return { texts, copper, ownedButtons, recruitButtons };
      };
      let state = readState(runner.lastWalk);
      const buyDeadline = Date.now() + 60_000;
      let affordable = state.recruitButtons.find(
        (entry) => (state.copper ?? -1) >= entry.price,
      );
      while (
        !affordable &&
        state.recruitButtons.length > 0 &&
        Date.now() < buyDeadline
      ) {
        await sleep(5_000);
        await runner.walk();
        const refresh = selectNodes(runner.lastWalk, {
          name: "button-刷新",
          pathIncludes: "HeroRecruitSceneView",
        })[0];
        if (!refresh) throw new Error("英雄招募场景缺少刷新按钮");
        await runner.tap(refresh, "heroRecruit-refresh");
        await sleep(500);
        await runner.walk();
        state = readState(runner.lastWalk);
        affordable = state.recruitButtons.find(
          (entry) => (state.copper ?? -1) >= entry.price,
        );
      }
      if (!affordable) {
        throw new Error(
          `60 秒内没有可购买英雄：${JSON.stringify({ copper: state.copper, buttons: state.recruitButtons.map((entry) => entry.price) })}`,
        );
      }
      const beforeCopper = state.copper;
      if (beforeCopper === null) throw new Error("购买前铜币不可读");
      const beforeOwned = state.ownedButtons;
      const beforeShot = await runner.shot("hero-recruit-before-buy");
      await runner.tap(affordable.node, affordable.node.name);
      const purchased = await runner.waitFor(
        "英雄购买成功并刷新铜币与拥有状态",
        (walk) => {
          const next = readState(walk);
          const notice =
            next.texts.find((text) => text.startsWith("已招募 ")) ?? null;
          return next.copper === beforeCopper - affordable.price &&
            next.ownedButtons === beforeOwned + 1 &&
            notice
            ? { copper: next.copper, ownedButtons: next.ownedButtons, notice }
            : null;
        },
        20_000,
      );
      const afterShot = await runner.shot("hero-recruit-after-buy");
      const backToSettings = selectNodes(runner.lastWalk, {
        name: "button-返回",
        pathIncludes: "HeroRecruitSceneView",
      })[0];
      if (!backToSettings) throw new Error("英雄招募场景缺少返回按钮");
      await runner.tap(backToSettings, "heroRecruit-return");
      await runner.waitFor(
        "返回设置面板",
        (walk) =>
          selectNodes(walk, { name: "SettingsView" }).length > 0 ? true : null,
      ...
      return {
        ...
        purchase: {
          ...
          copperBefore: beforeCopper,
          copperAfter: purchased.copper,
          ownedBefore: beforeOwned,
          ownedAfter: purchased.ownedButtons,
          notice: purchased.notice,
        },
        screenshots: [beforeShot, afterShot],
      };
    });

    if (options.heroBuyOnly) {
      report.ok = true;
      return 0;
    }

    const kicked = await runner.step(
      "运营强制下线：内部动作入口 → 客户端提示 + 回登录页",
      async () => {
        ...
        const response = await postInternalAction({
          ...
          payload: {
            type: "lobbyKick",
            actionParams: { uid, sId: options.sid },
          },
        });
        if (response.status !== 200)
          throw new Error(
            `内部动作入口 HTTP ${response.status}：${JSON.stringify(response.body)}`,
          );
        if (response.body?.code !== 0)
          throw new Error(`内部动作入口失败：${JSON.stringify(response.body)}`);
        const kickedFlag = response.body?.data?.json?.kicked;
        if (kickedFlag !== true)
          throw new Error(
            `入口必须报告确实踢掉了一个在线连接，实际 ${JSON.stringify(response.body)}`,
          );

        const prompt = await runner.waitFor(
          "强制下线提示",
          (walk) =>
            selectNodes(walk, { kind: "label", text: REVOKED_MESSAGE })[0] ??
            null,
          20_000,
        );
        ...
        const confirm = await runner.waitFor(
          "强制下线提示「确定」按钮",
          (walk) =>
            selectNodes(walk, { text: "确定", pathIncludes: "Confirm" })[0] ??
            null,
          10_000,
        );
        ...
        await runner.waitFor(
          "回登录页 btn_login",
          (walk) =>
            selectNodes(walk, { name: "btn_login" }).length > 0 ? true : null,
          30_000,
        );
        return {
          uid,
          kicked: kickedFlag,
          prompt: prompt.text,
          confirmAt: confirm.center,
          shot,
          backToLogin: await runner.shot("kicked-login"),
        };
      },
    );

    await runner.step(
      "恢复：重新登录回到首屏，且写入的偏好被保留（证明写落了真实存储）",
      async () => {
        await runner.tap(runner.find({ name: "btn_login" })[0], "btn_login");
        await runner.waitFor(
          "PromoHomeView",
        ...
        if (account.userId !== firstAccount.userId)
          throw new Error(
            `恢复后账号变了：${account.userId}（应为 ${firstAccount.userId}）`,
          );
        await runner.tapText("设置", { pathIncludes: "PromoHomeView" });
        await runner.waitFor("SettingsView", (walk) =>
          selectNodes(walk, { name: "SettingsView" }).length > 0 ? true : null,
        ...
        if (restored !== write.after)
          throw new Error(
            `偏好没有保留：写入后 ${write.after}，重登后 ${restored}`,
          );
        return {
          userId: account.userId,
          musicOn: restored,
          generalTap,
          shot: await runner.shot("recovered-settings"),
        };
      },
    );
    ...
    if (client) {
      try {
        report.console = await client.evaluate(
          "window.__creatorPreviewLogs || []",
        );
      } catch {}
  ...
  console.log(
    `页面 console：${report.console.length} 条（error/uncaught ${bad.length} 条）`,
  );
  for (const entry of bad.slice(0, 10))
    console.log(`  ✘ [${entry.level}] ${entry.text.slice(0, 300)}`);
  const sockets =
    report.sockets.length > 0 ? report.sockets.join(", ") : "（无）";
  console.log(`页面 WebSocket：${sockets}`);
  console.log(
    `${report.ok ? "✔ 全部通过" : `✘ 失败：${report.error}`} → ${outDir}`,
  );
