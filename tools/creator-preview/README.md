...
| `mapOriginal` | 设置中的「原版大地图」入口 → L0 地表/资源件、点击选中与洛阳城址 → L1 原件缓存 → L2 地貌缓存、郡名和城市标记 → L3 全图适配与缓存释放 → 返回 L2、缩略图跳转、回到 L0。标题为 `LOD n/3`；L1/L2 会读取真实 RenderTexture 的邻块重叠带，校验投影/裁剪一致性及 48 MiB 预算。全图步骤从公开 mesh 顶点验证完整地图落入地图视口；节点、文本、网格、贴图读取之外只使用普通输入，不调用 Logic 或发 RPC。画面设置仍只有画质一行；原版 2D 无运行时 LOD，四档属于宿主策略。独立触发，不加入 `all`。 |
...
截图人工复核或所需纹理格式覆盖缺失时明确记 pending；成功运行不代表 SC0 退出。
...
明确区别于自然低端硬件。WebGL1 冷启动仅接受原有精确、时间有界的 WebGL2 初始化失败诊断。
这些桌面证据不替代 SC4-B2 的 50 特效或 SC4-B3 的 low 退化与故障恢复验收，也不提高 SC0 冻结容量。
