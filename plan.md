...
> 当前开放问题、实施状态与验收证据以 [plan-v2.md](plan-v2.md) 为唯一真相。本文件保留此前审阅过程、
> 历史基线和已收口条目的上下文，不再接受本轮状态回写。

> 审阅日期：2026-08-29
>
> 实现基线：分支 `new`；截至历史 HEAD `27792b5`，已完成 P0-01 至 P0-05、P1-01 至 P1-09、P2-01 至 P2-03 的计划内收口，
...
> 上述为历史实现基线摘要，该阶段的收口提交与验证结果见下方第九轮记录（其后另有第十至十二轮）。
...
>
> 文档状态：`plan.md` 已纳入 Git，作为历史审阅归档保留
>
...
> 第九轮（2026-08-29，HEAD `27792b5`）复核 `ca8251c..27792b5` 的 12 个提交：
> `Main.startGameplay` 的三类失效、FGUI manifest 编排反例与显式路径、旧 View 句柄身份门、玩法 presentation 动态装配、
> 默认入口真实监听/SIGTERM 正常退出、ready marker 低频权威复核、gameplay 就近 README 和 legacy 全源严格检查均已由
> 执行级测试或生成/同步门禁锁定；`27792b5` 同步刷新了 codegen 变更后的 canonical FGUI manifest 哈希。当前实测为
...
> gameplay 就近 README 三件套与镜像逐字节一致。`verify:all`（服务端 207/207、客户端 222/222）、
> `test:fgui`（50/50）、`test:int`（103/103，本地栈实测）、`test:faults:int`（4 组 fault point 全部
> 实测执行）在本轮重跑复现。本轮修正 P1-06「65 处调用点」的不准确计数（任何口径都复算不出 65，已改为
> 可 grep 复核的持久表述）。第四/五轮的提交计数（12/47）因历史改写无法在当前 git log 复算，维持其
> 「过程记录」定性，不作为代码证据。
> 范围说明：`ca8251c..0ea018c` 实为 14 个提交；`0ea018c`（把 `todo-godogen.md` 入库）不在第九/第十轮的
> 复核范围内，其登记问题见 P1-09 复核备注。调用方在 `1c1a728` 上跑出的基线对代码侧结论仍然有效
> （`0ea018c` 只动文档）。
> 第十一轮（2026-08-29，HEAD `0ea018c`）按子系统分 4 组并行复核、每组配独立怀疑者做对抗式验证，
> 共 41 条裁决：24 条「上一轮登记项已闭合且有能失败的测试锁定」，17 条需要回填。本轮对 4 个纯测试提交与 `df5d8d6` 的守护代码
> 逐用例做了变异推演（删掉被守护的生产代码后用例是否变红），确认 `df5d8d6` 的三处守护代码、
> `e7ba622` 的进程级停服、`67f9a34` 的 Main 世代失效、`403ac1a` 的身份门与 `b7a876b` 的 presentation
> 正向链路都能变红；同时登记 5 组需回填的附带断言：P0-04 的 sId 包装变量自证、P0-04 的
> future/bad/boundary/真实 Redis reader 未覆盖、P0-05 的固定 timer 顺序、P1-08 的 strict
> `noUnusedLocals` 守门，以及 P1-09 的命令表完整性。它们不推翻任何 ✅。
> 真实 Creator 预览和完整 smoke 仍不纳入 Node/本地栈测试的通过声明。
> 第十二轮（2026-08-29，代码基线 `5c7818f`）收口上一轮列出的可执行测试与文案缺口：
> `0ca9f74` 锁定 ready marker 时间戳边界，`03d67ba` 锁定真实 `zoneCtx` ALS 透传，`b5c5e4e` 覆盖
> Redis `readCharacterRegistration` 的严格解析，`5c7818f` 消除停服任务顺序的 wall-clock 竞态，
> `2816e85` 守住 strict client probe 的 `noUnusedLocals`，`7160d08` 校准配置单位文案。
> 当前实测为服务端 208/208、客户端 222/222、FGUI 50/50、集成 104/104；故障矩阵 unit 123/123、
> integration 149/149，所有 fault point 均实际执行并 fail-closed。历史轮次保留原基线数字，仅作过程记录。
...
仓库根的 `todo-godogen.md`（`0ea018c` 入库，`3eb54e8` 合并为单一计划）是对外部项目的对照吸收计划，
不是本仓承诺；其中的 P1/P2 标记只描述该计划自身的取舍顺序，不与本文件的核心优先级竞争，也不构成核心
完成门槛。
**未登记的**根目录 Markdown 不在 `verify:inventory` 的覆盖面内（`plan.md` 自身已被登记并纳入检查），
其登记状态靠人工维护——详见 P1-09 的登记边界。

P0 表示继续扩展核心玩法前应先修复的确定性问题；P1 表示把现有 Demo 接缝收敛为可复用框架；P2 表示
在正确性稳定后再做的增强。条目内的六类小标题含义不同：「取舍」＝为达成本条目标而有意接受的行为代价
（已实施、非待办）；「未覆盖判据／覆盖面」＝对某条断言实际能否失败的如实说明；「复核备注（已收口）」＝有测试/门禁证据的结论；
「原审阅证据（已收口）」＝实现前的复现记录，仅说明改动动机，不代表当前基线仍存在这些缺陷；「保留
边界」＝已知且当前不打算做的范围限制；「已知语义回退（待闭合）」＝为达成本条目标而付出的运行行为代价，
已识别闭合方案但尚未实施（与「保留边界」区别在于它改变了行为，而非只是测得不够；**当前无条目使用**——
P0-04 原有的该类条目已随 `df5d8d6` 实施闭合方案，残余取舍改由该条正文的作用域表述承载）。
`✅` 表示条目正文声明的验收范围已完成并有测试/门禁证据；它可以与取舍、覆盖面或保留边界共存，
不表示没有任何遗留限制。`◐` 表示条目验收范围内仍有可执行缺口或只完成了一部分（当前无条目使用）。
...
本轮有一处明确记录的运行语义收紧：`9dff8f9` 将 `setField` 文本的接受域从 UTF-16 码元统一为 UTF-8
字节（CJK 值的可写长度实际缩到约 1/3），并已重钉协议指纹；它没有改变房间 join/envelope，durable effect
仍由独立的 `EFFECT_SCHEMA_VERSION=1` 描述。当前仍保持 `PROTOCOL_VERSION=5` 是开发期的明确评估结果：仓内没有
已部署的旧版线上客户端，当前不存在混合版本兼容义务；首次承担线上兼容义务的发布前，必须把该语义变更提升为
新协议版本并按迁移窗口发布。P0-04 的 ready marker 现在只在有限复核窗口内走本地快路径，
窗口过期会重新向权威登记复核并自愈，不再存在永久信任的未闭合回退；允许的最长陈旧窗口是显式配置的
可用性/一致性取舍——其另一半代价是窗口过期后若 WebPlatform 不可用则该次 join 被拒（解冻冷档在故障期
一律受影响），详见 P0-04 的「取舍」。
其余本轮新增项均为测试覆盖、生成物或文档门禁，不改变运行行为。
...
| `npm --workspace @game/server run test` | 208/208 通过 | 服务端单元测试；不等于真实外部服务或 Creator 集成已验证 |
| `npm --workspace @game/server run test:int` | 104/104 通过 | 使用本地 Redis/MySQL 的集成测试；外部 WebPlatform 边界使用契约兼容测试替身 |
| `npm run test:client` | 222/222 通过 | 客户端无头行为、运行时 wire/生命周期、生产页面/渲染接缝、vendor lock integrity 和 strict include 守门 |
| `npm run test:fgui` | 50/50 通过 | FGUI manifest 编排、codegen、registry 与结构契约专项测试 |
...
| `npm run verify:all` | 通过 | 完整执行一次 `verify:core`，再追加服务端 208 个单元测试；不重复客户端测试 |
...
| `npm run test:faults` / `npm run test:faults:int` | unit 123/123；integration 149/149 | unit 为 server-boundaries 66 例 + client-transitions 57 例（7 个 fault point）；integration 超集再加 storage-effects 11 例 + character-ready 15 例（共 13 个 fault point），全部要求实际执行并 fail-closed |
...
与配置检查，结果与表中一致；服务端单元测试为 208/208，客户端为 222/222，FGUI 为 50/50，集成测试为
104/104，故障矩阵四组共 149/149（unit 123 + integration 额外 26）。`test:int`/`test:faults:int` 仍依赖本地 Redis/MySQL 与当前
...
旧 controller；旧实例的清理不会触碰后来建立的会话。`mainGameplay.test.ts` 进一步在最小 Cocos 桩下动态加载
真实 `Main`，让 `startGameplay` 的 deferred 结果分别跨越 `destroyed`、`AbortSignal` 和 session generation
三类失效边界，直接断言旧 controller 被清理、新 controller 不受影响以及大厅关闭只发生一次。`roomClientOwnership.test.ts`
中的源码断言继续补充 `Main.ts` 的 registry/controller/catalog 与 `presentationHost` 装配约束；真实 Creator 生命周期
继续由编辑器预览承担。
...
`character-ready.test.ts` 覆盖 orchestrator 向 marker reader 透传 `sId`（s1 ready 不得短路 s2 首次登记）、
真实 `zoneCtx.getStore()?.sId` 的 ALS 上下文和过期 marker 自愈（`03d67ba`、`0ca9f74`）。删掉
`character.ts` 的 `zoneCtx.run` 会使该用例变红；真实 per-zone key 隔离仍由 `int/lobby-zone.test.ts`
承担。`character-repair.test.ts` 直接断言 worker 的 marker/时间戳写入和 intent 清理；`b5c5e4e` 新增
真实 Redis `readCharacterRegistration` 的合法/非法 checkedAt 解析覆盖；`lobby-zone.test.ts`
覆盖真实首登状态转移。
另：新增的档字段 `characterRegistration` 与 `characterRegistrationCheckedAt` 都不在
`apps/shared/src/protocol/lobbyRpc/economy.ts` 的 `EFFECT_RESERVED_FIELDS` 内。当前不可利用
（`EFFECT_FIELD_ALLOWLIST` 未收录这两个字段，且值规则对 allowlist 穷尽，扩表必须同时补规则），且其它
`CREATE_USER` 写入的字段（如 `registerTime`）同样不在保留表内，故不是
与现有习惯的不一致。⚠ 两个字段的风险并不对等：`characterRegistration` 被误加进 allowlist 时客户端最多把
自己标成 ready，而 `characterRegistrationCheckedAt` 是复核窗口的新鲜度闸门——客户端若能自刷时间戳，就能
把 marker 永久钉在快路径上，正好绕开本次修复。将来扩 `EFFECT_FIELD_ALLOWLIST` 必须同时把这两个字段加入
`EFFECT_RESERVED_FIELDS`。ready marker 仅作为有限复核窗口内的快路径；外部登记删除最多在该窗口内保持陈旧，
窗口到期即重新探测并通过 repair 路径恢复，不再是永久信任或不可自愈的语义回退。

取舍：「外部登记不可用时不再拒绝全量 join」现在只在复核窗口内成立——窗口过期的热档与全部解冻冷档在下次
join 时必打一次 `hasCharacter`；外部不可用时该次 join 被拒（与首次建档失败同码），重试仍会被拒，只留
durable repair intent 待外部恢复后收敛。默认 24 小时窗口下，WebPlatform 故障期每小时约有 1/24 的回访热档
用户受影响；冷档解冻后 `characterRegistrationCheckedAt` 随档原样搬回、必为旧值，因此解冻用户在故障期
一律受影响。这是**有意的 fail-closed 姿态**（P0-04 的契约首句就是「初始化失败/超时拒绝本次 join」），
不是待降级项；故障期唯一的运维缓解是调大 `CHARACTER_REGISTRATION_RECHECK_MS`，而它是模块级常量、
只在进程加载时读一次，改了必须重启，系统没有运行时降级或宽限开关。

覆盖面：新鲜度判据的四个边界已由 `0ca9f74` 的「未来/坏时间戳与恰好到期都不得走 ready 快路径」用例锁定
（未来时间戳 `checkedAtMs > nowMs`、负值、超 `Number.MAX_SAFE_INTEGER`、差值恰等于 `recheckMs`），
删掉 `character.ts:121` 的 `nowMs >= info.checkedAtMs` 会变红；`b5c5e4e` 的 Redis 集成用例直接
覆盖 `readCharacterRegistration` 对合法与坏值的解析。其新鲜窗口快路径由 `int/lobby-zone.test.ts`
的 outage 用例端到端覆盖——该用例先让真实 join 落下真实 marker 与时间戳，再注入全抛的 WebPlatform
客户端并要求二次 join 成功。
...
- 重复关闭与启动半失败时，每个资源恰好释放一次（registry 级 `lifecycle.test.ts`，真实入口子进程
  ...
  默认入口的真实依赖装配、`listen(app, PORT)` 以及「只注册一处 aggregator」由
  `index-startup-lifecycle.test.ts` 的源码接缝断言补充；同一文件的进程级用例会真实执行 `index.ts` 和
  Colyseus Server，在 TCP 成功监听后发送 `SIGTERM`，直接断言 admission 关闭、producer → character-ready
  → detached task → registered resources 的释放顺序、所有 disposer 完成以及 `{ code: 0, signal: null }` 正常退出。
  「同一 Server 禁止注册第二个 `onBeforeShutdown`」仍由 `shutdown-aggregator.test.ts` 的行为用例锁定；真实
  Colyseus `gracefullyShutdown(false)` 的底层句柄释放也有独立测试。XREAD、repair worker、loop monitor、
  stream depth alert 和存储入口仍有连续 start/stop 与停服关闸用例。

保留边界：完整外部 Redis/MySQL/WebPlatform smoke、真实 Creator 预览和目标设备采样仍不在 Node 证据内；
子进程用例替换的不止外部适配器，还包括 `app.config` 组合根（rooms/routes/express）、`infra/config`
以及 loader、loopMonitor、matchConsumer、kickBus、character、characterRepair、push、redisRoute、mysql、
webPlatformClient 等内部模块的探针桩，运行时房间数为 0；其中 `player/character.ts` 被桩掉意味着
character-ready 阶段本身也是探针，只证明阶段次序、不证明真实 ready 排空。它证明的是 admission 关闭时点、producer → character-ready → detached
task → registered resources 的释放顺序与 exit 0；不证明真实房间在 `onBeforeShutdown` 与 `onShutdown`
之间被排空，也不证明真实依赖装配——后者仍由同文件的源码接缝断言与本地 smoke 承担。
`5c7818f` 已将 detached task 的 settle 改为由 ready-drain promise 显式触发，`drain-ready` → `task-settled`
顺序不再依赖 wall-clock 定时器；真正的生产房间排空和外部依赖可用性仍由本地 smoke/真实部署验证。
默认进程的真实启动依赖这些本地/外部服务，因此不等于生产拓扑或第三方服务可用性承诺。
...
`checkManifest`，以动态 fixture 覆盖 root、package id、package.xml 资源/组件声明、package/View 缺失与多余、
View/AUTO 漂移；纯校验层同时覆盖 source/export 哈希不符、路径越界、导出物多重归属、manifest 结构非法、
重复 package name/id、非法 `ui://` 引用和 package.xml 伪声明。manifest 缺失与 JSON 解析失败有直接诊断用例；
CLI 用显式 `--manifest <path>` 统一 check/write，漂移返回非零且不会隐式改写目标，重复/未知参数会拒绝，
`FGUI_MANIFEST_PATH` 已明确禁用，避免环境变量静默改道。`27792b5` 已刷新 codegen 可见性变更后的 canonical
manifest 哈希；真实仓库资源闭包仍保留只读正例。

保留边界：FairyGUI 编辑器导出、资源导入和运行时 `autoClearItems` 等行为仍需 Creator 编辑器预览；Node 门禁
验证的是资源/生成物闭包与编排契约，不替代真实引擎和目标设备验证。
...
1. 完整数字 parser 拒绝尾随垃圾、指数写法、NaN/Infinity 和非安全整数；负值与上下界由调用方传入的
   min/max 选项拒绝（parser 本身不内置非负约束，`numbers.ts:16`/`:36` 的正则显式允许前导 `-`），全部
   调用点显式传 min（每处可 grep 复核，不钉死随代码演进的计数）。
...
2. `test:fgui` 已拆成准确的 `test:client` 与 FGUI 专项命令，并提供分层 `verify:core`/`verify:all`；
   `27792b5` 刷新了 codegen 变更后的 canonical manifest，`verify:fgui` 会拒绝陈旧哈希。
...
7. `d96186b` 先补齐了 legacy `cc-stub.d.ts` 的 `Color`、`EventTouch`、`Graphics`、`Input`、`UITransform`、
   ...
   全部 View 所需的 cc/FairyGUI 声明桩，`npm run typecheck:client:legacy` 才覆盖全部客户端源码。
   `efbd628` 将 FGUI codegen 的 AUTO 字段生成可继承的 `protected`，因此 `apps/client/tsconfig.json` 与
   `tsconfig.test.json` 均恢复 `noUnusedLocals: true`，业务代码与全部客户端源码继续受严格未使用声明检查；根
   ...
   反例，确认 ES2017 API floor 和根聚合命令均不可被静默削弱。`2816e85` 另为 strict 探针
   (`apps/client/tsconfig.test.json`) 增加 `noUnusedLocals: true` 守门断言，legacy 与 ES2022 两侧均受保护。
...
本轮已把聚合入口 `verify:core` / `verify:all` 与阶段命令 `verify:project` 补进 `CLAUDE.md`/`AGENTS.md`
的「常用本地命令」——此前该表逐条列出了 `verify:core` 的多个下游阶段却漏掉入口本身，而
`docs/OVERVIEW.md` 的口径是「改动合入前至少应通过 `verify:core`」。保留边界：`verify:inventory` 的条款
检查不覆盖命令表完整性，同类遗漏仍无机检。`7160d08` 已将
`apps/server/test/config-guard.test.ts:78` 的用例名校准为「1..2592000000 毫秒（最长 30 天）」；
配置单位与 `docs/SERVER.md` 保持一致。
...
`gameplay-boundary.defaultEntry` 因而有就近入口说明。

登记边界：`verify:inventory` 与 Markdown 链接检查覆盖的是登记表内的文档与就近 README，不扫描仓库根的
散装 Markdown。根目录 `todo-godogen.md`（`0ea018c` 入库，`3eb54e8` 由三层笔记合并为单一计划）因此不在
