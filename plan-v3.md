...
> `test:faults:int` 四组 72/67/12/15 全绿。
>
> **本轮回写快照**（§24 承重钉推广到四条链后同一工作树实测）：`verify:core`、`verify:all`、
> `typecheck` 全绿；服务端单测 297/297、客户端 258/258、`test:int` 154/154、
...
- `39838db`（`docs/Non-intrusive.md`，1066 行设计提案）**事实准确率 18/21、无铁律冲突、定位克制**，
...
1. `[已完成]` **承重钉体系的三个残留形态**（`06bc422`，一次机制修复）：
   - R3 新增链静默无钉：测试文件本地重建的 4-key 映射按 §24.3 自己的标准就是「无断言副本」。
...
2. `[已完成]` **Non-intrusive.md 三处事实修正 + 登记**（`b29d973`、`9d830e0`）：见 §25.1。
...
- Non-intrusive.md 的客户端 host 层（11 个新模块 + pages.ts 状态机迁移）是该提案最重风险，
