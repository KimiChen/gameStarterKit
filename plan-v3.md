...
> **本文件是历史归档。** 当前开放问题、实施状态与验收证据以 [plan-v4.md](plan-v4.md) 为准；
> 本文件保留该轮逐项实施的状态与验收证据，不再接受新状态回写，也不得从其中的完成标记推导当前状态。
> [plan-v2.md](plan-v2.md) 与 [plan.md](plan.md) 同为更早的历史归档。
>
...
> `test:faults:int` 四组 72/67/12/15 全绿。
>
> **上一轮回写快照**（§28 两条边界关闭后同一工作树串行实测）：`verify:core`、`verify:all`、
> `typecheck` 全绿；服务端单测 297/297、客户端 **262/262**、`test:int` 154/154、
> `test:inventory` 96/96；五张矩阵 3/3、2/2、5/5、4/4、3/3；
> `test:faults:int` 四组 72/67/12/15 全绿。
>
> **本轮回写快照**（§30 复核 HEAD `0d2333e` 同一工作树串行实测）：`verify:core`、`verify:all`、
> `typecheck` 全绿；服务端单测 297/297、客户端 **264/264**、`test:int` 154/154、
> `test:inventory` 96/96；五张矩阵 3/3、2/2、5/5、**5/5**、3/3；
> `test:faults:int` 四组 72/67/12/15 全绿。
>
...
- `[不阻塞·有意保留]` inventory 与 Markdown 链接检查只覆盖登记表内文档和就近 README，不扫描任意根目录
  Markdown；`plan-v3.md` 当轮通过 `routeOfTruth.corePlan`、`plan-v2.md` 与 `todo-godogen.md` 通过
  `referenceDocs` 纳入检查（⚠ 归档补注：真相指针已迁至 `plan-v4.md`，本文件现经 `referenceDocs`
  纳入检查——这正是本文 §30 记录过的「归档内以现在时自称当前真相」形态，按同一处置改为当轮时态）。本轮由复核者独立对 `git ls-files "*.md"` 全量（52 个 tracked `.md`）做了相对
...
- 两份新文档互为配套的「框架能力 vs 首版产品」分层（撮合预留 vs 首版不做公开匹配）与
  「默认入口归属」——后者**已拍板（2026-08-31）：snakeoff 优先替代 ballMove 入口**，wsrpc 的
  undergroundIdle 迁移入口路线次之；落地顺序按此执行。
...
## 29. 第二十二轮复核（2026-08-31）

复核对象：`f53c8cc..9ced4eb`（同事第二十一轮的 `f31d874`/`5663de4` + §28 回写 `b9b815f` +
策划案优化 `9ced4eb`），及同事对第二十轮表述更正的确认。方法：对抗式独立核查（隔离副本 +
真实 npm 逐形态实测）。

### 29.1 对第二十一轮的判定

- `f31d874`（workspace 闸表补后缀式）**修复成立但前提成色有误**：前后缀 6 个变体全部闸住；
  但「不需要合谋就能触发」不实——改写链形态必须动被 `CHAIN_LOAD_BEARING` 守门的声明表，
  干净逃逸仍需声明表+链+钉三处同改（§28.2 已就地更正）。且对抗复核发现仍漏三类等价形态
  （`npm run -w Y X`、`-w <dir>` 路径选择器、`--workspaces --if-present`，真实 npm 均跑钩子
  而双闸全绿），本轮已收口（`25d5404`）。
- `5663de4`（--check 扫描面扩 + 豁免带理由）**成立**：扫描面命中集合与四条豁免理由逐条核实
  （仅 vendor-lock 一条论证失真，已更正 `9d0a455`）；三个变异（新增未登记脚本/空理由/删豁免）
  各自变红。仍漏 `./` 前缀、解释器夹 flag、子目录脚本三条逃逸，本轮已补（`d9ce5b2`）。
- `b9b815f`（§28 回写）**两处不实**：「不需要合谋」与 §28.3「`--prefix` 真出现时链条覆盖判定
  会先红」——后者被隔离副本证伪（三处同改后双闸全绿；`commandReferences` 的失败关闭只在
  有 requires 锚点时触发，`verify:all` 无锚），两处均已就地更正（`11ee744`）。
- `9ced4eb`（Underground Idle 策划案优化）**快查无实质问题**：引用与仓内物一致、未实施状态
  声明克制、登记仍有效；两点观察（wsrpc 回链被删成单向、新 FGUI 包名 `idle` 小写偏离既有
  大写前缀惯例）登记在此，落地前宜拍板。
- 同事对第二十轮两处表述更正（§26 数字、§25.2「静默」）的确认属实。

### 29.2 本轮新发现并修掉的缺陷

1. `[已完成]` **workspace 调用形态的三类漏判**（`25d5404`）：见 §29.1；闸改为「前后缀命名式 +
   `--prefix <dir>` + `-w <dir>`（按目录解析）可闸，不可归类即拒」。四条反例 + 三组变异。
2. `[已完成]` **--check 扫描面三条逃逸形态**（`d9ce5b2`）：`./` 前缀、解释器夹 flag、子目录
   脚本；扫描面抽出为 `checkScriptPaths` 并扩展，正反向断言各若干，回退旧正则即红。
3. `[已完成]` **vendor-lock 豁免理由论证失真**（`9d0a455`）：写模式实际会收敛（实测），
   正确依据是「重新祝福当前字节而非从独立真源恢复 + 集合不匹配直接抛错」。
4. `[已完成]` **plan-v3 两处证伪更正**（`11ee744`）：见 §29.1。

### 29.3 仍登记的已知边界

- 豁免表条目本身靠评审（自指极限，§28.3 已登记）；`npm exec`/`npm run x -- --check` 等更偏
  的形态不在扫描面内（出现即需显式决策）。
- 不完整 `workspaces` 声明下的引用解析失败按跳过处理（真实 npm 会响亮拒绝该命令，钩子在
  命令跑不起来的前提下没有可藏之处）。
- `9ced4eb` 的两点观察已有结论：Idle 的 FGUI 包名已按拍板改为 `Idle`（首字母大写）；Home 入口仲裁
  已拍板 **snakeoff 优先**（§27.3 末条）。wsrpc 单向回链保留为备注，不构成门禁问题。

实测：`verify:all` 通过（服务端 297/297、客户端 264/264、FGUI 50/50、五矩阵 3/2/5/5/3）、
`test:inventory` 96/96、`test:int` 154/154（本地栈）。

## 30. 第二十三轮复核（2026-08-31）

复核对象：`b9b815f..0d2333e`（第二十二轮的三个门禁/测试修复、两处对我的表述更正、§29 回写，
以及 FGUI 包名与默认入口仲裁两条决策登记）。

**本轮我没有发现需要修的新缺陷**——同事的三个修复经逐形态实测覆盖完整，而我上一轮写进 §28
的两处判断**都是错的**，本节如实记录。

### 30.1 我上一轮两处判断错在哪

1. **「workspace 后缀式不需要合谋就能触发」——错。** 我自己的探针当时就同时改了
   `verify-toolchain.mjs`、`toolchainContract.test.ts`（承重钉）与 `package.json` 三处；
   承重钉的双向 `deepEqual` 决定了改写链形态**必然**要动声明表，干净逃逸确实需要三处同改。
   §27.3 原本的「自指边界」判断是准确的，我不该把它改掉。
   **但修复本身仍然该做**，理由要换成准确的那个：等价写法的改写是一次**正当重构**，
   开发者会自然地把三处一起改（钉子逼他改），于是「重构完成、门禁全绿、闸悄悄没了」。
   威胁模型不是恶意合谋，是**无恶意的等价改写静默丢闸**。
2. **「`--prefix` 出现时链条覆盖判定会先红」——错，已被直接证伪。** 我在 `b9b815f` 上实测：
   `--prefix` 三处同改 + 给 `apps/server` 加 `pretest` + 按命令表要求登记该钩子（全是正当步骤），
   `verify-toolchain` 与 `verify-inventory` **双双 exit 0**。
   我先前看到的那次红来自**不相干的闸**（workspace 脚本未登记），把它误读成了覆盖判定生效。
   根因是 `commandReferences` 的失败关闭只在**有 requires 锚点**时才产生断言，
   而 `verify:all` 在 `docs/inventory.json` 里根本没有登记（§24.1/§26 已确认过这一点，
   我写 §28.3 时没把它联起来）。

方法论教训：这两条都是**我没有一路测到底就写下的断言**。§28.2 我刚写完「反例先做变异确认它
真的会红」，同一节里就放了两条没做这一步的判断。

### 30.2 对第二十二轮修复的验证

- `25d5404`（workspace 形态）：我用真实 npm 建 workspace 探针，逐条确认 **10 种调用形态全部
  会跑 workspace 的 `pretarget` 钩子**（含 `-w <dir>`、`--prefix <dir>`、`npm run -w Y X`、
  `--workspaces`、`--workspaces --if-present`）；再逐条打门禁：**7 种被直接闸住并点名 `pretest`，
  3 种走「不可归类失败关闭」**，无一静默放行。设计取舍正确——不追着枚举形态，而是
  白名单可归类形态、其余一律拒。
- `d9ce5b2`（扫描面）：五种形态（裸、`./` 前缀、解释器夹 flag、`scripts/` 子目录、`tools/` 多层
  子目录）实测**全部要求显式分类**。
- `9d0a455`（vendor-lock 豁免理由）：**是对我的有效纠正**。我原写「没有写模式语义」，
  而 `writeVendorLock` 确实存在（`scripts/vendor-lock.mjs:200`）。改后的理由
  （「重新祝福当前字节而非从独立真源恢复」+「集合不匹配直接抛错」`:205-209`）与代码相符。
- `ecedee8` / `0d2333e` 两条决策登记属产品侧，本轮不作技术判定；`0d2333e` 已把 §27.3 的
  待拍板项标记为已决。

### 30.3 边界

- `[不阻塞·有意保留]` `--workspaces` 系形态走「不可归类失败关闭」：若将来真要在链里用它，
  必须先扩判定而不是直接写——这是有意的摩擦。
- `[不阻塞·有意保留]` 解析不到 workspace 的引用**跳过而非报错**：真实 npm 会 `No workspace found`
  响亮失败，命令跑不起来时钩子无处可藏。该取舍的前提是「仓内 workspaces 声明完整」。
- `[不阻塞·有意保留]` 豁免表条目本身仍靠评审（§28.3 已登记，自指极限）。
