...
§27.3 把它们登记为「属自指边界 / 靠评审」，但两条都**不需要合谋**就能触发，因此该关而不是该登记：
...
- `[不阻塞·有意保留]` workspace 闸表仍不认**路径形态**（`npm --prefix apps/server run test`）。
  该形态今天仓内不存在，且 `commandReferences` 侧对 `--prefix` 一律失败关闭（§18），
  真出现时链条覆盖判定会先红。
