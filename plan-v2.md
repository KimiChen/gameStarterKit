...
> `plan.md` 中标注为「原审阅证据（已收口）」的历史复现不列为当前问题；§6 另登记 `plan.md` 自身的文档问题。
>
> 抽取基线：`plan.md` @ `371d72e`（与 `63e6fc0` 内容一致）。文中 `plan.md:NNN` 是该快照的行号，
> 随 `plan.md` 演进会漂移，仅作定位辅助。
...
- 对局没有时长上限；是否增加属于规则设计决策。它不属于已闭合的内存有界性缺陷，但当前也没有收口。
...
- WebPlatform consumer map 另登记了仓内未调用的 `Livez`/`Readyz` 两个契约（属 consumer 子集而非生成全集，当前无实际暴露面）。

来源：`plan.md:525-526`、`plan.md:530-531`
...
- `test:int` / `test:faults:int` 依赖本地 Redis/MySQL 与当前 Node/Colyseus 运行时；环境中触发的既有
  schema decorator 兼容错误按约定记录为环境边界，不得误写成新回归。

来源：`plan.md:631-632`、`plan.md:668-669`、`plan.md:148`、`plan.md:154-156`
...
- 顶部说明（`plan.md:8`）写“其后另有第十、十一轮”，但正文已经记录第十二轮——前向引用没有随新轮次更新。
- 第十轮记录（`plan.md:59`）内嵌“当前树 HEAD 为 `0ea018c`”：这是撰写时快照，随分支推进立即过期。
  过程记录中的自指表述应写成纯历史叙述（“复核时 HEAD”已足够），不宜保留第二个“当前”表述。
- 第十一轮（`plan.md:75`）写“见 P0-04「未覆盖判据」”，但 P0-04 实际使用的小节标题是“覆盖面”
  （`plan.md:373`），引用无法直接定位；同轮登记的 5 处恒真/无覆盖附带断言只给了条目范围
  （P0-04 及 P0-05／P1-08／P1-09），没有逐项行号，读者无法核对。
- `✅` 表示限定范围已完成，但 P0-04、P0-05、P1-06、P1-07 等条目仍保留明确的安全、可用性、证据或扩展边界，状态标记容易被误读为“没有遗留问题”；本清单的存在即为缓解，长期可在 `plan.md` §1 图例中把 `✅` 与保留边界的共存关系写得更醒目。
- `PROTOCOL_VERSION=5` 目前只因仓内没有已部署旧客户端而保持不变；首次承担线上兼容义务前，需要重新评估版本升级和兼容矩阵（`plan.md:125-126`）。
- 本文件（`plan-v2.md`）与 `todo-godogen.md` 同属根目录散装 Markdown，不在 `verify:inventory` 的登记
  覆盖内；若长期保留，应登记到 README、AGENTS/CLAUDE 与 `docs/EXTRAFEATURES.md`，或迁入 `docs/`
  纳入链接检查（处置口径同 `plan.md:709-716` 对 `todo-godogen.md` 的分析）。
