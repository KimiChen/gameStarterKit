...
> `plan.md` 中标注为「原审阅证据（已收口）」「复核备注（已收口）」的历史复现与已闭合结论不列为当前问题；
> §7 另登记 `plan.md` 自身的文档问题。
>
> **本文件是 `plan.md` 的只读派生视图，不是第二真相。** 任何结论变更一律回写 `plan.md`，本文件随之重生成；
> 开放项全部收口后应整体作废删除，而不是与 `plan.md` 长期并存
> （见 `plan.md:12`「本文件是核心改进优先级的唯一真相」与 §10 完成定义第 1 条）。
>
> 抽取基线：`plan.md` @ `371d72e`（该文件自此未再变动，故文中 `plan.md:NNN` 行号当前有效；
...
- `[不阻塞·待补齐]` 顶部说明（`plan.md:8`）写「其后另有第十、十一轮」，但正文已经记录到第十二轮——
  前向引用没有随新轮次更新。
...
- `[不阻塞·待补齐]` 本文件（`plan-v2.md`）与 `todo-godogen.md` 同属根目录散装 Markdown，不在
  `verify:inventory` 的登记覆盖内（`plan.md` 自身已登记并纳入检查）。作为 `plan.md` 的只读派生视图，
  它的正当处置是随开放项收口而作废删除。若确实长期保留，处置口径同 `plan.md:709-716` 对
  `todo-godogen.md` 的分析，但**不宜登记进 `docs/EXTRAFEATURES.md`**——按 `plan.md:677`，该文件是
  「额外能力真相」，而本文件是 `plan.md` 的派生视图、不是一项额外能力；宜迁入 `docs/` 并在
  README、AGENTS/CLAUDE 登记以纳入链接检查。
