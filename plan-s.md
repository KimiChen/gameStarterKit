...
> `[已拍板·待实施]`、`[进行中]`、`[已完成]`、`[阻塞·需 Creator/真机]`、`[有意不做]`；
> 文档落盘本身不代表任何实现阶段完成。
...
- 异形屏、折叠屏和平板只调整安全区与 UI 密度，不修改世界单位。
...
| `newEndlessPortraitV2` | 后续候选 | 单独取证后决定 | 单独取证后决定 | 另行做容量、网络和性能评审 |
...
> 人日为单工程师粗估，不含最终 FGUI 美术制作、多目标真机排队和产品数值反复。
...
| S5 | [已拍板·待实施] | 验收与发布 | 自动测试、Creator、真机、故障演练、性能与兼容证据 | 全量门禁、Creator 和目标真机证据齐全 | 3–5 人日 |

完整首发合计约 25–40 人日。S3 可用于内部试玩，面向玩家宣称“养成系统完成”必须等 S4 的可靠奖励闭环完成。
...
- [ ] 750×1624 Creator 预览与目标真机截图留证。
- [ ] 低端目标机性能、内存、资源加载尖峰和断线重连留证。
- [ ] 回写本文件状态、commit、命令结果、截图和真机证据路径。
...
在当前上限 `8 蛇 + 315 食物 + 240 残骸` 下：

- 目标机型稳定 60 FPS；记录平均、P95 与最差帧。
- 食物保持单 atlas 批；每蛇最多一条主体 mesh。
- 不出现 315 个 Sprite 节点或同量级 draw call。
- 继续满足 64 KiB Snake 快照预算；新增 variant/kind 后重新记录最坏包大小。
- 皮肤资源预加载只包含本局真人装备、AI 池和默认 fallback；衣柜预览使用有界缓存。

### 11.4 Creator 与真机证据

- Creator 预览确认所有纹理、rect、pivot、动画、混合和层级正确。
- 至少覆盖一个低端目标设备、一个主流设备和一种异形屏/平板布局。
- 留存战场 golden、衣柜、结算、断线重连、资源缺失 fallback 和性能截图/日志。
...
| 每食物一个节点 | 真机帧率与内存恶化 | 单 atlas/material 批量 mesh |
...
| 阶段 | 状态 | commit | 自动验证 | Creator/真机证据 | 备注 |
|---|---|---|---|---|---|
| S0 | [已拍板·待实施] | — | — | — | — |
| S1 | [已拍板·待实施] | — | — | — | — |
| S2 | [已拍板·待实施] | — | — | — | — |
