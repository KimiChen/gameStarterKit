...
> 的既有门禁口径，也不改变 [plan.md](plan.md) 的核心优先级；两者分工见 §7。
>
...
3. 产物落盘前先做往返自检，防住「静默丢东西看起来像成功」这一类失败。
4. 把可复用的玩法规格转换为跨 shared/server/client/View/测试登记点的确定性脚手架。
...
- liangdabiao 用 `git clone` Godot **master** 生成 API 文档，却把引擎二进制钉在具体版本路径——
  文档与引擎按构造必然漂移。这恰好反证 T6「API 资料必须绑定锁定版本」的必要性。
...
```text
T1 产物往返自检（不依赖真引擎，成本最低，补的是一类全新门禁维度）
  → T2 verify:creator 最小路径
  → T3 资产 manifest（先「意图尺度」最小子集，再扩全字段）
  → T4 Underground Idle 最小垂直切片 → codegen:gameplay 泛化
  → T5 视觉验收 case
  → T6 Cocos/FGUI 运行时陷阱库
  → T7 助手指令单源化（成本近零，可随时插入）
  → T8 付费资产任务器与离线工具（有真实需求再做）
```

排序理由：T1 不需要真引擎、不需要新基础设施，却能立刻补上现有门禁完全没有的一个维度，因此提到最前；
T3 拆成两步，避免完整字段表拖住整条链。每一步都保留现有核心门禁，Creator/资产证据作为**额外能力**
登记，不把尚未覆盖的真实引擎边界写成 `verify:core` 已通过。

## 4. 任务

### T1 产物往返自检（P1）

**为什么**：两个仓库**各自独立**在场景构建脚本里放了同一段自检——
`build → 立刻反序列化 → 数节点 → 数量不符即 exit(1) 且不落盘`。它防的是一类**静默**失败：`owner` 链
未设全时 `PackedScene.pack()` 会悄悄丢节点，编译通过、保存成功、运行时少东西。原文的定性很准确：
`A silent drop otherwise looks like success.` 两处实现都强调 **gate the save on the validation result**。

对本仓的意义：现有 `verify:fgui` 是**哈希型**门禁，回答「这个文件还是我记下的那个吗」；它回答不了
**「这个产物反序列化回来还是我以为的那个内容吗」**。FGUI `.bin` 导出、`.meta` uuid、Cocos 场景序列化
都属于同一家族。往返自检是一类**新的**门禁维度，不是现有内容锁的重复。
...
- [ ] 其余候选（Cocos 场景、`.meta` 集合）先只登记不实施；需要真引擎的部分并入 T2。

**完成标准**：删掉导出物里的一个命名元素后，本地检查稳定变红且产物未被更新；正例在真实仓库上为绿。

### T2 verify:creator 最小路径（P1）
...
- **`plan.md` 的考古层张力**：htdt `AGENTS.md` 的编辑规则要求改动特性时**直接描述新状态**，
  不要写「现在改成了 X（原来是 Y）」，以免文档变成考古层。本仓 `plan.md` 已累积多轮复核记录与
  「原审阅证据（已收口）」分层。**不建议照办**——本仓的审计轨迹在多方复核场景下有真实价值
  （能回答「这个结论是谁在哪个基线上验的」）。若将来确实影响可读性，折中是把轮次记录移入独立历史文件，
  `plan.md` 只保留当前状态。此处仅登记该张力，当前不动。
...
- [plan.md](plan.md) 是**核心改进优先级的唯一真相**，其条目状态由既有门禁背书。
- 本文是**额外能力的对照吸收计划**：任务完成前不得写进 `plan.md` 的完成声明，也不得据此放宽
  `verify:core` / `verify:all` 的口径。
- 本文任务若产出新的门禁（如 T1 的往返自检、T2 的 `verify:creator`），应在落地时按本仓既有规则
  登记到 `docs/inventory.json` 并补正反例，再考虑是否并入聚合命令。
