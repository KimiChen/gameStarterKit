...
Cocos Creator **3.8.8** 微信小游戏工程 + FairyGUI（fairygui-cc 1.2.2）+ Oops ECS 库。
整体设计意图见 [OVERVIEW.md](OVERVIEW.md)；服务端见 [SERVER.md](SERVER.md)。

> 客户端**不在 npm workspaces**（Cocos 要自己的工程目录结构）；第三方库以源码/UMD 形式直接放在
> `assets/` 内。代码全在 `apps/client/assets/src/`。
...
```bash
# 仓库根目录先跑：
npm run sync:shared    # 生成 assets/src/shared/（双端共享层，已入库）
npm run fetch:fgui     # 拉 fairygui-cc 运行时到 extensions/fairygui-cc/runtime/（每台机一次）
```

然后在 Cocos 里：

1. 用 **Cocos Dashboard 3.8.8** 打开 `apps/client`，等首次导入（生成 `temp/`、`library/`、`.meta`）；
2. 选中 `assets/src/lib/colyseus/colyseus.js` → 属性检查器勾 **导入为插件** + 各平台加载，应用后重启编辑器；
3. 菜单「扩展 → 扩展管理器 → 已安装扩展」确认 **fairygui-cc** 已启用
   （挂载只读 `db://fairygui-cc/fairygui.mjs`）；
4. 新建场景（含 Canvas），把 `Main.ts` 挂到 Canvas 节点，保存；
5. 仓库根 `npm run dev:server`（http://localhost:2568）；
6. 编辑器 **预览**：控制台应输出 mock 登录成功 + 进房日志，**按住屏幕可拖动小圆点**。

> `.meta` 由编辑器生成后**连同资源一起提交**（uuid 不稳会丢引用）。
> `fetch:fgui` 版本钉死 1.2.2（⛔ 不飘 latest 免各机分叉）；运行时 600K+ 可再生、`.gitignore` 忽略、
> 每台机跑一次；扩展外壳（package.json/browser.js）入库。
...
## 2. 目录导览（`assets/src/` 根 = 入口两文件 + 6 目录，每目录有 README）
...
| `lib/` | 第三方锁定区：`ecs/`（8 文件字节锁）· `colyseus/`（UMD 插件）——⛔ 不可手改 |
...
`assets/resources/ui/`**、新 `.ts` 需开一次 Cocos 生成 `.meta`——见本文件末尾「迁移页面的一次性手动步骤」。
...
```
① FairyGUI 编辑器出图 → 发布 bin 到 assets/resources/ui
② npm run codegen:fgui -- <Pkg> <Comp>   生成 view/XxxView.ts（四个 AUTO 区块）
   契约条目加进 fguiContracts.FGUI_CONTRACTS；新 View 加进 tsconfig.typecheck 排除清单
...
真源 `designSpec.ts`，与 `settings/v2/packages/project.json` 的 designResolution（fitWidth=true 烘焙值）、
...
（命名 = 契约，改了 CI 契约测红）。改完**发布 `.bin` + 图集到 `apps/client/assets/resources/ui/`**——
...
7. 连 localhost 需关开发者工具的合法域名校验。该开关在**构建产物**的 `project.private.config.json` 里，
   重新构建会重置——已用构建模板 `build-templates/wechatgame/project.private.config.json`（urlCheck:false）
...
- **主包 4MB 限制**：`colyseus.js` 约 440KB。`npm run report:size` 出主包/分包体积报告（3.5MB 触发水位
  提示走资源分包）。
- **ECS 全局状态**：组/系统注册是模块级全局，用单例（`GameECS.inst`），场景重载重复 new 会泄漏 group 回调。
...
1. **FairyGUI 编辑器**：打开 `apps/art/fairygui/FairyGUI.fairy`，把这 12 个包发布 `.bin`+图集到
   `apps/client/assets/resources/ui/`：`View_AreaList_Login`/`View_AreaList_AreaList`/
...
2. **Cocos Creator**：打开 `apps/client` 生成新 `.ts` 的 `.meta`（view/logic/net 下的迁移文件已手工建 meta，
