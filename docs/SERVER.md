...
npm --workspace @game/server run db:bootstrap    # 建 game_<projectId> 库 + 全量 DDL + 预置行
...
- **多项目并行（同机多个本框架项目共用这一套栈）**：隔离靠**根目录 `.env.development` 的
  `projectId`**（缺省 `gono`）——Redis 全部键带 `<projectId>_` 前缀（`infra/keys.ts` 唯一拼接点）、
  MySQL 库名 `game_<projectId>`（`db:bootstrap` 自动建）。第二个项目只需改根 `projectId` +
  `apps/server/.env.development` 的 `PORT`（两个 dev server 不能同端口）。
  进阶（真要物理分栈）：`dev-stack.sh` 会从 `.env.development` 的三个连接 URL 派生栈端口、
...
> 下表为**逻辑键名**；实际存储一律带 `<projectId>_` 运行时前缀（`infra/keys.ts` 唯一拼接点，
...
| `PROJECT_ID / REDIS_KEY_PREFIX` | 根 .env 的 projectId（缺省 gono）/ `<projectId>_` | 多项目共用栈的命名空间：Redis 键前缀 + MySQL 库名 `game_<projectId>` |
