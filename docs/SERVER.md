...
- kit 表（`k_<id 小写>_*`，来自 `apps/kits/<id>/sql/`）按 `kit.json.sql.tables[].zone` 分区：`per-zone` 表必须有
  ...
  `core/infra/zoneTables.ts`（`perZoneTables` / `globalTables` / `allTables`，catalog 可注入）给出，关单区 /
  统计 / 冷档遍历与 smoke 的表清单都从这里取，⛔ 不再手抄。
...
- **09·R1–R9**：key 只由构造器产出（框架键在 `keys.ts`，玩法键经 `kGameplay` 工厂在
  ...
  `core/<pluginId>/keys.ts` 定义；⛔ 业务代码一律禁手拼）；按需字段读；相关 Lua key 同槽；durable/cache 语义隔离；
...
| kit-api/server 门面 | `apps/server/src/core/infra/kitApi.ts`（docs/KIT.md §4）：`withKitTx(kitId, sId, fn)` = READ COMMITTED 事务 + 只能碰 `k_<kitId 小写>_*` 表的 `tx.query()`（运行时表标识符闸 `assertKitTableAccess`，fail-closed：框架表 / 别的 kit / schema 限定 / DDL / 多语句一律拒）、`tx.debit` / `tx.credit`（currency.ts `debitInTx` / `creditInTx`，sId 已绑定）、`tx.enqueueEffect`（outbox `insertOutboxIntent`，ODKU no-op ⇒ "DUP"），提交后逐 uid `invalidateBalanceCache`；`kitOpId(kitId, uid, sId, op, clientReqId)` = `deriveOpId` 的 `kit:<kitId>:<op>` 命名空间；kit 需要的错误类型 / `CUR_GOLD` / `kKitUser` 从此文件再导出（kit 从 `apps/server/src/kits/<id>/**` 相对导入 `../../core/infra/kitApi`，⛔ 不直接 import 其他 core/infra 模块）。契约测试 `apps/server/test/kit-api.test.ts` |
| MySQL DDL | `apps/server/sql/schema.sql`（含 kit 迁移账本 `kit_migration` 与 `singleton_lease('db_bootstrap')` 预置行）；兼容升级逻辑在 `tools/db-bootstrap.ts`；kit 表来自 `apps/kits/<id>/sql/NNN-<name>.sql`，由 `tools/kit-migrations.ts` 在 `db_bootstrap` 租约下按账本逐条语句应用（白名单 lint、已应用文件 sha256 变化 fail-closed），按区表登记在 `core/infra/zoneTables.ts` |
