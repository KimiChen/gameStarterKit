...
> - [§12 开发约束 64 条规则目录](#12-开发约束64-条规则目录)——服务端 56 处 `09·XX` 注释锚定这里；
...
真实玩法链路（wx 登录、排行、工会、充值、冷档）需要本地 Redis×2 + MySQL：
...
| `core/` | 服务端底座（横切原语 + infra/auth/economy/rank/archive/match/guild/compute 子模块） |
...
客户端 join 房间 `lobby` 后，所有取数/排位/邮件/工会请求走**单一 `rpc` 消息通道**。Colyseus 的
...
- **HTTP（`http/`）**：真实端点，Colyseus 0.17 typed router（`createEndpoint`，zod 校验 body）。
  ...
  token_epoch 撤销）、`POST /pay/wx-notify`、`GET /version`、`GET /clock/now`、`POST /rank/*`、
...
| 加排行榜种类 | `rank/rankService.ts` 的 RANK_TYPES |
...
- `rank`：Redis ZSET 榜 + encodeScore 时间戳 tie-break + 赛季轮换
...
> **DB** MySQL 纪律 · **G** 网关与协议 · **K** 排行/结算/流 · **F** 冷档 · **S** Schema 演进。
> PR 审查顺序：写路径 → A（真源）→ I（幂等）→ L（锁/fence）；涉 Redis 查 R 全组、涉 MySQL 查 DB 全组；
> 钱 + 道具查 X 全组；冷档查 F 全组；结算/排行/流查 K 全组；新增常量/key/错误码先更新 §13。
...
- **X7** relayer/freeze worker/赛季轮换是单例——`singleton_lease`+fence_token，续租 UPDATE 与业务批写同一 MySQL 事务、守卫作第一句、0 行即 ROLLBACK 自杀；⛔ 禁 GET_LOCK 替代。
...
- **R3** per-user key 一律 `{uid}` hash-tag 同槽；⛔ 跨用户 key（rank:*、active:lru:{bucket}、stream:match）不进同一条 Lua。
...
- **K1** encodeScore 分母 = 赛季长度 `SEASON_LEN_S`（⛔ 非绝对 epoch），intScore ~1e12 内；累加必须单条 Lua 内 `ZSCORE→重算→ZADD`，⛔ 禁直接 ZINCRBY。
- **K2** 结算去重键 = `lb:dedup:{matchId}:{uid}`，⛔ 禁只按 matchId。
- **K3** ranked 发奖延迟到无头重放校验通过（先 pending 再 commit），发奖类强制 100% 校验；suspect 走 clawback（新 op_id 反向冲销，注意 X8）；状态落 MySQL `rank_award UNIQUE(season,uid)`。
...
| `rank:{type}:{season}` | ZSET | 赛季 | 排行榜（member=uid, score=encodeScore） |
...
| `RANK_UNAVAILABLE` | Redis 榜不可用 | 提示稍后 |
...
货币/outbox/充值/邮件附件→M8 结算 ; M7 排行→M8 ; M9 冷档(需 M2+M6,水位启用) } ; M10 运维贯穿`。
...
| M7 排行 | encodeScore + 单条 Lua 更新分 + 去重键 / 取榜 / 赛季轮换 / 发奖（需 rating 拍板） |
| M8 对局结算 | M8a 休闲（matchId + match_index 幂等闸 + stream:match 证据链）；M8b ranked 发奖闸（需 M7） |
| M9 冷档冻结层 | resolve(fence 新鲜度) + ensureLive + freezeWorker + janitor；relayer cold stub 换真 ensureLive |
| M10 运维收口 | 看板 / 例行任务（分区维护、死号清退、旧榜 TTL）/ DR + PITR runbook |
