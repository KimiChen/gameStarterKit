...
| MK1 世界闭环（B1–B6 ✅ 2026-09-20 交付；退出待拍板，§12） | movement 面、AOI 接入、两图交接、检查点恢复、附近 / 世界聊天与队伍包装、客户端基础适配 | MF5（a + b）、MF6、MF8 | 断线 / 重启 / 交接故障矩阵全过 ✅；基准台达 kill criterion（热点 100 人 ❌ / 50 人 ✅ 贴线） |
...
下一动作：**框架段 MF0–MF11 全部退出（tag `mmo-framework-v1`）；`mmo` kit MK0 已退出（tag `mk0-exit`）；MK1 世界闭环 B1–B6 已于 2026-09-20 交付、退出待拍板；MK2 模拟闭环 B1–B3 已于 2026-09-20 退出（tag `mk2-exit`）**——场景 B 热点 100 人未达 kill criterion（p99 55 ms / 126 KB/s；50 人贴线达标，§12 MK1 行偏差 ⑧）：拍板项 = ① 先建 bots 独立进程的基准台分离服务端本体成本再定，② 收紧节拍 / 角色可见上限 / 位置量化后重测，③ 或接受「50 人热点」为 v1 上限并改 §11.2（只许收紧的方向是收紧数字，不是放宽阈值——③ 需明示为例外）；拍板后补 tag `mk1-exit`；MK2 已退出 ⇒ 下一阶段 MK3 资产闭环（inventory 面物品半边：grantItem / moveItem / claimLoot 全在 withWorldTx、唯一物品 / 容器 / 装备 / 掉落归属、角色保存定稿、长跑）可开工；PS1 + PS4 入口拆分可随时落地；**MF5a 已退出 ⇒ slg 2b 与 lvr 视图房可开工**（已通知 slg.md §10.8，接法见 KIT.md §4 / SERVER.md §5）；门① 已过，线上部署方按 SERVER.md §8.2 SOP 执行迁移；MF7a 已退出 ⇒ slg / lvr 无人在线结算可开工（已通知 slg.md §0.1 / lvr.md §4.3），各自落地后在此回写一行；门③ 已过：`WORLD_ROOM_PROTOCOL_VERSION=1` 一次定型，首个客户端发版前仍可 revert。MF0 行登记的基线红项（Creator 镜像 `.meta` 同步——现连带 `test:sync-mirror-matrix` 8 例、`docs/evidence` 跟踪文件政策）仍待处置，⛔ 不算 MMO 阶段偏差。
