...
进程形态（v1.2，D27）：lobby（LobbyRoom + 框架域 + 在线表 / push / kick / mailwake）、game（GameRoom 全部 match mode）、world（WorldRoom）**三个进程**，各自 `apps/server/src/entries/<name>.ts` + `<name>.config.ts` + 端口（`LOBBY_PORT` / `GAME_PORT` / `WORLD_PORT`，缺省都等于 `PORT`）；本地 `npm run dev` 缺省仍是合体入口（`index.ts` 合并三份 config），`dev:split` 起三进程；客户端从游戏 HTTP `/version` 取 `lobbyWs / gameWs / worldWs`（缺省回落目录的 `gameWsUrl`，⛔ 不动外部 WebPlatform 契约；反代按路径分流是部署选项）；Lobby 只做定位与签票：私房票交客户端在 game 端 `create` / `joinById`，`world.enter` 建查实例行并签票后，客户端按返回 endpoint（为空回落 worldWs）在 world 端 `joinOrCreate`，由该端创建或复用 WorldRoom。lobby ↔ world 经 coord Redis 凭据 / 登记 / 流（§6.3 `kind=room`）与 MySQL 表互通，world 进程也跑 push 消费者但只处理 `kind=room`（只唤醒已存在实例）；`room.resolve` ⛔ 不再读 matchmaker 房间快照（容量 / start fence 由 game 入房端判断）。Colyseus 的 matchmaker 是进程级单例，拆分单位只能是进程。施工批次见 MMO-PLAN.md §5（PS1–PS5）。
...
| `entries/world.ts` + `world.config.ts`（D27：world 独立进程，PS4 独立入口 + `WORLD_PORT`） | `[RoomName.World]: defineRoom(WorldRoom).filterBy(["sId","mode","profile","mapId","line"])` 登记在 world 进程的 config；合体入口 `index.ts` 合并三份 config 时一并带上；⛔ 不登记进 lobby / game 的 config |
...
| `world.config.ts`、`config.ts` | `WORLD_MULTI_PROCESS=1` 只在**多个 world 进程之间**启用 RedisDriver / Presence（lobby / game 不加入该池：`world.enter` 通过 `WorldDirectory` 定位分线并签票，endpoint 优先取 `WorldRegistry` 的节点公开地址；客户端在 world 端执行 `joinOrCreate`，该端撮合或创建目标分线，D27）；`REDIS_COLYSEUS_URL` 必须 ≠ durable / coord（加载期断言）；`selectProcessIdToCreateRoom` 放置钩子只选 world 节点；首次无登记时回落配置的 world 端点，权威房激活后经 `WorldRegistry` 登记 publicAddress |
...
**兼容扩展**：content 面 v4 另声明可选 `hud: { kind: "module", ends: ["client"], export: "hud" }`。插件导出 `{ packId, load() }`，只为自有内容包提供 `IMmoHudModule`；`load()` 动态 import View，`IMmoHud` 由 kit 按 mount / render / unmount 管理。无贡献时使用默认 HUD，实体层与世界触摸仍由 kit 管理（版本与交付记录见 §12 MG2）。
...
| `content` | 内容包 schema + validator + 引用 / 可达性检查 | 注册表（读 `contributions.generated`）：`packFor(mapId)`、`creature` / `item` / `region` | 表现映射注册表 + 客户端地图几何；v4 加 `hudForPack`、`IMmoHud` / `IMmoHudContext` 与纯操作几何导出 |
...
| `orchestration` | 事件 / 命令 / 只读 API / 模块契约、`defineOrchestration()`、`MMO_ORCHESTRATION_VERSION = 1`、命令 validator | 运行器（内部）；对外 `readCheckpointedVars(addr, packId)`、`listCheckpointedVars(sId, mapId, packId)`、`createOrchestrationHarness()`（snapshot / restore） | `subscribeScriptState(source, packId, listener)`；迟订阅立即回放当前快照、断线清空，重连按新代次接收 |
...
| MK3 资产闭环（✅ 2026-09-22 退出，§12；24 h 长跑 stable） | inventory 面（唯一物品 / 容器 / 装备 / 掉落归属）、角色保存定稿 | — | 并发拾取 / 重复请求 / 重启重放不复制物品或奖励；币账仍在框架 |
| MK4 编排与验收（✅ 2026-09-22 退出，§12；tag `mmo-kit-v1-frozen`） | orchestration 面 + 运行器 + harness；api 冻结、说明书；内置内容包改为经贡献点装载；容量证据；打 tag `mmo-kit-v1-frozen` | MF9、MF11 | 干净安装全链闭环；卸载 / 升级识别在途交接与 pending 事件；框架零追加手改 |
...
| 预算 | 每 tick 每 pack：wall ≤ `ORCH_TICK_BUDGET_MS`（冻结 2 ms，§11.2）、命令 ≤ 64、奖励目标累计 ≤ 200（单条 grant ≤ 100 个去重目标）、事件队列 ≤ 256、存活脚本 spawn ≤ `limits.maxSpawnsAlive`、vars ≤ 4 KB | **fail-closed**：本 tick 命令整批丢弃，该 pack 在该分线 `suspended`（timers 保留、不再收事件），写 `k_mmo_world_event kind=packSuspended`；`mmoAdmin.resumePack` 或分线重启恢复 |
...
| `grantItem` | `{ toCharacterId, itemTemplateId, count, reason }` 或以 `toCharacterIds` 替代单人目标 | 模板 ∈ pack；`count ≤ limits.maxGrantCount`；写 `k_mmo_world_event kind=grantItem`（op_id 由 `kitOpId` 派生，包含 mmo / orchestration 命名空间、instanceId、sId、packId、eventSeq、idx，并加 orch: 前缀；行带 `checkpoint_rev`，按 §7.3 原子规则门控）→ worker `withWorldTx` → `inventory.grantItem` → `grantResult` |
| `grantCurrency` | `{ toCharacterId, amount, reason }` 或以 `toCharacterIds` 替代单人目标 | `amount ≤ limits.maxCurrencyPerGrant`（kit 硬上限）；同上路径经 `creditInTx`（persona owner） |
...
单人 `toCharacterId` 与批量 `toCharacterIds` 互斥；批量必须非空、无重复且 ≤100 人。kit 先确认整条名单都仍在本分线，再逐人扩展 durable 事件；保留单人旧 op_id，批量在原派生输入后附 `"recipients"` 与 characterId，名单重排不改变个人键。每次 dispatch 跨事件累计奖励目标 ≤200，在越限事件摘要和全部 durable 事件扩展之前拒绝，整个 dispatch 的暂存结果作废并暂停。
...
export type OrchestrationGrantRecipients =
  | { toCharacterId: string; toCharacterIds?: never }
  | { toCharacterId?: never; toCharacterIds: readonly string[] };
...
export type OrchestrationCommand =
  ...
  | ({ op: "grantItem"; itemTemplateId: string; count: number; reason: string } & OrchestrationGrantRecipients)
  | ({ op: "grantCurrency"; amount: number; reason: string } & OrchestrationGrantRecipients)
...
server 面提供 `readCheckpointedVars` / `listCheckpointedVars` 只读查询与 `createOrchestrationHarness({ module, pack, mapId, seed })`（`emit` / `advance` / `vars` / `replay` / `snapshot` / `restore`）；恢复封装实际 runner 的 vars / timers / regions / eventSeq，校验 pack / map / seed / 步长身份。精确签名以面导出与 api-freeze 为准。

### 8.6 交付方式与判据

1. `plugin.json`：`requires.kits.mmo = { content:1, world:1, characters:1, orchestration:1, … }`；`contributes.mmo = { content: "apps/plugins/<id>/content", presentation: "apps/client/src/plugins/<id>/mmoPresentation.ts", orchestration: "apps/server/src/core/<id>/mmoOrchestration.ts" }`（三条都在既有所有权 dir 规则内）；可另贡献 `hud: "apps/client/src/plugins/<id>/mmoHud.ts"` 替换本包 HUD，HUD 依赖 content ≥4；使用新增脚本订阅 / 批量奖励时另声明 orchestration ≥3。
2. `codegen:plugins` 生成 `apps/server/src/kits/mmo/contributions.generated.ts` 与客户端同名文件；实际均导出 `KIT_CONTRIBUTIONS`，server 收录 content / orchestration，client 收录 content / presentation / hud。
...
| D17 | 贡献点形态 | `contributions.<id> = { kind: data \| module, … }`；content / presentation / orchestration；content v4 兼容新增可选 hud（§12 MG2） |
...
| 24–72 h 长跑 | MK3-B3 4 分钟冒烟 stable + 正式 24 h stable（officenvidia，288 样本，`2026-09-20T234434-mmo-hotspot-soak-24h-officenvidia.json`，§12 MK3 行） | ✅ 2026-09-22 |
...
- **MG0 内容插件样本 1 `mmodemo`**（2026-09-22，提交 c99b5193（kit 反馈修复，tag 重打）/ acf22252（插件）/ 62472f1d（登记）/ 本行所在提交（B3）；**B1–B3 交付并退出**）：B1 拍板——样本名按计划默认 `mmodemo`（三贡献点全用）/ `mmohold`（MG2）；`apps/plugins/mmodemo/plugin.json`（`requires.kits.mmo = { content: 3, orchestration: 1 }`、`contributes.mmo = { content: content/pack.json, presentation: apps/client/src/plugins/mmodemo/mmoPresentation.ts, orchestration: apps/server/src/core/mmodemo/mmoOrchestration.ts }`、空客户端 entry）；内容包 `demoVale` v1 一份 JSON：图 2400×2400（山脊 + 巨石碰撞位图、nav 100）、区域 den / ambush、职业 fighter / caster（id 与 kit MMO_CLASS_IDS 同）、怪 wolf（aggro）/ valeBoar（被动）/ alphaWolf（boss，checkpointOnDeath，只由编排刷）、技能 strike / warcry / fireball / heal / bite / howl、物品 10、掉落表 3、NPC merchant（interacts trade）；B2——编排 `defineOrchestration({ packId: "demoVale" })` 分派到 encounters/{bossTimer,ambush,merchant}：① instanceStarted → startTimer(bossSpawn 600 s repeat)；timer → 狼穴无活 boss 才 spawn alphaWolf@den（±60 抖动经 api.rng）+ notice + sayWorld；creatureDied(tag boss) → setVar bossKills durable + 对 party.membersInInstance(killer) 逐个 grantItem alpha-fang（封顶 50 ⇒ ≤ 64 条命令）；② regionEntered(ambush, character) 且 tick − ambushAt ≥ 1200 → 围着来者 spawn wolf ×3（tag ambush，120 s 收回，不可走跳过）+ setVar + notice；③ interact(trade@merchant) → prompt（领药水 / 离开），choice(gift) → grantItem vale-tonic；`limits { maxSpawnsAlive 8, maxGrantCount 5 }`；表现映射 16 条（键与内置灰盒不重叠）。用例：`mmodemo-content`（validator / 职业覆盖 / boss・区域・NPC / packId 与 interacts 交叉核对 / 与灰盒同装载一图一包・物品不撞）、`mmodemo-orchestration`（harness：boss 周期与不叠刷、奖励只发同队在场者、70 人封顶不撑爆预算、伏击冷却、行商、同种子重放逐条相等・换种子落点不全同）、客户端 `mmodemo-logic`（表现覆盖全部 presentationId、键不重叠、经 presentationMap 生效）；`plugin -- pack` 17 文件 → 仓内首装 `plugin -- install`（锁 [package]）→ check 7 包一致。**首个真实插件暴露的 kit / 框架缺口（同批修，kit 0.1.24 → 0.1.25，`mmo-kit-v1-frozen` 重打到修复提交）**：① kit 测试假设贡献点为空（服务端 `mmo-contributions` 断言生成物 `content: []` 且交叉核对只认灰盒；客户端 `mmo-content` 断言包数 2、`mmo-character-select-logic` 写死首图 greybox）⇒ 改为与填充无关；② **kit 客户端缺陷**：`MmoWorldGameplay` 用 `classOf(classId)`（贡献包优先）取职业模板 ⇒ 多包并存时进灰盒图技能栏 / 预测速度取的是 demoVale 的战士 ⇒ 改按当前图所在包解析（`packForMap(room.mapId).classById`，与服务端准入同源；`classOf` 面签名不动）；③ 框架 `plugin-codegen.test.ts` 的隔离根只拷固定输入目录，不拷插件 `contributes` 指向的模块（apps/server/src/core/<id>/）⇒ 首个带贡献点的树内插件让 23 例红 ⇒ 补拷贡献文件及其所在目录。偏差：`dash` 改 `warcry`（v1 无位移技能族）；内容包单文件；行商是赠送（无扣币命令）；boss 奖励封顶 50；可选域 `mmodemo.bossBoard` 归 MG1-B2；**B3 动线验收（§9.4，相对 tag `mmo-kit-v1-frozen`）全过**：#1 插件提交 24 条 = 手写 17 + 生成物 6 + 锁 1、placement 0；#2 `plugin -- changed --base mmo-kit-v1-frozen --dry-run`（在插件提交的检出上跑）24 条全部落在 mmodemo 所有权推导集、foreign = []；#3 冻结路径 `':!*.generated.*'` diff 为空、`mmo.lock` 字节不变、`git grep mmodemo` 在 kit 路径只命中生成物与 kit README 样本名说明文字（B4 说明书 / MG0 反馈段，非代码耦合，登记为例外）；#4 writer 重跑零变、两个 codegen `--check` fresh；#5 mmodemo.lock 新增、指纹 e03aae620dcd02d3 不变、三处协议版本不变；#6 host.json 未动；#7 worktree at tag：pack zip 17 文件 → install → codegen / sync → `db:bootstrap` 两遍 kit_migration 11 → 11（插件零 DDL）→ check 7 包一致 → `plugin -- test mmodemo` 15 例 → 服务端 typecheck 零错（客户端 typecheck 在裸检出缺未跟踪的 `ui-uniflex/generated`，与插件无关）→ harness 事件序列 → uninstall 17 文件收回 → check ✔；#9 边界机检绿、65 条命令注入 ⇒ suspend、同种子重放相等。**MG0 退出（§9.5：拍板 / 内容包 + 表现 + 编排 / 接入贡献点 ✅）**；下一阶段 MG1（竖屏操作 + 可选域页面 bossBoard）、MG2（`mmohold`）。
- **MG1 竖屏操作模型 + 可选域页面**（2026-09-22，提交 2790218e（B1，kit 0.1.26 → 0.1.27，`mmo-kit-v1-frozen` 重打到本提交）/ 38947677（B2，mmodemo 0.1.2 → 0.2.0）；**B1–B2 交付并退出**）：B1——操作模型纯逻辑 `apps/client/src/logic/rooms/mmoWorld/hudControls.ts`（hudLayout 竖屏布局 / joystickFrame + JoystickSession（movement 面 dirFromJoystick 死区归一 + IntentThrottle 限频合并：死区内未动过不发、变向立即发、抬手只在动过时发停）/ wheelSlotPositions（正上 → 正左弧形分槽，弦长 ≥ 槽边长，溢外圈不越过内圈最左）/ screenToWorld・worldToScreen / pickEntity・resolveWorldTap（最近命中排除本人、再点同一目标清除、空地 ⇒ moveTo））；kit 默认 HUD `view/rooms/mmoWorld/MmoWorldView.ts` 换成摇杆（节点级触摸）+ 世界层轻点选目标 / 点地走（≤ 12 px）+ 技能轮盘（就绪高亮 / 冷却秒数）+ 目标红环，四向按钮退役，HUD 仍画在世界节点内（3d.md SC1-B9 前退路）；**Creator 预览证据**：`tools/creator-preview/run.mjs mmoWorld` 新场景（设置「进入世界」→ 选角（无角色先建角）→ 进入 → MmoWorldLayer → 摇杆按住向右拖 1.2 s → 视野无别的实体则摇杆探索 → 轻点实体 → 轮盘第一槽 → 离开）10 步全过、console 零错误（docs/evidence/creator-2026-09-22/mmoWorld，未跟踪）：建角后进 demoVale（HP 120/120、strike / warcry）；拖摇杆地面相对左移 −33.6 px 且旋钮回中；向下探索 6.4 s 遇 wolf-pack:2；轻点灰狼 ⇒「目标 灰狼 45/45（再点清除）」+ target-ring；点 strike ⇒ cast 发出、服务端回「rejected：range」进提示；离开回首屏。同批 kit orchestration 面 1 → 2（B2 前置，minSupported 不动、api-freeze 重钉）：服务端只读 `listCheckpointedVars(sId, mapId, packId)`（k_mmo_instance 按 map / pack 列分线 ≤ 64 → 各取最新检查点 rev / tick / 该 pack vars；只读两张 kit 表）——§9.4 #8 口径「样本需要改 kit 回 MK 阶段 bump 面版本」。B2——域 `mmodemo.bossBoard`（query，无参数无错误码：`{ mapId, packId, lines[{ instanceId, rev, tick, bossKills }], totalKills }`，validator ≤ 64 条 / 不重复 / 合计 = Σ）；服务端 `core/mmodemo/bossBoard.ts` 只经 kit 面 listCheckpointedVars（坏 var ⇒ 0 fail-soft）+ 端点 + 向量；客户端 `plugins/mmodemo/{logic/mmoDemoRuntime, logic/MmoDemoBoardLogic, view/MmoDemoBoardView}`（击杀降序、运行时长 tick × TICK_MS、失败保留旧数据）；plugin.json domains / route `bossBoard` / menu「头狼战报」（`requires.kits.mmo.orchestration` 1 → 2）；指纹 e03aae620dcd02d3 → 51b024b2e9b385e0（只加域，`LOBBY_PROTOCOL_VERSION` 不 bump）；安装 `--reinstall-from-tree mmodemo --allow-identity-change`（domains: - → mmodemo 有意变更）。用例：客户端 mmoWorld-hud-controls 6 / mmodemo-board-logic 3；服务端 mmo-orchestration 加 listCheckpointedVars、mmodemo-bossboard 3（假面注入 / 常量交叉核对 / validator 反向）；变异四处转红后复原（死区短路 / pack_id 过滤 / `value >= 0` / 排序）。验证：两端 typecheck 零错、服务端 1210、客户端 712、`plugin -- test mmodemo` 21、plugin check 7 包一致、codegen 三 `--check` fresh、verify:sync 绿、受保护路径未变。**B2 相对 tag 的 §9.4 动线验收**：#1 插件提交 39 条 = 手写 22 + 生成物 / 镜像 .meta 15 + 锁与指纹 2、placement 0；#2 在 tag + cherry-pick(B2) 的临时检出上 `plugin -- changed --base mmo-kit-v1-frozen --dry-run --root <wt>`：37 条全部落在 mmodemo 所有权推导集（13 条生成物 / 镜像由 codegen --check 与 verify:sync 把关）⇒ foreign = []、packages = [mmodemo]；#3 冻结路径 `':!*.generated.*'` diff 为空、`mmo.lock` 字节不变；#4 三个 codegen `--check` fresh；#5 mmodemo.lock 0.2.0、指纹重钉、GAME_ROOM / WORLD_ROOM 不变；#6 host.json 未动；#7 干净安装本批未重复（MG0-B3 已对同一插件走通 pack → install → bootstrap → check → test → uninstall；0.2.0 走 `--reinstall-from-tree --allow-identity-change` 与 check 一致）；#9 边界机检绿（未改编排）。偏差：战报读的是最新已落库检查点（⛔ 运行中实时值；分线检查点周期 30 s）；NPC 行商在 v1 不是世界实体（内容包 npcs 只喂 interact 目录），预览证据改点灰狼；预览里 strike 因射程被拒是真实回执路径，不是缺陷。**MG1 退出（竖屏操作模型 + 预览证据 + 可选域页面 ✅）**；下一阶段 MG2（`mmohold`）。
- **MG2 第二样本 `mmohold`**（2026-09-23；kit 前置 291fec60 / B1 c13f969b / B2 fcbb3854 / Creator 工具 68ec5efe；**B1–B3 交付并退出，tag `mg2-exit`**）：
  - **kit 反馈先回 MK，再冻结**：0.1.27 → 0.1.28，content 面 3 → 4（自有 HUD 贡献、纯操作几何），orchestration 面 2 → 3（客户端 `subscribeScriptState`、公开 harness snapshot/restore、100 人批量奖励）；minSupported 不变、api-freeze 重钉、从树重装，`mmo-kit-v1-frozen` 重打到 291fec60。首次进入 / baseline 请求补全量，断线清空；单人奖励键兼容、批量每命令 ≤ 100 人且每 dispatch 总目标 ≤ 200。实际预览另修 MySQL LIMIT 参数兼容、当前包 presentationId 解析、managed=orchestration 出生被 kit 初始化重复生成；旧错误检查点恢复不会回灌副本。kit 135 例、真实 mode 30 例、真实 MySQL 查询 2 例通过。
  - **B1 内容 / 编排**：`holdRidge` 1600×1200、A/B 两圆形据点、两职业 / 两阵营、sentinel 与三件奖励；每秒按存活阵营多数占领并得分，无人 / 平票保留归属且停分，易主换 2 名守卫。先到 100 分给当时全部在场胜方（含死亡者）1 件轮换物品 +25 金币，关闭 60 s 后重开；同 tick 双方达标先比分，再同分按轮次奇曙光 / 偶暮光。公开 harness 验证 100 人长 ID、恢复不重发奖 / 不延长休战、重放与预算；0.1.0 pack 15 文件首装，B1 包测试 10 例。
  - **B2 自有 HUD / 域入口**：0.2.0 新增 hud 贡献，显示比分 / 归属 / 轮次 / 休战并提供摇杆、技能轮盘、前往 A/B、拾取与离开；`mmohold.standings` 只读最近检查点，分页 3 分线，失败保留旧数据；设置「据点争夺」经公共 characters 面建角 / 入场。包测试 20 例，51 文件从树重装；指纹 51b024b2 → 8f243650，新增域不改 g8 / l7 / WORLD_ROOM 整数，host.json 零改。**Creator 16/16 步、11 截图、console 0**：比分 13 →14；本人实体出现后 3 ms 内观测到当前快照，占领 B 后及重进后均精确 4 名活守卫（A/B 各 2），战况读 rev21 / tick5406 的比分 13。证据 `docs/evidence/creator-2026-09-23/mmohold/` 本地保留。守卫是中立 idle 驻守标记，非阵营友军 AI；检查点战况允许 30 s 周期延迟。
  - **B3 §9.4 验收**：相对 tag 只重放 B1+B2 的检出，68 条 diff = 手写 26 / 生成物与镜像 40 / 锁与指纹 2 / placement 0；foreign=[]、packages=[mmohold]，26 手写全在 zip files.lock，51 制品文件逐字节一致；冻结路径 diff 空、mmo.lock 不变，两样本均复核；40 条生成路径均在 generatedWriterOwned，依次 writer / sync / 指纹重算后 tracked 差异为零，三个 codegen --check、verify:sync 与指纹检查通过；kit grep 除生成物外仅命中 tag 已有 README 样本说明（沿用 MG0 的文字例外，无样本条件分支）。两样本干净首装各自新数据库：kit_migration 均 11 →11 →11；mmodemo 37 文件 /21 测试、mmohold 51 文件 /20 测试；卸载全收回、check 通过。安装后两树各有一份新域的 shared Cocos 镜像需 Creator `.meta`；确认 TS 与主树逐字节一致后复用真实 Creator 已生成文件，记录来源与 SHA（不合成 UUID、不放宽矩阵）。全量测试临时目录改用外置盘，避开系统临时盘 ENOSPC；失败进程移盘后 46/46 通过。**两树最终 `verify:all` 均 exit 0**：mmohold 304 s（客户端 740 / 服务端 1230），mmodemo 397 s（客户端 732 / 服务端 1218）；各含 inventory 116、同步矩阵 21、UniFlex 60 全绿，随后卸载 / check 均成功、临时库已删除。完整报告与日志归档在 `docs/evidence/mg2-2026-09-23/`（本地不入库）。#9 边界 / 编排 22 例绿，65 命令整批暂停、同种子逐条重放一致；11 个 kit import 均为公共 api，常规宿主适配沿样板使用。
  - **验收基线修正（独立提交 35702977）**：inventory 夹具显式移除自己临时树中的历史证据，验证缺文件豁免及相邻坏前缀拒绝（116/116）；UniFlex 用 AST 检查实际 import，避免说明注释误报（60/60）。两项各做一次反向变异并复原；真实证据 / UI 功能不改。旧 node_modules converter 0.1.6 与锁 0.1.11 漂移导致的另 13 项失败，通过本地离线安装恢复锁定包，package.json / package-lock 字节不变。最终 mmohold 安装树服务端 1230 / 客户端 740 例与两端 typecheck、同步通过。未将其他任务的 sgzzmap / mapOriginal 改动计入本轨道。
- **PS 进程拆分独立轨道**（2026-09-22；PS3 fe8bd32e / PS5 93edb6e7 / PS2 0964310f / PS1 46f81560 / PS4 588a9a77；**PS1–PS5 交付并退出，tag `ps-exit`**）：
  - **入口与启停**：正式 `entries/{lobby,game,world}.ts`、三份房型 config 与公共 `bootstrapProcess`；大厅装配在线表 / kick / mailwake / 角色修复及用户推送，game 装配结算流监控，world 只消费 room 推送；角色隔离、启动半失败清理、停服排空与端口占用拒启均有实际故障验证。`dev` 继续合体；`dev:split` 的一个 watcher 监督三个真实子进程，独立端口 / NODE_ID、任一失败停止整组、父进程消失经 IPC 断连回收。MF4 已先退出，world 直接装完整 WorldRoom；监督器随 PS4 交付，未制造空房型过渡版。
  - **发现与入房**：游戏 `/version` 可选 `lobbyWs / gameWs / worldWs`，配置与 shared 共用 WS origin 校验；客户端登录先发现并按选择代次隔离迟到结果，Lobby 连接 / 重连走 lobby、对局走 game、世界默认走 world，缺字段兼容原目录 `gameWsUrl`。`room.resolve` 只读邀请租约并签票，容量 / start fence 留 game 预留与 admission；外部 WebPlatform 契约未改，协议整数仍 g8 / l7 / world1，指纹显式重钉。真实三进程集成经生产入口验证错房型拒绝、game 创建 / 加入、world 建角 / 入房 / 检查点复入及 Snake 再准入换装；默认 catalog 无 private profile，其正向私房准入由独立 PS3 夹具验证，不为测试添加产品入口。
  - **Snake 1.2.3**：每次准入重新水合 Redis，只合并并发读；装备字段独写，奖励 / 合成 / 复活按最新字段有界 CAS 合并，同 uid 写工厂串行且纳入停服排空。避免旧 game 档覆盖 lobby 换装或并发奖励；保持 demo 同步回执 / best-effort，未增加生产经济账本或持久幂等奖励承诺。`mmo` 仍 0.1.28，mmo.lock 与冻结 tag 291fec60 均未变。
  - **验收**：完整 `verify:all` exit 0（客户端 750、服务端 1268、inventory 116、同步矩阵 21、UniFlex 60）；最后 WS 非法 DNS 校验修正经先红后绿的定向 9 例、服务端 typecheck 及最终全测 **1272/1272**。全量真实集成 **243/243、零跳过**；最终故障矩阵 **203/203、25/25 点**；MMO 非侵入矩阵 **34/34**；生命周期 / 推送、端点、Snake CAS / 因果队列和监督器的实际变异均转红后复原。真实 Creator 合体预览 **12/12 步、7 截图**，四文件 sourceMap 与新镜像逐字节相等，网络实际 `/version` 200 后建立 Lobby / Game 两条连接，证明发现和缺字段回落；独立三进程分流由上述集成验证。首次预览被已有 Spine 3.8.99 / runtime 4.2 不匹配引出的 `skins` 空值错误浮层阻断；使用同期地图任务的独立 runner 快照记录并关闭浮层后行为链通过，**console 仍 4 条，不算渲染零错**，首次失败与工具 SHA 一并保留，未改引擎 / 素材。证据 `docs/evidence/ps-2026-09-22/` 本地 ignored。
  - **边界与偏差**：外部身份服务 smoke 未执行；当前安装树全闸通过不等于所有包卸载树全绿。独立卸载审计确认新增 PS 测试可选包边界、服务端纯源码及 codegen 通过；完整卸载树 typecheck 仍受 5 个旧 server fixture / world-bench 文件和 1 个旧 client Snake 测试硬依赖阻断，六文件本轮均未改，详情见本地审计报告。MMO 矩阵自身记录的 59 行既有 `.meta` 基线未冒充修复；全量闸另在已生成 AOT 镜像的当前字节快照通过。单 lobby / game / world 是本轮范围，多 game 私房任意节点直连与既有 world 容量优化未扩大。
下一动作：**框架段 MF0–MF11 全部退出（tag `mmo-framework-v1`）；`mmo` kit MK0 已退出（tag `mk0-exit`）；MK1 世界闭环 B1–B6 已于 2026-09-20 交付、同日用户拍板选项 ③ 退出（tag `mk1-exit`；§11.2 v1 例外「热点互见 ≤ 50 人」）；MK2 模拟闭环 B1–B3 已于 2026-09-20 退出（tag `mk2-exit`）；MK3 资产闭环已于 2026-09-22 退出（24 h 长跑 stable，tag `mk3-exit`，§12 MK3 行）**——热点 100 人的根因（互见 × 更新率 O(N²) 扇出 + 同进程机器人污染）与候选收紧（分层节拍 / 角色可见上限 / 位置量化 / 聊天限频）留在 §12 MK1 行偏差 ⑧，独立进程基准台与收紧留作 v1.x 的可选项（⛔ 阻塞 MK3）；MK4 编排与验收已于 2026-09-22 退出（B1–B6；tag `mmo-kit-v1-frozen` 含 mmo.lock，kit 0.1.22；场景 B 50 人整窗 tick p99 回归 37.2 / 25.1 / 32.6 ms 已拍板接受为 v1 已知回归、优化留 v1.x；§12 MK4 行）⇒ **`mmo` kit v1 冻结，MK0–MK4 全部退出**；MG0 内容插件样本 1 `mmodemo` 已于 2026-09-22 退出（B1–B3，§12 MG0 行；首个真实插件暴露的 kit 测试假设 / 客户端职业解析缺陷 / codegen 夹具缺口同批修，kit 0.1.26，`mmo-kit-v1-frozen` 重打到修复提交；§9.4 #1–#7、#9 全过）；MG1 已于 2026-09-22 退出（B1 竖屏操作模型 + Creator 预览证据 mmoWorld 场景，kit 0.1.27 含 orchestration 面 v2、tag 重打到 2790218e；B2 可选域页面 mmodemo.bossBoard，mmodemo 0.2.0；§12 MG1 行）；MG2（`mmohold`）已于 2026-09-23 退出（B1–B3；tag `mg2-exit`，kit 0.1.28 / mmohold 0.2.0；两样本 §9.4 全链通过，见 MG2 行）⇒ **MF0–MF11 / MK0–MK4 / MG0–MG2 全部退出**；PS1–PS5 已于 2026-09-22 退出（tag `ps-exit`，见本节 PS 行），可用 `npm run dev:split` 启动三进程；**MF5a 已退出 ⇒ slg 2b 与 lvr 视图房可开工**（已通知 slg.md §10.8，接法见 KIT.md §4 / SERVER.md §5）；门① 已过，线上部署方按 SERVER.md §8.2 SOP 执行迁移；MF7a 已退出 ⇒ slg / lvr 无人在线结算可开工（已通知 slg.md §0.1 / lvr.md §4.3），各自落地后在此回写一行；门③ 已过：`WORLD_ROOM_PROTOCOL_VERSION=1` 一次定型，首个客户端发版前仍可 revert。MF0 行登记的同步 / 夹具旧红项在本轮按真实 Creator `.meta` 导入前提与独立夹具修正收口（见 MG2 行）；历史已跟踪的 `docs/evidence` 文件与 ignore 政策差异仍保留原状，不在本轮清理，⛔ 不算 MMO 阶段偏差。
