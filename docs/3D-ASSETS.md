...
> - 日期：2026-09-24。状态：**规范 v1.6**（含 3D-PLAN 3D-41–3D-45 审阅修订与 SC4-B3 验收范围调整），与 [3d.md](3d.md) v1.6 对齐；SC0 已完成验证原型与 §15 预算冻结；正式框架能力仍按阶段交付。
...
9. **机检优先**：`verify:assets3d` 守格式 / `.meta` / UUID 依赖闭合与归属 / 体积 / 命名 / 导入选项 / 授权台账；Creator 目检与 `--perf` 不进自动聚合门禁，但所列证据是阶段人工退出条件。
...
| 基线设备 | Android：麒麟 970 / 骁龙 835；iOS：A10 | 规格参考，未认定手机下限（§15） | SC0 桌面基线为实际 Apple M4；不外推手机性能 |
...
SC4-B3 桌面交付（2026-09-24）：low 的浮点 / RGBA8 预烘焙为 50 单位 + 8 特效；instancing 不可用时保留烘焙、执行 25 单位上限；关节纹理无法采样或消费方提供的实际帧时超过明确预算时，显式允许后使用 25 单位非 instancing 实时蒙皮，LOD2 换消费方公告板，沿用原 hideAtLod / details 门。所有四条桌面路径验证 PNG 实际加载、近远切换与 20 次回收，摘要见 [B3 验收](perf/stage3d/2026-09-24-sc4-b3.json)；本批按桌面 WebGL1 退化与故障恢复范围验收完成，故障注入只代表所记录的能力路径。
...
真实运行覆盖单列 §15.2，包含已完成的 SC3 / SC4 容量与退化、SC5 离线链证据。
下表中的 SC0 初始负载不是最终资产统计。调整本表数值须记录依据并回写 3d.md §10。

| 项 | 冻结值 | 依据与后续验收 |
| --- | --- | --- |
| 包源资产总量 / 单 GLB / 单 PNG | 64 MB / 8 MB / 4 MB | 沿用 v1.5 的入库上限；SC0的7份源资产合85,679 B，SC1-B5/B7 已实施边界与越限反例 |
| 包 `3d/textures/` 总量 | 32 MB | 沿用作者态限制；lightmap 即使不在此目录，仍计入包总量；不等同 GPU 内存 |
| 面数：世界实体近 / 远、建筑、主角 | 3,000 / 300、10,000、20,000 三角 | 作者态上限；SC0网格最高96三角，SC5静态 / 蒙皮样例均为1200→297→118，不代表最大复杂度容量 |
| 同屏总面数 | medium 500,000 三角上限 | 调度预算；SC0全场实测峰值16,068，移动端容量未获证明 |
| 贴图尺寸档 | 256 / 512缺省 / 1024 / 2048登记 | 保留 POT、mip 与精确例外规则；SC0实际64²棋盘及1024² lightmap，SC5另验512²外置BC；其它规格不自动成为已实测 |
...
| 骨骼 / 权重 | 单位30骨、主角80骨、每顶点最多4个非零权重 | 作者态上限；SC4容量仍用两骨，SC5四骨四非零权重验证离线链，但primitive内权重一致；复杂角色和30/80骨容量未获证明 |
| 单特效 / 并发（low / medium / high） | 每个≤200粒子、≤2材质；并发8 / 24 / 48 | 初始调度上限；SC4已验100蒙皮+50特效及low退化，50特效为压测覆写，生产预算不变 |
| 同屏蒙皮单位（low / medium / high） | 50 / 100 / 100 | high 原 `100+` 收敛到已测数量100；扩容须新证据。当前100为两骨灰盒，不能外推30/80骨容量；low 退化证据见 SC4-B3 |
| 出档延迟释放 | 5 s | 沿用资源抖动控制策略；SC3-B2时钟/取消测试及SC3-B5资源实测验收 |
| 桌面基线 | Apple M4、ANGLE Metal、Chrome153、Creator3.8.8 WebPipeline；375×812 CSS / DPR2 | 当前可重放基线；麒麟970 / 骁龙835 / A10保留为规格参考，未认定为可支持的手机下限 |
...
不覆盖连续缩放、mip链或UV岛padding。RGBA8 注入仅证明该采样路径，不外推手机容量。

SC4退出补充（2026-09-24，tag `sc4-exit`）：正式100两骨蒙皮+50个capacity50 / rate30 / lifetime1特效，
桌面M4 / WebGL2、high / shadows=0、375×812 CSS / DPR2；显式引擎帧率上限120时，实际240帧窗口
平均59.97fps、p95 / max为18.3 / 19.0ms，另连续3,600帧约60秒平均60.00fps、p95 / max为18.9 / 21.7ms。
同屏每帧55次绘制提交，20次回收后业务引用归零、场景节点回10、GFX增量0。
沿用本节的实际吞吐口径，120是测试上限，原始间隔没有改写；默认60上限的49.83fps / p95 33.7ms
及空场景跳帧对照保留为已知引擎帧调度限制，生产配置未改，测试结束和失败时均恢复原值。
50特效是既有压测覆写，生产8 / 24 / 48及30 / 80骨等作者态预算不变；复杂骨架 / 权重和消费方容量仍须另测。
完整证据与默认配置边界见 [SC4汇总](perf/stage3d/2026-09-24-sc4-review.json)，WebGL1 low退化沿用
[B3摘要](perf/stage3d/2026-09-24-sc4-b3.json)。

SC5退出补充（2026-09-24，tag `stage3d-v1-frozen`）：静态 / 蒙皮两份原创样例均为
1200 → 297 → 118 面、两材质槽、512²独立BC PNG；蒙皮四骨、每点四个非零权重、两段各1秒动画。
真实Creator WebGL2 / WebGL1均完成0→1→2→0切档、材质与图片核对、动画挂点推进和20次回收，
节点回4、六份Prefab引用归零、GFX回预热基线。自动蒙皮LOD只接受primitive内一致的joints / weights，
复杂角色须审核手工变体；此证据不证明30/80骨、最大面数、手机或高负载容量。
转换 / 往返与变异证据见 [SC5-B1](perf/stage3d/2026-09-24-sc5-b1.json)，
全阶段复核与冻结源码索引见 [SC5汇总](perf/stage3d/2026-09-24-sc5-review.json)。
...
- 2026-09-22 v1.4：按 3D-PLAN 3D-41–3D-45 同步素材相关契约：蒙皮按 jointTexture / 布局分批与 RGBA8 回退、实时蒙皮禁 instancing；UUID / 子资产引用闭合与包归属、内置资源 allowlist 和干净安装反例；raw wall frame interval、预热后稳定内存基线。⛔ 本次仅文档修订，未实施能力、未冻结候选值、未勾阶段完成。
- 2026-09-22 v1.5：按当前任务范围收窄验收项，保留源资产与运行时预算、WebGL1 目标；⛔ 仅文档调整，未实施能力、未冻结候选值、未勾阶段完成。
...
- 2026-09-24 v1.6：按用户要求将 SC4-B3 的素材验收范围统一为桌面 WebGL1 low 退化、加载 / 激活失败清理与恢复；档位预算、20 次预热后回收和 SC4 阶段性能要求继续执行。
