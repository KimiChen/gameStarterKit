...
本目录规定 `Underground Idle` 的确定性通用模块生产目标。计划制作八个可编辑 SVG，并确定性导出透明 RGBA
PNG；该批次不计划依赖 ImageGen。本文不表示 SVG、PNG 或 manifest 已经生成或通过审计。
...
计划生成 `ug_art02_modules_contact_sheet_v01.svg/.png` 仅用于审稿：金色十字表示登记 pivot，右下棋盘格小窗
...
候选生成后，机器可读尺寸、可见 bbox、pivot、连接点和文件名必须登记到 `manifest.json`。自动检查与人工缩小
审阅均完成前，本包状态保持 `To audit`。
