...
> - 日期：2026-09-19；同日 **v1.2** 按三文档交叉审阅修订，**v1.3** 采纳第四轮审阅 3D-33–3D-40（§1.4）；**v1.4（2026-09-22）** 采纳第五轮审阅（§1.5）；**v1.5（2026-09-22）** 按当前范围精简平台验收条目；**v1.6（2026-09-24）** 调整 SC4-B3 验收范围，现行第五轮审阅保留 3D-41–3D-45。依据 [3d.md](3d.md) **v1.6** 与 [3D-ASSETS.md](3D-ASSETS.md) **v1.6**；[lvr-3d.md](../lvr-3d.md) 为消费方需求 v1.6。初稿基线 `f6fad19f`；MF9 已退出（MMO-PLAN §9，MF9-B2 = `745f5ca6`），保护面修改的前置已满足。v1.6 为当前设计基线；SC0 已于2026-09-22完成预算冻结与原型移交，SC1 / SC2 已于2026-09-23退出，SC3 已于2026-09-24退出，批次进度只在 §8 登记；政策上限与实测覆盖分列在3D-ASSETS §15。
...
### 1.5 第五轮审阅（2026-09-22，对当前 v1.3 计划与仓内实现）——3D-41–3D-45

以下为 **v1.4 文档修订** 采纳、经 **v1.6** 范围精简后保留的五项；审阅结论不等于实施完成，当前批次状态见 §8。§1.1–§1.4 保留历史原文，与本节冲突的旧处置以本节及修订后的施工批次为准。
...
| SC4-B3 WebGL1 low 退化与故障恢复（**门**，SD10） | Chrome `--disable-webgl2` 跑 `stage3d --perf`；low 退化实现并有用例：先验证预烘焙浮点 / RGBA8 关节纹理的实际采样能力，实际不可用或超预算才切关闭 instancing 的实时蒙皮上限 + 公告板远档；instancing 不可用 → 降低单位 / 绘制预算；ASTC 不可用 → PNG 回落。加载 / 激活失败时不遗留半就绪舞台 / 引用，恢复后重试须成功。复跑步骤放 `tools/creator-preview/README.md`，摘要回写 3d.md §8 / §10 与 3D-ASSETS.md §11；lvr A3 消费同一退化与恢复口径 | WebGL1 四条 low 路径的动画 / 提交 / 近远切换及 20 次回收证据齐全；退化 / 故障恢复用例绿。变异：删实时材质退化或失败清理 → 对应用例红 | Creator + Chrome WebGL1；`npm run test:client` |
| SC4-B4 文档 + 退出 | 3d.md §10 登记 SC4；lvr-3d.md §8 通知「A3 可开工」，须附 SC4-B3 WebGL1 low 退化证据并满足阶段性能要求；mmo 如后续选择 3D，在消费方新增增量接入记录，⛔ 回头改写已退出的 MK 阶段或既有 MMO 批次 | — | 文档提交；tag `sc4-exit` |
...
- **显存与机型**：桌面数字不等于手机；`--perf` 标注设备、画质、WebGL 版本并保留原始帧间隔 ms；每阶段 WebGL1 证据仍是退出条件。SC4-B3 验证 low 退化与故障恢复；lvr A3 同口径。
...
- [x] SC4-B1（04bb5e60） [x] SC4-B2（8330dbf8） [x] SC4-B3（5855bc3f；按本日调整后范围验收） [ ] SC4-B4
- [ ] SC5-B1 [ ] SC5-B2
- 消费方：[ ] lvr A0（随 SC0-B3） [ ] lvr A1 [ ] lvr A2 [ ] lvr A3 [ ] lvr A4 [ ] lvr A5 ｜ [ ] mmo（按 SD9） ｜ [x] slg 纯数学（SC2） [x] slg 资源 / 全局设置（SC3）

- 2026-09-24 SC4-B3 完成（实现 `5855bc3f`；按本日用户要求调整验收范围）：low 能力 / 实测预算策略、25 单位非 instancing 实时路径与远档公告板、PNG 回落、整组加载 / 激活失败清理和舞台失败关闭已交付。RGBA8 可用时仍烘焙；缺 instancing 仅降为 25 个烘焙单位。修复 Creator 3.8.8 实时材质隐式实例未销毁导致的每轮 16,800 字节 buffer 泄漏，捕获只属于当前 renderer / 本池的实例，在节点及队列退休后先释放实例再归还共享资产。最终实际 WebGL1 low 浮点 / RGBA8 / 缺 instancing / 无顶点纹理采样四条路径分别为 50 / 50 / 25 / 25 单位 + 8 特效，稳态原始间隔 p95 **32.6 / 17.5 / 17.7 / 32.6 ms**，平均 **56.69 / 59.26 / 57.60 / 56.25 fps**，draw calls 为 13 / 13 / 36 / 36；各 20 次近远交替开关后场景节点回 10、业务引用归零、GFX 增量 0。100 单位 WebGL2 的跨图集 / 实时切换 / socket / 回收回归、38 项定向回归、4 项隔离变异与真实 cc.d.ts 编译通过；最终 `verify:all` **3033 项全过**（客户端 1345 / UniFlex 80 / 服务端 1392，含其他门禁及两套客户端类型检查），完整机检与失败候选留在 [B3 验收摘要](perf/stage3d/2026-09-24-sc4-b3.json)。四条能力缺失均明确标为桌面启动前故障注入；较慢采样不删、不提高冻结预算，也不视为持续 60fps。[WebGL1 复跑说明](../tools/creator-preview/README.md#sc4-b3-low-退化) 已交付。按调整后的范围勾选 B3；SC4 的持续 60fps 要求与 B4 收尾待完成，lvr A3 仍等 SC4 退出。

- 2026-09-24 SC4-B2 完成：正式 `Vfx` 复用 EntityPool 的 Prefab 池与分帧预算，叠加同键 capacity / 全池 maxEffects、世界坐标 at / follow、从实际激活起算的单调毫秒寿命、目标失效与失败回收。LOD2 及内容隐藏门结束旧播放，返回近档不重播；降档释放超额，舞台租约取消清空加载 / 节点 / 帧订阅。Cocos 先 stop / clear 再停用节点，复用时重新播放，默认粒子材质在节点退休后的 AFTER_DRAW 回收，源资源不被修改。独立开发场景新增「50 特效（压测）」开关与 `stage3d --perf --vfx`，生产 8 / 24 / 48 上限不变，50 为显式 high 压测覆写；原生粒子序列化尺寸使用 `startSize`，探针断言实际作者参数与四份编译 source map，拒绝旧缓存。WebGL2 / 实际 WebGL1 均验证 **100 两骨蒙皮 + 50 个粒子系统**、实际提交、跟随 / 复用 / 定时 / LOD / 48 上限及 20 次联合回收；关闭后场景节点回到10、活动句柄 / 粒子 Prefab 引用为0、GFX增量0。60帧预热后240帧原始间隔 p95 为 **19.5 / 30.6 ms**，平均 **57.12 / 56.67 fps**，各55 draw calls；首载最大间隔41.9 / 50.5 ms。15项新增客户端回归、2项探针拒绝测试、删 LOD 门的隔离变异、真实 cc.d.ts 编译、独立 typecheck / 1335项 test:client 与 `verify:all` 3020项（客户端1335 / UniFlex80 / 服务端1389，含其他门禁）均通过。证据见 [B2 验收摘要](perf/stage3d/2026-09-24-sc4-b2.json)。旧缓存 / 视口 / 作者参数候选报告原样保留，未替换成成功证据；最终窗口不证明持续60fps，也不外推复杂骨架、手机、low档。SC4仍未退出，下一批B3，阶段性能门保留。

- 2026-09-24 SC4-B1 完成：正式 `SkinnedUnits` 以 EntityPool 为底，共用逐帧预算 / LOD / 取消 / 租约回收；全池 maxUnits 与显式实时子上限生效。Cocos 适配器以真实 mesh / 源材质 / jointTexture / 完整实例布局隔离全部材质 pass，跨图集 `play` 同帧重分组；实时路径先换非 instancing 材质，并按 clip 首次重建求值器。`socket` 跨 clip / 模式保持节点，支持 onEnable 重入关闭；图集登记按 hash 幂等、拒绝冲突及非对齐行宽。Creator 3.8.8 的 WebGL2 / 实际 WebGL1 / 显式浮点缺失 RGBA8 三路径均验 100 两骨单位、两主 clip 同播加跨图集样本、实际队列与上传实例、实时矩阵与 socket 推进和 20 次回收。稳态各 240 帧原始毫秒 p95 为 **34.1 / 17.6 / 17.8 ms**，各 5 draw calls / 100 instances / 9,794 triangles；首次激活最大间隔 44.3 / 42.6 / 43.6 ms，节点与 GFX 回预热基线。WebGL2 保留较慢窗口，不据此宣布 60fps 或 SC4 阶段退出。新增 15 项客户端回归与 3 项探针拒绝测试；删 jointTexture、删布局、实时仍启 instancing 的三次隔离变异均打红；同步后最终 `verify:all` 3003 项全过（客户端 1320 / UniFlex 契约 80 / 服务端 1387，含两套客户端类型检查；同步前隔离验收 2993 项亦全过）；真实引擎声明结果、15 份浏览器模块 / source map / 真源一致性及截图复核见 [B1 验收摘要](perf/stage3d/2026-09-24-sc4-b1.json)。初次视口 / 边缘裁剪 / 队列重复计数失败报告保留。SC4-B2 特效与 B3 low 退化门仍待交付，下一批 SC4-B2。

- 2026-09-24 SC3-B6 完成：CLIENT §8.2 补齐 `stage3d --perf` 的前置、双 WebGL / 画质复跑、原始毫秒帧间隔、首次激活与稳态窗口、20 次回收及不进自动聚合门禁的边界；slg.md §10.8 / lvr-3d.md §8 已通知正式资源 / 调度接口可消费，lvr 内容接入未勾选。核对 B1–B5 共 116 项哈希引用、33 份当前源码 / 测试 / 桩 / 工具及 SLG 既有测试体；8 项旧哈希的后续变更均有提交与对应验收承接。重新计算 B5 六份报告的 2,532 条原始帧记录与 120 次开关回收，全部与摘要一致；本批不改运行时、不重采浏览器、不新增变异，沿用 B1–B4 已验证的 8 项变异。本批隔离验收 `verify:all` 2975 项全过，含两套客户端类型检查。完整机检、源码沿革及桌面容量边界见 [SC3 汇总](perf/stage3d/2026-09-24-sc3-review.json)，阶段退出见 [3d.md §10](3d.md#10-实施状态回写)，tag `sc3-exit`。下一批 SC4-B1；SC4 蒙皮 / 特效与 SC5 工具 / 冻结仍待交付。

- 2026-09-24 SC3-B5 完成：`run.mjs stage3d --perf` 使用独立 `stage3d-dev.scene` 与真实 Creator 3.8.8，按 `performance.now()` 相邻差值在 `AFTER_DRAW` 保留首次加载 / 激活 120 帧、稳态 60 帧预热 + 240 帧原始间隔；同帧记录独立序号、实体数、draw call / triangles / instances、GFX buffer / texture 字节，引擎 dt 仅辅助。采样器安装经显式就绪确认，前后台污染或不完整窗口保留原始失败报告并拒绝出 PASS 摘要；1.2 s 长帧、`frameCount` 重置与秒 / 毫秒混用反例由工具测试覆盖。原生 `WebpageFullScreen` 下固定 375×812 CSS / 750×1624 backing，WebGL2 / 实际 WebGL1 各跑 low / medium / high：low 500 次请求中 100 名额隐藏、激活 0，medium / high 激活 300 / 500；两种上下文的 draw call 均为 3 / 4 / 4，triangles 均为 194 / 3794 / 6194，实例数均为 0 / 300 / 500。稳态 p95 毫秒 WebGL2 为 33.5 / 33.9 / 34.0，WebGL1 为 33.6 / 17.9 / 34.6；六份报告 console 均为 0，首次关闭预热后各 20 次开关的场景节点、业务实体及 GFX 内存均回基线。64 项定向测试与最终 `verify:all` 全过（客户端 1295 / UniFlex 契约 80 / 服务端 1384 项，含两套客户端类型检查）。原始 `report.json` 留本地 `.cache/stage3d/sc3-b5/`，六份数字摘要、哈希与限制见 [B5 验收摘要](perf/stage3d/2026-09-24-sc3-b5.json)。这些是桌面灰盒观察值，不承诺 60 fps 或移动端容量；SC3 未退出，下一批 SC3-B6。
...
- 2026-09-22 SC0-B3：真实 Cocos 页面与 Snake 载体的输入原型、相机叠加、蒙皮分组及回收验收通过，当前批次 `verify:all` 通过。WebGL2 / WebGL1 浮点 / WebGL1 显式 RGBA8 能力故障注入各完成 18 步；500 立方体实际提交、100 蒙皮中两动画同播及跨关节纹理切换正确，实时样本关闭 instancing 并可恢复。三路径各预热 60 帧、采样 240 帧，原始帧间隔 p95 为 18.7 / 18.9 / 18.8 ms，启动长帧最大值 120.5 / 148.7 / 126.6 ms 保留；各连续开关 20 次，关闭节点与业务引用回基线，GFX 增量为 0。Snake 两上下文各 10 步、32 条 trusted DOM 输入通过；wheel 由夹具覆盖。汇总见 [B3 审阅报告](perf/stage3d/2026-09-22-spike-review.json)，原始报告及截图留本地。该结果限于桌面原型，不是正式 SC1 框架交付；B1 已按修订后的要求完成，B4 预算与原型移交尚未冻结，SC0 未退出。
...
- 2026-09-22 文档 v1.4：采纳 §1.5 的 3D-41–3D-45，修订 raw-input 前置归属 / 取消、蒙皮 jointTexture 分组、SC1 真实同步 retainer、序列化 UUID 闭合 / 干净安装、原始帧间隔 ms；mmo 接入改为冻结 2D 基线之后的可选增量并对齐 mount / unmount 与 services 入口。仅文档修订，素材数字仍候选，未实施任何能力、未新增批次勾选。
- 2026-09-22 文档 v1.5：按当前范围精简平台验收条目；现行第五轮审阅保留 3D-41–3D-45 五项，编号不重排。仅文档修订，素材数字仍候选，未实施任何能力、未新增批次勾选。

- 2026-09-24 文档 v1.6：按用户要求收敛 SC4-B3 为桌面 WebGL1 low 退化与故障恢复；删除对应设备验收条目与操作模板，现行第五轮审阅保留 3D-41–3D-45 五项。依据 `5855bc3f` 已有证据完成 B3；SC4 阶段性能要求与 B4 收尾继续执行。
