...
> - 日期：2026-09-19；同日 **v1.2** 按三文档交叉审阅修订，**v1.3** 采纳第四轮审阅 3D-33–3D-40（§1.4）；**v1.4（2026-09-22）** 采纳第五轮审阅 3D-41–3D-47（§1.5）。依据 [3d.md](3d.md) **v1.4** 与 [3D-ASSETS.md](3D-ASSETS.md) **v1.4**；[lvr-3d.md](../lvr-3d.md) 为消费方需求 v1.4。初稿基线 `f6fad19f`；MF9 已退出（MMO-PLAN §9，MF9-B2 = `745f5ca6`），保护面修改的前置已满足。⛔ 本次只修订文档，未实施 3D 能力、未勾选新批次，素材预算数字仍为候选、渠道包体硬上限待 SC0 核验。
> - 定位：**施工单 + 审阅记录**——§1 保留历次审阅（3D-13–3D-47、L01–L09），§3–§4 把 3d.md §6 的每个阶段拆成可独立提交、可独立验收的批次（`SCx-Bn`），写清文件落点、机检退出条件与命令。⛔ 本文不是设计真源：机制以 3d.md 为准、素材以 3D-ASSETS.md 为准；施工细化须同步回写对应真源，⛔ 不在本文另立口径。
...
消费方 lvr A1 ← SC1–SC3；lvr A3 ← SC4；lvr A0 并入 SC0（用 lvr 样本资产做第二份证据）；**lvr = SD10 首发小游戏 / WebGL1 消费方**（lvr-3d.md R0：A1–A5 每阶段附 WebGL1 证据、A3 加真实微信设备缓存证据、开发者工具只作兼容证据、等 SC4-B3 门）
...
### 1.5 第五轮审阅（2026-09-22，对当前 v1.3 计划与仓内实现）——3D-41–3D-47

以下已随本次 **v1.4 文档修订** 采纳；§8 不勾选任何实施批次。§1.1–§1.4 保留历史原文，与本节冲突的旧处置以本节及修订后的施工批次为准。
...
| 3D-46 | Medium | 只记源资产预算及构建体积，未给小游戏主包 / 单分包 / 全部分包的硬上限和统计口径，不能判断包体门 PASS / FAIL | SC0-B1/B4 核验当前渠道规则，登记来源 / 核验日期 / 构建统计口径并冻结三类适用限制（含无独立上限的官方依据）；按实际构建产物逐项比对，任一超限退出失败；⛔ 本轮填未经核验的数值 |
| 3D-47 | Medium | 开发者工具的文件系统 / 缓存行为不能替代真实微信环境，远程 bundle 首次下载与缓存回收未被验收覆盖 | SC4-B3 在真实微信设备做冷缓存 / 满缓存 / LRU 淘汰 / 重启再进入；记录平台 / 微信版本 / 存储状态 / 请求与缓存命中 / 失败恢复，开发者工具只保留兼容证据；lvr A3 同步 |
...
| SC0-B1 引擎面清单 | 打开 `apps/Cocos`：把 `engine.json` 改为显式模块清单（SD11：`custom-pipeline` + `custom-pipeline-builtin-scripts` + `custom-pipeline-post-process` ON、`legacy-pipeline` OFF；关 terrain / tiled-map / dragon-bones / xr / physics / particle-2d 等），确认当前管线（CDP 读 `cc.director.root.pipeline` 构造名）、`project.json` 声明两个保留层位、补 `wechatgame.json` 并做一次微信小游戏构建（SD10）；Spine 切 4.2 并确认无既有用法；`apps/Cocos/README.md` 记录引擎 / 相机与可重复构建步骤。包体证据按实际构建产物分别统计主包、每个分包及全部分包，列明是否包含主包、远程 bundle 排除规则、压缩前后口径与产物路径；核验当前渠道官方规则并登记来源、核验日期和适用版本，移交 B4 冻结（3D-46，唯一限值表 = 3D-ASSETS.md §15.2）；同时盘点既有 `resources`，在框架构建配置中记录保留 / 过滤 / 远程策略，实际产物必须覆盖这些旧资源 | README 段落存在；小游戏构建产物、逐包明细与渠道来源可复核，⛔ 用源 `glb/png` 体积替代构建统计；`verify:inventory` 绿（AGENTS / CLAUDE 不变） | 人工 Creator；文档提交 |
...
| SC0-B4 冻结与回填（← B1/B2/B3/B5） | 3d.md §9.2 与 3D-ASSETS.md §15.1 素材数字由候选经实测后冻结；B1 核验的当前渠道主包 / 单分包 / 全部分包或总发布包硬上限、来源 / 核验日期 / 适用版本、计量单位及统计口径只登记在 3D-ASSETS.md §15.2（无独立单分包上限时填官方依据），3d.md §9.2 引用该表，证据报告保留本次限额快照，⛔ 本轮文档修订预填未知上限。按 B1 实际构建产物逐包比较（含既有 resources 的过滤 / 远程策略验证），缺项 / 任一超限 = FAIL；SC0 探针报告保留限额与实测值，并用人为调低限额的反例证明超限失败。回填 WebGL1 退化、内置管线在小游戏构建的可用性、raw-input 接缝、蒙皮分组与 B5 烘焙重载结果；不可用则修订设计并记录新拍板项，不带失败进入 SC1；3d.md §10 登记 SC0 | 待冻结表已由证据填实（素材候选与渠道硬限分别有依据）；三个包体检查均 PASS、降低限额反例 FAIL；全部接缝及烘焙重载 PASS；记录原型移交 / 移除清单 | 文档提交 |
...
| SC4-B3 WebGL1 / 真实微信设备退化与缓存（**门**，SD10） | Chrome `--disable-webgl2` 跑 `stage3d --perf`；low 退化实现并有用例：先验证预烘焙浮点 / RGBA8 关节纹理的实际采样能力，实际不可用或超预算才切关闭 instancing 的实时蒙皮上限 + 公告板远档；instancing 不可用 → 降低单位 / 绘制预算；ASTC 不可用 → PNG 回落。微信开发者工具保留构建 / 渲染兼容证据；**退出另须真实微信设备**运行同一构建，覆盖冷缓存首下载，随后在真实客户端实际触发缓存写入失败，再观察 LRU 淘汰 / 清理、重试、重新访问与退出重启再进入，记录设备 / 系统 / 微信版本 / 构建版本 / 基础库版本 / 资源版本 / 平台实际存储容量 / 限制 / 存储状态、可复现填满或触发失败步骤与真实错误、请求及缓存命中、可见资产与失败恢复；不假设引擎存在可配置容量开关，接近满但未触发失败不能通过；失败时不遗留半就绪舞台 / 引用，恢复后重试须成功。使用最终测试构建按 3D-ASSETS §15.2 的 SC0-B4 冻结口径复验实际主包 / 单分包 / 全部分包或总发布包，任一超限 FAIL。步骤与证据模板放 `tools/creator-preview/README.md`，摘要回写 3d.md §8 / §10 与 3D-ASSETS.md §11；lvr A3 等本批并提供内容侧同口径证据 | WebGL1 报告 + 真实微信冷缓存 / 写入失败 / LRU / 重试 / 重启证据及判定均齐全，开发者工具 PASS 不能替代；逐包构建体积通过硬上限；退化 / 故障恢复用例绿。变异：删实时材质退化或缓存失败清理 → 对应用例红 | Creator + Chrome 标志；微信开发者工具（兼容）；真实微信设备（退出） |
...
| --- | --- | --- | --- |
| R0 平台目标（SD10） | — | lvr 为首发小游戏 / WebGL1 消费方：low 档必达，A1–A5 每阶段附 WebGL1 证据，A3 加真实微信设备冷缓存 / 实际写入失败 / LRU / 重试 / 重启证据（同 SC4-B3），开发者工具仅作兼容证据；小游戏构建按 SC0-B4 核验冻结的主包 / 单分包 / 全部分包硬上限与统计口径验收 | R0 登记（lvr-3d.md v1.4）；渠道 SDK / 发版打包 / 审核仍按 lvr.md §9.3 不做 |
...
- **显存与机型**：桌面数字不等于手机；`--perf` 标注设备、画质、WebGL 版本并保留原始帧间隔 ms；每阶段 WebGL1 证据仍是退出条件。SC4-B3 另需真实微信设备的冷缓存 / 实际写入失败 / LRU / 重试 / 重启证据，开发者工具只判兼容；lvr A3 同口径。
...
- 2026-09-22 文档 v1.4：采纳 §1.5 的 3D-41–3D-47，修订 raw-input 前置归属 / 取消、蒙皮 jointTexture 分组、SC1 真实同步 retainer、序列化 UUID 闭合 / 干净安装、原始帧间隔 ms、小游戏构建硬上限核验、真实微信缓存退出门；mmo 接入改为冻结 2D 基线之后的可选增量并对齐 mount / unmount 与 services 入口。仅文档修订，素材数字仍候选、渠道硬限待 SC0 核验，未实施任何能力、未新增批次勾选。
