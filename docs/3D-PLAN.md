...
> - 日期：2026-09-19；同日 **v1.2** 按三文档交叉审阅修订，**v1.3** 采纳第四轮审阅 3D-33–3D-40（§1.4）。依据 [3d.md](3d.md) **v1.3** 与 [3D-ASSETS.md](3D-ASSETS.md) **v1.3**；[lvr-3d.md](../lvr-3d.md) 为消费方需求 v1.2。初稿基线 `f6fad19f`；MF9 已退出（MMO-PLAN §9，MF9-B2 = `745f5ca6`），保护面修改的前置已满足。⛔ 本次只修订文档，未实施 3D 能力、未勾选新批次。
...
| 3D-35 | Medium | SC0-B3 原型体量 ≈ SC1-B9，「门」变成半个实施 | SC0-B3 加规模上限（只改 `syncInput` 一处 + 一个 overlay 标记 + presentation host 一处闸）与时间盒（候选 3 人日），超出即停并记 3d.md §8 |
| 3D-36 | Low | SC1-B2 提前实现 AssetLease 同步 retainer，只为资源型全局 patch | SC1 全局 patch 只含枚举 / 数值字段；SC1-B2 只定义 retainer 接口 + FakeRetainer；资源型字段与真实持有归 SC3-B1 |
...
| SC1-B2 `Stage3D` 核心（无头） | `view/scene3d/stage3dLayers.ts` 与 `Stage3D.ts`：引擎适配器、单舞台租约、相机 / 灯 / 视口 / 射线接口；舞台 `setGlobals` 与 `acquireGlobals(owner, patch)` 共用 token 覆盖表（3d.md §3.2），首 token 记录场景基线；更新替换自己的补丁且不改变优先级，删除任意 token 后按基线 + 仍有效补丁逐字段重算，⛔ release 写回自己的旧快照；owner 关闭自动删除 token；SC1 的 `Stage3DGlobalsPatch` 只含枚举 / 数值字段（3D-36）；`AssetLease.ts` 本批只定义内部 loaded-asset retainer **接口**并以 FakeRetainer 测覆盖表持有顺序，⛔ 真实持有与加载 I/O（归 SC3-B1）；owner 用 `Stage3DOwner { signal; isActive() }` 最小接口（3D-34）；`logic/scene3d/viewport.ts` 做设计矩形换算 | FakeNode：单舞台忙拒、生命周期自动释放、幂等；舞台与独立全局 token 两种乱序释放 / 同字段覆盖 / 不同字段合并 / 更新不抢优先级 / 最后释放回基线 / 失败回滚；资源不提前释放；20 次开关回基线。变异：任一 release 改回旧快照 → 交错关闭测试红 | `npm run test:client` |
...
| SC1-B9 非模态 HUD 输入（← SC0-B3） | 将 spike 结论正式落入 `view/defineView.ts` 的 `ViewMeta.inputMode?: "modal" \| "overlay" \| "passive"`（3D-39：缺省由 `interactive` 别名推导 true ⇒ modal / false ⇒ passive，矛盾组合拒绝；仅 FGUI 可 overlay）、sidecar 校验 / `apps/server/tools/plugin-codegen/` 登记输出、`ViewMgr.ts` / `FguiView.ts` / `CocosView.ts` 与框架输入适配器；最高 modal 决定阻断边界，overlay 不暂停下层世界；HUD 只命中控件、空白穿透，pointer 以起点锁定归属直到结束 / 取消，wheel 按当前命中；关闭 / 重挂 / 模态抢占清理在途 pointer；⛔ 改 vendor；**gameplay 载体**（3D-33）：`GameplayPresentationHost` / `GameplayInstanceHost.dispatchInput` 旁加同一套 pointer 归属闸，HUD 命中的 pointer ⛔ 转发给世界 | 既有默认仲裁测试零改动绿；snake 局 + overlay HUD：HUD 点击不产生世界输入；新混合输入序列：HUD 点击无世界事件、空白拖拽无 HUD 事件、跨边界不截断、wheel 命中分流、打开弹窗取消并阻断世界、关闭恢复但不补 click 或续接旧拖拽、root 重建清理；非法 sidecar 拒绝；真实 Creator WebGL2/WebGL1 证据；更新受保护路径锁 | `codegen:plugins`；`npm run sync:client`；`npm run test:client`；`npm run verify:protected-paths` |
...
| --- | --- | --- |
| 首版（MK0–MK1） | 2D 公告板：`UIMeshRenderer` / Sprite + `cameraRig` 跟随 + `lodBands`（等 SC2）+ FGUI HUD 输入归属（等 SC1-B9 的 gameplay 载体闸；退路 = HUD 画在世界节点内则不等 B9，3D-33）；世界视图经 `WorldPresentation` 适配器接内容，`EntityPool` 键 = presentationId，`cameraRig` 投影无关；`IPresentationMap` 预留可选 `model` 字段（`content` 面 v1 增量） | MK0-B4 骨架 ⛔ 不等本轨道；MK1 接 FGUI HUD 前需 SC1-B9 或退路（MMO-PLAN MK0-B4 行已加注记）；MK1-B1 客户端预测 / 和解与本轨道无关 |
