...
```text
波 0   SC0 可行性 spike（门）：引擎面清单 ‖ 灰盒 glb 生成器 ‖ CDP 探针实测 → 拍板（3d.md §9.2 冻结 + SD9）
...
消费方 lvr A1 ← SC1–SC3；lvr A3 ← SC4；lvr A0 并入 SC0（用 lvr 样本资产做第二份证据）
       mmo：MK0-B4 最小 HUD 不等 3D；世界视图是否 3D = SD9（SC0-B4 拍板）；若是 → MK1 起 ← SC3 / SC4
...
| 3D-21 | Low | §6.3 | 「mmo MK0 客户端世界引擎 ← SC3」把 mmo 世界视图默认成 3D；MMO.md §7.6 / MMO-PLAN MK0-B4 只写「bitECS 实体池 + 插值 + 相机 + 最小 HUD」，未拍板 2D 还是 3D | `docs/MMO.md:755`；`docs/MMO-PLAN.md:232` | 立 **SD9**：mmo 世界视图形态（2D `UIMeshRenderer` 公告板 vs 3D Stage3D），SC0-B4 与 MK0-B4 之前拍板；本文 §4 按两种结果各写接法；回写 3d.md §9 |
...
| SC0-B4 拍板 | 3d.md §9.2 冻结表逐行由候选变冻结值（clear 方式、保留层位数、体积预算、60fps 判据设备、`AssetLease` deadline、`--perf` 采样窗）；**SD9**：mmo 世界视图 2D / 3D；SD10：WebGL1 / 微信是否首版目标（决定 SC4-B3 是门还是证据）；3d.md §10 登记 SC0 | §9.2 无「候选」字样；SD9 / SD10 写进 3d.md §9.1；⛔ 任一接缝判据 FAIL ⇒ 先重估 3d.md 再开 SC1 | 文档提交 |
...
退出：B1–B6；tag `sc1-exit`。回滚：可回退（新增文件 + `AppPorts` 一个字段）。
...
| SC4-B3 WebGL1 / 微信退化实证 | Chrome `--disable-webgl2` 跑 `stage3d --perf`：预烘焙蒙皮（浮点关节贴图）与 instancing 在 WebGL1 的可用性与退化（引擎回退 vs 我们退化到非烘焙 / 公告板）；结论写进 3d.md §8；按 SD10 决定是门还是证据 | 报告存在（`docs/perf/stage3d/<date>-webgl1.json`）；若 SD10 = 首版目标则退化路径有用例 | Creator + Chrome 标志 |
...
### 4.2 `mmo` kit（按 SD9 两种结果）

| SD9 结果 | 接法 | 与 MMO-PLAN 的交点 |
| --- | --- | --- |
| 2D（缺省，未拍板前） | 世界视图用 `UIMeshRenderer` 公告板 + `cameraRig` 跟随 + `lodBands`（SC2 即可） | MK0-B4 不等本轨道；MK1-B1 客户端预测 / 和解与本轨道无关 |
| 3D | `MmoWorldView` 在 `onOpen` 取 `ports.stage3d` 租约；实体 = `EntityPool` + `SkinnedUnits`（`IPresentationMap.prefab` 即池键）；特效 = `Vfx`；相机 = `cameraRig.follow(self)` | MK1（← SC3 / SC4）；MMO-PLAN MK1 行加注记（SC4-B4） |
...
| P9 | lvr-3d.md L01–L09 对齐为消费方需求 v1.1（SC0-B0，待用户确认） | lvr-3d.md |
...
- **显存与机型**：桌面数字不等于手机；`--perf` 报告标注设备；SD10 决定 WebGL1 是门还是证据。
