...
| 选中高亮原版化 | ✅ 已完成：原版机制查明（`choose_00_group.prefab` = 240×112 罩格地块面 choose2.png + pivot 中心 + 循环动画 + ring_white 子件 —— 面积罩而非描边线，所以原版无视觉错位）；落地 = `build_choose.py` 从 `grid-1.ktx` 切 240×112 帧入 `choose.png`（台账 A 节 47 条），选中框改为「罩格地块面（pivot 中心对格心、51.2×23.9 世界像素 = 0.8 格）+ 保留细线短条 + 1.4s 呼吸动画」；真机重放 15 步全绿、选中实拍见图证 `docs/evidence/creator-2026-09-23/maporiginal-choose/`（不入库） | （本轮） | 2026-09-23 |
