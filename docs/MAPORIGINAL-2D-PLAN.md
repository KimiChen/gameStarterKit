...
>
> 本文 §2–4 保留建单时的问题与施工设计，不能当作当前缺陷清单。M/N 批次沿革只见 §5；
> [MAPORIGINAL-2D-NEXT.md](MAPORIGINAL-2D-NEXT.md) 是 2026-09-23 的历史交接单。
> 当前机制对照见 [真源 §9](MAPORIGINAL-2D.md#9-本-kit-与原版对照表)，
> 素材和加载优化状态只见 [优化方案 §9](MAPORIGINAL-2D-OPTIMIZATION.md#9-实施状态唯一登记处)。
...
此处保留依赖关系；完成与后修记录只在 §5。

| 批 | 内容 | 依赖与约束 |
|---|---|---|
| M2-B1 | 地表底：独立 POT 底纹 + 整数次 REPEAT | 不能将可重复底纹并入普通图集；按 §1.4 生成草地块 UV |
| M2-B2 | 雪／沙 block 覆盖层及 top | 原版多边形与路径表；精确世界 UV 见真源 §1.8，不能套草地 UV |
| M3-B1 | 道路层 | road_info.lua 与 bytes 的 42,018 条记录互证；type_info 与 client_res ID 直接对应，无 +1 |
| M3-B2 | 河流多边形及 top | 原几何库 + 独立 504² 河格；ninegrid2pos 计算摆位，见真源 §4.1 |
...
| 批次 | 状态 | 提交 | 日期 |
|---|---|---|---|
| M2-B2 / M3-B2 的后修 | ✅ 雪沙改用原生世界 UV，103 种几何的 3,412 个顶点与原生计算核一致；河格按 ninegrid2pos 核对全部 31,140 条摆位、修正 15,595 条奇数行偏移。概览与小地图重烘；130 项地图回归、竖版 41 步真实引擎重放通过，详见 MAPORIGINAL-2D §9.3 | （本轮） | 2026-09-24 |
| 四档 LOD（宿主策略） | ✅ 已实施并通过专项验收：L0/L1/L2/L3 标称档界 0.55/0.20/0.04、8% 滞回与动态全图下限；L1 原件缓存、L2 地貌缓存、L3 同源概览，48 MiB LRU、16 位网格完整拆批、地名避让与城市标记。真实引擎查出并修复离屏 UI 投影污染及山体单边裁剪；103 项地图回归、四档往返/雪沙/拖动/释放与横竖地图范围检查通过，详见 MAPORIGINAL-2D §9.2。原版 2D 无运行时 LOD 的结论不变 | （本轮） | 2026-09-24 |
...
| M1-B1 grid 层止血 | ✅ 2026-09-22 先关门控并加守门；2026-09-24 A12 已补原贴图格线并启用 | （本轮） | 2026-09-22 |
...
| M3-B1 道路层 | ✅ 已完成；初版用结构签名，随后改由 client_res 实测绑定，见下方后修行 | （本轮） | 2026-09-23 |
...
| N4 研究项 | ✅ 已查明（交接单 N4，B4 按原判不阻塞不做）：B3 **grid 层证据有了** —— 原版无独立线框网格层，但 2D 地表内嵌贴图格线子系统（FRAME=1400、1px 淡黄 α24%、26.57°、静态合批、远档隐；MAPORIGINAL-2D §9 对照表）⇒ 当时改判可做；2026-09-24 已由 A12 实现；B1 minimap 四表查明（缩略图标注原版化可行、无数据缺口）、city 余桶粗筛（`[5]` 名胜地标登记为新开放项）；B2 三表勾销 | （本轮，调研） | 2026-09-23 |
| 普通点选（后修） | 2026-09-23 的 choose_00/11009 行军选择图已被 2026-09-24 A10 取代：普通点选使用 city_shape.GRID.click_res=2080 的八片 UI XML 布局、Scale/Color 轨道、24 fps 与 Quad.Out；以真源 §2.2 / §9.1 为准 | A10 记录 | 2026-09-24 |
...
| 运行时任意下载原版单件资源 | 当前消费导出后的稳定 textureId、图片表和版本绑定 Bundle；未实现原版 share_res 全集及动态下载，不是图集在技术上禁止按逻辑 ID 换件 |
