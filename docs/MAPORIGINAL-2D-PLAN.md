...
| M3-B1 | 道路层（§4.2） | ~~`road_info.bytes` 的方向编码未解~~ **结构已解开**（75 B 前缀 + 1125² 网格头 + 42,018 × 5 B `{row, col, 组号}`，字节账精确吻合）；⛔ **坐标系未定**：1125² 不是逻辑格网格，1:1 与 ×4/3 两种换算都无信号。⚠ 另：记录只带组号**没有片号** ⇒ 片是**运行时按邻接拼**，与河的「制图期烘死」是两套机制 |
...
| M2-B2 snow/desert 块层 | ✅ 已完成（`_top_group` 手摆件与河流同批留后） | （本轮） | 2026-09-23 |
| M3-B1 道路层 | ⏸ 勘察完成、**卡在坐标系**（`recon_road.py`） | （本轮） | 2026-09-23 |
