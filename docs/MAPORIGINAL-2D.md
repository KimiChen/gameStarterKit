...
| 道路 | 选片**制图期烘死**（`type_info` 下标 + 水平翻转），三套皮肤 | ✅ **已建**（M3-B1）：路格 1125²、半宽 200/半高 100 = 4/3 逻辑格，42,018 片 | ~~整层缺失~~ 已补。⚠ id→精灵绑定是 `[推断]`（结构签名） |
| 建筑城营 | **AOI 驱动的 unit**，两级配置表选件 | 城址件按「面积前 8 大」**启发式**挑 | 机制不同（AOI 需服务端）。⚠ **选件**这半边卡在 `city_info`/`city_res`（在未解的 base.cw 里，§11-1） |
| 归属状态 | `grid_state` 层，两个 z 档（2000 / 3800） | 无 | 需服务端 |
| 分层深度 | render_layer + `MAP_ZORDER`（留缝） + 层内画家序**三级** | ✅ **已补第 ② 级**（2026-09-23）：`MAPO_LAYERS` 每层带 `zorder`（照抄 MAP_ZORDER、留缝），**每层一个容器节点**按它升序建 | ~~缺第 ② 级刻度~~ 已补。⚠ 补之前实测有真缺陷：次序取决于「谁先 render」，地表底挂在路/河/山之后把它们全盖住 |
...
1. ⚠ **`base.cw`（66,776,016 B ctable）已解开一部分**（2026-09-23）。
   ...
   ★ `[实测]` **顶层表目录**：一段按**字母序**排的「表名 → u32」键值对（可见 1,339 项，
   `build` … `zhongshuling_market_level_cfg`）。判据用「池里前一字节是 NUL」——
   真键必指向串**起点**，⛔ 软判据（「像标识符」）会在 `IER_1` / `RY_6` 这类池内巧合上截断。
   ⛔ **但那个 u32 不是对象句柄**：实测 `client_res` 的行落在句柄 ~214,746 而目录值是
   164,841、`land` 是 ~505,131 vs 460,421，差值还不是常数 ⇒ **语义未知**，
   目前只能用它确认某表存在 / 拿全部表名，⛔ 别拿它定位行。
   ...
   **同一张表的行定长且对齐**。已把 `client_res` 的布局定死（100 B / 25 个 u32）：
   ...
   ⛔ **仍未解**（2026-09-23 攻过一轮，把边界划清）：
   - **列表列的对象编码**：`offset_2d` / `vector` / `variant_*_client_res_id_list` /
     `reward_*` / `even_res_center` 的值是**连号句柄**（同一行的多个列表按序分配）。
     句柄经对象偏移表能拿到对象（8–20 B），但**对象内部编码没有一致结构**
     （同一列的多个实例既有全零 16 B、也有内容迥异的 20 B）⇒ **2 向量读不出来**，
     件的美术锚点偏移仍无解。要啃得从 `libnative-lib.so` 里的 ctable C 模块读解码器。
   - **变长行的边界**：`client_res` 里有 27 行带 `even_res_center` 等额外列 ⇒ 该表
     **不是**处处定长。「往前扫到键不成立为止」的行首启发式在变长行上**不可靠**
     （实测同一列两次扫出不同句柄），⛔ 别用它。
   - `land_shape`（实测列只有 `historic/id/name/season`）、`city` / `city_res` /
     `city_shape` / `city_shape_grids`、`minimap_plate`。

   ⚠ **两处名字更正**：① **没有 `city_info` 这张表**（真名是 `city` / `city_res` /
   `city_shape` / `city_shape_grids`）；② **`even_res_center` 不在 `land_shape`**，
   它在一张**形状表**里（同行列有 `even_grids` / `even_logic_center` / `odd_res_center` /
   `shape_box`，`allias = H_SHAPE`）。
