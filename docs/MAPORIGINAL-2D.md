...
>
> ★ `[实测]` **路格没有奇偶行错位**（2026-09-23 道路不连贯根因查明）：42,018 条 tiles 的
> 邻居直方图在 (0,±1) / (±1,0) / (±1,±1) 全方向密集 ⇒ 路格是 8 连通的普通菱形格网，
> 位置式 = `x=(row−col)·半宽, y=−(row+col+1)·半高`（无错位）。原版路/河/官道走的是
> `coord_util.ninegrid2pos`（与逻辑格 `grid2pos` **不是**一条式子）；河格对它传 `/3`
> 分数坐标（`river_grid.lua`）⇒ 它本就不带奇偶性。⛔ 早先本 kit 照逻辑格抄了奇偶错位，
> (±1,0) 方向的邻居被打成 (+0.5hw,−1.5hh)/(+1.5hw,−0.5hh) 交替 ⇒ **行方向的路碎成虚线**
> （列方向完好，与症状一致）。锚点「底边中点对格心」在无错位下相邻件菱形边精确重合。
> 2D 消费现场：`2d/background/road_layer_view`（disasm）逐 tile `ninegrid2pos(r,c,200,100)`。
>
...
> 本 kit 的 4 档 LOD + 同源地貌概览 + 常显缩略图整套是**自创**——见 §10 的定性（结论不变）。
...
自建档界是宿主策略（⛔ 拿 3D 相机距离比套 2D 缩放是跨维度套用）。
`lodref.data.ts` 保存的原始参考仍为 **6 档**，层结构
（LOD_0 特判 `is_lod_0_hide` + LOD_1..5 表 `lod_hide_cfg`，1=隐）、方向（档大=远）、
**各层隐藏模式**用于交叉检查显示次序（11 层映射带依据，river 为豁免项）。
2026-09-24 本 kit 已改为 **4 档**（`MAPO_LOD_MAX=3`），实时层集中在 L0；L1/L2 由原件缓存承接，
不能再把参考表档数写成运行时档数。⚠ 极性陷阱：kit 的 scale **大=近**，原版 3D 的 scale **大=远**，
...
### 9.2 四档 LOD 实施与验收（2026-09-24，宿主策略）

运行时由六档改为四档：L0 `[0.55,2]`、L1 `[0.20,0.55)`、L2 `[0.04,0.20)`、
L3 `[scaleMin,0.04)`；切档采用 8% 双向滞回。`scaleMin` 按地图视口与全图包围盒计算，
缩放到底固定居中。原版 2D 没有这些分档，§8.2 的原包结论不变。

L1 把原始地貌、道路、水面静态填充与完整资源 prefab 烘入分块纹理，城市单独显示；
L2 保留地貌并改用城市标记，L3 使用同源的 2048×1024 概览。实时层、缓存和离线概览
共用坐标/几何生成函数；旧 res 调色板远图退役。缓存上限 48 MiB，含颜色与深度附件及待建块，
进入 L3 清空缓存，页面关闭释放专用相机、材质、网格和纹理。

本轮真实引擎检查还修复了两项：离屏 UI 网格在 Creator 3.8.8 自定义管线受主 UI/Profiler
投影影响，改为独立相机下的普通网格并显式排序；山体中心锚点向下跨块时，旧二分查询
只扩展单边而漏件，改为双向候选余量。GPU 验收读取相邻 RenderTexture 的 4 texel 重叠带，
直接比较同一世界位置的像素；不只以“节点存在”判定缓存成功。

机检：103 项 mapOriginal 回归、客户端严格类型检查、资产/镜像校验及全仓 `npm run verify:all` 通过。
Creator 主重放 12 步、
追加洛阳拖动/释放 2 步通过，console=[]；雪地、沙地各覆盖 L0/L1/L2，另验证横屏地图适配。
竖屏地图视口 750×1110.53 的下限为 0.007032422，横屏地图视口 750×207.57 为 0.003891297，
公开网格顶点均完整落在地图视口内；这是地图适配检查，不代表宿主固定 HUD 已完成横屏布局验收。
缓存峰值实测 48 MiB，L3 为 0；相邻块重叠带最大单组平均差低于 0.014/255，关闭后残留 mapo 节点为 0。
...
| 自创项 | 定性 | 理由 |
|---|---|---|
| 合并 mesh + 主画面正交 UI 相机 + 离屏缓存相机 + 兄弟序 | **必要补充** | Cocos 下上万节点不可行。⚠ 代价见 §7.1 |
| `mapoPainterCompare` 把画家序做进索引顺序 | **必要补充** | 与原版 `-pos_y ∝ (row+col)` 同序；先全局排序再拆批，批间保持兄弟序 |
| 4 档 LOD + 视口适配缩放下限 | **宿主策略**（2026-09-24） | 标称档界 0.55 / 0.20 / 0.04，8% 滞回；最小值按全图与地图视口计算。L1 原件缓存、L2 地貌缓存、L3 单张概览；不能反推原版行为，详见 kit README §六 |
| ~~8 自造粗类 × 4 变体 + 逐格 UV 翻转~~ | ✅ **已退役**（M2-B1，2026-09-23） | 整套在绕开「图集里不能 GL_REPEAT」这个自我限制。产物 / shared API / mesh builder / 渲染器 / 烘焙函数全部删除 |
| 原图乘色/加色 + 独立 UI 标记颜色 | **宿主适配** | 地貌原图直接按 RGBA 输出，避免在实时层、缓存与概览之间重复应用 tone mapping；标记属于宿主界面 |
| ~~`regions.bin` 连通域 + 无锚区补件~~ | ✅ **已被 res 锚点取代**（M0-B1，2026-09-22） | 它存在的原因是我们自己销毁了锚点信息。连通域与 scipy 依赖整套删除 |
| ~~`plate-lod4/5` 按 res 调色板烘图~~ | **已退役**（2026-09-24） | 此做法丢失雪沙带与真实水面。新 `overview` 与缓存直接消费近景原始地貌几何、贴图与锚点 |
