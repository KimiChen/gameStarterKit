...
1. ⚠ **`base.cw`（66,776,016 B ctable）已解开一部分**（2026-09-23）。
   ...
   ★ `[实测]` **那个 u32 就是根容器的子项索引**（2026-09-23 逆出 ctable 解码器后更正）。
   ⚠ 本文档一度写成「不是对象句柄、语义未知」，**那是错的** —— 当时把 entries 当成了
   ...
   **同一张表的行定长且对齐**。已把 `client_res` 的布局定死（100 B / 25 个 u32）：
   ...
   - **嵌套容器的头部布局**：根容器是 `[4]=count` / `[8..]=entries`，而子容器不同 ——
     `client_res` 容器头实测是 `[0]=串基址 [4]=109 [8]=8 [0xC]=328`，`0x10` 起是一大片
     全 `04` 的标签字节 ⇒ **行的键/值数组落点还没坐实**，`even_res_center` 仍读不出来。
     要继续得细追 `0xb3fc00` 那段行遍历的寄存器来源（`x22` / `x27` 的 provenance）。
