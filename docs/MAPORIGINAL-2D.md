...
| 建筑城营 | **AOI 驱动的 unit**，两级配置表选件 | 城址件按「面积前 8 大」**启发式**挑 | 机制不同（AOI 需服务端）。~~**选件**卡在 base.cw~~ ✅ **已解除阻塞**（§11-1）：249 座城的真名/类型/等级/形状已入 `MAPO_CITY_SITES`；`city_res` 1,532 行带 `editor_brush_res_path`，⚠ 但那是 **3D `.group` 件**，2D 沙盘要的 2D 件另找 ⇒ **画城址件仍未做** |
