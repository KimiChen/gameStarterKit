...
| 建筑城营 | **AOI 驱动的 unit**，两级配置表选件 | 城址件按「面积前 8 大」**启发式**挑 | 机制不同（AOI 需服务端）。~~**选件**卡在 base.cw~~ ✅ **已解除阻塞**（§11-1）：`city_res` 1,532 行全出，带 `editor_brush_res_path` 直指 `.group` 件；⚠ 接上去是**后续批次**，本版仍是启发式 |
...
1. ✅ **`base.cw`（66,776,016 B ctable）格式已完全解开**（2026-09-23，逆自 `libnative-lib.so`）。
   ...
   **`child(i) = base + 8 + count*4 + entries[i]`**。按它解出 `client_res` → 容器 @0x98ffac、
   ...
   | `city` | 249 | 40 | ⚠ 不是城址表（`city#1` 名「部队攻击」），是别的语义 |
   ...
   | `city_shape_grids` | — | — | **不是行表**，是 2,689 项的扁平 `grid_key → u32` 图 |
   ...
   ⇒ **对本 kit 当前画的层没有影响**：山体走 `mountain_*_group.prefab` 的 transform（已读）、
   城址走 `city_center.lua` + 启发式，渡口 / 码头本 kit **不渲染**。
   ⛔ 别据此去调山体或城址的摆位 —— 它们不在这 8 个形状里。
