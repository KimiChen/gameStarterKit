...
- [x] 完成筛选（全部/已拥有/未拥有/可合成）、装备、合成、错误提示（只按 `code` 分派，
      ⛔ 不解析文案）和设备本地红点（坏值/写失败降级为内存态）。
- [x] 页面关闭后释放监听（`onCloseLifecycle` 断开 `onChanged` 并清引用）。
- [x] **皮肤预览已接**：懒加载 `previewAsset`（S1 已产出的运行时资源），裁掉顶部 28px 技术标题条。
      ⚠ 是**静态合成图**（tail+body+head 各取 normal 首帧），⛔ 不是逐帧动画；
      且底色为烘死的不透明深蓝，行底板上会有矩形色差——要透明底须改生成器重出 16 张并动 S1 证据字节，
      ⛔ 不在本专项范围。加载失败的 fallback 是「不画图」，⛔ 不退回默认皮肤（那会在别的皮肤行上画皮肤 1）。
- [x] 稳定回退：装备值失效时服务端回退皮肤 1（S3-03），客户端只渲染服务端下发的目录。

> ⚠ **预热限制**：`getSnapshot` 只在打开衣柜时调用（feature 非 resident；`resident` 只豁免
> idle-release，**不等于**启动期装载）。冷启动直接进 snake 会用默认皮肤 1——服务端 resolver
> 的回退路径已覆盖、不阻塞进房。是否补「join 前预热」接缝见 [README §9.6](README.md#96-剩余待决项不阻塞-s3-开工) 的 **S3-d**。
