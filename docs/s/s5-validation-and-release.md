...
⚠ **F5/F6/F7/F8 的真引擎目视验证尚未完成**（Creator 未重新编译，判据见下方环境限制）；
其余 F1/F2/F3/F4/F10/F11/F12 已真机验证。
⚠ **F7 没有 Node 侧回归闸**：`WardrobeView` 目前只有 logic 层测试，视图层要连
`getSnakeCosmeticRuntime` 与 `resources.load` 一起搭台，本轮未做——F7 仅靠代码审阅与真机目视。
...
| S5-CR-06 | 衣柜行的皮肤预览与长名排版不挤压按钮区 | ⚠ 代码已修（F7 顺序 bug），⛔ 真机目视与 Node 回归闸都还没有——`04-wardrobe.png` 仍是修复前的画面 |
...
| S5-05 Creator 预览 | `[进行中]` | 8 张证据 | - | - | F1～F8、F10～F12 全部修复；F1/F2/F3/F4/F10/F11/F12 已真机验证，F5～F8 待 Creator 重开后目视 |
