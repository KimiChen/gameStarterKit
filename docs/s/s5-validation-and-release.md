...
| `04-wardrobe.png` | F7 修复后重拍：预览缩略图落在行内（149×46.8），名称与稀有度不再被压 | ✅ |
| `05-wardrobe-craftable.png` | 「可合成」筛选空态「该筛选下没有皮肤」（新账号 0 碎片，符合预期） | ✅ |
| `06-battle.png` | 六条修复后重拍：自机与 AI 蛇身均为离散重叠圆点、弯道逐点旋转正确、白齿消失；自机细白轮廓 vs AI 无轮廓；头身色调一致；零告警 | ✅ |
| `07-relive.png` | F6 修复后重拍：全屏压暗 + 居中面板，文字与按钮完全可读 | ✅ |
| `08-result.png` | F5/F8 修复后重拍：五行文字全在底板内、战斗 HUD 已收起、点原按钮位不再弹确认框 | ✅ |
...
- **F4｜已修**：根因比字面更深——effect 名 `builtin-ui` **在引擎里根本不存在**，材质因此拿到
  **零 pass**，于是任何 `setProperty` 都必然报 `illegal property name`。⚠ 改用 `for2d/builtin-sprite`
  ...
  转向翻正负，背面剔除会让一半转弯段消失。
- **F5｜已修**：两处。
  **F5a 排版**：底板 `snake_result_bg` 是固定 **674×694** 的贴图且不缩放（上下界 ±347），而文字全部按
  `view.getVisibleSize().height` 取比例定位。`ResolutionPolicy.FIXED_WIDTH` 下视口高度随机型变化，
  1624 高时标题落在 **+389.8**、「返回主页」落在 **−357.3**，双双出界；只有 ~1400 以下的矮视口才碰巧
  落在板内——所以 Node 桩与 typecheck 永远看不见。⚠ 还有一处潜伏：`lines` 最多 7 行
  （`SnakeHud.ts`：2 基础 + 复活 + 金币 + 经验 + 碎片 + 解锁），第 7 行原本落在 **−373.5**，
  与 `返回主页` 只差 16px 会叠字，取证时只有 3 行故未显形。
  修复：全部改为相对**底板半高**排布（标题 0.73、原因 0.55、行首 0.32 步长 0.155、返回主页 −0.78），
  按 7 行取值仍在板内。⛔ 别再按视口比例定位任何一行。
  **F5b HUD 未收起**：`showRunResult` 只销毁复活层，从不动 `hudLayer` / `controlLayer`。
  ⚠ 指望底板遮住是不行的：排行榜在 x=240、摇杆在 y=−592、加速键在 (245,−402)，全在 674×694 板外，
  且触摸监听仍然活着。修复：结算时把两层 `active = false`。
- **F6｜已修**：复活提示（与同类的「结束确认」）此前只有容器 + 三个 Label，**没有任何底**。
  ⚠ 这两层显示期间 `render()` 并不早退（只有 `resultLayer` / `battleBlocked` 才早退），世界仍在逐帧
  渲染并跟随镜头移动，蛇身从字底下穿过时近白色的 `金币复活` 就完全读不出来。
  修复：新增 `newBackdrop()`——全屏压暗 + 居中面板，两层都在建文字**之前**调用（同层内后加的画在上面）。
  ⛔ 别改成只调文字颜色或加描边：底下是活动的战场、颜色随机。
- **F7｜已修**：根因是一处**顺序 bug**，不是尺寸算错。`sprite.spriteFrame = frame` 执行时
  `sizeMode` 还是默认的 `TRIMMED`，引擎在 `_applySpriteFrame → _applySpriteSize` 里用 `frame.rect`
  覆写了 UITransform 尺寸（420×132）；下一行才设 `CUSTOM` 已经晚了——它只抑制**后续**的自动改尺寸，
  ⛔ 不回滚。于是预览条半宽从 74.5 涨到 210：左溢出面板 79px、并压住名称与稀有度行约 120px。
  修复：`sizeMode = CUSTOM` 移到赋 `spriteFrame` **之前**，并在其后再显式钉一次 UITransform 尺寸
  （不依赖该顺序在未来引擎版本继续成立）。⚠ 与 S3-04 已登记的「预览图底色色差」是两回事，
  那条是烘死在 PNG 里的不透明底。对应 S5-CR-06。
- **F8｜已修**：run 已结束、结算页在显示时「结束本次」仍可点，确认框与结算页叠加互相压字。
  修复两道：`showRunResult` 关掉整个 `hudLayer`（按钮随之消失），且 `showEndRunConfirmation` 在
  `resultLayer` 存在时直接拒绝——后者是兜底，防将来有人改动可见性时回归。
...
⛔ F9 仅为观察项，F1～F8、F10～F12 均已修复；S5-05 的剩余缺口是 16 套皮肤逐一装备验证与真栈 int 未跑。
**F1～F8、F10～F12 全部已在 Creator 3.8.8 真机验证。** F5～F8 的实测判据：
- F5a：结算层子节点 y 实测 标题 **253**、原因 191、三行 111/57/3、返回主页 **−271**，全部落在底板
  ±347 内（修复前分别是 385 与 −353，出界）；目视五行文字均在蓝色底板内（`08-result.png`）。
- F5b：`SnakeWorld.Hud.active = false`、`SnakeWorld.Controls.active = false`；画面上左右手/结束本次/
  排行榜/摇杆/加速键全部消失。
- F6：`SnakeWorld.Relive` 的第一个子节点是 `backdrop`，全屏压暗 + 居中面板，文字与两个按钮
  完全可读（`07-relive.png`）。
- F7：`preview-*` 节点 UITransform 实测 **149 × 46.8**、`sizeMode = 0(CUSTOM)`、左边缘 world x=86
  （修复前是 420 宽、左边缘越出面板）；目视缩略图落在行内且不压文字（`04-wardrobe.png`）。
- F8：结算页显示时点击原「结束本次」位置，`SnakeWorld.EndRunConfirm` 不出现。

⚠ **F7 没有 Node 侧回归闸**：`WardrobeView` 目前只有 logic 层测试，视图层要连
`getSnakeCosmeticRuntime` 与 `resources.load` 一起搭台，本轮未做——F7 只有代码审阅 + 上面这条
真机实测撑着，⛔ 别当成已被 `verify:all` 覆盖。
（原文保留）F1/F2/F3/F4/F10/F11/F12 已修，且**六条几何/材质类均已真机验证**（见下）。
...
##### 真机验证结论（Creator 3.8.8，证据 `06-battle.png`）

- 自机与 AI 的蛇身均为**离散重叠圆点**，弯道处逐点旋转正确，白齿消失（F10）。
- 自机有细白轮廓、AI 无，与目录 `identity.self.outline = "fine-white"` / `ai.outline = "none"` 一致。
- 头与身体色调一致（F11）；战斗期控制台**零告警**。
- 运行时判据：出生瞬间蛇身网格 48 顶点 = **12 个四边形**，与目录算得的
  `floor((51 - firstBodyPointDistance) / repeatedBodyPointDistance) + 1 = 12` 吻合；
  旧几何是 204 顶点 = 51 个 = `initialPointCount - 1`。
- 顺带实测 GFX Buffer Mem 由 56.31M 降至 20.08M（动态网格按实际用量上传 + 绘制量随之收缩）。

⚠ **取证环境限制（登记在案，后来者必读）**：本仓位于 `/Volumes/KimData` 非启动卷，Creator 的
资源监听**收不到该卷的文件变更**——实测原子替换、浏览器硬重载均无效，**必须手动重开 Creator**
才会重新导入与编译。⛔ 别把「改完代码预览没变」当成修复无效：先用上面那类运行时判据确认
预览跑的是哪一版。本轮就因此白验了一轮，误以为 F10 没生效。
...
| S5-CR-02 | `750 x 1624` 下 HUD、摇杆、按钮和弹窗无重叠 | ⚠ 部分：结算/复活/确认三层的重叠已修并实测（F5/F6/F8）；⛔ `safeBottom=0/100` 两组对比未做 |
| S5-CR-03 | 16 套皮肤、衣柜、复活和结果页完整可操作 | ⚠ 部分：衣柜/复活/结果页已取证，身体几何与贴图均已真机验证；⛔ 16 套逐一装备验证未做 |
...
| S5-CR-06 | 衣柜行的皮肤预览与长名排版不挤压按钮区 | ✅ 真机实测 149×46.8、CUSTOM、左边缘在面板内（`04-wardrobe.png`）；⚠ 仍无 Node 回归闸 |
...
| S5-05 Creator 预览 | `[进行中]` | 8 张证据 | - | - | F1～F8、F10～F12 全部修复**且全部真机验证**；剩余缺口：16 套皮肤逐一装备、`safeBottom=0/100` 两组对比 |
