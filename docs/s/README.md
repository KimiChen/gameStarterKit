...
| S5 | `[进行中]` | 本次 | S5-01 候选已冻结；S5-02 `verify:all` exit 0（client 436/436、server 609/609、FGUI 66/66、inventory 110/110） | Creator 3.8.8 预览首跑完成，8 张证据见 `evidence/s5/creator/`；⛔ 真栈 int 仍未跑 | S5-05 首跑修复 F1（`SpriteFrame.pivot` 只读赋值废掉整条资源装载链）与 F2/F3/F4（`UIMeshRenderer` 无渲染组件 + 静态网格 + `builtin-ui` effect 不存在，三者叠加使蛇身/食物网格零渲染）；F10（蛇身几何改为原作的离散重叠精灵）、F11（ACES 色调映射致身体与头色差）与 F12（`updateSubMesh` 不同步 InputAssembler，绘制量永不收缩）亦已修，六条均已真机验证；F5～F8（结算页排版与 HUD 未收起、复活提示无底、衣柜预览条 sizeMode 顺序 bug、结算页上重复开确认框）亦已修，四条均已真机验证（结算层 y 实测 253/−271 落在底板 ±347 内、HUD 与操作区 inactive、复活层首子节点为 backdrop、预览条 149×46.8 CUSTOM）；⛔ 不得据此宣称 demo 可放行 |
...
- **`identity`：性质被推翻——它不是「永不渲染」。** 细白自机轮廓、AI 名字识别、顶点恒白都在跑。
  ...
  ✅ 已补：`snakePresentation.test.ts` 把可观察行为（自机无名牌 / 他人有名牌）绑到目录声明值，
  并经变异验证（去掉 View 的自机排除即转红）。
