...
> **主要输入：** 固定的原作归档 `/Users/kimi/work/tanchishe/wegameVersion/`、当前仓库 Snake 实现、
> 用户在 2026-09-03 拍板的竖版和地图覆盖口径。<br>
> **主要输出：** `newEndlessPortraitV2Map4096TotalTime0` 的可审计配置栈、横版/竖版 golden、
...
- 对当前实现与目标口径形成逐项差异表，并拍板尚不能由 V2 配置直接推导的规则。
...
V2 没有给出一套可直接覆盖本项目 fixed-step 联机规则的完整配置。下列值不能借“选择 V2”静默换算：

| 项目 | 当前实现 | 原作静态参考 | S0 必须产出 |
|---|---:|---:|---|
| Star 长度/分值 | 5 | 10 | 拍板首发值、理由和边界测试 |
| 基础速度 | 160 unit/s | 约 `4.5 px/frame @ 60fps` | 拍板是否保持当前联机值，禁止无证据单位换算 |
| 加速倍率 | 1.6 | 2 | 拍板值及客户端/服务端一致性 |
| 转向速度 | 已为竖版联机收敛 | 无可直接覆盖的完整 V2 值 | 保持或变更的明确决策与测试向量 |
| 出生保护 | 已为竖版联机收敛 | 来源流程含复活后 3 秒保护 | 区分首次出生与复活保护；复活保护固定为含 provisional 首 tick 的 60 tick 半开区间 |

任何变更都要更新 shared 真源、测试和版本说明；S0 只形成批准结果，不在差异表中把候选值伪装成已实现。
...
- [ ] 确认参考根目录只读，并记录归档的版本标识；若没有 Git commit，生成覆盖本阶段已读取配置、源码和
  场景序列化文件的路径 + SHA-256 清单。
...
- [ ] 使用固定构建、固定 V2 对象、固定随机种子和固定实体 fixture 启动原作新版无尽。
- [ ] 固定 `1624 × 750` 设计视口、主题、相机长度档位和截图时刻。
- [ ] 至少留存出生态、普通游玩态、长蛇缩放态、边界态、Star/残骸态和 AI 多皮肤态。
- [ ] 截图旁记录配置 hash、种子、实体坐标/长度、相机参数和获取步骤。

**产物**

- 原作横版 golden 集及其机器可读/可人工复现的元数据。
- 每张图与命名配置、种子、视口和状态的一一映射。

**验证**

- 清空临时状态后按记录重跑，核心截图尺寸、配置 hash 和 fixture 状态一致。
- 截图中确认原作网格、背景、边界、七色 Dot、Star、AI 原色皮肤均可识别。
...
- [ ] 对 S0-02 的世界实体按 `(x,y) -> (-y,x)` 转换，以 `750 × 1624` 目标视口生成参照。
...
- [ ] 固定 `100/200/300/300/300`、5 秒、最多五次成功复活和 3 秒保护；把 `ad_card` 标为仅来源证据。
...
- [ ] 逐项评审 Star 分值、基础速度、加速倍率、转向速度、首次出生保护和复活保护。
- [ ] 对每项记录“保持当前/采用来源/新值”、理由、单位换算、玩法影响和回退值。
- [ ] 若证据不足，选择明确的首发值并标为项目适配，不能伪装成原作事实。
- [ ] 为所有拍板值定义 shared/server/client 一致性和测试向量。
...
- 不使用 `px/frame` 到 `unit/s` 的隐式近似；任何换算都有公式与采样前提。
- 首次出生保护与复活后 60 tick 保护分别建模，不共享含糊字段；边界向量覆盖 start、start+59、until。

### S0-10 · 封装可复现证据并完成阶段交接

**动作**

- [ ] 汇总五层配置、组合 hash、golden、调色板、差异表、规则拍板和 fixture 结果。
...
- 配置来源、`totalTime=0` 和截图均能独立重复核验的阶段结论。

**验证**

- 另一名开发者只依据证据说明即可复现配置 hash 和核心截图。
- 所有文件路径存在、所有 hash 可复算、所有通过项有原始输出。
- 五层 hash、组合 hash、golden 元数据与差异表引用同一配置身份。
...
- [ ] 原作归档和所有已读取源文件有不可歧义身份，参考目录保持只读且不是运行时依赖。
- [ ] 原作横版 golden、竖版世界参照、UI 标注稿和边界 fixture 可重复生成。
...
- [ ] Star、速度、加速、转向、首次出生保护和复活保护均已拍板并有测试向量。
...
| 截图不可复现 | 像素差异无法归因 | 每图绑定视口、主题、种子、配置 hash、相机和实体 fixture；缺任一字段即重做 |
| 未拍板规则被实现者自行猜测 | S2 出现隐性产品决定 | S0-09 未闭合则不得进入相关实现；需要变更时回退为明确的当前值并重新批准 |
