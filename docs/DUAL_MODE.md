...
| 职责 | 内容 | 现状接缝 |
|---|---|---|
| ① 身份 | `accounts(openid→uid,status,token_hash)` + `seq(uid)` + `login_audit` | WebPlatform `lib/{login,auth}.ts`；组侧 `platform/inProcessLogin.ts`、`session.ts:auditLogin` |
| ② 令牌 | 不透明 `{uid}.{hex}` 签发/校验/撤销，库存 sha256（⛔ 非 JWT） | WebPlatform `lib/auth.ts`；组侧 `platform/inProcessAccount.ts:verifyBearer` |
| ③ 角色/足迹注册表 | `character(uid, handle, created_at)`（U5 定案，账号服务持有）：答 `ul` **且**是「uid 在 handle 有角色」的**权威**记录（喂 F4，见 §2.6）；句柄不透明、状态不感知（不知 handle=sId、不感知区内余额/进度） | WebPlatform `lib/character.ts`（接缝 `account.character.*`） |
...
> **⚑ 最终定案（M12d 落地，2026-07 简化）**：本节下方的 epoch/maxEpoch/outbox/TTL-兜底 推导是**历史设计**，
> 实施时按下述更简模型收敛。两次简化的理由：① 「每节点自筛踢」落地后，踢与 maxEpoch 依赖同一条广播，
> maxEpoch 只多兜几毫秒的在途 RPC；② 撤销的送达保证改由 **GM 工具**承担（见下 SOP），故周期回权威的
> `verifiedAt` 兜底也一并取消——快路径退化为**纯缓存 hash 比对、零权威回源**。
>
> **封号语义 = 账号级（封人、所有区）**：角色级/区级封禁是**另一套机制**（准入闸 + 单区踢），未纳入本轮。
>
> **权威真相只有两位**：`accounts.status`（1=封禁）+ `accounts.token_hash`（NULL=已撤销/换发）。
> 封号 = **一条 UPDATE**（⛔ 无 `token_epoch` fence、⛔ 无 `revocation_log` outbox、⛔ 无 `verifiedAt` 周期回源）。
>
> ### 🔴 封号 SOP：两步**都必做**（缺第二步无自动收敛）
>
> | 步 | 动作 | 保证 |
> |---|---|---|
...
>
> **GM 工具契约（运营/工具侧必须实现；已列为服务端规则 [09·G7b](SERVER.md#12-开发约束63-条规则目录)）**：
> 1. **顺序**：先 ①（权威）成功，再 ②（踢）——反序会让用户在权威写入前被踢、随即正常重连。
> 2. **遍历全部在役节点**：用户只连在一个节点，故绝大多数节点返回 `kicked:false` 属**正常**，非错误。
> 3. **重试到确认**：踢幂等（不在线即 no-op），失败必须重试；**最终仍失败要告警**。
>    ⚠ 「节点不可达」通常无害——节点挂了其上连接也没了；真正危险的是「节点活着在服务玩家但 GM 够不到」。
...
| 现状 | 服务化后 |
|---|---|
| `verifySession`（快路径 HMGET） | 组本地读 tokenHash（**纯缓存、零回源**；在线撤销靠 GM 踢） |
| `verifySessionStrict`（回源 accounts） | `verify(token)` 远调账号服务 |
| `issueSession`（登录内联签发） | 账号服务 `login` 拥有；组本地 `sess:{uid}` 由 onAuth 从 verify 结果懒填 |
| `banUser`/`revokeSessions` | 账号服务写权威（status/token_hash）+ 广播 kick（便捷扇出）；**送达保证走 GM `/admin/kick`**；业务侧不再 del sess |
