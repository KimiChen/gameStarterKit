...
MK4 退出（待）：B1–B5 交付（✅ 2026-09-20；§12 MK4 行）；退出条件三条：干净安装全链闭环 ✅、卸载 / 升级识别在途交接 + pending 事件 + 运行中分线 ✅、框架零追加手改 ❌→ 两处显式侵入（MK1-B4 / MK4-B3，锁重钉可 revert）；B6 `mmo-kit-v1-frozen` 只待 ① MK3 长跑报告（② 场景 B 50 人整窗 tick p99 回归已于 2026-09-20 用户拍板接受为 v1 已知回归、优化留 v1.x）。回滚：可回退（kit 0.1.17 → 0.1.12 重装，表结构未变；B3 框架侵入随锁重钉 revert）。
