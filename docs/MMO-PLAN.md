...
MK1 退出：B1–B6 全部交付（✅ 2026-09-20）；故障矩阵 ✅；kill criterion 场景 B 热点 100 人 ❌（50 人 ✅ 贴线）⇒ ⛔ 未 tag `mk1-exit`，拍板待用户（bots 独立进程基准台 / 收紧节拍・可见上限・量化 / 或接受 50 人为 v1 热点上限并改 §11.2）；数字与偏差 ①–⑫ 见 MMO.md §12 MK1 行。回滚：可回退（B4 框架侵入 = 新增钩子 / 端口，mode 未实现即退化）。
