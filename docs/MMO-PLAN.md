...
MK3 退出：B1（inventory 面物品半边）/ B2（角色保存定稿）/ B3 基准台 + 冒烟 全部交付（✅ 2026-09-20）；验收「并发拾取 / 重复请求 / 重启重放不复制物品或奖励」机检成立（int/mmo-inventory 并发恰一个 conflict、回执重放零写入；int/mmo-loot 至少一次 + 回执去重）、「币账仍在框架」不变；**24 h 长跑 ✅ stable（2026-09-20 23:44 → 09-21 23:44 HKT，officenvidia，288 样本，RSS +1.1 MB/h / heap +0.17 MB/h / 资源 −0.02/h / 积压 0 / 检查点表有界；§12 MK3 行）⇒ MK3 退出 2026-09-22，tag `mk3-exit`**。回滚：可回退（kit 0.1.12 → 0.1.9 重装，表结构未变；保留策略只删旧行）。

MK2 退出：B1–B3 全部交付（✅ 2026-09-20）；验收三条（有序执行与预算上限 / AI 分桶不挤占主 tick / 可无头重放）全部机检成立 ⇒ tag `mk2-exit`（⛔ 不依赖 MK1 退出拍板）；偏差见 MMO.md §12 MK2 行。回滚：可回退（kit 0.1.9 → 0.1.6 重装，表结构未变）。

MK4 退出：B1–B5 交付（✅ 2026-09-20）+ B6 冻结（✅ 2026-09-22；§12 MK4 行）；退出条件三条：干净安装全链闭环 ✅、卸载 / 升级识别在途交接 + pending 事件 + 运行中分线 ✅、框架零追加手改 ❌→ 两处显式侵入（MK1-B4 / MK4-B3，锁重钉可 revert）；B6 tag `mmo-kit-v1-frozen`（含 `scripts/packages/mmo.lock`，kit 0.1.22）已于 2026-09-22 打，冻结前 `verify:kit-clean-install` 25 步 ✅；前置 ① MK3 退出 ✅（24 h 长跑 stable）② 场景 B 50 人整窗 tick p99 回归已于 2026-09-20 用户拍板接受为 v1 已知回归、优化留 v1.x。回滚：可回退（kit 0.1.22 → 0.1.12 重装，表结构未变；B3 框架侵入随锁重钉 revert）。

MK1 退出：B1–B6 全部交付（✅ 2026-09-20）；故障矩阵 ✅；kill criterion 场景 B 热点 100 人 ❌、50 人 ✅ ⇒ 2026-09-20 用户拍板选项 ③「接受 50 人热点为 v1 上限」（§11.2 登记为明示例外；50 人三次重跑（种子 7 / 8 / 9，2026-09-20T1615*）tick p99 20.8 / 24.2 / 24.8 ms、出站 p50 69.1 / 68.8 / 68.5 KB/s/会话）⇒ tag `mk1-exit`；数字与偏差 ①–⑫ 见 MMO.md §12 MK1 行。回滚：可回退（B4 框架侵入随 protected-paths 锁重钉可 revert；kit 0.1.6 → 0.1.0 重装，表 004 列保留）。
...
MG0 退出：B1 拍板（默认样本名 `mmodemo` / `mmohold`）/ B2 内容包 + 表现 + 编排 / B3 §9.4 #1–#7、#9 全部交付（✅ 2026-09-22；MMO.md §12 MG0 行）。首个真实插件暴露的 kit 测试假设、客户端职业解析缺陷、codegen 夹具缺口在插件提交前以 kit 0.1.24 → 0.1.26 修复并把 `mmo-kit-v1-frozen` 重打到修复提交，插件提交相对该 tag 只含插件文件 + 生成物 + 锁（#2 / #3 判据成立）。回滚：`plugin -- uninstall mmodemo`（#7 已实证 17 文件收回、check ✔）。
MG1 退出：B1 竖屏操作模型（kit 默认 HUD：摇杆 + 轻点选目标 / 点地走 + 技能轮盘；纯逻辑 `hudControls.ts` 单测 + Creator 预览证据场景 `mmoWorld` 10 步全过）/ B2 可选域页面（`mmodemo.bossBoard` + 端点 + 向量 + `MmoDemoBoardLogic/View` + 菜单入口）全部交付（✅ 2026-09-22；MMO.md §12 MG1 行）。B2 需要 kit 提供「按图列分线的检查点 vars」——按 §9.4 #8 口径回 MK 阶段：orchestration 面 1 → 2 只加只读 `listCheckpointedVars`（kit 0.1.27，api-freeze 重钉，`mmo-kit-v1-frozen` 重打到 B1 提交 2790218e），插件提交相对该 tag 再做 #1–#6、#9（#7 干净安装未重复，MG0-B3 已走通）。

## 5. 进程拆分独立轨道 PS（四项已拍板：MMO.md D27）
...
| PS1 入口拆分 | `apps/server/src/entries/{lobby,game}.ts` + `{lobby,game}.config.ts`；共用 `bootstrapProcess()`（lifecycle 登记、后台循环按进程装配：mailwake / kick / 角色修复 → lobby，结算流深度告警 → game，监控与 shutdown 聚合器每进程一份）；`index.ts` / `app.config.ts` 合并三份 rooms；`LOBBY_PORT` / `GAME_PORT`（缺省 = `PORT`）；`start:lobby` / `start:game` 与 `dev:split`（并发三进程，`dev` 缺省仍合体）；`docs/inventory.json` 登记入口；正式入口三进程 int helper、`test/smoke.ts` 两端点发现；`protocol-version-matrix` 直接测 onAuth/onCreate，不依赖合体 boot | MF3 / MF4 均已退出，可直接接完整房型 |
...
| PS3 解耦 `room.resolve` | 删 `matchMaker.driver.findOne` 快照（③ 已拍板）：`room.resolve` 只做 lease 读取 + 凭据签发，满员 / 已开局由 game 端预留或 admission 拒绝（现有 `RoomFull` / `GameAlreadyStarted` 数字错误码，⛔ 不伪装成 Lobby RPC 字符串）；`private-room.test.ts` 相关期望同批改 | `core/rooms/privateRoomRpc.ts` |
| PS4 world 独立入口 | `entries/world.ts` + `world.config.ts` + `WORLD_PORT`；MF4-B6 已登记 `WorldRoom`，故本批直接装完整 world rooms，毋须再退回空占位；MF10-B2 的 RedisDriver 只在多个 world 进程之间；`WorldDirectory` 记录实例所在节点 publicAddress，world 消费者只处理 `kind=room` | D27 的占位是 MF4 之前的施工顺序，房型能力以已退出的 MF4 为准 |
| PS5 snake 皮肤缓存 | `rooms/modes/snake/cosmeticProfile.ts` 的进程内 Map 在拆进程后跨进程失效：snake 房每次 join 从 Redis 重新水合，只合并并发读；装备独写，奖励 / 合成 / 复活按字段 CAS 合并，防止旧 game 缓存覆盖 lobby；同 uid 写工厂串行并纳入停服排空 | 与 MMO 无关，PS1 的必做项；保持 demo 非生产经济边界 |
...
- **PS 与 MF4 的时序**：原建议在 MF4 前先占位；实际 MF4 已先退出，PS1 + PS4 直接装配现有 `world.config.ts` 的完整 WorldRoom；`docs/inventory.json` 与正式入口 int helpers 随 PS1 同批。
...
- [x] MK1-B1（8fe30a45：`movement` 面三端 + content v2（职业模板 + 碰撞位图）+ 直发 `s2c.mmoWorld.pos`（modeVersion 3）+ 客户端预测按 seq 和解 + kit 0.1.1 从树重装） [x] MK1-B2（b3e5ca51：kit 内部 `aoi/{grid,visibility}.ts` 网格候选 + 位面 / 隐身 × 阵营规则 + 最近优先截到 256；名片 `factionId` ⇒ world 面 v2、modeVersion 4、kit 0.1.2） [x] MK1-B3（f9d3a8fc：portal → `c2s.mmoWorld.transfer` → 框架 MF8 → perSession `transferReady`；落点经 persona 快照 `arrival`；客户端带参重进；灰盒 v3 东郊 + 一对门；真栈 int `mmo-transfer`（reply-lost 注入）；kit 0.1.3） [x] MK1-B4（e5980eb9：框架 persona 级强制点（`onPersonaCheckpoint` / `forcePersonaCheckpoint` / `savePersona`，离座 / 交接只落该 persona，显式框架侵入 WorldMode / WorldRuntime / WorldCheckpoint / WorldRoom）+ kit-api 再导出 CheckpointPort 三型、端口迁回 `kits/mmo/persistence/checkpoint.ts`、角色检查点 rev 按角色单调 + 004 `instance_rev`、快照 v2（cooldowns / loot / scriptVars / timers / regions）+ 真栈 `mmo-checkpoint`（硬杀 → 恢复 → 回退窗口）；kit 0.1.4） [x] MK1-B5（3a0e8c29：`social` 面三端（worldChannelId / partyOf / sayWorld / partyLocate / partyPanelRows / 附近聊天行映射）+ `mmoSocial` 域 partyLocate + 端点 + 向量 + kit-api `readPartyView` + 真栈 `mmo-social`；kit 0.1.5） [x] MK1-B6（本行所在提交：剧本 `mmo-hotspot` 逐级 25 / 50 / 100 + 生产节拍 `MMO_WORLD_TUNING`（10 Hz 位置 / 200 ms 兴趣集，相位错开）+ 场景 A 回归 −47% 出站；kit 0.1.6）— MK1 B1–B6 交付 2026-09-20，**退出待拍板**（热点 100 人未达 kill criterion），见 MMO.md §12 MK1 行
- [x] MK2-B1（本行所在提交：`combat` 面 shared / client + 服务端施法管线（读条 / 瞬发 / 打断 / 二次校验 / 直伤・治疗・aura / 死亡・复活 / checkpointOnDeath）+ 框架 `calcDamageWithDefense` 公式族 + private 流 cooldowns / casting（modeVersion 5）+ 灰盒 v4 技能族；kit 0.1.7） [x] MK2-B2（本行所在提交：`ai` 面 shared（decide / 分桶 / nav A*）+ 服务端 PathfinderPort（进程内缺省、compute 任务形态、迟到即丢）+ kit 内部 AiScheduler（分桶 + wall 预算顺延）+ mode 怪物脑（aggro / chase / cast / evade / patrol）+ 灰盒 v5 野猪・田鼠；kit 0.1.8） [x] MK1 退出（本行所在提交：用户拍板选项 ③，50 人热点三次重跑 ✅，§11.2 明示例外；tag `mk1-exit`） [x] MK2-B3（本行所在提交：`inventory` 面掉落半边 shared（rollLoot 分线随机流确定性 / nearestLoot / lootClaimed 载荷闸）+ wire `count`（modeVersion 6）+ mode 掉落实体・拾取 ⇒ `lootClaimed` 事件随分线检查点落库・快照 loot / lootSeq + worker `grantItemInTx`（回执幂等）+ 客户端拾取 + 灰盒 v6 物品・掉落表；真栈 `int/mmo-loot`；kit 0.1.9）— MK2 退出 2026-09-20，见 MMO.md §12；tag `mk2-exit` [x] MK3-B1（本行所在提交：`inventory` 面 v2 物品半边——shared 背包 wire / planGrant / checkEquip / bagAttrs、`ItemStore`（rev CAS）+ `sqlItemStore`、服务端 grantItem / moveItem / claimLoot / bagOf / moveItemFor、`mmo.bag` / `mmo.moveItem`（contractVersion 2）、mode 掉落归属 + 私有流 bag（预热 + 拾取后轮询）、客户端 bag 模型 / HUD / runtime；真栈 int/mmo-inventory（并发恰一个 conflict）+ mmo-loot ⑥；kit 0.1.11） [x] MK3-B2（本行所在提交：`validatePersonaSnapshot` fail-closed + mode 坏快照走出生点、`k_mmo_character` 列集冻结用例、检查点保留策略 KEEP 32 / 16（同事务删旧行）） [x] MK3-B3 基准台（本行所在提交：`world-bench --sample-every` 长跑采样 + 最小二乘增长 + 检查点表有界判定 + 4 分钟冒烟 stable；kit 0.1.12）— MK3 退出 2026-09-22（24 h 长跑 stable，officenvidia；§12 MK3 行；tag `mk3-exit`） [x] MK4-B1（本行所在提交：`orchestration` 面 shared 契约 + validator + 摘要、kit 内部运行器（预算 / 有界状态 / 整批作废 suspend / 环形日志）+ 注册表、服务端 `readCheckpointedVars` / `createOrchestrationHarness` / `pollGrantResults`、mode 全链接线（十三种事件源、十五种命令落地、快照）、worker grantItem / packSuspended、边界机检；kit 0.1.13） [x] MK4-B2（本行所在提交：kit.json 三个贡献点 content（data）/ presentation（module）/ orchestration（module）；`contributions.generated.ts` 两端生成；服务端内容注册表多包（贡献优先、内置兜底、一图一包）+ mode 按图解析 + 编排注册表收录贡献模块 + 启动期 packId 交叉核对；客户端内容 / 表现 / 物品模板改经贡献包；kit 0.1.14） [x] MK4-B3（本行所在提交：卸载分线闸 `tools/plugin/instanceGate.ts`（`world_instance` 候选行 × coord Redis 权威租约键 × 登记 `mode` 归属，fail-closed、⛔ bypass，`check` 告警）+ 框架 WorldRegistry 登记加 `mode`（显式侵入、锁重钉）+ api 面冻结 `apps/kits/mmo/api-freeze.json` / `mmo-api-freeze.test.ts`（九面 version / minSupported 数字 + 三端导出符号集，升级方向 KIT.md §4）+ 真栈 `int/kit-instance-gate`；kit 0.1.15） [x] MK4-B4（本行所在提交：kit README 重排为「一、说明书」（身份与边界 / 九面表（自 api-freeze.json）/ 三贡献点 / mmoWorld 与 wire / 编排事件・命令清单与预算 / 数字表 / 数据・事件・回退窗口 / 运维 / 验证）+「二、施工记录」历史保留；kit 0.1.16） [x] MK4-B5（本行所在提交：场景 A / B 最终数字（A ✅、B 25 ✅、B 50 整窗 p99 37.2 / 25.1 / 32.6 ❌ 回归、100 ❌ 已知）+ 分窗诊断（预热段）+ 故障矩阵全表（§10.2 十二行 → 证据；`world-transfer` 组用例竞态修复）进 MMO.md §12 MK4 行；kit 0.1.17） [x] MK4-B6（本行所在提交：tag `mmo-kit-v1-frozen` 含 mmo.lock，kit 0.1.22；冻结前干净树 25 步 ✅）— MK4 退出 2026-09-22，见 MMO.md §12；tag `mmo-kit-v1-frozen`
- [x] MG0-B1（本行所在提交：拍板 mmodemo / mmohold 默认名与范围；plugin.json + README + content/pack.json（demoVale v1）） [x] MG0-B2（本行所在提交：编排 mmoOrchestration + encounters/{bossTimer,ambush,merchant} + 表现映射 + 三份用例；仓内首装锁 [package]；同批修 kit 0.1.25 三处缺口，`mmo-kit-v1-frozen` 重打） [x] MG0-B3（本行所在提交：§9.4 #1–#7、#9 全过，结果表在 apps/plugins/mmodemo/README.md「验证」与 MMO.md §12 MG0 行；mmodemo 0.1.2 [tree]）— MG0 退出 2026-09-22 [x] MG1-B1（提交 2790218e：hudControls 操作模型 + MmoWorldView 摇杆 / 轻点选目标 / 技能轮盘 + creator-preview `mmoWorld` 场景 10 步全过；kit 0.1.27 含 orchestration 面 v2 listCheckpointedVars，tag 重打） [x] MG1-B2（提交 38947677：域 mmodemo.bossBoard + 端点 + 向量 + MmoDemoBoardLogic / View + 菜单入口；mmodemo 0.2.0；指纹重钉）— MG1 退出 2026-09-22 [x] MG2-B1（c13f969b：holdRidge 内容 / 编排、0.1.0 首装；kit 前置 291fec60 已单独冻结） [x] MG2-B2（fcbb3854：自有 HUD / standings、0.2.0；Creator 工具 68ec5efe，16 步全过） [x] MG2-B3（本行所在提交：§9.4 双样本全链通过；两树 verify:all exit 0，安装 / 迁移 11→11→11 / 卸载通过，冻结路径与锁不变；详见 MMO.md §12 MG2 行）— MG2 退出 2026-09-23（tag `mg2-exit`）
- [x] PS0（2026-09-19 拍板 → MMO.md D27） [x] PS1（46f81560：lobby / game 正式入口、公共 bootstrap、角色启停与正式三进程 int helper） [x] PS2（0964310f：游戏 `/version` 端点发现、客户端代次隔离 / 重连 / world 路由、smoke 两端点） [x] PS3（fe8bd32e：`room.resolve` 只读邀请租约并签票，容量与 start fence 留 game 准入） [x] PS4（588a9a77：完整 world 独立入口、room-only 推送、三进程 dev 监督器） [x] PS5（93edb6e7：Snake 1.2.3 每次准入水合、字段 CAS 与同 uid 写队列）— PS1–PS5 退出 2026-09-22（tag `ps-exit`）；监督器随 PS4 补齐，完整闸 / 真实集成 / 故障矩阵与 Creator 结果及偏差见 MMO.md §12 PS 行
