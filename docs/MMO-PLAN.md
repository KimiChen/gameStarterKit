...
- [x] MK1-B1（8fe30a45：`movement` 面三端 + content v2（职业模板 + 碰撞位图）+ 直发 `s2c.mmoWorld.pos`（modeVersion 3）+ 客户端预测按 seq 和解 + kit 0.1.1 从树重装） [x] MK1-B2（本行所在提交：kit 内部 `aoi/{grid,visibility}.ts` 网格候选 + 位面 / 隐身 × 阵营规则 + 最近优先截到 256；名片 `factionId` ⇒ world 面 v2、modeVersion 4、kit 0.1.2） [ ] MK1-B3…B6 [ ] MK2-B1…B3 [ ] MK3-B1…B3 [ ] MK4-B1…B6
