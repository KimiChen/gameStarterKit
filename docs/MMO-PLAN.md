...
MK3 退出：B1（inventory 面物品半边）/ B2（角色保存定稿）/ B3 基准台 + 冒烟 全部交付（✅ 2026-09-20）；验收「并发拾取 / 重复请求 / 重启重放不复制物品或奖励」机检成立（int/mmo-inventory 并发恰一个 conflict、回执重放零写入；int/mmo-loot 至少一次 + 回执去重）、「币账仍在框架」不变；**24–72 h 长跑报告未跑 ⇒ ⛔ 未 tag `mk3-exit`**（需要机器连续跑一天，用户择时执行 kit README 基准段命令，报告回写 MMO.md §12 后补 tag）。回滚：可回退（kit 0.1.12 → 0.1.9 重装，表结构未变；保留策略只删旧行）。
