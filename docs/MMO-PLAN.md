...
- [x] MK1-B1（8fe30a45：`movement` 面三端 + content v2（职业模板 + 碰撞位图）+ 直发 `s2c.mmoWorld.pos`（modeVersion 3）+ 客户端预测按 seq 和解 + kit 0.1.1 从树重装） [x] MK1-B2（b3e5ca51：kit 内部 `aoi/{grid,visibility}.ts` 网格候选 + 位面 / 隐身 × 阵营规则 + 最近优先截到 256；名片 `factionId` ⇒ world 面 v2、modeVersion 4、kit 0.1.2） [x] MK1-B3（f9d3a8fc：portal → `c2s.mmoWorld.transfer` → 框架 MF8 → perSession `transferReady`；落点经 persona 快照 `arrival`；客户端带参重进；灰盒 v3 东郊 + 一对门；真栈 int `mmo-transfer`（reply-lost 注入）；kit 0.1.3） [x] MK1-B4（本行所在提交：框架 persona 级强制点（`onPersonaCheckpoint` / `forcePersonaCheckpoint` / `savePersona`，离座 / 交接只落该 persona，显式框架侵入 WorldMode / WorldRuntime / WorldCheckpoint / WorldRoom）+ kit-api 再导出 CheckpointPort 三型、端口迁回 `kits/mmo/persistence/checkpoint.ts`、角色检查点 rev 按角色单调 + 004 `instance_rev`、快照 v2（cooldowns / loot / scriptVars / timers / regions）+ 真栈 `mmo-checkpoint`（硬杀 → 恢复 → 回退窗口）；kit 0.1.4） [ ] MK1-B5…B6 [ ] MK2-B1…B3 [ ] MK3-B1…B3 [ ] MK4-B1…B6
