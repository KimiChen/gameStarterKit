...
- [x] MG0-B1（本行所在提交：拍板 mmodemo / mmohold 默认名与范围；plugin.json + README + content/pack.json（demoVale v1）） [x] MG0-B2（本行所在提交：编排 mmoOrchestration + encounters/{bossTimer,ambush,merchant} + 表现映射 + 三份用例；仓内首装锁 [package]；同批修 kit 0.1.25 三处缺口，`mmo-kit-v1-frozen` 重打） [ ] MG0-B3 [ ] MG1-B1…B2 [ ] MG2-B1…B3
