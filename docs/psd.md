...
| PSD/design 源图 → UniFlex Web Golden | `ui:render-source` 按 `design.json` 独立合成 RGB 源图，`ui:verify` 按颜色阈值 `0.1`、区域差异 `5%` 比较，并检查组件、交互、九宫格和字体契约 | **主验收链已实现**；Backpack RGB 源图与 Web proposal 整图 `AE=0`，但两个越界 group frame 保留诊断警告 |
