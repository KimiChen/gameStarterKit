...
`ui:export-fgui` 从 UniFlex 组件树 + 布局快照写出一份**候选**独立 FairyGUI 工程（只写 `--out`，不写 `apps/art/fairygui`，不伪造 Cocos `.bin` / 图集 / `.meta`）。共享组件来自 `defineComponent` 目录（`components.generated.json`，出现在快照里才进 `UniFlex_Common`）。纯色背景写成 9 宫格填充图，不使用 Graph。`--screen` 出单页包 `UniFlex_<Page>`；`--screens a,b` 或 `--all` 把多页收进同一工程，预览用 `?screen=` 切换。`PopupFrame` 因 Slot 限制只出外壳模板；各页是特化树。变体组件（如 `Tab` 的 skin）按首次出现做模板，可见图不同则内联，不生成 controller。`virtual-list` / `scroll-view` 导出为独立的滚动组件（行收进子组件、裁剪 + 触摸滚动，隐藏滚动条；方向按行溢出自动判定：宽出 → horizontal、高出 → vertical、都超 → both），不再是拍平行。捕获还会把 TabBar/PanelTab 逐个点一遍：内容有差异的页签各出一份变体页 `<Page>Tab<N>`（picker/目录不显示），预览里点页签文字在同族变体间切换（含点激活页签回基础页）；捕获同时逐屏滚动 VirtualList 合并全量行。`ui:preview-fgui` 用锁定的官方 `fairygui-dom@1.0.0` 预览发布态包；`ui:verify-fgui-dom` 用真实 Chrome（CDP，优先 9222）逐屏验证加载、滚动拖拽、目录跳转与数量控件（依赖浏览器，⛔ 不进 verify:core / test:uniflex-ui-contract）。未完成 Editor 保存—重开前，不要把它当成已发布 FGUI 资源。
