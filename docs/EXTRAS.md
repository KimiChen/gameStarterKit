...
gono 的核心定位是**开发期游戏基础框架**：客户端、服务端、shared 契约，以及本地开发、
...
以 plugin 单源目录（`apps/plugins/<id>/plugin.json`）登记的能力有一份机器生成的索引：
[docs/plugins.generated.md](plugins.generated.md)，由
`npm --workspace @game/server run codegen:plugins` 生成（生成物禁手改），只记录
`planned` / `registered` / `source-present` 三种可机检的结构状态。普通 plugin 的 extra
...
`gono` 的 workspace、能力清单或核心验证命令当作本仓能力。
...
- 「兑换码」插件 `apps/plugins/redeem`（plugin 插件标准形态的端到端样本：静态码表 + 本 plugin 钱包，
  ⛔ 不接经济主钱包；见 [apps/plugins/redeem/README.md](../apps/plugins/redeem/README.md)）；
- 「点数赛」插件 `apps/plugins/tally`（gameplay 形态插件的端到端样本：先点满目标次数者胜的最小实时玩法，
  无奖励、无 evidence；见 [apps/plugins/tally/README.md](../apps/plugins/tally/README.md)）；
...
| 原编号 | 原内容摘要 | 归并结果 |
| --- | --- | --- |
| D1 | 客户端 app/session/scene/room 状态机、玩法 registry、统一回登录出口、可注入持久化、timeout/cancel | 历史归档：`plan.md` P0-01、P1-01；保留边界的原始记录在已删除的 `plan-v3.md`（见 Git 历史） |
...
（`plan.md` / `plan-v2.md` / `plan-v3.md` / `plan-v4.md`，2026-09-06 已从工作树删除，正文在 Git 历史）
经同轮核验**没有任何未被承接的开放项**。
...
- 条目 2 的 XADD 失败在 GameRoom stub 侧无注入：int 侧已覆盖真实故障注入，单测侧只注入返回结果，
  ⛔ 不重复建桩。
- 已删除的历史归档 `plan-v3.md` 的 78 条 `[不阻塞·有意保留]` 与其尾部各轮的保留边界（`--workspaces`
  形态失败关闭、workspace 引用解析跳过、豁免表条目靠评审等）继续有效；**原始记录与证据只在 Git 历史**
  （`git show 5940c85:plan-v3.md`），本文件不复制。
  ⚠ 仓内多处代码注释仍按小节号引用它（§9.5 / §11.3 / §12.3 / §15.3 / §18 / §30 / P1-08，以及
  `plan-v4` 的条目号），`grep -rn 'plan-v[34]'` 可列全。那些是**出处标注**，不是待办：读时按上面的
  `git show` 去翻归档正文即可。活测试的**失败信息**已全部改为指向本节（§5.3），
  ⛔ 不会再叫人去改一份工作树里不存在的文件。
