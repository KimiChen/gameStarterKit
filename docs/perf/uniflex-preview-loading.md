...
最初验证时另补齐了基线缺失的 `StarLevel.tsx.meta`；集成到 `new` 时保留目标分支已补交的
Creator 元数据，不再重放该临时补齐提交，也不修改星级组件源码。
...
### 集成到 new 的复验（2026-09-23）

预览优化重放到 `new` 的 `4ae2908c` 后重新构建并同步忽略目录中的 AOT 镜像：

- `verify:core` 通过，包括 1104 项客户端测试；UI 契约 77 项全部通过，目标分支的
  `3a5c2f01` 已修复上述 3 项 PSD 夹具问题。
- 浏览器复验通过：勾选、全屏状态保留、皮肤切换、13 个分类、离屏保留上限、搜索、快速切分类、
  直接 hash 进入；字体实例保持 1，未出现运行错误。
- 服务端 1375 项中 1366 通过、0 断言失败、9 取消，全部来自 `character-ready.test.ts`，
  原因仍是事件循环结束时 Promise 未完成。在尚未合并的干净 `new`（`4ae2908c`）上单独运行该文件，
  同样得到 3 通过、9 取消。因此本次 `verify:all` 仍未全绿；该既有问题不属于预览优化改动。
