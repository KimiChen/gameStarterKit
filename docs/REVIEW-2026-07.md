# 2026-07 同事评审 · 逐条归档与回复

> **正式归档**（曾自述"发完可删"的草稿，因它一度是 19 条「真没做」的唯一记录处而转正）。
> v3 · 2026-07-26：19 条「真没做」已全部有正式去处——可小修的已修（客户端三处泄漏、多区×freeze
> 断言等），其余登记 `todo.md`（**A1–A6**、D6、GameRoom 房级区上下文、既有支付条目）与 W4 契约；
> §0/§5 按 HEAD 现状更新，⚠ 含两处**对信中原有结论的更正**（正文就地标注，⛔ 不静默改写）。
> 本文自此只作历史归档——**登记与进度以 `todo.md` 和各 docs 为准，⛔ 不再回写本文**。
...
验证（发信时点基线；现行数字**只看 CLAUDE.md 现状段**，本文不再更新）：typecheck 三端 + verify:sync / int 115 / 服务端单测 28 / 客户端 79。
...
| U6 发奖无基础 | `docs/HANDOFF-M12.md` U6 | ⚠ **有一处比 U6 更急**：§4.1 明文要求「GameRoom.onCreate 读 options.sId 设房级区上下文」，而 `rooms/GameRoom.ts` 把 options 整个丢弃——规格与代码漂移，且进度表把 M11 标成已落地。建议把「房级 sId + MatchEvidence 带 sId」从 U6 拆出来单排。<br>⚠ **v3 更正**：「把 options 整个丢弃」「M11 虚标」两句**写下当时就已过期**——`908b7e8`（早本信 3 小时）已落地 `filterBy(["sId"])` 撮合 + onAuth 读 `options.sId` 硬闸，是我们对着旧代码写了回复。真实剩余的只有 **onCreate 不建房级 zoneCtx** 那一半，拆单承诺已兑现：`todo.md`「GameRoom 房级区上下文」 |
...
| **大厅连 `getBaseUrl()`、战斗连所选 `wsUrl`** | 成立，客户端侧零登记。<br>⚠ **唯一澄清**：今天打不出来（目录还是 demo 静态表、所有区同 wsUrl；单组下 GROUP_ZONES 通常为空即放行），但 W4 或 M16 任一落地就是必现事故。**v3：已登记 todo.md A5**（连同你 §4 那条「endpoint + sId 纳入连接复用判据」一起收进去了） |
...
- **选服结果应含 HTTP/WS/Portal 三类端点** —— 这半零登记（`IAreaServer` 只有 wsUrl，W4 也只承诺 wsUrl），真缺口，建议随 W4 定契约；coordinator 那半已在 D2 目标里。**v3：已并入 W4 处置栏**（契约要一次定全三类；portal 形状挂回问 Q7）。
- **`exceptHash` 不是单调栅栏，积压事件可能误踢赢家** —— 机制层面成立。⚠ 两点保留只影响定级：触发窗很窄（流游标从 `$` 起、重启不重放，要出事得消费循环卡顿超过「下一次登录 + registerOnline」）；后果是赢家被踢一次、重登即恢复，无数据损坏。**v3：已登记 todo.md A6**（若 Q2「/verify 扩单调量」拍了板，本条与 A1 复用同一个单调量、⛔ 别另造）。
...
**认领的六处 —— v3 时点：五处已改完，一处剩一半**（发信时全部原样在错）：

| 位置 | 问题 | v3 状态 |
|---|---|---|
| `platform/inProcessLogin.ts:52` | 「split 天然无此竞态」——**说错了**，正是你这条打中的地方 | ✅ 已改（`54f1b47`）：收窄成「split 只是没有**本路径**这条」，并把它自己那条（httpAccount 两步无 fence）指向 todo.md **A1** |
| `core/infra/config.ts:161` | 「专用 HA 实例，唯一合法跨组通道」——过期口径，与已改的 kickBus/redisRoute 打架 | ✅ 已改（`54f1b47`）：本组 coord、扇出只到组内；跨组顶号缺口指向 todo.md **A3** |
| `docs/GM-TOOL-SPEC.md:202` | 顶号「系统自动完成，⛔ 不需要 GM 工具介入」——多物理组下是假承诺 | ✅ 已改（`3e8cfcc`）：只对单组成立 + 指向 A3。⚠ 但 **Q3 的拍板仍未发生**，改注释不等于拍了板 |
| `docs/DUAL_MODE.md:18` + `docs/HANDOFF-M12.md:24` | 「split 全链 e2e 绿」——同进程同库，**过度声称** | ✅ 已改（`3e8cfcc`）：收窄成「证接缝、⛔ 不证部署形态」；真拓扑覆盖立账 todo.md **A4** |
| `docs/DUAL_MODE.md:24` | 「经济按区隔离」——因邮件那条属**过度承诺** | ✅ 已改（`3e8cfcc`）：收窄成「货币/背包按区隔离」；邮件/公会立账 todo.md **A2** |
| `docs/DUAL_MODE.md:434` §4.1 vs `rooms/GameRoom.ts` | 规格漂移（见 §2 U6 那行） | ⚠ **剩一半**：撮合 + onAuth 硬闸其实**发信前就已落地**（`908b7e8`，见 §2 那行的 v3 更正）；剩 onCreate 房级 zoneCtx，已单排 todo.md「GameRoom 房级区上下文」 |
