...
审核、灰度、热更新、合规检查和商店发行流程。上述缺失不是 `plan-v3.md` 的核心阻塞项。

## 5. 原待办归并与非绑定候选

原独立待办同时混合了核心正确性问题和可选扩展方向。当前核心项以
[plan-v3.md](../plan-v3.md) 的收口状态与证据为准；额外项在本节保留。下面的映射用于证明原内容已有明确
...
和本地验证闭环属于核心代码缺口，不在本节降级为可选项；当前开放边界统一进入 [plan-v3.md](../plan-v3.md)。
...
核心问题、实施状态与验收证据以 [plan-v3.md](../plan-v3.md) 为准；本文件中的成熟度说明不自动转化为计划任务。
