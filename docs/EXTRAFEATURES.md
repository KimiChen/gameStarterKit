...
项目说明站已迁移到独立的 Sites 项目仓库，不再属于 `gameStarterKit` monorepo。本仓库已移除
`apps/website/` 的页面源码、托管元数据、独立 package、构建/预览/部署脚本和站点测试，也不再提供
说明站相关的 npm 命令。站点的域名、托管、发布和后续维护由独立仓库负责；本节只保留迁出记录，不能
据此推导本仓库提供站点交付或线上可用性。
### 3.2 GM、账号管理与强制下线参考
