...
审核、灰度、热更新、合规检查和商店发行流程。上述缺失不是 `plan-v2.md` 的核心阻塞项。

## 5. 原待办归并与非绑定候选

原独立待办同时混合了核心正确性问题和可选扩展方向。当前核心项以
[plan-v2.md](../plan-v2.md) 的收口状态与证据为准；额外项在本节保留。下面的映射用于证明原内容已有明确
去向，不构成第二份路线图，也不改变本文件的非承诺定位。

| 原编号 | 原内容摘要 | 归并结果 |
| --- | --- | --- |
| D1 | 客户端 app/session/scene/room 状态机、玩法 registry、统一回登录出口、可注入持久化、timeout/cancel | 历史归档：`plan.md` P0-01、P1-01；当前边界见 `plan-v2.md` |
...
和本地验证闭环属于核心代码缺口，不在本节降级为可选项；当前开放边界统一进入 [plan-v2.md](../plan-v2.md)。
...
核心问题、实施状态与验收证据以 [plan-v2.md](../plan-v2.md) 为准；本文件中的成熟度说明不自动转化为计划任务。
