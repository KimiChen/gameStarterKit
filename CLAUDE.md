...
> - [docs/MMO.md](docs/MMO.md)：MMO 整合设计基线（2026-09-09；v1.1 / v1.2 2026-09-19：按 MMO-REVIEW M01–M20 修订、按 MMO-PLAN §7 细化与 D27 进程形态拍板（lobby / game / world 三进程，dev 缺省合体）；Nakama 定形、AzerothCore 定实；框架阶段 MF0–MF11（MF5 / MF7 各分 a / b：a = GameRoom 路径 / kit worker 通用半边，slg 2b 与 lvr 只等 a 半边）→ `mmo` kit MK0–MK4 → 内容插件 MG0–MG2；实施状态只在其 §12 回写，⛔ 不进 plan-v5）
