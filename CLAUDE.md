...
> - [docs/MAPORIGINAL-2D.md](docs/MAPORIGINAL-2D.md)：原版《三国志·战略版》**2D 沙盘**实现机制（2026-09-22；`mapOriginal` kit 复刻对象的唯一真源，代码注释/README/机检的判据都引它）。证据分四档标注 `[干净集]`/`[实测]`/`[disasm]`/`[推断]`，⛔ 不许把低档当高档引用。核心结论：地表底是「一块 10×10 格 + 一张 256² 底纹整数次 GL_REPEAT」而非逐格贴片；`res.bytes` 值 48..61 **就是多格地形的锚点**、`res_multi` 只是覆盖掩码（六项判据实测全中）；原版 2D **没有「远档」**（拉到头是换视图开小地图面板）。⛔ 不是施工单、不进 plan-v5
> - [docs/MAPORIGINAL-2D-PLAN.md](docs/MAPORIGINAL-2D-PLAN.md)：mapOriginal 对齐原版 2D 机制的施工单（2026-09-22；按「先纠错 → 再止血 → 再对齐 → 最后补层」拆成 `Mx-Bn`：文件落点、**机检退出**、风险、依赖。M0 纠错 4 批（锚点模型 / 件的 transform / 山体换基础季 / 城格抑制资源件）、M1 止血 3 批、M2/M3 有前置未排期；批次勾选只在其 §5，阶段级完成回写 MAPORIGINAL-2D.md 的对照表；⛔ 不是设计真源、不进 plan-v5
...
> - [docs/3d.md](docs/3d.md)：框架级 3D 舞台与资产管线设计提案（2026-09-19；v1.1 对照 Cocos Cyberpunk 校正，v1.2 / v1.3 审阅修订，2026-09-22 v1.5 补齐原始输入分流、蒙皮合批、同步 retainer、UUID 依赖、帧时 / 真机缓存验收（3D-13–3D-45、3D-47）；由 lvr-3d.md 审阅提升：Stage3D 相机 / 层位 / 场景全局租约、`AppPorts.stage3d`、AssetLease、`logic/scene3d` 纯数学（lodBands 落 shared）、EntityPool / SkinnedUnits / Vfx / quality 画质分档、cc 类型桩 3D 面、`verify:assets3d` 与 `tools/art3d`；阶段 SC0–SC5，消费方 lvr / mmo kit / slg 2b；SD9–SD12 已于 2026-09-19 拍板（mmo 世界视图 2D 首版 + 3D-ready、小游戏为首版目标且首发消费方 = lvr、引擎内置新管线、每包一个 bundle）；SC0 / SC1 / SC2 / SC3 已退出（2026-09-22 / 23 / 24，tag `sc0-exit` / `sc1-exit` / `sc2-exit` / `sc3-exit`），SC4-B1 / B2 已完成、SC4 进行中，SC5 未开始；实施状态只在其 §10 回写，⛔ 不进 plan-v5）
...
> - [docs/3D-PLAN.md](docs/3D-PLAN.md)：3D 实施施工单（2026-09-19 初稿、2026-09-22 v1.5；含 3D-13–3D-45、3D-47 五轮审阅记录与 L01–L09，六项新增问题按批次实施；SC0 / SC1 / SC2 / SC3 已退出，SC4-B1 / B2 已完成、SC4 进行中，SC5 未开始；SC0–SC5 拆成 `SCx-Bn` 批次：文件落点、机检退出、命令；消费方 lvr A0–A5 / mmo（SD9）/ slg 接法；批次勾选只在其 §8，阶段级完成仍回写 3d.md §10；⛔ 不是设计真源、不进 plan-v5）
