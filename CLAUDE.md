...
> - **[docs/SERVER.md](docs/SERVER.md)** —— 服务端目录/ws-RPC/outbox/冷档/广播/**62 条 `09·XX` 规则目录 + 07 契约表**
...
8. **服务端写路径对照 62 条规则**（[docs/SERVER.md §12](docs/SERVER.md#12-开发约束62-条规则目录)；代码注释 `09·XX` 即编号；⛔ 禁 HGETALL/INSERT IGNORE/ZINCRBY/无 fence 写档等）。新增常量/key/错误码先进 [§13 契约表](docs/SERVER.md#13-契约与配置redis-key--字段--错误码--常量) 再进 `core/infra/config.ts`/`keys.ts`（错误码另进 shared `RPC_ERR_CODES`）；登记点清单见 `apps/server/src/core/README.md`。
