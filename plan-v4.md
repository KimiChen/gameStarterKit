...
**仍登记**：`fb777ce` 未形成复现-修复-复现闭环（已自报），若全量套件再现熔断红需重新取证。
（本轮登记的另两条已在第二十二轮修掉：1200ms 贴边预算 → `89eebb4`；CDATA 内容被扫成引用 → `e40f9ef`。）
...
## 第二十三轮复核：修复批 + Non-intrusive 全批（2026-09-01）

**复核范围**：同事的两大批 commit——修复批 `b6790a5..c102ba4`（15 个 commit：11 个修复线
——flaky 根因、死代码/no-op 清理、文档更正——加 4 个并行文档线 `0aa2318`/`f695a08`/
`a208e46`/`e703452`；后 4 个未单列复核，随各自主题文档在期内自明）与 Non-intrusive 实现批
`37ed8b2..947296e`（阶段 1–9 + 治理收尾，共 13 个 commit）。
...
**复核范围**：同事对第二十三轮成果的修复批 `64d25f0..c897741`（9 个：4 代码修复 + 5 文档/登记更正；⚠ git 范围左端不含，写 `7c3065b..c897741` 只覆盖 8 个）
